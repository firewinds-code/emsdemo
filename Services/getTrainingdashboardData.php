...
echo "Executing API for Training Dashboard  Start";
echo "<br>";
...
(select count(t2.EmployeeID)ActiveCount,Alias, t2.cm_id,bt.BacthID as BatchID,BacthName,client,clientid from status_table t1 inner join ActiveEmpID t2 on t1.EmployeeID=t2.EmployeeID left join status_training tr on tr.EmployeeID=t2.EmployeeID left join batch_master bt on bt.BacthID=tr.BatchID where bt.createdon>'2020-10-01' and t1.Status in (2,3) group by cm_id,bt.BacthID,BacthName,client,clientid,cast(bt.createdon as date))t
left join
(select ifnull( count(t.EmployeeID),0)traing,BatchID,cm_id,cast(min(createdon) as date)StartDate,cast(max(date_cer_1) as date)EndDate,reporttoname(max(Trainer))TrainerName,Trainer as TrainerID from status_training t inner join AllEmpID e on t.EmployeeID=e.EmployeeID where cast(createdon as date)>=cast('2020-10-01' as date) group by BatchID,cm_id) tr1 on tr1.BatchID=t.BatchID and tr1.cm_id=t.cm_id 
left join (select count(t.EmployeeID) ncns,e.cm_id,s.BatchID from login_ncns_smsmail t left join ActiveEmpID e on t.EmployeeID=e.EmployeeID left join status_table s on e.EmployeeID=s.EmployeeID where status in (2,3) and cast(t.createdOn as date)=cast(now() as  date)  and type='NCNS' group by cm_id,s.BatchID)nc on nc.cm_id=tr1.cm_id and nc.BatchID=tr1.BatchID
left join 
( select count(a.EmployeeID) present,a.cm_id,s.BatchID from ActiveEmpID a inner Join (SELECT distinct EmpID FROM biopunchcurrentdata where dateOn=cast(now() as date)) b  on b.EmpID=a.EmployeeID left join status_table s on a.EmployeeID=s.EmployeeID where status in(2,3) group by cm_id  ,s.BatchID
...
echo "<br>Executing API for Training Dashboard with No. of records=" . $i;
//store_execution_time($i. 'End');
// function store_execution_time($i){
// 	 	$myDB= new MysqliDb();
// 		$ClientD=$myDB->query("Insert into process_eod_status_log set start_time=now(),end_time=now(),No_of_execution='Training dashbord ".$i."'");
// }
