...
if (empty($mysql_error) && count($result) > 0) {
	foreach ($result as $key => $value) {
		//echo $value['inojt'];
		$myDB = new MysqliDb();
		$sql_bio = "select id from biopunchcurrentdata where empid='" . $value['EmployeeID'] . "' and dateon='" . $date . "' limit 1";
		$result_bio = $myDB->query($sql_bio);
		if (count($result_bio) > 0) {
			echo $sql_roster = "select count(*) as `count`  from roster_temp where EmployeeID = '" . $value['EmployeeID'] . "' and (InTime like '%WO%' or OutTime like '%WO%') and  DateOn between '" . $value['inojt'] . "' and '" . $date . "'  limit 1";
