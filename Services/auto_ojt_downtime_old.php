...
$sql = "select t1.EmployeeID,cast(InOJT as date) as inojt,DATEDIFF(cast(date_add(now(), interval -1 day) as date), cast(InOJT as date))+1 as dd from status_table t1 join employee_map t2 on t1.EmployeeID=t2.EmployeeID where t2.emp_status='Active' and t1.Status=5  and t2.df_id in (74,77,146,147,148,149) and cast(t1.InQAOJT as date)<=cast(date_add(now(), interval -1 day) as date);";
...
if (empty($mysql_error) && count($result) > 0) {
	foreach ($result as $key => $value) {
		//echo $value['inojt'];
		$myDB = new MysqliDb();
		$sql_bio = "select id from biopunchcurrentdata where empid='" . $value['EmployeeID'] . "' and dateon=cast(date_add(now(), interval -1 day) as date) limit 1";
		$result_bio = $myDB->query($sql_bio);
		if (count($result_bio) > 0) {
			$sql_roster = "select count(*) as `count`  from roster_temp where EmployeeID = '" . $value['EmployeeID'] . "' and (InTime like '%WO%' or OutTime like '%WO%') and  DateOn between '" . $value['inojt'] . "' and cast(date_add(now(), interval -1 day) as date)  limit 1";
