...
if($_REQUEST['key']=="pfesic")
{
		if($_REQUEST)
		{
			...
			$Query="select a.EmployeeID,p.EmployeeName,c.mobile,a.doj,s.uan_no,s.esi_no from ActiveEmpID a inner join salary_details s on a.EmployeeID=s.EmployeeID inner join personal_details p on a.EmployeeID=p.EmployeeID inner join contact_details c on a.EmployeeID=c.EmployeeID where a.EmployeeID='".$_REQUEST['empid']."'";
