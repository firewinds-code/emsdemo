...
$sql = 'select EmployeeID from transfer_testEmp';
$selQ = $conn->prepare($sql);
$selQ->execute();
$resu = $selQ->get_result();

if ($resu->num_rows > 0) {
    foreach ($resu as $key => $values) {
        $emp = $values['EmployeeID'];

        $sqlConnect = 'select EmployeeID, location,old_loc from transfer_emp where EmployeeID=?';
        ...
        if ($result->num_rows > 0) {
            ...
            // die;
            $selQr = "select img from personal_details where EmployeeID=?";
            // $myDB = new MysqliDb();
            // $res = $myDB->query($selQr);
            $selQ = $conn->prepare($selQr);
            $selQ->bind_param("s", $EmployeeID);
