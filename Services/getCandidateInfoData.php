...
if ($iid != "" && $tempid != "" && $hrid != "" && $loc != "") {
	...
	if (count($data_array2) > 0) {
		foreach ($data_array2 as $key => $value) {
			...
			echo "<br>";
			$selcqry = $myDB->query("select INTID from vaccination_data where INTID='" . $iid . "'");
			if ($selcqry[0]['INTID'] != '') {
				echo $updateVerifiactionQuery = "Update  vaccination_data set EmpID='" . $tempid . "',EmpName='" . addslashes($fullname) . "', Vac1='" . $value->Vac1 . "',Vac2='" . $value->Vac2 . "',VacFile='" . $value->VacFile . "',CreatedOn='" . $value->CreatedOn . "',CreatedBy='" . $hrid . "',UpdatedOn='" . $value->UpdatedOn . "',NextSchedule='" . $value->NextSchedule . "' where INTID='" . $value->INTID . "' ";
			} else {
				echo $inserquery = "INSERT INTO vaccination_data set INTID='" . $value->INTID . "',EmpID='" . $tempid . "',EmpName='" . addslashes($fullname) . "', Vac1='" . $value->Vac1 . "',Vac2='" . $value->Vac2 . "',VacFile='" . $value->VacFile . "',CreatedOn='" . $value->CreatedOn . "',CreatedBy='" . $hrid . "',UpdatedOn='" . $value->UpdatedOn . "',NextSchedule='" . $value->NextSchedule . "'";
				$myDB->query($inserquery);
			}
			echo "<br>";
			...
			if ($value->sign_image != "") {
				$imsrc = CANDIDATE_INFO_URL . "Signature/" . $value->sign_image;
				$filename = $value->sign_image;
				$target_dir = ROOT_PATH . $dir_location . 'Docs/Other/';

				$target_file = $target_dir . $filename;
				if (copy($imsrc, $target_file)) {
					echo "Signature copied";
				} else {
					echo "Signature not copied";
				}
				echo "<br>";

				echo $insertSignData = "call manageDocData('Other','Signature','Signature','" . $value->sign_image . "','" . $tempid . "','" . $hrid . "','" . $iid . "','')";
				echo "<br>";
				$myDB = new MysqliDb();
				$myDB->query($insertSignData);
				if ($myDB->getLastError() != "") {
					echo 'MySql Error1 -: ' . $myDB->getLastError();
					echo "<br>";
				}
			...
			if ($value->edu_exp_file != '') {
				$imsrc = CANDIDATE_INFO_URL . "Edu_exception/" . $value->edu_exp_file;
				$filename = $value->edu_exp_file;
				$target_dir = ROOT_PATH . $dir_location . 'Docs/Edu_exception/';
				$target_file = $target_dir . $filename;
				if (copy($imsrc, $target_file)) {
					echo "Education exception file copied";
				} else {
					echo "Education exception file not copied";
				}
				echo "<br>";
				echo $insertResumeData = "call manageDocData('Education Exception','Education Exception','Education Exception','" . $value->edu_exp_file . "','" . $tempid . "','" . $hrid . "','" . $iid . "','')";
				echo "<br>";
				$myDB = new MysqliDb();
				$myDB->query($insertResumeData);
				if ($myDB->getLastError() != "") {
			...
			if ($value->VacFile != "") {
				$imsrc = CANDIDATE_INFO_URL . "Vacination/" . $value->VacFile;
				$filename = $value->VacFile;
				$target_dir = ROOT_PATH . $dir_location . 'Vacination/';
				$target_file = $target_dir . $filename;
				if (copy($imsrc, $target_file)) {
					echo "Vacination image copied";
				} else {
					echo "Vacination image not copied copied";
				}
				echo "<br>";
			}
	...
	$insertData = "call `manageAadharVerification`('" . $tempid . "','" . $iid . "')";
	$myDB = new MysqliDb();
	$myDB->query($insertData);
} else {
