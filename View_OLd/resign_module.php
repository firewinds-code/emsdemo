...
if (!empty($_POST['txt_search_empid'])) {
	$textsearch = cleanUserInput($_POST['txt_search_empid']);
	(isset($textsearch)) ? $EmployeeID = $textsearch : $EmployeeID = '';
}

$__user_logid = clean($_SESSION['__user_logid']);
$txt_search_empid = isset($_POST['txt_search_empid']);
if ($txt_search_empid) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		$txt_empid = cleanUserInput($_POST['txt_empid']);
		$nt_start  = cleanUserInput($_POST['txt_nt_start']);
		$nt_end  = cleanUserInput($_POST['txt_nt_end']);
		$remark = cleanUserInput($_POST['txt_search_remark']);
		$filename = '';
		...
		$srtddoQry = 'SELECT resign_details.EmployeeID from resign_details inner join employee_map on employee_map.EmployeeID = resign_details.EmployeeID where rg_status > 0 and rg_status < 9 and employee_map.emp_status = "Active" and resign_details.EmployeeID = ? and final_acceptance is null;';
		$stmt = $conn->prepare($srtddoQry);
		$stmt->bind_param("s", $txt_empid);
		$stmt->execute();
		$srtddo = $stmt->get_result();
		// $mysql_error = $myDB->getLastError();
		// $rowCount = $myDB->count;
		if ($srtddo->num_rows > 0 && $srtddo) {
			...
			$srtddoQry2 = 'SELECT resign_details.EmployeeID from resign_details inner join employee_map on employee_map.EmployeeID = resign_details.EmployeeID where employee_map.emp_status = "Active" and rg_status >= 9 and resign_details.EmployeeID = ? and last_day(resign_details.modifiedon) >= date_sub(date_format(NOW() ,\'%Y-%m-01\'), interval 3 month) and resign_details.modifiedon is not null';
			$stmt1 = $conn->prepare($srtddoQry2);
			$stmt1->bind_param("s", $txt_empid);
			$stmt1->execute();
			$srtddo2 = $stmt1->get_result();
			// $mysql_error = $myDB->getLastError();
			// $rowCount = $myDB->count;
			if ($srtddo2->num_rows > 0) {
					...
					if (file_exists(ROOT_PATH . 'Resign/' . $filename) && $filename != '') {
						...
						if ($years == 0 && $months == 0) {
							if ($days == 0 || $days == 1 || $days == 2 || $days == 3) {
								$sql = 'call manage_resign("' . $txt_empid . '","' . $remark . '","' . $filename . '","0","' . $nt_start . '","' . $nt_end . '","' . $__user_logid . '")';
								$myDB = new MysqliDb();
	...
	$(document).ready(function() {
		...
		$('#btn_save').click(function() {
			...
			if ($('#txt_search_remark').val() == '' || $('#txt_search_remark').val() == 'NA') {
				...
				if ($('#span_txt_search_remark').length == 0) {
