		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				<div class="input-field col s12 m12" id="rpt_container">
					...
					<div class="input-field col s4 m4">
						<Select name="txt_dept" id="txt_dept" required>
							...
							if ($user_type == 'ADMINISTRATOR' || $user_type == 'CENTRAL MIS' || $user_type == 'HR' || ($user_type == 'HR' &&  $status_ah == clean($_SESSION['__user_logid']))) {
								...
								$Getinfo = 'select distinct Process,cm_id from new_client_master where account_head=? or th=? or qh=? or oh=?';
								$selectQ = $connn->prepare($Getinfo);
								$selectQ->bind_param("ssss", $user, $user, $user, $user);
								$selectQ->execute();
								...
								if ($rowData->num_rows > 0) {
									...
									echo '<option >' . clean($_SESSION['__user_process']) . '</option>';
								}
							}
							?>
						</Select>
				...
				if ($btn_view) {
					...
					$myDB = new MysqliDb();
					$chk_task = $myDB->query('call apr_getappraisalReport("' . clean($_SESSION['__user_logid']) . '","' . $date_From . '","' . $date_To . '")');
