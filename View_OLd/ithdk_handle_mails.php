...
if (isset($_SESSION)) {
	...
	if (clean($_SESSION["__user_type"]) != 'ADMINISTRATOR' &&  $user_logid != 'CE10091236') {
		$location = URL . 'Login';
...
if (($btn_add_new)) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		$mailType = cleanUserInput($_POST['mailType']);
		$emailType = (isset($mailType) ? $mailType : null);
		$emailIT = cleanUserInput($_POST['emailIT']);
		$email = (isset($emailIT) ? $emailIT : null);
		$CogentC121 = cleanUserInput($_POST['CogentC121']);
		$locationC21 = (isset($CogentC121) ? $CogentC121 : null);
		$CogentC100s = cleanUserInput($_POST['CogentC100']);
		$CogentC100 = (isset($CogentC100s) ? $CogentC100s : null);
		$CogentA61s = cleanUserInput($_POST['CogentA61']);
		$CogentA61 = (isset($CogentA61s) ? $CogentA61s : null);
		$BangaloreGopalnas = cleanUserInput($_POST['BangaloreGopalna']);
		$BangaloreGopalna = (isset($BangaloreGopalnas) ? $BangaloreGopalnas : null);
		$BangaloreHebbals = cleanUserInput($_POST['BangaloreHebbal']);
		$BangaloreHebbal = (isset($BangaloreHebbals) ? $BangaloreHebbals : null);
		$Vadodaras = cleanUserInput($_POST['Vadodara']);
		$Vadodara = (isset($Vadodaras) ? $Vadodaras : null);
		$MangaloreRajTowers = cleanUserInput($_POST['MangaloreRajTower']);
		$MangaloreRajTower = (isset($MangaloreRajTowers) ? $MangaloreRajTowers : null);
		$MangaloreFortunes = cleanUserInput($_POST['MangaloreFortune']);
		$MangaloreFortune = (isset($MangaloreFortunes) ? $MangaloreFortunes : null);
		$Meeruts = cleanUserInput($_POST['Meerut']);
		$Meerut = (isset($Meeruts) ? $Meeruts : null);
		$Bareillys = cleanUserInput($_POST['Bareilly']);
		$Bareilly = (isset($Bareillys) ? $Bareillys : null);
...
<div id="content" class="content">
	...
	<div class="pim-container row" id="div_main">
		...
		<div class="form-div">
			...
			<h4>Handle Emails
				<a class="btn-floating btn-large waves-effect waves-light modal-trigger tooltipped" id="ContentAdd" onclick="javascript:return openModelToAdd();" data-position="bottom" data-tooltip="Add Email"><i class="material-icons">add</i></a>
			</h4>
		...
		if (Cnfm) {
			...
			xmlhttp.onreadystatechange = function() {
				if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
					...
					window.location.replace("../View/ithdk_handle_mails.php");
