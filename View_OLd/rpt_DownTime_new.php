		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				<div class="col s12 m12" id="rpt_container">
					...
					<div class="input-field col s4 m4">
						...
						<Select class="form-control" name="emp_status" style="min-width: 200px;" id="status">
							<option value='Active' <?php $emp_status = clean($_POST['emp_status']);
													if (isset($emp_status) && $emp_status == 'Active') {
														echo "selected";
													} ?>>Active</option>
							<option value='InActive' <?php $emp_status = clean($_POST['emp_status']);
					...
					<div class="input-field col s6 m6 l6">
						<select id="clientName" name="clientName">
							...
							$Query = 'select distinct t1.client_name ID,t3.client_name from new_client_master t1 join report_map t2 on t1.cm_id=t2.processID join client_master t3 on t1.client_name=t3.client_id where t2.EmpID=? and reportID=6';
				...
				if (isset($_POST['btn_view'])) {
					...
					$empid = clean($_SESSION['__user_logid']);
					$chk_task  =  $myDB->query('call sp_getDawnTime_Report_new("' . $empid . '","' . $date_From . '","' . $date_To . '","' . $empStatus . '","' . $clientname . '")');
					$my_error = $myDB->getLastError();
