...
if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
	$clean_ddf_process = cleanUserInput($_POST['ddl_clfs_Process']);
}
if (isset($clean_ddf_process)) {
	$process = $clean_ddf_processs;
}
			...
			<div class="schema-form-section row">
				...
				if (isset($_POST['btn_view'])) {
					...
					if ($clean_ddf_process == 'ALL') {
						...
						$selectQury = $conn->prepare($select);
						$selectQury->bind_param("ss", $clean_ddf_process, date("F Y", strtotime($DateTo)));
						$selectQury->execute();
						$chk_task = $selectQury->get_result();
					}
