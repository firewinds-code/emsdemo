...
if (isset($_SESSION)) {
	if (!isset($user_logid)) {
		...
		if ($user_logid == '' || $user_logid == null) {
			...
			exit();
		} else if (!(clean($_SESSION["__ut_temp_check"]) == 'COMPLIANCE' || clean($_SESSION["__user_type"]) == 'ADMINISTRATOR' || clean($_SESSION['__user_logid']) == 'CE12102224')) {
	...
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		...
		if ($uploadOk == 0) {
			...
			if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
							...
							for ($i = $begin; $begin <= $end; $i->modify('+1 day')) {
								...
								if ($flag != 0) {
									...
									if (strtotime($date1) > strtotime($date2)) {
										$query = 'select t1.EmployeeID, designation, windowstart,windowend,t3.work_from from whole_dump_emp_data t1 inner join process_window t2 on t1.cm_id=t2.cm_id inner join roster_temp t3 on t1.EmployeeID=t3.EmployeeID where t1.EmployeeID= ? and t3.DateOn= ? ';
										$select = $conn->prepare($query);
										$select->bind_param("ss", strtoupper($row['A']), $date2);
										$select->execute();
										$res = $select->get_result();
										$ds_APR = $res->fetch_row();

										if ($res->num_rows > 0 && $res) {
											if ($ds_APR[1] == 'CSA' && $ds_APR[4] != '') {
												$url = 'http://10.147.20.14/apr_processing/user_apr.php?apr_date=' . $date2 . '&username=' . strtoupper($row['A']) . '&intime=' . $time_inout1 . '&outtime=' . $time_inout2 . '&WorkFrom=' . $ds_APR[4] . '&windowstart=' . $ds_APR[2] . '&windowend=' . $ds_APR[3];
												//echo $url;
												$curl = curl_init();
				...
				} else if (cleanUserInput($_POST['txt_Type_Upload']) == "Back Date Roster") {
					$row_counter = 0;
					foreach ($activeSheetData as $row) {
						if ($row_counter > 0 && !empty($row['A']) && $row['A'] != '') {
							...
							if ($rst_insert_bkd != 0) {
								...
								$date2 = $DateOn;
								if (strtotime($date1) > strtotime($date2)) {
									$sql = 'select t1.EmployeeID, designation, windowstart,windowend,t3.work_from from whole_dump_emp_data t1 inner join process_window t2 on t1.cm_id=t2.cm_id inner join roster_temp t3 on t1.EmployeeID=t3.EmployeeID where t1.EmployeeID= ? and t3.DateOn= ? ';
									$sel = $conn->prepare($sql);
									$sel->bind_param("ss", $EmployeeID, $date2);
									$sel->execute();
									...
									if (count($ds_APR) > 0 && $ds_APR) {
										if ($ds_APR[1] == 'CSA' && $ds_APR[4] != '') {
											$url = 'http://10.147.20.14/apr_processing/user_apr.php?apr_date=' . $date2 . '&username=' . $EmployeeID . '&intime=' . $InTime . '&outtime=' . $OutTime . '&WorkFrom=' . $ds_APR[4] . '&windowstart=' . $ds_APR[2] . '&windowend=' . $ds_APR[3];
											//echo $url;
											$curl = curl_init();
											curl_setopt($curl, CURLOPT_URL, $url);
											curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
				...
				if (file_exists($target_dir . basename($_FILES["fileToUpload"]["name"]))) {
					...
					rename($target_file, $target_dir . time() . '_' . $uploader . "_Roster_" . $txt_Type_Upload . "." . $ext);
				}
			} else {
