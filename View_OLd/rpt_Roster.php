...
if (time() >= strtotime('08:30:00') && time() <= strtotime('10:30:00')) {
	$location = URL;
...
<div id="content" class="content">
	...
	<div class="pim-container row" id="div_main">
		...
		<div class="form-div">
			...
			<h4>Roster Report <a class="btn-floating btn-large waves-effect waves-light modal-trigger tooltipped" id="ContentAdd" href="#myModal_content" data-position="bottom" data-tooltip="Filter"><i class="material-icons">ohrm_filter</i></a></h4>
			...
			<div class="schema-form-section row">
					...
					<div class="modal-content">
						...
						<div class="modal-body">
							...
							<div class="input-field col s6 m6">
								...
								<Select name="txt_dept" style="min-width: 175px;" id="txt_dept">
									<?php
									if (clean($_SESSION['__user_type']) == 'ADMINISTRATOR' || clean($_SESSION['__user_type']) == 'CENTRAL MIS') {
										$myDB = new MysqliDb();
										...
										$data = 'select distinct Process from new_client_master where account_head=? or qh=? or oh=? or th=?';
										$select = $conn->prepare($data);
										$select->bind_param("ssss", $userid, $userid, $userid, $userid);
										$select->execute();
										$rowData = $select->get_result();
							...
							<div class="input-field col s12 m12 right-align">
								...
								<?php if (clean($_SESSION['__user_type']) == 'HR' || clean($_SESSION['__user_type']) == 'ADMINISTRATOR' || clean($_SESSION['__user_type']) == 'CENTRAL MIS') {  ?>
				...
				if ($monthCount != "") {
					if (($dateLimit < $date_selected)) {
						...
						$chk_task = 'call sp_get_roster_Report("' . clean($_SESSION['__user_logid']) . '","' . $date_To . '","' . $date_From . '","' . $dept . '","' . $EmpStatus . '","' . clean($_SESSION["__location"]) . '")';
					} else {
						$viewTable = 'roster_history_dump';
						$chk_task = 'call sp_get_roster_history_Report("' . clean($_SESSION['__user_logid']) . '","' . $date_To . '","' . $date_From . '","' . $dept . '","' . $EmpStatus . '")';
					}
					//echo $chk_task; die;
...
<script>
	...
	function downloadexcel(el) {
		...
		var sp = "call sp_get_roster_Report('" + usrid + "','" + date_to + "','" + date_from + "','" + dept + "','" + status + "','" + loc + "')";
