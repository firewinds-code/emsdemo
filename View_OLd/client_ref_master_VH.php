...
if ($btn_RefMaster_Edit) {
    if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
        ...
        $approver = cleanUserInput($_POST['approver']);
            ...
            if (empty($mysql_error)) {
                if ($myDB->count > 0) {
                    ...
                    if ($approver == "Approve") {
                        ...
                        $UserLogin = clean($_SESSION['__user_logid']);
                        $insertQuery = "insert into ref_amount_master(RefID,cm_id,amount,1st_pay,2nd_pay,window_month,emp_type,ApplicableFrom,ApplicableTo,createdby)values(?,?,?,?,?,?,?,?,?,?)";

                        $stmt = $conn->prepare($insertQuery);
                        $stmt->bind_param("ssssssssss", $DataID, $_cm_id, $_RefAmount, $_FPay, $_SPay, $_WinMonth, $emp_type, $_from, $_to, $UserLogin);
                        if (!$stmt) {
                            echo "failed to run";
                            die;
                        }
        ...
        // $insert = "insert into ref_amount_master(RefID,cm_id,amount,1st_pay,2nd_pay,window_month,ApplicableFrom,ApplicableTo,createdby)values('" . $DataID . "','" . $_cm_id . "','" . $_RefAmount . "','" . $_FPay . "','" . $_SPay . "','" . $_WinMonth . "','" . $_from . "','" . $_to . "','" . clean($_SESSION['__user_logid']) . "')";
        ...
        <div class="form-div">
            ...
            <div class="schema-form-section row">
                ...
                <div id="pnlTable">
                    ...
                    $flag = 0;
                    $sqlConnect = "select t2.id,t2.cm_id,t1.process,t1.sub_process,t2.amount,t2.ApplicableFrom,t2.ApplicableTo,t2.1st_pay,t2.2nd_pay,t2.window_month,t2.emp_type,t2.createdby,t2.createdon,t1.location,t3.client_name,t4.location as locname,flag from new_client_master t1 join client_ref_master t2 on t1.cm_id= t2.cm_id join client_master t3 on t1.client_name= t3.client_id join location_master t4 on t4.id=t1.location where flag=? and t1.cm_id not in (select cm_id from client_status_master) ";
