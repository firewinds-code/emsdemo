...
$createBy = $clean_u_login;
$EmployeeID = $clean_u_login;
...
if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
	if (isset($_POST["submit"]) && $clean_employeeid != "") {

		// print_r($_POST);
		// die;
		$EmployeeName = cleanUserInput($_POST['EmployeeName']);
		$Email = cleanUserInput($_POST['Email']);
		$Contactno = cleanUserInput($_POST['Contactno']);
		$Doj = cleanUserInput($_POST['Doj']);
		$Designation = cleanUserInput($_POST['Designation']);
		$ImmediateManager = cleanUserInput($_POST['ImmediateManager']);
		$Assignment = cleanUserInput($_POST['Assignment']);
		$linkdinLink = cleanUserInput($_POST['linkdinLink']);
		// $abc = $createBy;
		$ID = cleanUserInput($_POST['ID']);
		$sqlResponse = "UPDATE mail_template set `name` =? , `email`=?, `contact_no`=?, `doj`=?, `designation`=?, `immediate_manager`=?, `assignment`= ?,`linkdinLink`=?,flag=1, ModifiedBy = ?, Modifiedon = now() where id = ?";
		$stmt = $conn->prepare($sqlResponse);
		$stmt->bind_param("ssssssssss", $EmployeeName, $Email, $Contactno, $Doj, $Designation, $ImmediateManager, $Assignment, $linkdinLink, $createBy, $ID);
		$updt = $stmt->execute();
		// $myDB =  new MysqliDb();
		// $Results = $myDB->rawQuery($sqlResponse);
		$mysql_error = $myDB->getLastError();
		$EmployeeID = cleanUserInput($_POST['EmployeeID']);
		$sqlResponse = "UPDATE contact_details set mobile= ?, `ofc_emailid`=? where EmployeeID = ?";
		$stmt = $conn->prepare($sqlResponse);
		$stmt->bind_param("sss", $Contactno, $Email, $EmployeeID);
		$updt = $stmt->execute();
		// $myDB =  new MysqliDb();
		// $Results = $myDB->rawQuery($sqlResponse);
		$mysql_error = $myDB->getLastError();
        ...
        <div class="form-div">
            ...
            <div class="schema-form-section row">
                ...
                <form method="POST" name="" action="<?php echo URL . 'View/mail_template.php'; ?>">
                    ...
                    <div class="row">
                        <div class="col s12 center"><input type="submit" class="btn" name="submit" id="submit"
                                value="Submit"></div>
                    </div>
                </form>
                <?php
