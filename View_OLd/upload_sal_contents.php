...
if (isset($_SESSION)) {
	...
	if (!isset($clean_u_logid)) {
		...
		if (!($clean_u_logid == 'CE03070003' || $clean_u_logid == 'CE10091236')) {
		...
		if ($uploadOk == 0) {
			...
			if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
				...
				if (count($activeSheetData) > 0 && $activeSheetData) {
					foreach ($activeSheetData as $row) {
						if ($row_counter > 0 && !empty($row['A']) && $row['A'] != '') {
							...
							$J = clean($row['J']);
							$clean_u_logid = clean($_SESSION['__user_logid']);
							$__user_logid = $clean_u_logid;
							$sql_insert = '';
							$clean_type_upload = cleanUserInput($_POST['txt_Type_Upload']);
							if ($clean_type_upload === 'Incentive') {
								$sql_insert =  'INSERT INTO payroll_incentive_upld(EmployeeID,month,year,ClientIncentive,Arrears,OtherIncentive,OtherNarration,createdby)VALUES(?,?,?,?,?,?,?,?)';

								$stmt = $conn->prepare($sql_insert);
								$stmt->bind_param("ssssssss", $A, $B, $C, $D, $E, $F, $G, $__user_logid);
								$inst = $stmt->execute();
								// 
							} elseif ($clean_type_upload === 'Deduction') {
								$sql_insert =  'INSERT INTO payroll_deduction_upld(EmployeeID,month,year,AssetDamage,Id_Access_Card_Damage,Guest_House_Rent,TDS,NoticeRecovery,OtherLess,OtherNarration,createdby)VALUES(?,?,?,?,?,?,?,?,?,?,?)';
								$stmt = $conn->prepare($sql_insert);
								$stmt->bind_param("sssssssssss", $A, $B, $C, $D, $E, $F, $G, $H, $I, $J, $__user_logid);
								$inst = $stmt->execute();


								// 
							} elseif ($clean_type_upload === 'Status') {
								$E = addslashes($row['E']);
								$sql_insert =  'INSERT INTO payroll_final_sl_status (EmployeeID,month,year,SalaryStatus,Remarks,createdby)VALUES(?,?,?,?,?,?)';

								$stmt = $conn->prepare($sql_insert);
								$stmt->bind_param("ssssss", $A, $B, $C, $D, $E, $__user_logid);
								$inst = $stmt->execute();
				...
				if (file_exists($target_dir . basename($_FILES["fileToUpload"]["name"]))) {
					...
					$clean_type_upload = cleanUserInput($_POST['txt_Type_Upload']);
					rename($target_file, $target_dir . time() . '_' . $uploader . "_PayrollNarration_" . $clean_type_upload . "." . $ext);
				}
			} else {
