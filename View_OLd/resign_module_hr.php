			...
			if ($count_check > 0) {
				...
				foreach ($_POST['cb'] as $val) {
					...
					if ($Status != 'NA' || $Status != 'Na') {
						...
						$sql = 'update resign_details set status_hr = "1" where EmployeeID = ?';
						...
						if ($upQ->affected_rows === 1) {
							...
							// $myDB = new MysqliDb();
							$sql1 = 'INSERT INTO resign_comment_log(`EmployeeID`,`CreatedBy`,`CreatedOn`,`Comment`,`type`)VALUES(?,?,now(),?,"HR")';
							$insQ = $conn->prepare($sql1);
							$insQ->bind_param("sss", $EmpID, $createBy, $Remark);
							$insQ->execute();
							$rstdt = $insQ->get_result();
...
<div id="content" class="content">
	...
	<span id="PageTittle_span" class="hidden">Employee transfer to HR - Resigned [HR] </span>
	...
	<div class="pim-container ">
		...
		<div class="form-div">
			...
			<h4>Employee transfer to HR - Resigned [HR] </h4>
			...
			<div class="schema-form-section row">
				...
				<div id="pnlTable">
					...
					if ($location == "1") {
						$chktask = 'select wh.EmployeeID,wh.EmployeeName,rs.remark,rs.nt_start,rs.rg_status ,rs.nt_end,wh.Process,wh.sub_process,wh.clientname,pd1.img from resign_details  rs inner join whole_details_peremp wh on wh.EmployeeID = rs.EmployeeID left outer join personal_details pd1 on pd1.EmployeeID = rs.EmployeeID left outer join personal_details pd on pd.EmployeeID = ReportTo where (curdate() between cast(DATE_SUB(rs.nt_end, INTERVAL 3 DAY) as date)  and cast(rs.nt_end as date)) and  (select true from whole_details_peremp where cm_id  = "37" and des_id in  (1,5,7,8,10) and EmployeeID = ?) and (status_hr  is null or status_hr = 0) and nt_end >= curdate() and rg_status < 9 and wh.location in (1,2)';
						$selectQ = $conn->prepare($chktask);
						$selectQ->bind_param("s", $userlogid);
						$selectQ->execute();
						$chk_task = $selectQ->get_result();
					} else {
						$chktask = 'select wh.EmployeeID,wh.EmployeeName,rs.remark,rs.nt_start,rs.rg_status ,rs.nt_end,wh.Process,wh.sub_process,wh.clientname,pd1.img from resign_details  rs inner join whole_details_peremp wh on wh.EmployeeID = rs.EmployeeID left outer join personal_details pd1 on pd1.EmployeeID = rs.EmployeeID left outer join personal_details pd on pd.EmployeeID = ReportTo where (curdate() between cast(DATE_SUB(rs.nt_end, INTERVAL 3 DAY) as date)  and cast(rs.nt_end as date)) and  (select true from whole_details_peremp where cm_id  = "37" and des_id in  (1,5,7,8,10) and EmployeeID = ?) and (status_hr  is null or status_hr = 0) and nt_end >= curdate() and rg_status < 9 and wh.location in (?)';
						$selectQ = $conn->prepare($chktask);
						$selectQ->bind_param("si", $userlogid, $location);
						$selectQ->execute();
						$chk_task = $selectQ->get_result();
					}
