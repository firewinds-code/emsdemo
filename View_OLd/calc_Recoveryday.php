...
if (count($chk_task) > 0 && $chk_task) {
	foreach ($chk_task as $key => $value) {
		...
		$resdataQry = "select t1.nt_start,t1.nt_end,t2.rt_type from resign_details t1 join salary_details t2 on t1.EmployeeID = t2.EmployeeID where t1.EmployeeID=?";
		$stmt = $conn->prepare($resdataQry);
		$stmt->bind_param("s", $EmployeeID);
		$stmt->execute();
		$resdata = $stmt->get_result();
		$resdataRow = $resdata->fetch_row();
		// print_r($resdata);
		// die;
		if ($resdata->num_rows > 0 && $resdata) {
			/*$resstart=$resdata[0]['nt_start'];
				$resend=$resdata[0]['nt_end'];*/
			$result_array = getRecoveryDay($value['EmployeeID'], $resdataRow[0], $resdataRow[1], $resdataRow[2]);
			// echo "<pre>";
			// print_r($result_array);
			// die;
			$myDB = new MysqliDb();

			$sqlInsertrecday = 'call sp_InsertRecovery("' . $value['EmployeeID'] . '","' . $resdataRow[0] . '","' . $resdataRow[1] . '","' . $result_array[1] . '","' . $result_array[0] . '","' . $result_array[2] . '","' . date('m', strtotime($resdataRow[1])) . '","' . date('Y', strtotime($resdataRow[1])) . '","' . $resdataRow[2] . '")';
