	...
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		...
		if ($uploadOk == 0) {
			...
			if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
				...
				$clean_tyype_upload = cleanUserInput($_POST['txt_Type_Upload']);
				if ($clean_tyype_upload == 'Next Week') // && (date('l',time()) =='Friday' or date('l',time()) =='Saturday'))
				{
					...
					foreach ($activeSheetData as $row) {
						if ($row_counter > 0 && !empty($row['A']) && $row['A'] != '') {
							...
							$daycount = 1;
							// $myDB = new MysqliDb();
							// echo "select FirstPre,SecondPre from rosterpref where EmpID ='" . $row['A'] . "' and WeekNo ='" . $dt_First . " To " . $dt_Last . "' order by id desc ";
							...
							$stmt->bind_param("ss", $EmpID, $WEEKNO);
							if (!$stmt) {
								echo "failed to run";
								die;
							}
							...
							$EmployeeID = $row['A'];
							$Query_check_roster = "select rt_type from salary_details where EmployeeID =? limit 1";

							$stmt = $conn->prepare($Query_check_roster);
							...
							$stmt->execute();
							$check_roster = $stmt->get_result();
							$row = $check_roster->fetch_row();
							$count = $check_roster->num_rows;


							//if (count($check_roster) > 0) {
							if ($count > 0) {
								//$check_roster = $check_roster[0]['rt_type'];
								$check_roster = clean($row[0]);
							...
							$EmployeeID = $row['A'];
							$QuerycheckGender = "select Gender from personal_details where EmployeeID =? limit 1";

							$stmt = $conn->prepare($QuerycheckGender);
							$stmt->bind_param("s", $EmployeeID);
							if (!$stmt) {
								echo "failed to run";
								die;
							...
							$stmt->execute();
							$checkGender = $stmt->get_result();
							$row = $checkGender->fetch_row();
							$count = $checkGender->num_rows;
							...
							$EmployeeID = $row['A'];
							$QuerycheckAccount = "select account_head,Process,sub_process from employee_map inner join new_client_master on new_client_master.cm_id = employee_map.cm_id where EmployeeID =? limit 1";

							$stmt = $conn->prepare($QuerycheckAccount);
							$stmt->bind_param("s", $EmployeeID);
							if (!$stmt) {
								echo "failed to run";
								die;
							...
							$stmt->execute();
							$checkAccount = $stmt->get_result();
							$row = $checkAccount->fetch_row();
							...
							if ($count > 0) {
								...
								$account_head = clean($row[0]);
								$process_macth = clean($row[1]) . '-' . clean($row[2]);
							}
							if ($account_head == $uploader && ($process_macth == $dept || $dept == 'ALL Process')) {
								...
								for ($i = $begin; $begin <= $end; $i->modify('+1 day')) {
									...
									$roster_type = 1;
									/*$str = 'call InsertRoaster_New("'.strtoupper($row['A']).'","'.$col.'","'.$val.'","'.$clean_u_logid.'","'.date('Y-m-d').'","'.$month_row['month'].'","'.$row['J'].'")';*/
					...
					<div class="col s12 m12">
						<div class="input-field col s12 m12">
							<Select name="txt_dept" id="txt_dept">
								<?php
								// $myDB = new MysqliDb();
								// $rowData = $myDB->query('select distinct Process,sub_process from new_client_master where account_head="' . $clean_u_logid . '"');
								$clean_u_logid = clean($_SESSION['__user_logid']);
								$UserLoginID = $clean_u_logid;
