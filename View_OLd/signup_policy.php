...
$clean_u_logid = cleanUserInput($_SESSION['__user_logid']);
$EmployeeID = strtoupper($clean_u_logid);
...
if (isset($_POST['btnSave'])) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		...
		if ($clean_empid != "") {
			...
			$selectQ->bind_param("s", $clean_u_logid);
			$selectQ->execute();
			$rscovidack = $selectQ->get_result();

			// $rscovidack = $myDB->query($covidQuery);
			if ($rscovidack->num_rows < 1) {
				$query = "Insert into `signup_policy_ack` set  EmployeeID= ? ";
				$insert = $conn->prepare($query);
				$insert->bind_param("s", $clean_empid);
				$insert->execute();
	...
	<div class="pim-container row" id="div_main">
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				<p>I <?php $clean_u_name = clean($_SESSION['__user_Name']);
						echo $clean_u_name; ?> , do hereby declare that I have fully read and understood the Code of Conduct policy of Cogent E Services and agree to comply to the same. I understand that any non-compliance to the above policies may lead to disciplinary sanctions that can include up to termination of employment and even criminal prosecution under applicable laws.</p>
				<input type='hidden' name='empID' id="empID" value="<?php echo $clean_u_logid; ?>">
