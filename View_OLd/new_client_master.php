...
if ($btn_client) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		...
		$_txt_QualityID = (isset($_POST['txt_QualityID']) ? $_POST['txt_QualityID'] : null);
		$_txt_TrainingID = (isset($_POST['txt_TrainingID']) ? $_POST['txt_TrainingID'] : null);
		$_txt_OpsID = (isset($_POST['txt_OpsID']) ? $_POST['txt_OpsID'] : null);
		...
		if ($_HRID != "" && $_ITID != "" && $_ReportsTo != "") {
			...
			$Insert = 'CALL add_client_new("' . trim($_Name) . '","' . $_ach . '","' . $_dept . '","' . trim($_proc) . '","' . $_oh . '","' . $_qh . '","' . $_th . '","' . trim($_subproc) . '","' . $createBy . '","' . $_HRID . '","' . $_ITID . '","' . $_ReportsTo . '","' . $sitespoc . '","' . $_Stipend . '","' . $_StipendDays . '","' . trim($_er_scop) . '","' . trim($_er_spoc2) . '","' . trim($_er_spoc3) . '","' . trim($_from_joiningdate) . '","' . trim($_from_floordate) . '","' . trim($_rotation_date) . '","' . $txt_vertical_head . '","' . $_location . '","' . $EmpID . '","' . $_txt_QualityID . '","' . $_txt_TrainingID . '","' . $_txt_OpsID . '")';
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				?>-->
						...
						$resultBy=$myDB->rawQuery($sqlBy);*/
									?>
									<select id="txt_ERSPOC2" name="txt_ERSPOC2" required>
										<!-- <option value="">----Select----</option>	
						<?php
						if ($resultBy) {
							foreach ($resultBy as $key => $value) {
						...
						?> -->
									...
									<label for="txt_ERSPOC2" class="active-drop-down active">ER SPOC 2</label>
								</div>
						...
						$resultBy=$myDB->rawQuery($sqlBy);*/
									?>
									<select id="txt_ERSPOC3" name="txt_ERSPOC3" required>
										<!-- <option value="">----Select----</option>	
						<?php
						if ($resultBy) {
							foreach ($resultBy as $key => $value) {
						...
						?> -->
									...
									<label for="txt_ERSPOC3" class="active-drop-down active">ER SPOC 3</label>
								</div>
							...
							<div class="col s12 m12">
								<div class="input-field col s4 m4">
									<select id="txt_QualityID" name="txt_QualityID" required> </select>
									<label for="txt_QualityID" class="active-drop-down active">Downtime Quality</label>
								</div>
								<div class="input-field col s4 m4">
									<select id="txt_TrainingID" name="txt_TrainingID" required> </select>
									<label for="txt_TrainingID" class="active-drop-down active">Downtime Training</label>
								</div>
								<div class="input-field col s4 m4">
									<select id="txt_OpsID" name="txt_OpsID" required> </select>
									<label for="txt_OpsID" class="active-drop-down active">Downtime Ops</label>
								</div>
							</div>
						...
						<table id="myTable" class="data dataTable no-footer row-border" cellspacing="0" width="100%">
							<thead>
								<tr>
									...
									<th class="hidden">ER scop 2</th>
									<th class="hidden">ER scop 3</th>
									<th>Sub Process</th>
									<th>Location</th>
									<th>Manage Client</th>
									<th>Create New</th>
									...
									<th class="hidden">dtqa</th>
									<th class="hidden">dttr</th>
									<th class="hidden">dtops</th>
							...
							<tbody>
								...
								foreach ($result as $key => $value) {
									...
									echo '<td class="er_spoc2 hidden ">' . $value['er_spoc2'] . '</td>';
									echo '<td class="er_spoc3 hidden ">' . $value['er_spoc3'] . '</td>';
									...
									echo '<td class="dtqa hidden">' . $value['QualityID'] . '</td>';
									echo '<td class="dttr hidden">' . $value['TrainingID'] . '</td>';
									echo '<td class="dtops hidden">' . $value['OpsID'] . '</td>';
...
<script>
	$(document).ready(function() {
		...
		$('#btn_Client_Edit,#btn_Client_Save').on('click', function() {
			...
			if ($('#txt_ERSPOC2').val() == 'NA') {
				$('#txt_ERSPOC2').parent('.select-wrapper').find('input.select-dropdown').addClass("has-error");
				if ($('#spantxt_ERSPOC2').size() == 0) {
					$('<span id="spantxt_ERSPOC2" class="help-block">Required *</span>').insertAfter('#txt_ERSPOC2');
				}
				validate = 1;
			}
			if ($('#txt_ERSPOC3').val() == 'NA') {
				$('#txt_ERSPOC3').parent('.select-wrapper').find('input.select-dropdown').addClass("has-error");
				if ($('#spantxt_ERSPOC3').size() == 0) {
					$('<span id="spantxt_ERSPOC3" class="help-block">Required *</span>').insertAfter('#txt_ERSPOC3');
				}
				validate = 1;
			}
	...
	function EditData(el) {
		...
		var er_spoc2 = tr.find('.er_spoc2').text();
		var er_spoc3 = tr.find('.er_spoc3').text();
		...
		var dt_qa = tr.find('.dtqa').text();
		var dt_tr = tr.find('.dttr').text();
		var dt_ops = tr.find('.dtops').text();
		...
		getProcess(locid, account_head, VH, oh, qh, th, Exception, SiteSpoc, er_scop, er_spoc2, er_spoc3, dt_qa, dt_tr, dt_ops);
		...
		$('#txt_ERSPOC2').val(er_spoc2);
		$('#txt_ERSPOC3').val(er_spoc3);
		//alert($('#txt_ERSPOC').val());
	...
	function getProcess(el, ach, VH, oh, qh, th, Exception, SiteSpoc, er_scop, er_spoc2, er_spoc3, dtqa, dttr, dtops) {
		...
		$.ajax({
			url: "../Controller/getalignmentByLocation.php?loc=" + $('#txt_location').val() + "&type=excep&val=" + dtqa,
			success: function(result) {
				//alert(result);
				$("#txt_QualityID").html(result);
				$('select').formSelect();
			}
		});

		$.ajax({
			url: "../Controller/getalignmentByLocation.php?loc=" + $('#txt_location').val() + "&type=excep&val=" + dttr,
			success: function(result) {
				//alert(result);
				$("#txt_TrainingID").html(result);
				$('select').formSelect();
			}
		});

		$.ajax({
			url: "../Controller/getalignmentByLocation.php?loc=" + $('#txt_location').val() + "&type=excep&val=" + dtops,
			success: function(result) {
				//alert(result);
				$("#txt_OpsID").html(result);
		...
		$.ajax({
			url: "../Controller/getalignmentByLocation.php?loc=" + $('#txt_location').val() + "&type=site&val=" + SiteSpoc,
			success: function(result) {
				//alert(result);
				$("#txt_Site_Spoc").html(result);
				// binddate1(result.trim());
				$('select').formSelect();
			}
		});
		...
		$.ajax({
			url: "../Controller/getalignmentByLocation.php?loc=" + $('#txt_location').val() + "&type=hr&val=" + er_spoc2,
			success: function(result) {
				//alert(result);
				$("#txt_ERSPOC2").html(result);
		...
		$.ajax({
			url: "../Controller/getalignmentByLocation.php?loc=" + $('#txt_location').val() + "&type=hr&val=" + er_spoc3,
			success: function(result) {
				//alert(result);

				$("#txt_ERSPOC3").html(result);
