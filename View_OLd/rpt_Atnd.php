	...
	<div class="pim-container row" id="div_main">
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
						...
						<div class="modal-body">
							...
							<div class="input-field col s4 m4">
								<Select name="txt_dept" id="txt_dept" required>
									<?php
									if ($__user_type == 'ADMINISTRATOR' || $__user_type == 'CENTRAL MIS' || $__user_type == 'HR' || ($__user_type == 'HR' &&  $__status_ah == $emp_id) || $emp_id == 'CE081930104') {
										...
										$data = 'select distinct Process from new_client_master where account_head=? or th=? or qh=? or oh=?  or VH=? ';
										$selectQ = $conn->prepare($data);
										$selectQ->bind_param("sssss", $emp_id, $emp_id, $emp_id, $emp_id, $emp_id);
										$selectQ->execute();
							...
							<div class="input-field col s12 m12 right-align">
								...
								<?php if ($__user_type == 'HR' || $__user_type == 'ADMINISTRATOR' || $__user_type == 'CENTRAL MIS') {  ?>
				...
				$myDB = new MysqliDb();
				$type = '';
				if (($__user_type == 'HR' &&  ($__status_ah == $emp_id || $__user_type == 'HR'))) {
					$type = 'ADMINISTRATOR';
				} else {
					$type = $__user_type;
				}
				//echo 'call sp_get_atnd_Report("'.$emp_id.'","'.$date_To.'","'.$date_From.'","'.$dept.'","'.$type.'","'.$_SESSION["__location"].'")'; 
				$loc = '';
				$locat = ($_SESSION["__location"]);
				if (isset($locat)) {
					$loc = clean($_SESSION["__location"]);
				}
				$chk_task = $myDB->query('call sp_get_atnd_Report("' . $emp_id . '","' . $date_To . '","' . $date_From . '","' . $dept . '","' . $type . '","' . $loc . '")');
				// echo 'call sp_get_atnd_Report("' . $emp_id . '","' . $date_To . '","' . $date_From . '","' . $dept . '","' . $type . '","' . $loc . '")';
...
<script>
	...
	function downloadexcel(el) {
		...
		$item = $(el);
		//call sp_get_atnd_Report("' . $emp_id . '","' . $date_To . '","' . $date_From . '","' . $dept . '","' . $type . '","' . $loc . '")
