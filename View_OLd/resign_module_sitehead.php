			...
			if ($count_check > 0) {
				...
				foreach ($checked_arr as $val) {
					...
					if ($Status != 'NA' || $Status != 'Na') {
						...
						// $sql = 'update resign_details set status_sitehead = "1" where EmployeeID = "' . $EmpID . '"';
						$sql = 'update resign_details set status_sitehead = "1" where EmployeeID = ?';
						...
						if ($resulti->num_rows > 0) {
							...
							// $sql1 = 'INSERT INTO resign_comment_log(`EmployeeID`,`CreatedBy`,`CreatedOn`,`Comment`,`type`)VALUES("' . $EmpID . '","' . $createBy . '",now(),"' . $Remark . '","SH")';
							$sql1 = 'INSERT INTO resign_comment_log(`EmployeeID`,`CreatedBy`,`CreatedOn`,`Comment`,`type`)VALUES(?,?,now(),?,"SH")';
							$stmt1 = $conn->prepare($sql1);
							$stmt1->bind_param("s", $EmpID, $createBy, $Remark);
							$stmt1->execute();
							$rstdt = $stmt1->get_result();
						} else {
			...
			<div class="schema-form-section row">
				...
				<div id="pnlTable">
					<div class="had-container pull-left row card dataTableInline">
						<div class="">
							<?php
							// $chk_task = $myDB->rawQuery('select rs.id, rs.EmployeeID,wd.EmployeeName,rs.remark, rs.file, rs.rg_status, rs.nt_start, rs.nt_end, rs.accept_time, rs.accept, rs.revoke_accept,rs.revoke_status, rs.accepter_remark, rs.accepter, rs.status_ah, rs.status_oh, rs.status_sitehead, rs.status_it, rs.status_hr, rs.final_acceptance, rs.createdon, rs.createdby, rs.revoke_ah, rs.revoke_hr, rs.revoke_on, rs.revoke_comment, rs.rv_hr_remark, wd.Process,wd.clientname,wd.sub_process,rs.rv_ah_remark,pd1.img from resign_details rs inner join whole_details_peremp wd on wd.EmployeeID = rs.EmployeeID left outer join personal_details pd1 on pd1.EmployeeID = rs.EmployeeID left outer join personal_details pd on pd.EmployeeID = ReportTo where (curdate() between cast(DATE_SUB(rs.nt_end, INTERVAL 3 DAY) as date)  and cast(rs.nt_end as date)) and  ("CE03070003" =  "' . $_SESSION['__user_logid'] . '") and (status_sitehead  is null or status_sitehead = 0) and nt_end >= curdate() and rg_status < 9');
							$chk_taskQry = 'SELECT rs.id, rs.EmployeeID,wd.EmployeeName,rs.remark, rs.file, rs.rg_status, rs.nt_start, rs.nt_end, rs.accept_time, rs.accept, rs.revoke_accept,rs.revoke_status, rs.accepter_remark, rs.accepter, rs.status_ah, rs.status_oh, rs.status_sitehead, rs.status_it, rs.status_hr, rs.final_acceptance, rs.createdon, rs.createdby, rs.revoke_ah, rs.revoke_hr, rs.revoke_on, rs.revoke_comment, rs.rv_hr_remark, wd.Process,wd.clientname,wd.sub_process,rs.rv_ah_remark,pd1.img from resign_details rs inner join whole_details_peremp wd on wd.EmployeeID = rs.EmployeeID left outer join personal_details pd1 on pd1.EmployeeID = rs.EmployeeID left outer join personal_details pd on pd.EmployeeID = ReportTo where (curdate() between cast(DATE_SUB(rs.nt_end, INTERVAL 3 DAY) as date)  and cast(rs.nt_end as date)) and  ("CE03070003" =  ?) and (status_sitehead  is null or status_sitehead = 0) and nt_end >= curdate() and rg_status < 9';
							$stVar = $conn->prepare($chk_taskQry);
							$stVar->bind_param("s", $userID);
							$stVar->execute();
							$chk_task = $stVar->get_result();
								...
								<table id="myTable" class="data dataTable no-footer row-border" cellspacing="0" width="100%">
									...
									<tbody>
										...
										foreach ($chk_task as $key => $value) {
											...
											<input  type="input" name="txt_EmployeeID_' . $count . '" id="txt_EmployeeID_' . $count . '" readonly="true" value="' . $value['EmployeeID'] . '" /></td>';

											echo '<td class="edit_info_body hidden"><textarea  name="txt_Remark_' . $count . '" id="txt_Remark_' . $count . '"  title="Remark" class="materialize-textarea materialize-textarea-size">NA</textarea></td>';
