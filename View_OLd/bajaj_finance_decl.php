...
$sqlquery = "select t1.EmpID,t1.EmpName,address_p,FatherName,case when df_id=74 or df_id=77 then 'Agent' else 'Support Staff' end as desig,t1.Gender from EmpID_Name t1 join employee_map t2 on t1.EmpID=t2.EmployeeID join address_details t5 on t2.EmployeeID=t5.EmployeeID where t2.EmployeeID= ?";
...
if (isset($_POST['btn_ack'])) {
    ...
    $query = "insert into bajaj_finance_decl (EmployeeID, EmpName,Address,fathername,designation,acknowledge) values(?,?,?,?,?,?) ";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("ssssss", $EmployeeID, $EmployeeName, $address, $fathername, $desig, $ackvalue);
