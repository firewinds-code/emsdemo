...
// setcookie('user', 'vinn', ['samesite' => 'None', 'secure' => true]);
// $cookie_name = "reload";
// setcookie($cookie_name, 'hidden', time() + (86400 * 30), "/", ['samesite' => 'None', 'secure' => true]); // 86400 = 1 day		
					...
					<div class="col s3" id="div_me" align="center">
						<div class="col s12">
							<div class="card">
								...
								<?php
								$en_empid = "";
								$sqlstr = "select empid from employee_qrcode where EmployeeID=? ";
								$myDB = new MysqliDb();
								$conn = $myDB->dbConnect();
					...
					<div class="col s9">
						<ul class="collapsible">
											...
											if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
												...
												if (empty($MysqliError)) {
													...
													if ($ChildStatus == 'Yes') {
														...
														$sqlConnect = " select * from child_details where EmployeeID=?";
														$myDB = new MysqliDb();
														$conn = $myDB->dbConnect();
							...
							<li>
								...
								<div class="collapsible-body">
									<div id="contact_div" class="list-container">
										...
										$sqlConnect = "select * from doc_details where EmployeeID=?";
										$myDB = new MysqliDb();
										$conn = $myDB->dbConnect();
										$stmt = $conn->prepare($sqlConnect);
										$stmt->bind_param("s", $EmployeeID);
										...
										if (isset($_REQUEST['empid']) || $EmployeeID != '') {
											...
											if (empty($MysqliError)) {
												...
												if (!empty($result_all[0]['account_head'])) {
													$emp_accoHead = $result_all[0]['account_head'];
													$myDB = new MysqliDb();
													$conn = $myDB->dbConnect();
													$resultah = 'select img,contact_details.mobile from personal_details inner join contact_details on contact_details.EmployeeID = personal_details.EmployeeID where personal_details.EmployeeID = ? limit 1';
													$selQr = $conn->prepare($resultah);
													$selQr->bind_param("s", $emp_accoHead);
													$selQr->execute();
													$res = $selQr->get_result();
													$result_ah = $res->fetch_row();
													if ($res->num_rows > 0 && $res) {
														$contact_ah = $result_ah[1];
														$imsrc_ah = $result_ah[0];
													}
												}

												if (file_exists(ROOT_PATH . 'Images/' . $imsrc_ah)) {
													$imsrc_ah = URL . 'Images/' . $imsrc_ah;

													echo "<td >Account Head </td><td  style='padding-left: 5px;vertical-align: middle;line-height: 40px;'>" . '<img  id="imgToempl_ah" name="imgToempl_ah" class="emp_image" src="' . $imsrc_ah . '"  style="height: 40px;width: 40px;border-radius: 10px;"/>' . "&nbsp;&nbsp;<b>" . $result_all[0]['AH'];
												} else {
													$imsrc_ah = '';
												...
												$srQ = 'select function_id from employee_map inner join df_master on df_master.df_id = employee_map.df_id where EmployeeID = ?';
												$myDB = new MysqliDb();
												$conn = $myDB->dbConnect();
												$selQr = $conn->prepare($srQ);
												$selQr->bind_param("s", $EmployeeID);
												$selQr->execute();
												$res = $selQr->get_result();
												$funcitonID = $res->fetch_row();
												// $funcitonID = $myDB->query($srQ);


												if ($funcitonID[0] == 7 || $funcitonID[0] == 8 || $funcitonID[0] == 10) {
													echo '<tr>';
													if (!empty($result_all[0]['oh'])) {
														$emp_oh = $result_all[0]['oh'];
														$myDB = new MysqliDb();
														$conn = $myDB->dbConnect();
														$resultoh = 'select img,contact_details.mobile from personal_details inner join contact_details on contact_details.EmployeeID = personal_details.EmployeeID where personal_details.EmployeeID = ? limit 1';
														$selQy = $conn->prepare($resultoh);
														$selQy->bind_param("s", $emp_oh);
														$selQy->execute();
														$resul = $selQy->get_result();
														$result_oh = $resul->fetch_row();

														if (count($result_oh) > 0 && $result_oh) {
															$contact_oh = $result_oh[1];
															$imsrc_oh = $result_oh[0];
														}
													}
													/* if (!empty($imsrc_oh)) {
														$imsrc_oh = URL . 'Images/' . $imsrc_oh;
													} */
													if (file_exists(ROOT_PATH . 'Images/' . $imsrc_oh)) {
														$imsrc_oh = URL . 'Images/' . $imsrc_oh;
														echo "<td>Operation Head </td><td  style='padding-left: 5px;vertical-align: middle;line-height: 40px;'>" . '<img  id="imgToempl_oh" name="imgToempl_oh" class="emp_image" src="' . $imsrc_oh . '"  style="height: 40px;width: 40px;border-radius: 10px;"/>' . "&nbsp;&nbsp;<b>" . $result_all[0]['OH'];
													} else {
														$imsrc_oh = '';
													...
													if (!empty($result_all[0]['qh'])) {
														$emp_qh = $result_all[0]['qh'];
														$myDB = new MysqliDb();
														$conn = $myDB->dbConnect();
														$resultqh = 'select img,contact_details.mobile from personal_details inner join contact_details on contact_details.EmployeeID = personal_details.EmployeeID where personal_details.EmployeeID = ? limit 1';
														$selQry = $conn->prepare($resultqh);
														$selQry->bind_param("s", $emp_qh);
														$selQry->execute();
														$resu = $selQry->get_result();
														$result_qh = $resu->fetch_row();

														if ($resu->num_rows > 0 && $resu) {
															$contact_qh = $result_qh[1];
															$imsrc_qh = $result_qh[0];
														}
													}
													/* if (!empty($imsrc_qh)) {
														$imsrc_qh = URL . 'Images/' . $imsrc_qh;
													} */
													if (file_exists(ROOT_PATH . 'Images/' . $imsrc_qh)) {
														$imsrc_qh = URL . 'Images/' . $imsrc_qh;
														echo "<td>Quality Head </td><td  style='padding-left: 5px;vertical-align: middle;line-height: 40px;'>" . '<img  id="imgToempl_qh" name="imgToempl_qh" class="emp_image" src="' . $imsrc_qh . '"  style="height: 40px;width: 40px;border-radius: 10px;"/>' . "&nbsp;&nbsp;<b>" . $result_all[0]['QH'];
													} else {
														$imsrc_qh = '';
													...
													if (!empty($result_all[0]['th'])) {
														$emp_th = $result_all[0]['th'];
														$myDB = new MysqliDb();
														$conn = $myDB->dbConnect();
														$resultth = 'select img,contact_details.mobile from personal_details inner join contact_details on contact_details.EmployeeID = personal_details.EmployeeID where personal_details.EmployeeID = ? limit 1';
														$selQry = $conn->prepare($resultth);
														$selQry->bind_param("s", $emp_th);
														$selQry->execute();
														$resu = $selQry->get_result();
														$result_th = $resu->fetch_row();
														if ($resu->num_rows > 0 && $resu) {
															$contact_th = $result_th[1];
															$imsrc_th = $result_th[0];
														}
													}
													/* if (!empty($imsrc_th)) {
														$imsrc_th = URL . 'Images/' . $imsrc_th;
													} */
													if (file_exists(ROOT_PATH . 'Images/' . $imsrc_th)) {
														$imsrc_th = URL . 'Images/' . $imsrc_th;
														echo "<td>Training Head </td><td  style='padding-left: 5px;vertical-align: middle;line-height: 40px;'>" . '<img  id="imgToempl_th" name="imgToempl_th" class="emp_image" src="' . $imsrc_th . '"  style="height: 40px;width: 40px;border-radius: 10px;"/>' . "&nbsp;&nbsp;<b>" . $result_all[0]['TH'];
													} else {
														$imsrc_th = '';
													...
													if (!empty($result_all[0]['Qa_ops'])) {
														$EmpID = $result_all[0]['Qa_ops'];
														$myDB = new MysqliDb();
														$conn = $myDB->dbConnect();
														$resultqa = 'select img,contact_details.mobile from personal_details inner join contact_details on contact_details.EmployeeID = personal_details.EmployeeID where personal_details.EmployeeID = ? limit 1';
														$stmt = $conn->prepare($resultqa);
														$stmt->bind_param("s", $EmpID);
														$stmt->execute();
														$resultss = $stmt->get_result();
														$result_qa = $resultss->fetch_row();
														if ($resultss->num_rows > 0 && $resultss) {
															$contact_qa = $result_qa[1];
															$imsrc_qa = $result_qa[0];
														}
													}
													// if (!empty($imsrc_qa)) {
													// 	$imsrc_qa = URL . 'Images/' . $imsrc_qa;
													// }
													if (file_exists(ROOT_PATH . 'Images/' . $imsrc_qa)) {
														$imsrc_qa = URL . 'Images/' . $imsrc_qa;
														echo "<td >Quality Analyst </td><td  style='padding-left: 5px;vertical-align: middle;line-height: 40px;'>" . '<img  id="imgToempl_qa" name="imgToempl_qa" class="emp_image" src="' . $imsrc_qa . '"  style="height: 40px;width: 40px;border-radius: 10px;"/>' . "&nbsp;&nbsp;<b>" . $result_all[0]['QA_OPS'];
													} else {
														$imsrc_qa = '';
												...
												if (!empty($result_all[0]['ReportTo'])) {
													$myDB = new MysqliDb();
													$conn = $myDB->dbConnect();
													$empID = $result_all[0]['ReportTo'];
													$resultrt = 'select img,EmployeeName,contact_details.mobile from personal_details inner join contact_details on contact_details.EmployeeID = personal_details.EmployeeID where personal_details.EmployeeID = ? limit 1';
													$stmt = $conn->prepare($resultrt);
													$stmt->bind_param("s", $empID);
													$stmt->execute();
													$results = $stmt->get_result();
													$result_rt = $results->fetch_row();

													if ($results->num_rows > 0 && $results) {
														$contact_rt = $result_rt[2];
														$imsrc_rt = $result_rt[0];
														$rtname = $result_rt[1];
													}
												}
												// if (!empty($imsrc_rt)) {
												...
												// }
												if (file_exists(ROOT_PATH . 'Images/' . $imsrc_rt)) {
													$imsrc_rt = URL . 'Images/' . $imsrc_rt;
													echo "<td >Reporting </td><td  style='padding-left: 5px;vertical-align: middle;line-height: 40px;'>" . '<img  id="imgToempl_rt" name="imgToempl_rt" class="emp_image" src="' . $imsrc_rt . '"  style="height: 40px;width: 40px;border-radius: 10px;"/>' . "&nbsp;&nbsp;<b>" . $rtname;
												} else {
													$imsrc_rt = '';
												...
												if ($result_all[0]['vh'] != NULL) {
													if (!empty($result_all[0]['vh'])) {
														$empid = $result_all[0]['vh'];
														$myDB = new MysqliDb();
														$conn = $myDB->dbConnect();
														$sql = 'select img,EmployeeName,contact_details.mobile from personal_details inner join contact_details on contact_details.EmployeeID = personal_details.EmployeeID where personal_details.EmployeeID = ? limit 1';
														$stmt = $conn->prepare($sql);
														$stmt->bind_param("s", $empid);
														$stmt->execute();
														$result = $stmt->get_result();
														$result_vh = $result->fetch_row();

														if ($result->num_rows > 0 && $result) {
															$contact_vh = $result_vh[2];
															$imsrc_vh = $result_vh[0];
															$vhname = $result_vh[1];
														}
													}

													// if (!empty($imsrc_vh)) {
													// 	$imsrc_vh = URL . 'Images/' . $imsrc_vh;
													// }
													if (file_exists(ROOT_PATH . 'Images/' . $imsrc_vh)) {
														$imsrc_vh = URL . 'Images/' . $imsrc_vh;
														echo "<td >Vertical Head </td><td  style='padding-left: 5px;vertical-align: middle;line-height: 40px;'>" . '<img  id="imgToempl_vh" name="imgToempl_vh" class="emp_image" src="' . $imsrc_vh . '"  style="height: 40px;width: 40px;border-radius: 10px;"/>' . "&nbsp;&nbsp;<b>" . $vhname;
													} else {
														$imsrc_vh = '';
								...
								<div class="collapsible-body">
									<div id="management_div" class="list-container">
										...
										if (isset($_REQUEST['empid']) || $EmployeeID != '') {
											...
											$getGenderEmp = "select Gender from personal_details where EmployeeID=?";
											$myDB = new MysqliDb();
											$conn = $myDB->dbConnect();
											$selQury = $conn->prepare($getGenderEmp);
											$selQury->bind_param("s", $EmployeeID);
											$selQury->execute();
											$result = $selQury->get_result();
											$resultgetGenderEmp = $result->fetch_row();
											...
											if (isset($resultgetGenderEmp[0])) {
												$gender = $resultgetGenderEmp[0];
											}
											$MysqliError = $myDB->getLastError();
							...
							if (isset($_REQUEST['empid']) || $EmployeeID != '') {
								...
								$getDetails = 'select t1.er_scop,t2.EmployeeName from new_client_master t1 join whole_details_peremp t2 on t1.er_scop=t2.employeeid where t1.cm_id=?';
								$myDB = new MysqliDb();
								$conn = $myDB->dbConnect();
								...
								if (!empty($result)) {
									echo '<table class="responsive-table">';
									echo '<tr>';
									if (!empty($result_all[0])) {
										$myDB = new MysqliDb();
										$conn = $myDB->dbConnect();
										$emp = $result_all[0];
										$result_ah = 'select img,contact_details.mobile from personal_details inner join contact_details on contact_details.EmployeeID = personal_details.EmployeeID where personal_details.EmployeeID = ? limit 1';
										$stmt = $conn->prepare($getDetails);
										$stmt->bind_param("s", $emp);
										$stmt->execute();
										$result = $stmt->get_result();
										$result_ah = $result->fetch_row();

										if ($result->num_rows > 0 && $result) {
											$contact_er = $result_ah[1];
											$imsrc_er = $result_ah[0];
										}
									}
									if (!empty($imsrc_er)) {
										$imsrc_er = URL . 'Images/' . $imsrc_er;
									}
									echo "<td >ER Spoc </td><td  style='padding-left: 5px;vertical-align: middle;line-height: 40px;'>" . '<img  id="imgToempl_er" name="imgToempl_er" class="emp_image" src="../Style/images/agent-icon.png"  style="height: 40px;width: 40px;border-radius: 10px;"/>' . "&nbsp;&nbsp;<b>" . $result_all[1];
									if (!empty($contact_er)) {
										echo '&nbsp;&nbsp;(&nbsp;' . $contact_er . '&nbsp;)';
									}
									echo '</b></td></tr>';
				...
				<li>
					...
					<div class="collapsible-body">
						<div id="process_div" class="list-container">
							<?php
							if (isset($_REQUEST['empid']) || $EmployeeID != '') {
								$rptToEmp = "select ReportTo from status_table   where employeeid=?";
								$myDB = new MysqliDb();
								$conn = $myDB->dbConnect();
								$sel = $conn->prepare($rptToEmp);
								$sel->bind_param("s", $EmployeeID);
								$sel->execute();
								$resultss = $sel->get_result();
								$resultrptToEmp = $resultss->fetch_row();
								// $resultrptToEmp = $myDB->query($rptToEmp);
								// $MysqliError = $myDB->getLastError();
								if (isset($resultrptToEmp[0])) {
									$rid = $resultrptToEmp[0];
								}
								$rptTortid = "select ReportTo from status_table   where employeeid=?";
								$myDB = new MysqliDb();
								$conn = $myDB->dbConnect();
								$selQu = $conn->prepare($rptTortid);
								$selQu->bind_param("s", $rid);
								$selQu->execute();
								$resul = $selQu->get_result();
								$resultrptTortid = $resul->fetch_row();

								// $resultrptTortid = $myDB->query($rptTortid);
								// $MysqliError1 = $myDB->getLastError();
								if (isset($resultrptTortid[0])) {
									$rrid = $resultrptTortid[0];
								}
								$rrrptTortid = "select ReportTo from status_table   where employeeid=?";
								$myDB = new MysqliDb();
								$conn = $myDB->dbConnect();
								$selQu = $conn->prepare($rptTortid);
								$selQu->bind_param("s", $rrid);
								$selQu->execute();
								$resul = $selQu->get_result();
								$resultrrrto = $resul->fetch_row();
								// $resultrrrto = $myDB->query($rrrptTortid);
								// $MysqliError2 = $myDB->getLastError();
								if (isset($resultrrrto[0])) {
									$rrrid = $resultrrrto[0];
								}
								if (empty($MysqliError)) {
									echo '<table class="responsive-table">';
									...
									if (!empty($rid)) {
										...
										$myDB = new MysqliDb();
										$conn = $myDB->dbConnect();
										$sql = 'select personal_details.img,emp_dt_map.designation,personal_details.EmployeeName,contact_details.mobile from personal_details inner join contact_details on contact_details.EmployeeID = personal_details.EmployeeID inner join emp_dt_map on personal_details.EmployeeID=emp_dt_map.EmployeeID where personal_details.EmployeeID = ? limit 1';
										$selQu = $conn->prepare($sql);
										$selQu->bind_param("s", $rid);
										$selQu->execute();
										$resul = $selQu->get_result();
										$result_rt = $resul->fetch_row();

										if ($resul->num_rows > 0 && $resul) {
											$contact_rt = $result_rt[3];
											$imsrc_rt1 = $result_rt[0];
											$rtname = $result_rt[2];
											$rdesignation = $result_rt[1];
										}


										if (!empty($imsrc_rt1)) {
											$imsrc_rt1 = URL . 'Images/' . $imsrc_rt1;
										}
									...
									if (!empty($rrid) && $rid != $rrid && empty($MysqliError1)) {
										...
										$myDB = new MysqliDb();
										$conn = $myDB->dbConnect();
										$sqlConnect = 'select personal_details.img,emp_dt_map.designation,personal_details.EmployeeName,contact_details.mobile from personal_details inner join contact_details on contact_details.EmployeeID = personal_details.EmployeeID inner join emp_dt_map on personal_details.EmployeeID=emp_dt_map.EmployeeID where personal_details.EmployeeID = ? limit 1';
										$stmt = $conn->prepare($sqlConnect);
										$stmt->bind_param("s", $rrrid);
										$stmt->execute();
										$results = $stmt->get_result();
										$result_rrt = $results->fetch_row();

										if ($results->num_rows > 0 && $results) {
											$contact_rrt = $result_rrt[3];
											$imsrc_rrt = $result_rrt[0];
											$rrtname = $result_rrt[2];
											$rrdesignation = $result_rrt[1];
										}


										if (!empty($imsrc_rrt)) {
											$imsrc_rrt = URL . 'Images/' . $imsrc_rrt;
										}
										echo "<td>$rrdesignation</td><td  style='padding-left: 5px;vertical-align: middle;line-height: 40px;'>" . '<img  id="imgToempl_ah" name="imgToempl_ah" class="emp_image" src="../Style/images/agent-icon.png"  style="height: 40px;width: 40px;border-radius: 10px;"/>' . "&nbsp;&nbsp;<b>" . $rrtname;
									...
									if (!empty($rrrid) && $rrrid != $rrid && $rrrid != $rid  && empty($MysqliError2)) {
										...
										$myDB = new MysqliDb();
										$conn = $myDB->dbConnect();
										$sqlConnect = 'select personal_details.img,personal_details.EmployeeName,emp_dt_map.designation,contact_details.mobile from personal_details inner join contact_details on contact_details.EmployeeID = personal_details.EmployeeID inner join emp_dt_map on personal_details.EmployeeID=emp_dt_map.EmployeeID where personal_details.EmployeeID = ? limit 1';
										$stmt = $conn->prepare($sqlConnect);
										$stmt->bind_param("s", $rrrid);
										$stmt->execute();
										$result = $stmt->get_result();
										$result_rrrt = $result->fetch_row();

										if ($result->num_rows > 0 && $result) {
											$contact_rrrt = $result_rrrt[3];
											$imsrc_rrrt = $result_rrrt[0];
											$rrrtname = $result_rrrt[1];
											$rrrdesignation = $result_rrrt[2];
										}


										if (!empty($imsrc_rrrt)) {
											$imsrc_rrrt = URL . 'Images/' . $imsrc_rrrt;
										}
				...
				<li>
					...
					<div class="collapsible-body">
						<div id="Education_div" class="list-container">
							...
							$sqlConnect = "SELECT i.ah_status,i.ah_subtype,i.ah_Datetime,d.Title,d.Document FROM warning_rth i inner join warning_rth_documents d on i.EmployeeID=d.EmployeeID and i.id=d.DataId where i.EmployeeID=? group by d.DataId;";
							$stmt = $conn->prepare($sqlConnect);
							$stmt->bind_param("s", $EmployeeID);
							$stmt->execute();
							$result = $stmt->get_result();
...
<script>
	$(function() {
		...
		// 	var checkFileimgsrc_ah = fileExistFun(imgsrc_ah);
		// 	if (checkFileimgsrc_ah == "true") {
		// 		$('#imgToempl').attr('src', imgsrc_ah);
		// 	} else {
		// 		$('#imgToempl').attr('src', '../Style/images/agent-icon.png');
		// 	}
		// 	/* if (imgsrc_ah == "") {
