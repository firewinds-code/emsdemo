...
if ($btnSave) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		...
		if ($QualityID != "" && $TrainingID != "" && $OpsID != "" && $HRID != "" && $ITID != "" && $ReportsTo != "") {
			...
			$update = "UPDATE downtimereqid1 set QualityID= ? , TrainingID= ? , OpsID= ? , HRID= ? , ITID= ? , ReportsTo= ?,updated_on=now()  where ID=?";

			$stmt = $conn->prepare($update);
			$stmt->bind_param("ssssssi", $QualityID, $TrainingID, $OpsID, $HRID, $ITID, $ReportsTo, $ID);
			$updt = $stmt->execute();
			...
			if ($stmt->affected_rows === 1) {
				...
				echo "<script>$(function(){ toastr.success('Data Not Updated ::Error :- <code>''</code>'); }); </script>";
			}
		}
	}
}
