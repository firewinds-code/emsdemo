			...
			if (!empty($ReportsTo)) {
				if (!empty($res[2]) && $DT_Start < $DT_End && date('Y-m-d', strtotime($DT_End)) <= date('Y-m-d', strtotime($DT_Start . ' +1 days'))) {
					...
					if (strtotime($TotalDT) > strtotime('00:00') && $RequestType != 'NA') {
						$myDB = new MysqliDb();
						$sqlInsertDT = 'call sp_InsertDTReq("' . $EmpID . '","' . $Process . '","' . $DateFrom . '","' . $DateTo . '","' . $TotalDT . '","' . $FAID . '","' . $res[3] . '","' . $EmployeeComment . '","' . $FAID . '","' . $Status . '","' . $ReportsTo . '","' . $RTStatus . '","' . $RequestType . '","' . $ITticketID . '","web-Downtime848")';
		...
		$.ajax({
			...
			success: function(result) {
				...
				if ($.trim(result).length > 0) {
					...
					// var AccountHeadName = 'Handler Remarks';
					// alert(AccountHeadName);
					...
					$.ajax({
						url: "../Controller/getCommentDownTime.php?ID=" + $item.attr("Data-ID") + '&Emp=' + EmployeeID + '&Emp1=' + $('#txtEmpID1').val(),
						success: function(result) {
