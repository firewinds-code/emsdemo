					...
					foreach ($data_ncns as $ncns_key => $ncns_value) {
						...
						if ((intval($days) >= 2) && $account_head_hr != 'CE07147134') {
							...
							if ($rowCount > 0) {
								...
								$myDB = new MysqliDb();
								$conn = $myDB->dbConnect();
								$sql = 'select mobile,altmobile from contact_details where EmployeeID = ? limit 1';
