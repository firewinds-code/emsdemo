					...
					if ($rowCount > 0) {
						...
						if ($MngrStatusID == "Approve") {
							if ($Exception == "Roster Change" || $Exception == "Shift Change") {
								...
								if ($dt1 == $dt2) {
									...
									if ($rowCount > 0) {
										...
										if (strtotime($date1) > strtotime($date2)) {
											$dsAPR = 'select t1.EmployeeID, designation, windowstart,windowend,t3.work_from from whole_dump_emp_data t1 inner join process_window t2 on t1.cm_id=t2.cm_id inner join roster_temp t3 on t1.EmployeeID=t3.EmployeeID where t1.EmployeeID= ? and t3.DateOn= ? ';
											$selectQury = $conn->prepare($dsAPR);
											$selectQury->bind_param("ss", $EmployeeID, $date2);
											$selectQury->execute();
											...
											if ($res->num_rows > 0 && $res) {
												if ($ds_APR[1] == 'CSA' && $ds_APR[4] != '') {
													$url = 'http://192.168.202.130/apr_processing/user_apr.php?apr_date=' . $date2 . '&username=' . $EmployeeID . '&intime=' . $ShiftIn . '&outtime=' . $ShiftOut . '&WorkFrom=' . $ds_APR[4] . '&windowstart=' . $ds_APR[2] . '&windowend=' . $ds_APR[3];
													//echo $url;
													$curl = curl_init();
								...
								$query = 'INSERT INTO leavehistry(EmployeeID,DateFrom,DateTo,ReasonofLeave,LeaveOnDate,TotalLeaves,EmployeeComment,MngrStatusID,HRStatusID,ManagerComment,HRComents,CreatedBy,HOD,LeaveType) VALUES (?,?,?,?,"' . date('Y-m-d') . '",?,"Approve","Approve","Approve",?,"",?,"' . ' SERVER' . '",?)';
								// $myDB = new MysqliDb();
								// $flag = $myDB->rawQuery($query);
								// $mysql_error = $myDB->getLastError();
								// $rowCount = $myDB->count;
								$insQu = $conn->prepare($query);
								$insQu->bind_param("ssssisss", $EmployeeID, $DateFrom1, $DateTo1, $ReasonofLeave, $TotalLeaves1, $ManagerComment, $CreatedBy, $LeaveType);
								$insQu->execute();
								$res = $insQu->get_result();
								// if ($rowCount > 0) {
								if ($insQu->affected_rows > 0) {
