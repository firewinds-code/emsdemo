 <?php
    ...
    $location = URL . 'Error';
    if (in_array($emp, $NotApplicable)) {
        header("Location: $location");
        exit();
    } else if (substr($emp, 0, 3) == 'CCE' || substr($emp, 0, 3) == 'EXT') {
        header("Location: $location");
        exit();
    } else {

        // $query = "Select status,fetcheEmail from appointmentlonline where EmployeeID='" . $EmployeeID . "'";
        $query = "Select status,fetcheEmail from appointmentlonline where EmployeeID=?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("s", $EmployeeID);
        $stmt->execute();
        $dataArray = $stmt->get_result();
        $dataArrayRow = $dataArray->fetch_row();
        // print_r($dataArrayRow);
        // die;
        // $dataArray = $myDB->rawQuery($query);
        $disable = '';

        if ($dataArray->num_rows < 1) {
            ...
            $select_email_arrayQry = "select EmployeeName,b.cm_id from personal_details a inner Join employee_map b on a.EmployeeID=b.EmployeeID where  a.EmployeeID=?";
            ...
            $cm_id = '';
            if (isset($select_email_arrayRow[1])) {
                $cm_id = clean($select_email_arrayRow[1]); //['cm_id'];
                $EmployeeName = clean($select_email_arrayRow[0]); //['EmployeeName'];
            }
            $myDB = new MysqliDb();
            ...
            $insert_QueryQry = "Insert Into appointmentlonline set EmployeeID=?,EmpName=?,cm_id=?,CreateDate='" . date('Y-m-d') . "',status='0',source_from='web427'";
            $stmt = $conn->prepare($insert_QueryQry);
            $stmt->bind_param("sss", $EmployeeID, $EmployeeName, $cm_id);
            $stmt->execute();
 ...
 <?php
    ...
    $disable = "";
    $message = "";
    if ($dataArray->num_rows > 0) {
        if (clean($dataArrayRow[0]) == 1) {
            $message = "You have already received your Appointment Letter on your email " . clean($dataArrayRow[1]); //['fetcheEmail'];
            $disable = "disabled='disabled'";
        } else if ($dataArray && clean($dataArrayRow[0]) == 0) {
            $disable = "";
            $message = "";
        }
    }/*else{
    ...
    if (isset($_POST['emailAddress']) && trim(cleanUserInput($_POST['emailAddress'])) != "") {
        if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
            ...
            $GetData = "select whole_details_peremp.EmployeeID,EmployeeName,location,DOJ,designation,ctc,status_table.onFloor,des_id,whole_details_peremp.function,Gender,tehsil from whole_details_peremp left outer join salary_details on salary_details.EmployeeID = whole_details_peremp.EmployeeID left outer join status_table on whole_details_peremp.EmployeeID = status_table.EmployeeID left join address_details on whole_details_peremp.EmployeeID = address_details.EmployeeID where whole_details_peremp.EmployeeID=?";
            //Flag=0";
            ...
            if ($resultsE->num_rows > 0) {
                foreach ($resultsE as $val) {
                    ...
                    $stm = $conn->prepare($Locationquery);
                    $stm->bind_param("i", $locationid);
                    $stm->execute();
                    $Locationresult = $stm->get_result();
                    $LocationresultRow = $Locationresult->fetch_row();
        ...
        ' . $location . '</p>';
            ...
            if ('OC' == substr($EmployeeID, 0, 2)) {
                ...
                $tcpdf->Image('../Style/images/cogent-logoNW.png', 160, 5, 40, 30, 'JPG'); //logo right cogent logo
                        ...
                        $select_email_arrayQry = "select mobile,b.cm_id from contact_details a inner Join employee_map b on a.EmployeeID=b.EmployeeID where  a.EmployeeID=?";
                        $stm1 = $conn->prepare($select_email_arrayQry);
                        $stm1->bind_param("s", $EmployeeID);
                        $stm1->execute();
                        $select_email_array = $stm1->get_result();
                        ...
                        if ($select_email_array->num_rows > 0) {
                            $contactNum = clean($select_email_arrayRow[0]); //['mobile'];
                        ...
                        if (file_exists($path)) {
                            ...
                            if (!$mail->send()) {
                                ...
                                $emaddress = addslashes($_POST['emailAddress']);
                                $updtQry = "update appointmentlonline set status=1,source_from='Web-146byemp',fetcheEmail=?,ReceivedDate=now() where EmployeeID=?";
                                $stm1 = $conn->prepare($updtQry);
                                $stm1->bind_param("ss", $EmployeeID, $emaddress);
                                $stm1->execute();
                                $updtResult = $stm1->get_result();
                                // print_r($updtResult);
                                ...
                                if (!empty($mobilenum)) {
                                    ...
                                    $msg = "Hi , your appointment letter has been sent on your given Email ID : " . cleanUserInput($_POST['emailAddress']) . "  - Cogent E Services";
                                    $url = SMS_URL;
                                    $token = SMS_TOKEN;
                                    $credit = SMS_CREDIT;
                                    $sender = SMS_SENDER;
                            ...
                            $UpdateQry = 'update doc_al_status set validate=1,validateby=?,validatetime = now(),comment=? where EmployeeID=?';
                            $stm1 = $conn->prepare($UpdateQry);
                            $stm1->bind_param("sss", $EmployeeID, $Comment, $EmpID);
                            $stm1->execute();
                            $updtResult = $stm1->get_result();
                            $body = "";
                            // $myDB->rawQuery($Update);
                            // $mysql_error = $myDB->getLastError();
                            if ($updtResult->num_rows > 0) {
                                ...
                                $stm1 = $conn->prepare($selectCountQry);
                                $stm1->bind_param("s", $EmployeeID);
                                $stm1->execute();
                                $selectCount = $stm1->get_result();
