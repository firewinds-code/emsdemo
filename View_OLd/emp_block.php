...
$classvarr = "'.By Aadhar'";
$emid = "";
$empid = clean($_SESSION['EmployeeID_s']);
$btn_block = isset($_POST['btn_block']);
if ($btn_block) {
    if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
        $aadharno = clean($_SESSION['aadharno']);
        // $_SESSION['aadharno'] = '';
        // unset($_SESSION['aadharno']);
        // $insert_EmpBlock = "insert into emp_block(EmployeeID,aadharNo)values('" . $empid . "','" . $aadharno . "')";
        $insert_EmpBlock = "insert into emp_block(EmployeeID,aadharNo)values(?,?)";
        $stmt = $conn->prepare($insert_EmpBlock);
        $stmt->bind_param("ss", $empid, $aadharno);
        $stmt->execute();
        $insResult = $stmt->get_result();
        // print_r($insResult);
        // die;
...
if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
    $aadhar = cleanUserInput($_POST['aadharno']);
}
?>
<script>
            ...
            <div class="schema-form-section row">
                ...
                <div class="By Aadhar" id="adharDiv">
                    ...
                    <div class="input-field col s5 m5">
                        ...
                        <input type="text" id="aadharno" name="aadharno" title="Enter Aadhar Number Must Not Less Then 12 Number" maxlength="12">
                ...
                <div id="pnlTable">
                    <?php
                    $btn_AadharNo = isset($_POST['btn_AadharNo']);
                    if ($btn_AadharNo) {
                        // $aadhar = cleanUserInput($_POST['aadharno']);
                        ...
                        if (count($result) > 0) {
                            ...
                            $selectQry = "select aadharNo from emp_block where aadharNo=?";
                            $stmts = $conn->prepare($selectQry);
                            $stmts->bind_param("s", $aadhar);
                            $stmts->execute();
                    ...
                    ?>
                                <div class="had-container pull-left row card" style="margin-top: 10px;width: 100%;padding: 15px;">
                                    <div class="">
                                        ...
                                        <?php $btn_AadharNo = isset($_POST['btn_AadharNo']);
                                        if ($btn_AadharNo) { ?>
                                            <div class="input-field col s12 m12 right-align">
