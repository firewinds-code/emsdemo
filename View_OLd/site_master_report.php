...
if (isset($_POST['send'])) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		...
		ssm.site_id, process, seat, mnth_year, ssm.createdon,
		scm.site_id, costitem, txt_date, price , scm.createdon,l.location,c.client_name as ClientName,ci.item
		from site_master sm 
		left join site_seat_master ssm on sm.id=ssm.site_id 
		left join site_cost_master scm on sm.id=scm.site_id and ssm.mnth_year=scm.txt_date 
