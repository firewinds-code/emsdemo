...
if ($statusdate >= '10'  && ($statusdate <= '15')) {
	...
	if (isset($clean_u_logid) && $clean_u_logid != "") {
		$sql = "select EmployeeID from status_table where ReportTo=?";
		$selectQury = $conn->prepare($sql);
		$selectQury->bind_param("s", $clean_u_logid);
		$selectQury->execute();
		$results = $selectQury->get_result();
		$resultrrt = $results->fetch_row();
		if ($results->num_rows > 0) {
			if (isset($resultrrt[0]) && $resultrrt[0] == NULL) {
				$location = URL . 'Login';
						...
						<div class="">
							...
							if ($result->num_rows > 0) { ?>
								...
								<table id="myTable" class="data dataTable no-footer row-border" cellspacing="0" width="100%">
									...
									<tbody id="emplist">
										<input class='empclass hidden ' type='text' name='reporttoid' id="reporttoid" value="<?php echo $clean_u_logid; ?>">
										<?php
										$i = 0;
										$j = 1;
