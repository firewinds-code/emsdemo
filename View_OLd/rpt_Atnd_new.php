	...
	<div class="pim-container row" id="div_main">
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
						...
						<div class="modal-body" style="height:300px ;">
							...
							<div class="input-field col s4 m4 l4">
								<select id="clientName" name="clientName">
									...
									$Query = 'select distinct t1.client_name ID,t3.client_name from new_client_master t1 join report_map t2 on t1.cm_id=t2.processID join client_master t3 on t1.client_name=t3.client_id where t2.EmpID=? and reportID=1';
				...
				<?php
				$myDB = new MysqliDb();
				$clntname = $_POST['clientName'];
				if (!empty($clntname)) {
					$clientname = cleanUserInput($_POST['clientName']);
				}
				$curMonth = date('m');
				$curYear = date('Y');
				$txtmonth = $_POST['txt_dateMonth'];
				if (!empty($txtmonth)) {
					$monthCount = cleanUserInput($_POST['txt_dateMonth']);
				}
				...
				if ($monthCount != "") {
					...
					$chk_task = $myDB->query('call sp_get_atnd_Report_new("' . $EmpID . '","' . $date_To . '","' . $date_From . '","' . $clientname . '") ');
					...
					if (count($chk_task) > 0 && $chk_task) {
						...
						foreach ($chk_task as $key => $value) {
							...
							$Querytemp  = 'SELECT rsnofleaving,dol,disposition from exit_emp where EmployeeID  = ?  order by id desc limit 1';

							$stmt = $conn->prepare($Querytemp);
							...
							$row = $temp_result->fetch_row();


							if (!empty(clean($row[0])) && $value['emp_status'] == 'InActive' && !empty(clean($row[1]))) {
								$i = 1;
								while ($i <= 31) {
									$date_1 = date('Y-m-d', strtotime(clean($row[1])));
									$date_2 = date('Y-m-d', strtotime($value['Year'] . '/' . $value['Month'] . '/' . $i));
									if ($date_2 == $date_1) {
										if (strtoupper(clean($row[0])) == 'RES' || strtoupper(clean($row[0])) == 'RES') {
											$table .= '<td>' . $value['D' . $i] . '</td>';
										} else if (!empty(clean($row[2]))) {
											$table .= '<td>' . clean($row[2]) . '</td>';
										} else {
											$table .= '<td>' . clean($row[0]) . '</td>';
										}
									} else if ($date_2 > $date_1) {
										if (!empty(clean($row[2]))) {
											$table .= '<td>' . clean($row[2]) . '</td>';
										} else {
											$table .= '<td>' . clean($row[0]) . '</td>';
										}
							...
							if (!empty(clean($row[0])) && $value['emp_status'] == 'InActive' && !empty(clean($row[1]))) {
								$table .= '<td>' . date('Y-m-d', strtotime(clean($row[1]))) . '</td>';
							} else {
								$table .= '<td></td>';
							}
