...
// 	$queryto = cleanUserInput($_POST['queryto']);
// 	$querysub = cleanUserInput($_POST['querysub']);
// 	$querybody = cleanUserInput($_POST['querybody']);
			...
			<div class="schema-form-section row">
				...
				<div id="pnlTable">
					...
					if ($result) {
						...
						echo '<thead><tr><th class="hidden">id</th><th class="">EmployeeID</th><th>Request By</th><th>Query </th><th>Communicated With</th><th>Request Date</th><th>Status</th><th>Manage</th></thead>';
						foreach ($result as $key => $value) {
							...
							if ($value['requestby'] == clean($_SESSION['__user_logid']) || ($value['requestby'] == 'CE03070003' && clean($_SESSION['__user_logid']) == 'CE01145570' && clean($_SESSION['__user_logid']) == 'CE021929762')) {
								echo '<td><a target="_self" class="btn waves-effect waves-green red darken-3 white-text" style="min-width: 5px; !important" href=" ' . URL . 'View/open_issue.php?ID*DSAad=refcode_' . base64_encode($value['id']) . '_dabhd" >Open</a></td>';
							} else {
								if ($value['status'] == 'Reopen') {
									echo '<td><a target="_self" class="btn waves-effect modal-action modal-close waves-red close-btn" style="min-width: 5px; !important" href="' . URL . 'View/check_issue.php?ID*DSAad=refcode_' . base64_encode($value['id']) . '_dabhd">View</a></td>';
								} else {
									echo '<td><a target="_self" class="btn waves-effect waves-green" style="min-width: 5px; !important" href="' . URL . 'View/check_issue.php?ID*DSAad=refcode_' . base64_encode($value['id']) . '_dabhd" >View</a></td>';
								}
							}
							echo '</tr>';
						}
