...
function getPath($val_type, $val_stype)
{
	$rootloc = '';
	if ($_POST['loc'] == "1" || $_POST['loc'] == "2") {
		if ($val_type == 'Proof of Identity') {
			return $filePath = ROOT_PATH . "Docs/IdentityProof/";
		} else if ($val_type == 'Proof of Address') {
			return $filePath = ROOT_PATH . "Docs/AddressProof/";
		} else if ($val_type == 'FnF') {
			return $filePath  = ROOT_PATH . "Docs/FnF/";
		} else if ($val_type == 'Apology Letter') {
			return $filePath  = ROOT_PATH . "Docs/ApologyLetter/";
		} else if ($val_type == 'Warning Letter') {
			return $filePath  = ROOT_PATH . "Docs/WarningLetter/";
		} else if ($val_type == 'Other' && $val_stype == 'CheckList') {
			return $filePath  = ROOT_PATH . "checklist_pdf/";
		} else if ($val_type == 'Other' && ($val_stype == 'Resume' || $val_stype == 'Curriculum Vitae')) {
			return $filePath  = ROOT_PATH . "Resume/";
		} else if ($val_type == 'Other' && $val_stype == 'Salary Proof') {
			return $filePath  = ROOT_PATH . "Docs/salaryslip/";
		} else if ($val_type == 'Other' && $val_stype == 'PAN Card') {
			return $filePath  = ROOT_PATH . "Docs/IdentityProof/";
		} else if ($val_type == 'Other' && $val_stype == 'Warning Letter') {
			return $filePath  = ROOT_PATH . "Docs/WarningLetter/";
		} else if ($val_type == 'BGV Report') {
			return $filePath  = ROOT_PATH . "Docs/BGV/";
		} else if ($val_type == 'Call Log') {
			return $filePath  = ROOT_PATH . "Docs/CallLog/";
		} else if ($val_type == 'Covid 19') {
			return $filePath  = ROOT_PATH . "Docs/Covid19/";
		} else if ($val_type == 'Apprenticeship') {
		...
		if ($val_type == 'Proof of Identity') {
			...
			return $filePath  = ROOT_PATH . "Meerut/checklist_pdf/";
		} else if ($val_type == 'Other' && ($val_stype == 'Resume' || $val_stype == 'Curriculum Vitae')) {
			return $filePath  = ROOT_PATH . "Meerut/Resume/";
		} else if ($val_type == 'Other' && $val_stype == 'Salary Proof') {
			return $filePath  = ROOT_PATH . "Meerut/Docs/salaryslip/";
		} else if ($val_type == 'Other' && $val_stype == 'PAN Card') {
			return $filePath  = ROOT_PATH . "Meerut/Docs/IdentityProof/";
		} else if ($val_type == 'Other' && $val_stype == 'Warning Letter') {
			return $filePath  = ROOT_PATH . "Meerut/Docs/WarningLetter/";
		...
		if ($val_type == 'Proof of Identity') {
			...
			return $filePath  = ROOT_PATH . "Bareilly/checklist_pdf/";
		} else if ($val_type == 'Other' && ($val_stype == 'Resume' || $val_stype == 'Curriculum Vitae')) {
			return $filePath  = ROOT_PATH . "Bareilly/Resume/";
		} else if ($val_type == 'Other' && $val_stype == 'Salary Proof') {
			return $filePath  = ROOT_PATH . "Bareilly/Docs/salaryslip/";
		} else if ($val_type == 'Other' && $val_stype == 'PAN Card') {
			return $filePath  = ROOT_PATH . "Bareilly/Docs/IdentityProof/";
		} else if ($val_type == 'Other' && $val_stype == 'Warning Letter') {
			return $filePath  = ROOT_PATH . "Bareilly/Docs/WarningLetter/";
		...
		if ($val_type == 'Proof of Identity') {
			...
			return $filePath  = ROOT_PATH . "Vadodara/checklist_pdf/";
		} else if ($val_type == 'Other' && ($val_stype == 'Resume' || $val_stype == 'Curriculum Vitae')) {
			return $filePath  = ROOT_PATH . "Vadodara/Resume/";
		} else if ($val_type == 'Other' && $val_stype == 'Salary Proof') {
			return $filePath  = ROOT_PATH . "Vadodara/Docs/salaryslip/";
		} else if ($val_type == 'Other' && $val_stype == 'PAN Card') {
			return $filePath  = ROOT_PATH . "Vadodara/Docs/IdentityProof/";
		} else if ($val_type == 'Other' && $val_stype == 'Warning Letter') {
			return $filePath  = ROOT_PATH . "Vadodara/Docs/WarningLetter/";
		...
		if ($val_type == 'Proof of Identity') {
			...
			return $filePath  = ROOT_PATH . "Manglore/checklist_pdf/";
		} else if ($val_type == 'Other' && ($val_stype == 'Resume' || $val_stype == 'Curriculum Vitae')) {
			return $filePath  = ROOT_PATH . "Manglore/Resume/";
		} else if ($val_type == 'Other' && $val_stype == 'Salary Proof') {
			return $filePath  = ROOT_PATH . "Manglore/Docs/salaryslip/";
		} else if ($val_type == 'Other' && $val_stype == 'PAN Card') {
			return $filePath  = ROOT_PATH . "Manglore/Docs/IdentityProof/";
		} else if ($val_type == 'Other' && $val_stype == 'Warning Letter') {
			return $filePath  = ROOT_PATH . "Manglore/Docs/WarningLetter/";
		...
		if ($val_type == 'Proof of Identity') {
			...
			return $filePath  = ROOT_PATH . "Bangalore/checklist_pdf/";
		} else if ($val_type == 'Other' && ($val_stype == 'Resume' || $val_stype == 'Curriculum Vitae')) {
			return $filePath  = ROOT_PATH . "Bangalore/Resume/";
		} else if ($val_type == 'Other' && $val_stype == 'Salary Proof') {
			return $filePath  = ROOT_PATH . "Bangalore/Docs/salaryslip/";
		} else if ($val_type == 'Other' && $val_stype == 'PAN Card') {
			return $filePath  = ROOT_PATH . "Bangalore/Docs/IdentityProof/";
		} else if ($val_type == 'Other' && $val_stype == 'Warning Letter') {
			return $filePath  = ROOT_PATH . "Bangalore/Docs/WarningLetter/";
		...
		if ($val_type == 'Proof of Identity') {
			...
			return $filePath  = ROOT_PATH . "Nashik/checklist_pdf/";
		} else if ($val_type == 'Other' && ($val_stype == 'Resume' || $val_stype == 'Curriculum Vitae')) {
			return $filePath  = ROOT_PATH . "Nashik/Resume/";
		} else if ($val_type == 'Other' && $val_stype == 'Salary Proof') {
			return $filePath  = ROOT_PATH . "Nashik/Docs/salaryslip/";
		} else if ($val_type == 'Other' && $val_stype == 'PAN Card') {
			return $filePath  = ROOT_PATH . "Nashik/Docs/IdentityProof/";
		} else if ($val_type == 'Other' && $val_stype == 'Warning Letter') {
			return $filePath  = ROOT_PATH . "Nashik/Docs/WarningLetter/";
		...
		if ($val_type == 'Proof of Identity') {
			...
			return $filePath  = ROOT_PATH . "Anantapur/checklist_pdf/";
		} else if ($val_type == 'Other' && $val_stype == 'PAN Card') {
			return $filePath  = ROOT_PATH . "Anantapur/Docs/IdentityProof/";
		} else if ($val_type == 'Other' && ($val_stype == 'Resume' || $val_stype == 'Curriculum Vitae')) {
			return $filePath  = ROOT_PATH . "Anantapur/Resume/";
		} else if ($val_type == 'Other' && $val_stype == 'Salary Proof') {
			return $filePath  = ROOT_PATH . "Anantapur/Docs/salaryslip/";
		} else if ($val_type == 'Other' && $val_stype == 'Warning Letter') {
			return $filePath  = ROOT_PATH . "Anantapur/Docs/WarningLetter/";
		} else if ($val_type == 'BGV Report') {
...
if (isset($_POST['btn_document_add']) && $EmployeeID != '' && $_POST['documentid'] == "") {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		$clean_documentid = cleanUserInput($_POST['documentid']);
		$myDB = new MysqliDb();
		$file_counter = 0;
		if (is_array($_FILES)) {
			$count = 0;
			foreach ($_FILES['txt_doc_name_']['name'] as $name => $value) {
				...
				if (is_uploaded_file($_FILES['txt_doc_name_']['tmp_name'][$name])) {
					...
					$clean_txt_doc_type_ = cleanUserInput($_POST['txt_doc_type_' . $count]);
					$val_type = trim($clean_txt_doc_type_);
					$clean_txt_doc_stype_ = cleanUserInput($_POST['txt_doc_stype_' . $count]);
					$val_stype = trim($clean_txt_doc_stype_);
					$clean_txt_doc_value_ = cleanUserInput($_POST['txt_doc_value_' . $count]);
					$docVal = trim($clean_txt_doc_value_);
					$filePath = getPath($val_type, $val_stype);
					...
					if ($val_type == 'BGV Report' || $val_type == 'Covid 19' || $val_stype == 'Curriculum Vitae' || $val_stype == 'Resume' || $val_stype == 'WFH Policy' || $val_stype == 'System Undertaking' || $txt_doc_stype == 'Apprenticeship' || $txt_doc_stype == 'Signature') {
					...
					if ($val_stype == 'Aadhar Card' || $val_stype == 'PAN Card') {
						...
						if (empty($clean_txt_doc_value_) || ($val_stype == 'Aadhar Card' && (strlen($clean_txt_doc_value_) != 12 || !is_numeric($clean_txt_doc_value_)))) {
							$validation_check = 1;
						} else {
							// $data_calid = $myDB->query('SELECT  whole_details_peremp.EmployeeID FROM whole_details_peremp INNER JOIN doc_details ON doc_details.EmployeeID = whole_details_peremp.EmployeeID where trim(doc_details.dov_value) = trim("' . $clean_txt_doc_value_ . '") and doc_details.doc_stype = "' . $val_stype . '" and doc_details.doc_type = "' . $val_type . '"');
							$docV = trim($clean_txt_doc_value_);
							$data_calidQry = 'SELECT whole_details_peremp.EmployeeID FROM whole_details_peremp INNER JOIN doc_details ON doc_details.EmployeeID = whole_details_peremp.EmployeeID where trim(doc_details.dov_value) = ? and doc_details.doc_stype = ? and doc_details.doc_type =?';
							$stmt = $conn->prepare($data_calidQry);
							$stmt->bind_param("sss", $docV, $val_stype, $val_type);
							$stmt->execute();
							$data_calid = $stmt->get_result();
							if ($data_calid->num_rows > 0) {
								$validation_check = 1;
							}
						}
					}
					...
					$data_validQry = "SELECT EmployeeID  from doc_details where doc_type=? and  doc_stype=? and  dov_value=? and  EmployeeID=?";
					$stm = $conn->prepare($data_validQry);
					$stm->bind_param("ssss", $val_type, $val_stype, $docVal, $EmployeeID);
					$stm->execute();
					$data_valid = $stm->get_result();
					// print_r($data_valid);
					// die;
					if ($data_valid->num_rows > 0) {
						$validation_check = 1;
					}
					if ($validation_check === 0 && $uploadOk == 1) {
						if (move_uploaded_file($sourcePath, $targetPath)) {
							$ext = pathinfo(basename($_FILES['txt_doc_name_']['name'][$name]), PATHINFO_EXTENSION);
							$filename = $EmployeeID . preg_replace('/\s+/', '', $clean_txt_doc_stype_) . '_' . date("mdY_s") . '.' . $ext;
							$file = rename($targetPath, $filePath . $filename);
							if (file_exists($filePath . $filename)) {
								$myDB = new MysqliDb();
								$sqlInsertDoc = 'call insert_empdoc("' . $clean_txt_doc_type_ . '","' . $clean_txt_doc_stype_ . '","' . $clean_txt_doc_value_ . '","' . $filename . '","' . $EmployeeID . '")';
							...
							echo "<script>$(function(){ toastr.error('The file not uploaded.') });</script>";
						}
					} else {
						if ($uploadOk) {
							echo "<script>$(function(){ toastr.error('Document not updated. May be document is alredy exist or the value you entered is not valid') });</script>";
						}
					}
				}
			}
...
if (isset($_POST['btn_document_save']) && $EmployeeID != '' && $_POST['documentid'] != "") {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		...
		$txt_doc_stype = '';
		$val_stype = '';
		$clean_doc_type1 = cleanUserInput($_POST['txt_doc_type_1']);
		if (isset($clean_doc_type1)) {
			$txt_doc_type = $val_type = $clean_doc_type1;
		} else {
			$val_type = 'Proof of Identity';
		}
		$clean_stype_1 = cleanUserInput($_POST['txt_doc_stype_1']);
		if (isset($clean_stype_1)) {
			$txt_doc_stype = $clean_stype_1;
		}
		$clean_doc_val1 = cleanUserInput($_POST['txt_doc_value_1']);
		if (isset($clean_doc_val1)) {
			$txt_doc_value = $clean_doc_val1;
		}
		if (isset($_FILES['txt_doc_name_']['name'][0]) && $_FILES['txt_doc_name_']['name'][0] != "") {
			$filePath = getPath($val_type, $txt_doc_stype);
			$sourcePath = $_FILES['txt_doc_name_']['tmp_name'][0];
			$targetPath = $filePath . basename($_FILES['txt_doc_name_']['name'][0]);
			$FileType = strtolower(pathinfo($targetPath, PATHINFO_EXTENSION));
			$uploadOk = 1;
			$validation_check = 0;
			if ($val_type == 'BGV Report' || $val_type == 'Covid 19' || $txt_doc_stype == 'Curriculum Vitae' || $txt_doc_stype == 'Resume' || $txt_doc_stype === 'WFH Policy' || $txt_doc_stype == 'System Undertaking' || $txt_doc_stype == 'Apprenticeship' || $txt_doc_stype == 'Signature') {
			...
			if ($validation_check === 0 && $uploadOk == 1) {
				if (move_uploaded_file($sourcePath, $targetPath)) {
					...
					$clean_doc_stype1 = cleanUserInput($_POST['txt_doc_stype_1']);
					if (isset($clean_doc_stype1)) {
						$filename = $EmployeeID . preg_replace('/\s+/', '', $clean_doc_stype1) . '_' . date("mdY_s") . '.' . $ext;
					} else {
					...
					if (file_exists($filePath . $filename)) {
						...
						// $selectDoc = $myDB->rawQuery("select doc_type,doc_file from doc_details where doc_id='" . $clean_documentid . "'"); //
						$docID = $clean_documentid;
						$selectDocQry = "select doc_type,doc_file from doc_details where doc_id=?";
						$st = $conn->prepare($selectDocQry);
						$st->bind_param("s", $docID);
						$st->execute();
						$selectDoc = $st->get_result();
						$selectDocRow = $selectDoc->fetch_row();
						$val_type = '';
						$efilename = '';
						if ($selectDoc->num_rows > 0) {
							$val_type = $selectDocRow[0]; //['doc_type'];
							$efilename = $selectDocRow[1]; //['doc_file'];
						}
						$efilePath = getPath($val_type,	$val_stype);

						if (file_exists($efilePath . $efilename)) {
							@unlink($efilePath . $efilename);
						}
						$docsID = $clean_documentid;
						// $updateDocument = "update doc_details set doc_file='" . $filename . "' where doc_id='" . $clean_documentid . "'";
						$updateDocument = "update doc_details set doc_file=? where doc_id=?";
						$stdoc = $conn->prepare($updateDocument);
						$stdoc->bind_param("si", $filename, $docsID);
						$stdoc->execute();
						$result = $stdoc->get_result();
						// print_r($result);
						// die;
		...
		$updateDocument .= " where doc_id='" . $clean_documentid . "'";
		$myDB = new MysqliDb();
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				<div class="col s12 m12">
					...
					$stt = $conn->prepare($sqlConnect);
					$stt->bind_param("s", $EmployeeID);
					$stt->execute();
			...
			<i class="material-icons edit_item imgBtn imgBtnEdit tooltipped" onclick="javascript:return EditDoc(this);"  data-file="' . $value['doc_file'] . '" id="' . $value['doc_id'] . '_' . $value['doc_type'] . '" data-position="left" data-tooltip="Edit">ohrm_edit</i>';
									if ($value['doc_type'] == 'BGV Report' && $FileType == 'pdf') {
										...
										} else if (file_exists('../' . $rootloc . '/Other/' . $value['doc_file'])) {
											echo '<i class="material-icons download_item imgBtn imgBtnDownload tooltipped"  data="' . $value['doc_file'] . '" id="' . $value['doc_type'] . '" data-position="left" data-tooltip="Download File"><a download  href="../' . $rootloc . '/Other/' . $value['doc_file'] . '" target="_blank">ohrm_file_download</a></i>';
										} else {
											echo "No File";
										}
									} else  if (($value['doc_type'] == 'Other') && $value['doc_stype'] == 'Resume' && $FileType4 == 'pdf') {
										...
										} else if (file_exists('../' . $dir_loc . '/AddressProof/' . $value['doc_file'])) {
											echo '<i class="material-icons download_item imgBtn imgBtnDownload tooltipped"  data="' . $value['doc_file'] . '" id="' . $value['doc_type'] . '" data-position="left" data-tooltip="Download File"><a download  href="../' . $dir_loc . '/AddressProof/' . $value['doc_file'] . '" target="_blank">ohrm_file_download</a></i>';
										} else {
											echo "No File";
										}
						...
						function changeevent() {
							$('*[id^=txt_doc_type_]').change(function() {
								...
								if ($(this).val() == 'Proof of Identity') {
									...
									$(this).closest('tr').children("td:nth-child(4)").find('*[id^=txt_doc_stype_]').empty().append('<option>Resume</option><option>Interview Form</option><option>Salary Proof</option><option>Undertaking</option><option>WFO-WFH Declaration</option><option>System Undertaking</option><option>WFH Policy</option><option>Signature</option>');
						...
						if ($(el).attr("data") != '') {
							...
							$.ajax({
							 ...
							 start  check inner file
							   */
									if ($(el).attr("id") == "Other" && ($(el).attr("id1") == "Curriculum Vitae" || $(el).attr("id1") == "Resume")) {
									...
									} else if ($(el).attr("id") == "Proof of Address" && $(el).attr("id1") == "Aadhar Card") {
										docfile1 = Docs + "AddressProof/";

										$.ajax({

											url: "../" + docfile1 + $(el).attr("data"),
											type: 'HEAD',
											error: function() {
												alert('No File Exist');
												//alert('1');							        //flag=1;
											...
											success: function() {
												...
												imgchecker = imgcheck("../" + docfile1 + $(el).attr("data"));

												if (imgchecker.match(/(jpg|jpeg|png|gif)$/i)) {
													getImageDimensions("../" + docfile1 + $(el).attr("data"), function(data) {
														var img = data;

														$('<img>', {
															src: "../" + docfile1 + $(el).attr("data")
														}).watermark({
															//text: 'ⓒ For Cogent E Services Ltd.',
															text: 'Cogent E Services Ltd.',
															//path:'../Style/images/cogent-logobkp.png',
													...
													window.open("../" + docfile1 + $(el).attr("data"));
													//window.open("../FileContainer/pdf_watermark/watermark-edit-existing-pdf.php?src="+"../.."+docfile1+$(el).attr("data"));
												} else {
													window.open("../" + docfile1 + $(el).attr("data"));
												}
									...
									} else if ($(el).attr("id") == "Other" && $(el).attr("id1") == "Salary Proof") {
										docfile1 = Docs + "salaryslip/";
									...
									} else if ($(el).attr("id") == "Other" && $(el).attr("id1") == "Warning Letter") {
										docfile1 = Docs + "WarningLetter/";
									...
									} else if ($(el).attr("id") == "Other" && $(el).attr("id1") == "PAN Card") {
										docfile1 = Docs + "IdentityProof/";
