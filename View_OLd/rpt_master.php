...
if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
	...
	$emp_statuss = cleanUserInput($_POST['emp_status']);
}

if (isset($emp_statuss)) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		$EmpStatus = cleanUserInput($emp_statuss);
	}
}
?>
			...
			<div class="schema-form-section row">
				...
				if (isset($_POST['btn_view'])) {
					...
					if ($EmpStatus == 'Active') {
						...
						$chktask = "select distinct t1.*,concat('\'',adhar.dov_value,'\'') as AdharCard,pan.dov_value as PanCard from View_EmpinfoInActive t1 LEFT JOIN  (select distinct EmployeeID,dov_value from doc_details where doc_stype='Aadhar Card' group by EmployeeID order by createdon desc) adhar on t1.EmployeeID=adhar.EmployeeID LEFT JOIN (select distinct EmployeeID, dov_value from doc_details where doc_stype='PAN Card' group by EmployeeID order by createdon desc) pan  on t1.EmployeeID=pan.EmployeeID where Year(DOJ)=?";
						$selectQ = $conn->prepare($chktask);
						$selectQ->bind_param("i", $Year);
						$selectQ->execute();
					}
