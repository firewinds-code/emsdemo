...
$emp = isset($_POST['EmployeeID']);
if ($ID) {
	$EmployeeID = clean($_REQUEST['ID']);
	$mobNo = clean($_REQUEST['mob']);
} else if ($emp) {
	$EmployeeID = cleanUserInput($_POST['EmployeeID']);
	$mobNo = cleanUserInput($_POST['mobno']);
}
...
if ($btnsave) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		if ($loc != '') {
			$contact_status = cleanUserInput($_POST['contact_status']);
			$issue = cleanUserInput($_POST['subb_issue']);
			$belongsto = cleanUserInput($_POST['queryto']);
			$issue1 = cleanUserInput($_POST['querysub']);
			$handler = cleanUserInput($_POST['handler']);
			$remark = cleanUserInput($_POST['remark']);
			$location = cleanUserInput($_POST['loc']);
			...
			if ($rowCount > 0) {
				...
				if (!empty($_POST['emailid'])) {
					...
					$selectQr = $conn->prepare($select_email);
					$selectQr->bind_param("si", $pagename, $location);
					$selectQr->execute();
					$select_email_array = $selectQr->get_result();
					...
					// $myDB = new MysqliDb();
					$query = "select max(id) as id from issue_tracker  where requestby =? and cast(request_date as date)= curdate() limit 1;";
					$selectQy = $conn->prepare($query);
					$selectQy->bind_param("s", $createdby);
					$selectQy->execute();
					$result = $selectQy->get_result();
					$refID = $result->fetch_row();
					$refID_id = clean($refID[0]);
					...
					if ($mobNo != '') {
						...
						$msg = "Your request for $issue Submited.We will try our best to resolve it as soon as possible. Thank You " . $gender_last . ". " . $EmployeeName;
				...
				echo "<script>$(function(){ toastr.success('Issuee Request Submited For " . $gender_last . " . " . $EmployeeName . " and " . $lbl_msg . " '); }); </script>";
			} else {
	...
	$(function() {
		...
		$('#btnSave').click(function() {
			...
			if ($('#contact_status').val() == 'NA') {
				...
				if ($('#span_contact_status').length == 0) {
