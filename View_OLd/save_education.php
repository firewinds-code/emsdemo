...
if (isset($_POST['btn_education_Add']) && $EmployeeID != '') {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		...
		$loc = cleanUserInput($_POST['loc']);
		$dir_location = cleanUserInput($_POST['dir']);
		if ($loc == "1" || $loc == "2") {
		...
		if ($uploadOk == 1) {
			...
			$select = 'select * from education_details where edu_name = ? and edu_level = ? and EmployeeID = ?';
			$selectQ = $conn->prepare($select);
			$selectQ->bind_param("sss", $Name_1, $lvl_1, $EmployeeID);
			$selectQ->execute();
			$select_exist_data = $selectQ->get_result();
			$counter_check = 0;
			if ($select_exist_data) {
				if ($select_exist_data->num_rows > 0) {
					$counter_check = 1;
				}
			}
			// echo $file;
			...
			if ($counter_check == 0) {
				...
				if (cleanUserInput($_POST['eduselect']) != '' && $select_exist_data->num_rows == 1) {
					$eduselect = cleanUserInput($_POST['eduselect']);
					$sqlInsertDoc = 'call save_education("' . $lvl_1 . '","' . $Name_1 . '","' . $Spc_1 . '","' . $bu_1 . '","' . $college_1 . '","' . $type_1 . '","' . $division_1 . '","' . $EmployeeID . '","' . $file . '","' . $createBy . '","' . $percentage_1 . '","' . $eduselect . '","' . $pass . '")';
					// echo $sqlInsertDoc;
					// die;
					$myDB = new MysqliDb();
					$result = $myDB->query($sqlInsertDoc);
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				<div class="container col s12 m12" id="childtables">
					...
					$mysql_error = $myDB->getLastError();
					if (($resultBy->num_rows > 0) and isset($_REQUEST['empid'])) {
					...
					<div class="input-field col s6 m6">
						...
						<select id="txt_edu_Name_1" name="txt_edu_Name_1">
							...
							// die;
							$selectQ = $conn->prepare($sql);
							$selectQ->bind_param("s", $edu_level);
							$selectQ->execute();
									...
									<table id="myTable" class="data dataTable no-footer row-border" cellspacing="0" width="100%">
										...
										<tbody>
											...
											foreach ($result as $key => $value) {
												...
												echo '<td class="manage_item" ><i class="material-icons edit_item imgBtn imgBtnEdit tooltipped" onclick="javascript:return Edit(this);" data="' . $value['edu_id'] . '"   data-position="left" data-tooltip="Edit">ohrm_edit</i>
