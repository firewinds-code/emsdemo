...
// echo $briefingId;
// die;
if ($briefingId && $briefingId != '') {
	// $briefingId = $_GET['id'];
	// $rows = $myDB->query("Select * from brf_question where BriefingID='" . $briefingId . "'");
	$rowsQry = "Select * from brf_question where BriefingID=?";
	$stmt = $conn->prepare($rowsQry);
	$stmt->bind_param("i", $briefingId);
	$stmt->execute();
	$rowsRes = $stmt->get_result();
	$rows = $rowsRes->fetch_row();
	// print_r($rows);
	// die;
	$my_error = $myDB->getLastError();
	//echo count($rows);
	if ($rowsRes->num_rows > 0) {
