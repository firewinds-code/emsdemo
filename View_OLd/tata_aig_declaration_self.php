...
if (isset($_POST['btnSave'])) {
    if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
        ...
        $query = "insert into tataaig_decl_self (EmployeeID, EmployeeName,FatherName,designation,mobile,location,status) values(?,?,?,?,?,?,?) ";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("ssssiss", $EmployeeID, $EmployeeName, $FatherName, $Designation, $mobile, $location, $status);
        $stmt->execute();
        $result = $stmt->get_result();
        if ($stmt->affected_rows === 1) {
            echo "<script>$(function(){toastr.success('Acknowledge Successfully')})</script>";
            echo "<scrip>location.href='index.php'; </scrip    t>";
        } else {
