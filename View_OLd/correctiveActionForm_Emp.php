...
if ($submit) {
    if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
        ...
        $corrective_Formid = cleanUserInput($_POST['hrFormid']);
        $empComment = addslashes(trim($_POST['emp_comment']));
        $QueryHr = "insert into corrective_action_formemp (corrective_Formid, emp_comment, created_by) VALUES (?,?,?)";
        $selectQury = $conn->prepare($QueryHr);
        $selectQury->bind_param("sss", $corrective_Formid, $empComment, $EmployeeID);
        $result_Hr = $selectQury->execute();

        $last_id = $myDB->getInsertId();
        if ($selectQury->affected_rows === 1) {
...
function warning_pdf($EmployeeID)
{
    ...
    $GetData = "select w.EmployeeID , w.EmployeeName , w.designation ,p.location, wr.issue_type,wr.issue_comment,w.Gender,head.pli, head.incentive from whole_dump_emp_data w inner join personal_details p on w.EmployeeID=p.EmployeeID inner join corrective_action_form wr on w.EmployeeID=wr.employee_id join corrective_action_formhead head on wr.id=head.corrective_Formid where w.EmployeeID=?;";
    $selectQury = $conn->prepare($GetData);
    $selectQury->bind_param("s", $EmployeeID);
    $selectQury->execute();
    $result = $selectQury->get_result();
    $resultsE = $result->fetch_row();

    if ($result->num_rows > 0) {
        $EmployeeID = $resultsE[0];
        $EmployeeName = $resultsE[1];
        $Designation = $resultsE[2];
        $locationid = $resultsE[3];
        $WarningReason = $resultsE[4];
        $Comments = $resultsE[5];
        $Gender = $resultsE[6];
        $pli = $resultsE[7];
        $incentive = $resultsE[8];
    }
    ...
    $tcpdf->Image('../Style/images/newLogo cogent.jpg', 140, 5, 60, 30, 'JPG'); //logo right
            ...
            <div class="schema-form-section row">
                <div id="pnlTable">
                    <div class="had-container pull-left row card" style="margin-top: 10px;width: 100%;padding: 15px;">
                        <div>
                            <?php if (isset($allData) and $allData->num_rows > 0) {
                            ?>
