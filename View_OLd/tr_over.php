	...
	if (isset($_POST['cb'])) {
		...
		if ($count_check > 0) {
			...
			foreach ($_POST['cb'] as $val) {
				...
				$myDB = new MysqliDb();
				$save = 'call manage_status_th_after("' . $empID . '","' . $txt_remark . '","' . $createBy . '","TH OVER")';
		...
		if (isset($_POST['cb'])) {
			...
			if ($count_check > 0) {
				...
				foreach ($_POST['cb'] as $val) {
					...
					$txt_remark = cleanUserInput($_POST['txt_Remark_' . $empID]);
					$myDB = new MysqliDb();
					$save = 'call manage_refer_hr("' . $empID . '","' . $createBy . '","' . $txt_remark . '","TH OVER REFER")';
					$resulti = $myDB->query($save);
					$mysql_error = $myDB->getLastError();
					if (empty($mysql_error)) {
			...
			if ($count_check > 0) {
				...
				foreach ($_POST['cb'] as $val) {
					...
					if ($check_rtr_q->num_rows > 0 && $check_rtr_q) {
						...
						$txt_remark = cleanUserInput($_POST['txt_Remark_' . $empID]);
						$myDB = new MysqliDb();
						$save = 'call manage_retrain("' . $empID . '","' . $createBy . '","' . $txt_remark . '","TH OVER Retrain")';
						$resulti = $myDB->query($save);
						$mysql_error = $myDB->getLastError();
						if (empty($mysql_error)) {
			...
			<div class="schema-form-section row">
				...
				<div class="form-inline">
					...
					<div id="pnlTable" class="col s12 m12 card">
						<?php
						$sqlConnect = 'call get_thchecklist_over("' . $user_logid . '")';
						$myDB = new MysqliDb();
