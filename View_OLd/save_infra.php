...
if (isset($_REQUEST['empid']) && clean($_REQUEST['empid']) != "") {
...
if ($result->num_rows > 0) {
	$loc = clean($resultRow[0]); //['location'];
}
if ($loc == "1" || $loc == "2") {
	$dir_location = '';
} else if ($loc == "3") {
...
if (strtoupper($_SERVER['REQUEST_METHOD']) == 'POST') {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		$sys_names = cleanUserInput($_POST['sys_name']);
		$sys_name = (isset($sys_names) ? $sys_names : null);
		$sys_availables = cleanUserInput($_POST['sys_available']);
		$sys_available = (isset($sys_availables) ? $sys_availables : null);
		$sys_processors = cleanUserInput($_POST['sys_processor']);
		$sys_processor = (isset($sys_processors) ? $sys_processors : null);
		$internet_avails = cleanUserInput($_POST['internet_avail']);
		$internet_avail = (isset($internet_avails) ? $internet_avails : null);
		$internet_types = cleanUserInput($_POST['internet_type']);
		$internet_type = (isset($internet_types) ? $internet_types : null);
		$service_providers = cleanUserInput($_POST['service_provider']);
		$service_provider = (isset($service_providers) ? $service_providers : null);
		$internet_plans = cleanUserInput($_POST['internet_plan']);
		$internet_plan = (isset($internet_plans) ? $internet_plans : null);
	}
} else {
...
$emplid = isset($_REQUEST['empid']);
if ($emplid && $EmployeeID == '' && !isset($_POST['txt_Infra_account'])) {
	$EmployeeID = clean($_REQUEST['empid']);
...
$btn_infra_Add = isset($_POST['btn_infra_Add']);
if (($btn_infra_Add || isset($_POST['btn_infra_Save'])) && $EmployeeID != '') {
	$username = $EmployeeID;
	...
	<div class="pim-container row" id="div_main">
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				$sqlInfraQuery = "select * from infra_details where EmployeeID=? ";
				$stmt1 = $conn->prepare($sql);
				$stmt1->bind_param("s", $EmployeeID);
				$stmt1->execute();
				$data_array = $stmt1->get_result();
				...
				if ($data_array->num_rows > 0) {
					$sys_name = clean($data_arrayRow[3]); //['sys_name'];
					$sys_available = clean($data_arrayRow[4]); //['sys_available'];
					$sys_processor = clean($data_arrayRow[5]); //['sys_processor'];
					$sys_img = clean($data_arrayRow[6]); //['sys_img'];
					$internet_avail = clean($data_arrayRow[7]); //['internet_avail'];
					$internet_type = clean($data_arrayRow[8]); //['internet_type'];
					$service_provider = clean($data_arrayRow[9]); //['service_provider'];
					$internet_plan = clean($data_arrayRow[10]); //['internet_plan'];
					$inlanproof_doc = clean($data_arrayRow[11]); //['inlanproof_doc'];
					$Infraid = clean($data_arrayRow[0]); //['ID'];
				}
