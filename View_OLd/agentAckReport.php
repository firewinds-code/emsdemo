		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				<div id="pnlTable">
					...
					$sqlConnect = "select a.*,b.heading,b.id,b.quiz from brf_acknowledge a INNER JOIN brf_briefing b on a.BriefingId=b.id where a.EmployeeID=? order by a.id desc";
								...
								<table id="myTable" class="data dataTable no-footer row-border" cellspacing="0" width="100%">
									...
									<tbody>
										...
										foreach ($result as $key => $value) {
											...
											$select_attempted = "select AttemptedDate from brf_quiz_attempted where BriefingID=? and EmployeeID=?";
											$stmt = $conn->prepare($select_attempted);
											$stmt->bind_param("is", $bf_id, $userID);
											$stmt->execute();
											$Quizresult = $stmt->get_result();
											$QuizresultRow = $Quizresult->fetch_row();
											// $myDB = new MysqliDb();
											...
											if ($Quizresult->num_rows > 0) {
												...
												$date_time = clean($QuizresultRow[0]); //['AttemptedDate'];
											} else {
												$attempt = 'No';
											}
					...
					<?php
						...
						echo "<script>$(function(){ toastr.error('Quiz Not Found ') }); </script>";
					}
