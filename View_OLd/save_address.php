...
if (strtoupper($_SERVER['REQUEST_METHOD']) == 'POST') {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		$txt_address_add = cleanUserInput($_POST['txt_address_add']);
		$add = (isset($txt_address_add) ? $txt_address_add : null);
		$txt_address_country = cleanUserInput($_POST['txt_address_country']);
		$country = (isset($txt_address_country) ? $txt_address_country : null);
		$txt_address_state = cleanUserInput($_POST['txt_address_state']);
		$state = (isset($txt_address_state) ? $txt_address_state : null);
		$txt_address_dist = cleanUserInput($_POST['txt_address_dist']);
		$dist = (isset($txt_address_dist) ? $txt_address_dist : null);
		$txt_address_city = cleanUserInput($_POST['txt_address_city']);
		$city = (isset($txt_address_city) ? $txt_address_city : null);
		$txt_address_tehsil = cleanUserInput($_POST['txt_address_tehsil']);
		$tehsil = (isset($txt_address_tehsil) ? $txt_address_tehsil : null);
		$txt_address_other = cleanUserInput($_POST['txt_address_other']);
		$other = (isset($txt_address_other) ? $txt_address_other : null);
		$txt_address_zip = cleanUserInput($_POST['txt_address_zip']);
		$zip = (isset($txt_address_zip) ? $txt_address_zip : null);

		$txt_address_add_p = cleanUserInput($_POST['txt_address_add_p']);
		$add_p = (isset($txt_address_add_p) ? $txt_address_add_p : null);
		$txt_address_country_p = cleanUserInput($_POST['txt_address_country_p']);
		$country_p = (isset($txt_address_country_p) ? $txt_address_country_p : null);
		$txt_address_state_p = cleanUserInput($_POST['txt_address_state_p']);
		$state_p = (isset($txt_address_state_p) ? $txt_address_state_p : null);
		$txt_address_dist_p = cleanUserInput($_POST['txt_address_dist_p']);
		$dist_p = (isset($txt_address_dist_p) ? $txt_address_dist_p : null);
		$txt_address_city_p = cleanUserInput($_POST['txt_address_city_p']);
		$city_p = (isset($txt_address_city_p) ? $txt_address_city_p : null);
		$txt_address_tehsil_p = cleanUserInput($_POST['txt_address_tehsil_p']);
		$tehsil_p = (isset($txt_address_tehsil_p) ? $txt_address_tehsil_p : null);
		$txt_address_other_p = cleanUserInput($_POST['txt_address_other_p']);
		$other_p = (isset($txt_address_other_p) ? $txt_address_other_p : null);
		$txt_address_zip_p = cleanUserInput($_POST['txt_address_zip_p']);
		$zip_p = (isset($txt_address_zip_p) ? $txt_address_zip_p : null);
	}
} else {
...
} elseif (isset($_POST['EmployeeID']) && cleanUserInput($_POST['EmployeeID']) != '') {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
...
$btn_address_Save = isset($_POST['btn_address_Save']);
if ($btn_address_Save && $EmployeeID != '') {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		$myDB = new MysqliDb();
		if ($EmployeeID) {
...
<script>
	$(document).ready(function() {
		...
		$('#txt_address_country').change(function() {
			$.ajax({
				url: "../Controller/getState.php?ctr=" + $(this).val(),
		...
		$('#txt_address_state').change(function() {
			$.ajax({
				url: "../Controller/getDist.php?ctr=" + $(this).val(),
				success: function(result) {
						...
						<select id="txt_address_state" name="txt_address_state" required>
							...
							if (empty($mysql_error)) {
								foreach ($result as $key => $value) { ?>
									<option <?php if ($state == $value['state']) {
												echo "selected";
					...
					<div class="input-field col s12 m12">
						<select id="txt_address_dist" name="txt_address_dist" required>
							...
							if (empty($mysql_error)) {
								echo '<option value="NA" >---Select---</option>';
								foreach ($result as $key => $value) { ?>
									<option <?php if ($dist == $value['district']) {
												echo "selected";
