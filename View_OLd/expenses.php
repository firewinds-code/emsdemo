...
if (isset($_POST['btn_food'])) //for food submit
{
    if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
        ...
        if (isset($_FILES['receipt_image']) && $_FILES['receipt_image'] != null) {
            ...
            move_uploaded_file($tempPath, $dir_locationToSave . $food_fileNameFinal);
        }
...
if (isset($_POST['btn_travel'])) //for travel submit
{
    if (isset($_POST["token1"]) && isset($_SESSION["token1"]) && $_POST["token1"] == $_SESSION["token1"]) {
        ...
        if (isset($_FILES['receipt_image']) && $_FILES['receipt_image'] != null && $_POST['modeOftravel'] != 'Car') {
            ...
            move_uploaded_file($tempPath, $dir_locationToSave . $receipt_image);
        }

        if ((isset($_FILES['car_km_receipt']['name']) ? $_FILES['car_km_receipt']['name'] : "")  && $_POST['modeOftravel'] == 'Car') {

            $fileName2 = $_FILES['car_km_receipt']['name'];
            $tempPath2 = $_FILES['car_km_receipt']['tmp_name'];
            $dir_locationToSave2 = __DIR__ . '/../ExpenseTravel/';
            $fileExt2 = strtolower(pathinfo($fileName2, PATHINFO_EXTENSION)); // get  extension
            $car_km_receipt = $EmployeeID . '_carKm_' . date('Y-m-d_His') . '.' . $fileExt2;
            move_uploaded_file($tempPath2, $dir_locationToSave2 . $car_km_receipt);
        }
        if ((isset($_FILES['car_parking_receipt']['name']) ? $_FILES['car_parking_receipt']['name'] : "") && $_POST['modeOftravel'] == 'Car') {

            $fileName3 = $_FILES['car_parking_receipt']['name'];
            $tempPath3 = $_FILES['car_parking_receipt']['tmp_name'];
            $dir_locationToSave3 = __DIR__ . '/../ExpenseTravel/';
            $fileExt3 = strtolower(pathinfo($fileName3, PATHINFO_EXTENSION)); // get  extension
            $car_parking_receipt = $EmployeeID . '_parking_' . date('Y-m-d_His') . '.' . $fileExt3;
            move_uploaded_file($tempPath3, $dir_locationToSave3 . $car_parking_receipt);
        }
...
if (isset($_POST['btn_hotel'])) //for hotel submit
{
    if (isset($_POST["token2"]) && isset($_SESSION["token2"]) && $_POST["token2"] == $_SESSION["token2"]) {
        if (isset($_FILES['receipt_image']) && $_FILES['receipt_image'] != null) {
            ...
            move_uploaded_file($tempPath, $dir_locationToSave . $hotel_fileNameFinal);
        }

        $insert_hotel = 'INSERT INTO expense_hotel(EmployeeID,empName,dateFrom,dateTo,noOfdays,hotelName,amount,receipt_no,receipt_image,remarks,req_status,reqType,reviewerStatus,mgrStatus,approverStatus) VALUES(?,?,?,?,?,?,?,?,?,?,"Pending","HotelRequest","Pending","Pending","Pending") ';

        $stmt3 = $conn->prepare($insert_hotel);
        $stmt3->bind_param("ssssisiiss", $EmployeeID, $empName, $_POST['dateFrom'], $_POST['dateTo'], $_POST['noOfdays'], $_POST['hotelName'], $_POST['amount'], $_POST['receipt_no'], $hotel_fileNameFinal, $_POST['remarks']);
...
if (isset($_POST['btn_miscellaneous'])) //for Miscellaneous submit
{
    if (isset($_POST["token3"]) && isset($_SESSION["token3"]) && $_POST["token3"] == $_SESSION["token3"]) {
        if (isset($_FILES['receipt_image']['name']) ? $_FILES['receipt_image']['name'] : "") {
            $fileName = $_FILES['receipt_image']['name'];
            $tempPath = $_FILES['receipt_image']['tmp_name'];
            $dir_locationToSave = __DIR__ . '/../ExpenseMiscellaneous/';
            $fileExt = strtolower(pathinfo($fileName, PATHINFO_EXTENSION)); // get  extension
            $misc_fileNameFinal = $EmployeeID . '_' . date('Y-m-d_His') . '.' . $fileExt;
            move_uploaded_file($tempPath, $dir_locationToSave . $misc_fileNameFinal);
        }
                    ...
                    <div class="col s12 m12">
                        ...
                        <div class="input-field col s12 m12 l12">
                            <select id="expenses1R" name="expenses1" onchange="this.form.submit();">
                                <!-- <select id="expenses1R" name="expenses1" onchange="pageRefresh()"> -->
            ...
            <div class="form-div" id="divHotelRRR">
                ...
                <div class="schema-form-section row">
                    ...
                    <div id="hotleTableDiv">
                        <?php
                        $sqlConnect = 'SELECT * FROM expense_hotel where EmployeeID=?';
                    ...
                    <div class="input-field col s12 m12">
                        <div class="col s12 m12">
                            <form action="" method="post" enctype="multipart/form-data">
                                ...
                                <div class="input-field col s12 m12 right-align">
                                    <button type="submit" name="btn_miscellaneous" id="btn_miscellaneous" class="btn waves-effect waves-green align-right">Save</button>
                                </div>
                            </form>
                        </div>
                    </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#btn_miscellaneous').on('click', function() {
            var validate = 0;
            var alert_msg = '';
            var fileExtension = ['jpeg', 'jpg', 'png', 'pdf', 'msg'];
...
<script>
    $('#food_date, #travel_date, #returnDate, #miscellaneous_date').datetimepicker({
        timepicker: false,
        format: 'Y-m-d',
        scrollMonth: false,
    });
