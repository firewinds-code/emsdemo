...
if ($btn_add) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		$txt_name = isset($_POST['txt_name']);
		$txt_name2 = cleanUserInput($_POST['txt_name']);
		$txt_cname = ($txt_name ? $txt_name2 : null);
		$txtrefcp = isset($_POST['txt_ref_cp']);
		$txtrefcp2 = cleanUserInput($_POST['txt_ref_cp']);
		$_cp = ($txtrefcp ? $txtrefcp2 : null);
		$txt_ref_cn = isset($_POST['txt_ref_cn']);
		$txt_ref_cn2 = cleanUserInput($_POST['txt_ref_cn']);
		$_cn = ($txt_ref_cn ? $txt_ref_cn2 : null);
		$txt_refName = isset($_POST['txt_refName']);
		$txt_refName2 = cleanUserInput($_POST['txt_refName']);

		$txt_refName = ($txt_refName ? $txt_refName2 : null);

		$createBy = clean($_SESSION['__user_logid']);
		$Insert = 'call add_consultancy("' . $txt_cname . '","' . $_cp . '","' . $_cn . '","' . $txt_refName . '","' . $createBy . '")';
		$myDB = new MysqliDb();
...
if ($btn_save) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		...
		$txt_name2 = cleanUserInput($_POST['txt_name']);
		$txt_cname = ($txt_name ? $txt_name2 : null);
		$txt_ref_cp = isset($_POST['txt_ref_cp']);
		$txt_ref_cp2 = cleanUserInput($_POST['txt_ref_cp']);
		$_cp = ($txt_ref_cp ? $txt_ref_cp2 : null);
		$txt_ref_cn = isset($_POST['txt_ref_cn']);
		$txt_ref_cn2 = cleanUserInput($_POST['txt_ref_cn']);
		$_cn = ($txt_ref_cn ? $txt_ref_cn2 : null);
		$txt_refName = isset($_POST['txt_refName']);
		...
		$Update = 'call save_consultancy("' . $txt_cname . '","' . $_cp . '","' . $_cn . '","' . $txt_refName . '","' . $ModifiedBy . '","' . $DataID . '")';
		...
		if (!empty($DataID) || $DataID != '') {
			...
			echo "<script>$(function(){ toastr.success('Something is wrong Please click to Edit Button First.If Not Resolved then contact to technical person.'); }); </script>";
		}
	}
}
?>
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				<div id="pnlTable">
					...
					$sqlConnect = "select id, ConsultancyName, ContactPerson, ContactNo, RefName from consultancy_master order by ConsultancyName desc";
					$myDB = new MysqliDb();
