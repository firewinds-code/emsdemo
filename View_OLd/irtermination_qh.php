...
if (isset($_POST['btn_Issue_Save'])) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		...
		$Insert = "update warning_rth set qh_status=?,`Termination Date`=?,`qh_remark`=?,`qh_modifiedon`=now() where id=?;";
		$insQu = $conn->prepare($Insert);
		$insQu->bind_param("sssi", $IRTStatusHR, $TerDate, $remark, $DataID);
		$insQu->execute();
		$res = $insQu->get_result();
		// $myDB->rawQuery($Insert);
		// $mysql_error = $myDB->getLastError();

		// if (empty($mysql_error)) {
		if ($insQu->affected_rows === 1) {
					...
					if (is_uploaded_file($_FILES['txt_doc_name_']['tmp_name'][$name])) {
						...
						if ($validation_check === 0) {
							if (move_uploaded_file($sourcePath, $targetPath)) {
								$emid = cleanUserInput($_POST['EmployeeID']);
								$ext = pathinfo(basename($_FILES['txt_doc_name_']['name'][$name]), PATHINFO_EXTENSION);
								$filename = $emid  . '_' . preg_replace('/\s+/', '', $_POST['IRTStatusHR']) . '_' . date("dmYhis") . '.' . $ext;
								...
								$insQ->bind_param("sssss", $emid, $dataid, $filename, $txt_doc_value_, $userid);
								$insQ->execute();
				...
				<form method="post">
					...
					<div id="pnlTable">
						...
						if (isset($_POST['btnSearch'])) {
							$SearchBy = cleanUserInput($_POST['SearchBy']);
							$sqlConnect = "SELECT * FROM warning_rth where `ah_status`=? and qh_status is null;";
					...
					else
					{
						 $sqlConnect = "SELECT wr.* FROM warning_rth wr inner join personal_details pd on wr.EmployeeID = pd.EmployeeID where wr.ah_status = '".$_POST['SearchBy']."' and wr.hr_status is null and pd.location in ('".$_SESSION["__location"]."');";
							...
							$selectQury->bind_param("s", $SearchBy);
							$selectQury->execute();
...
<script>
	...
	function EditData(el) {
		...
		$('select').formSelect();
		$.ajax({
			url: <?php echo '"' . URL . '"'; ?> + "View/bindDocIR.php?empid=" + EmployeeID_
		}).done(function(data) {
			$('#AppendDoc').html(data)
		});
