			...
			if ($count_check > 0) {
				...
				foreach ($_POST['cb'] as $val) {
					...
					if ($Status != 'NA' || $Status != 'Na') {
						...
						$sql = 'update resign_details set status_it = "1" where EmployeeID = ?';
						...
						if ($stmt->affected_rows === 1) {
							...
							$sql1 = 'INSERT INTO resign_comment_log(`EmployeeID`,`CreatedBy`,`CreatedOn`,`Comment`,`type`)VALUES(?,?,now(),?,"IT")';
							...
							echo "<script>$(function(){ toastr.error('Request Not Updated for " . $EmpID . "::Error :</code>'); }); </script>";
						}
					}
				}
			} else {
