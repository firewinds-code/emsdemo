...
if ($EmployeeID == 'CE07147134') {
    $getEmployees = "select t1.EmployeeID,t1.EmployeeName,concat(clientname,'|',Process,'|',sub_process) as `Dept`,cm_id, designation, ReportTo,t2.EmployeeName as `ReportToName` from whole_details_peremp t1 join personal_details t2 on t1.ReportTo=t2.EmployeeID where TH=? or OH =? or QH =? or ReportTo =? order by EmployeeName";
    $selectQury = $conn->prepare($getEmployees);
    $selectQury->bind_param("ssss", $EmployeeID, $EmployeeID, $EmployeeID, $EmployeeID);
    $selectQury->execute();
    $allEmp = $selectQury->get_result();
} else {
    $getEmployees = "select t1.EmployeeID,t1.EmployeeName,concat(clientname,'|',Process,'|',sub_process) as `Dept`,cm_id, designation, ReportTo,t2.EmployeeName as `ReportToName` from whole_details_peremp t1 join personal_details t2 on t1.ReportTo=t2.EmployeeID where TH=? or OH =? or QH =? order by EmployeeName";
    $selectQury = $conn->prepare($getEmployees);
    $selectQury->bind_param("sss", $EmployeeID, $EmployeeID, $EmployeeID);
    $selectQury->execute();
    $allEmp = $selectQury->get_result();
    // print_r($allEmp);
    // die;
...
if ($submit) {
    if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
        ...
        $issue = addslashes(trim($_POST['description_of_issue']));
        $employee_id = cleanUserInput($_POST['employee_id']);
        $employee_name = cleanUserInput($_POST['employee_name']);
        $issued_date = cleanUserInput($_POST['issued_date']);
        $position = cleanUserInput($_POST['position']);
        $department = cleanUserInput($_POST['department']);
        $supervisor_name = cleanUserInput($_POST['supervisor_name']);
        $issue_type = cleanUserInput($_POST['issue_type']);
        $Query = "insert into corrective_action_form (employee_id, employee_name, issued_date, position, department, supervisor_name, issue_type, description_of_issue, created_by) VALUES (?,?,?,?,?,?,?,?,?)";
        $selt = $conn->prepare($Query);
        $selt->bind_param("sssssssss", $employee_id, $employee_name, $issued_date, $position, $department, $supervisor_name, $issue_type, $issue, $EmployeeID);
        $selt->execute();
        $result_all = $selt->get_result();
        $last_id = $conn->insert_id;
        if ($selt->affected_rows === 1) {
                ...
                foreach ($_FILES['txt_doc_name']['tmp_name'] as $key => $value) {
                    ...
                    if (in_array(strtolower($file_ext), $allowed_types)) {
                        ...
                        if (move_uploaded_file($file_tmpname, $filepath)) {
                            ...
                            $fileQuery = "insert into corrective_action_form_files (corrective_action_form_id, file_path) VALUES (?,?)";
                            $selectQu = $conn->prepare($fileQuery);
                            $selectQu->bind_param("is", $last_id, $fileName);
                            $selectQu->execute();
                            ...
                            if ($selectQu->affected_rows === 1) {
                                ...
                                echo "<script>$(function(){ toastr.success('Successfully uploaded <br /> '); }); </script>";
                            }
                        } else {
                            //echo "Error uploading {$file_name} <br />";
                            echo "<script>$(function(){ toastr.warning('Error uploading'); }); </script>";
                        }
                    } else {
                        // If file extention not valid 
                        echo "<script>$(function(){ toastr.warning('Error uploading.'); }); </script>";
                        echo "<script>$(function(){ toastr.warning('file type is not allowed'); }); </script>";
                    }
                }
            }
        } else {
            ...
            <div class="schema-form-section row">
                ...
                <div id="pnlTable">
                    ...
                    <div class="had-container pull-left row card" style="margin-top: 10px;width: 100%;padding: 15px;">
                        <div>
                            <?php
                            $getAllData = "select * from corrective_action_form where created_by = ? order by created_at desc";
                            $selectQ = $conn->prepare($getAllData);
                            $selectQ->bind_param("s", $EmployeeID);
                            $selectQ->execute();
                            $allData = $selectQ->get_result();

                            if (isset($allData) and $allData->num_rows > 0) {
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#btn_document_add').click(function() {
            ...
            for (i = 1; i <= rowlen; i++) {
                if ($('#txt_doc_value_' + i).val().replace(/^\s+|\s+$/g) == '') {
                    $(function() {
        ...
        $('#submit').click(function() {
            ...
            if ($.replace(/^\s+|\s+$/g, $('#description_of_issue').val()) == '') {
                $('#description_of_issue').addClass('has-error');
                $validate = 1;

            }

            if ($.replace(/^\s+|\s+$/g, $('#description_of_issue').val()).length < 150) {
                $('#description_of_issue').addClass('has-error');
                $validate = 1;
            ...
            if (checkRepeat($.replace(/^\s+|\s+$/g($('#description_of_issue').val())))) {
                $('#description_of_issue').addClass('has-error');
                $validate = 1;
                if ($('#sremark').length == 0) {
