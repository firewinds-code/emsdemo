...
if (!isset($clean_user_logid)) {
	...
	if ($clean_user_logid == 'CE10091236' || $clean_user_type == 'ADMINISTRATOR' || $clean_user_type == 'CENTRAL MIS') {
...
$sql = "select EmpID, test_name, testid, test_score ,createdon  from test_score where  cast(createdon as date) between ? and ? ";
...
<div id="content" class="content">
	...
	<span id="PageTittle_span" class="hidden">Test Score Report</span>
						...
						<tbody>
							...
							if ($result->num_rows > 0) {
								foreach ($result as $key => $value) {
									...
									echo '<td  id="empid' . $count . '" >' . $value['EmpID'] . '</td>';
									echo '<td  id="test_name' . $count . '" >' . $value['test_name'] . '</td>';
									echo '<td  id="testid' . $count . '"  >' . $value['testid'] . '</td>';
									echo '<td  id="test_score' . $count . '"  >' . $value['test_score'] . '</td>';
