			...
			<div class="schema-form-section row">
				...
				<div class="input-field col s12 m12">
					<select name='altertype' id='altertype'>
						<option value='' <?php if ($logtype == '') {
												echo 'selected';
											} ?>>Select Alter Type</option>
						<option value='QAreports_to' <?php if ($logtype == 'QAreports_to') {
															echo 'selected';
														} ?>>Report To QA</option>
						<option value='reports_to' <?php if ($logtype == 'reports_to') {
														echo 'selected';
													} ?>>Report To</option>
						<option value='reactive' <?php if ($logtype == 'reactive') {
														echo 'selected';
													} ?>>Reactive </option>
						<option value='Designation' <?php if ($logtype == 'Designation') {
														echo 'selected';
													} ?>>Designation </option>
					</select>
				</div>
				...
				<div id="pnlTable">
					...
					function getEmpName($empID)
					{
						$myDB = new MysqliDb();
						$conn = $myDB->dbConnect();
						$myq = "SELECT EmployeeName from personal_details where EmployeeID=? ";
						$stmt1 = $conn->prepare($myq);
						$stmt1->bind_param("s",  $empID);
						$stmt1->execute();
						$empname_array = $stmt1->get_result();
						//print_r($empname_array);
						//die;
						// exit;
						// $empname_array = $myDB->query();
						$empname_array = mysqli_fetch_array($empname_array);
					...
					function getDesignation($dfid)
					{
						$myDB = new MysqliDb();
						$conn = $myDB->dbConnect();
						$query1 = "SELECT a.Designation  from designation_master a INNER JOIN df_master b ON a.ID=b.des_id where  b.df_id=? limit 0,10";
						$stmt2 = $conn->prepare($query1);
						$stmt2->bind_param("s",  $dfid);
						$stmt2->execute();
						$desig_array = $stmt2->get_result();
						// $desig_array = $myDB->query();
						$desig_array = mysqli_fetch_array($desig_array);
						if ($desig_array) {
					...
					if ($logtype != "") {
						$sqlConnect = "SELECT a.*,b.Process,b.sub_process,b.EmployeeName,b.DOJ,st.ReportTo,st.Qa_ops,b.df_id from tbl_log_altaration a inner join whole_dump_emp_data b  on a.EmployeeID=b.EmployeeID inner join status_table st  on a.EmployeeID=st.EmployeeID where type=? order by a.log_date desc limit 0,10 ";
					...
					<?php
							...
							echo '<div id="div_error" class="slideInDown animated hidden">Data Not Found:: <code ></code> </div>';
						}
					}
					?>
