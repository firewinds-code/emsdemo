...
$__user_type = cleanUserInput($_SESSION['__user_type']);
if ($__user_type != 'ADMINISTRATOR') {
...
$btn_ref_edit = isset($_POST['btn_ref_Save']);
if ($btn_ref_edit) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		...
		$txt_tenure	= cleanUserInput($_POST["txt_tenure"]);
		$txt_startdate	= cleanUserInput($_POST["txt_startdate"]);
		$txt_enddate	= cleanUserInput($_POST["txt_enddate"]);
		$txt_status	= cleanUserInput($_POST["txt_status"]);
		$username	= cleanUserInput($_POST["username"]);
		$txtlocation = cleanUserInput($_POST['txt_location']);

		$consultancy_id = (isset($txt_ref_Type) ? $txt_ref_Type : null);
		$txt_empmap_subprocess = (isset($txt_empmap_subprocess) ? $txt_empmap_subprocess : null);
		$txt_payout = (isset($txt_payout) ? $txt_payout : null);
		$txt_tenure = (isset($txt_tenure) ? $txt_tenure : null);
		$txt_location = (isset($txtlocation) ? $txtlocation : null);
		$txt_startdate = (isset($txt_startdate) ? $txt_startdate : null);
		$txt_enddate = (isset($txt_enddate) ? $txt_enddate : null);
		...
		if ($txt_payout <= 5 || is_numeric($txt_payout)) {
			$txt_payout	= cleanUserInput($_POST["txt_payout"]);
			...
			echo "<script>$(function(){ toastr.success('Payout not more than 5 digits.'); }); </script>";
		}
	}
}
...
if (($btn_ref_edit)) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		...
		$txt_payout = (isset($txt_payout) ? $txt_payout : null);
		$txt_tenure = (isset($txt_tenure) ? $txt_tenure : null);
		$txt_location = (isset($txt_location) ? $txt_location : null);
		$txt_startdate = (isset($txt_startdate) ? $txt_startdate : null);
		$txt_enddate = (isset($txt_enddate) ? $txt_enddate : null);
		$txt_status = (isset($txt_status) ? $txt_status : null);
					...
					<div class="modal-content">
		     		 ...
		     		 <?php
						...
						?>-->
								...
								<div class="input-field col s6 m6">
									<input type="text" class="form-control" id="txt_payout" name="txt_payout" required />
...
<script>
	$(document).ready(function() {
		...
		var minLength = 2;
		var maxLength = 5;
		$('#txt_payout').on('keydown keyup change', function() {
			var char = $(this).val();
