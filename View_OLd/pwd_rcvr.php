...
if (isset($_POST['btn_change_password'])) {
	...
	$userempid = cleanUserInput($_POST['txt_pwd_empid']);
	$txt_chg_pwd = cleanUserInput($_POST['txt_chg_pwd']);
	$txt_chg_pwd1 = cleanUserInput($_POST['txt_chg_pwd1']);
	...
	if (!$uppercase || !$lowercase || !$number || !$specialChars || strlen($txt_chg_pwd) < 8) {
		...
		$alert_msg =  "<script>$(function(){ toastr.error('Password contain 8 characters in length and should include at least one upper case letter, one number, and one special character.'); }); </script>";
	} else {
		// echo "<script>$(function(){ toastr.success('Strong password.'); }); </script>";
		$password_hash = md5($txt_chg_pwd);
		$password_hash1 = md5($txt_chg_pwd1);
		// $password = PASSWORD_HASH($_POST["newPassword"], PASSWORD_DEFAULT);
		if ($password_hash != $password_hash) {
			echo "<script>$(function(){ toastr.error('Password and Confirm Password Mismatch '); }); </script>";
		} else {
			...
			if (empty($mysql_error)) {
				...
				$alert_msg =  "<script>$(function(){ toastr.error('Record not updated.'); }); </script>";
			}
		}
	}
}
					...
					<div class="schema-form-section row">
						...
						<div class="scheduler-border">
							...
							if (isset($_POST['btn_save'])) {
								$empid = cleanUserInput($_POST['txt_empid']);
								$seqans = cleanUserInput($_POST['txtans']);
								$myDB = new MysqliDb();
							...
							?>
								...
								<div class="input-field col s6 m6 ">
									<input type="text" class=" clsInput" name="txt_dob" id="txt_dob" placeholder="YYYY-MM-DD" autocomplete="off" onclick="isDate(this)" />
			...
			$(function() {
				...
				$('#btn_check').click(function() {
					...
					if (validate == 1) {
						...
						$('#txt_dob').on('blur', 'input', validatedate);
						...
						function validatedate() {
							...
							if (/\d+\/\d+\/\d{4}/.test(text)) { // good input
								...
								$('#txt_dob').val('Please use mm/dd/yyyy format');
							}
						};
