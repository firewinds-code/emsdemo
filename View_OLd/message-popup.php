...
if (isset($_POST['btn_view'])) {
	...
	$sqlConnect = "select t1.ID, t1.ackstatus, t1.msg_date, t1.text_msg, t1.to_empid, t1.sender_empid, t1.sender_name,t2.EmployeeName from tbl_chat_message t1 inner join personal_details t2 on trim(t1.to_empid)=trim(t2.EmployeeID) $whereClause and cast(msg_date as date) between ? and ? order by msg_date";

	// $sqlConnect = "select t1.ID, t1.ackstatus, t1.msg_date, t1.text_msg, t1.to_empid, t1.sender_empid, t1.sender_name,t2.EmployeeName from tbl_chat_message t1 inner join personal_details t2 on trim(t1.to_empid)=trim(t2.EmployeeID) $whereClause and cast(msg_date as date) between ? and ? order by msg_date";
	$select = $conn->prepare($sqlConnect);
	$select->bind_param("ss", $date_From, $date_to);
	$select->execute();
} else {
	...
	<div class="pim-container row" id="div_main">
		...
		<div class="form-div">
			...
			<div id="pnlTable">
				...
				if ($btnsave) {
					$id = cleanUserInput($_POST['dataid']);
					$comment = cleanUserInput($_POST['txt_comment']);
					if ($id != "" && $comment != "") {

						date_default_timezone_set('Asia/Kolkata');
						$date = date('Y-m-d H:i:s', time());
						$ack = addslashes($_POST['txt_comment']);
						$query = "UPDATE tbl_chat_message SET acknowledge= ?, acknowledgedate=?, ackstatus=1 WHERE ID=?";
						$update = $conn->prepare($query);
						$update->bind_param("ssi", $ack, $date, $id);
						$update->execute();
			...
			<div class="input-field col s12 m12 " style="display:none;" id="messagebox">
				...
				<div class="input-field col s12 m12 right-align">
					<input type="submit" value="Acknowledge" name="btnSave" id="btnSave1" class="btn waves-effect waves-green" />
				</div>
		...
		$('#btnSave1').click(function() {
			...
			if (msg == "") {
				...
				if ($('#stxt_comment').size() == 0) {
					$('<span id="stxt_comment" class="help-block">Reply Message should not be empty.</span>').insertAfter('#txt_comment');
				}
			}
