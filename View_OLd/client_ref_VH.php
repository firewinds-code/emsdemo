...
$btn_RefMaster_Save = isset($_POST['btn_RefMaster_Save']);
if ($btn_RefMaster_Save) {
    if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
        $txt_Client = isset($_POST['txt_Client']);
        $txt_Client2 = cleanUserInput($_POST['txt_Client']);
        $txt_RefAmount = isset($_POST['txt_RefAmount']);
        $txt_RefAmount2 = cleanUserInput($_POST['txt_RefAmount']);
        $txt_from = isset($_POST['txt_from']);
        $txt_from2 = cleanUserInput($_POST['txt_from']);
        $txt_to = isset($_POST['txt_to']);
        $txt_to2 = cleanUserInput($_POST['txt_to']);
        $txt_1st_PayAmt = isset($_POST['txt_1st_PayAmt']);
        $txt_1st_PayAmt2 = cleanUserInput($_POST['txt_1st_PayAmt']);
        $txt_2nd_PayAmt = isset($_POST['txt_2nd_PayAmt']);
        $txt_2nd_PayAmt2 = cleanUserInput($_POST['txt_2nd_PayAmt']);
        $txt_WindowMonth = isset($_POST['txt_WindowMonth']);
        $txt_WindowMonth2 = cleanUserInput($_POST['txt_WindowMonth']);
        $emp_type = isset($_POST['emp_type']);
        $emp_type2 = cleanUserInput($_POST['emp_type']);

        $_cmid = ($txt_Client ? $txt_Client2 : null);
        $_RefAmt = ($txt_RefAmount ? $txt_RefAmount2 : null);
        $_from = ($txt_from ? $txt_from : null);
        $_to = ($txt_to ? $txt_to2 : null);
        $_FPay = ($txt_1st_PayAmt ? $txt_1st_PayAmt2 : null);
        $_SPay = ($txt_2nd_PayAmt ? $txt_2nd_PayAmt2 : null);
        $_WinMonth = ($txt_WindowMonth ? $txt_WindowMonth2 : null);
        $emp_type = ($emp_type ? $emp_type2 : null);
...
$btn_RefMaster_Edit = isset($_POST['btn_RefMaster_Edit']);
if ($btn_RefMaster_Edit) {
    if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
        ...
        $Update = 'call sp_Update_MasterRefScheme_VH("' . $DataID . '","' . $_cm_id . '","' . $_RefAmount . '","' . $_from . '","' . $_to . '","' . $_FPay . '","' . $_SPay . '","' . $_WinMonth . '","' . $emp_type . '","' . $ModifiedBy . '")';
        // die;
        $myDB = new MysqliDb();
    ...
    <div class="pim-container row" id="div_main">
        <div class="form-div">
            ...
            <div class="schema-form-section row">
                            ...
                            <div class="col s12 m12">
                                ...
                                <div class="input-field col s8 m8">
                                    <select id="txt_Client" name="txt_Client" required>
                                        ...
                                        $sqlBy = "select concat(t3.location,' | ',t2.client_name,' | ',t1.process,' | ',t1.sub_process) as process,cm_id from new_client_master t1 join client_master t2 on t1.client_name=t2.client_id join location_master t3 on t3.id=t1.location where vh=? and cm_id not in (select cm_id from client_status_master) order by t3.location";
                ...
                <div id="pnlTable">
                    <?php
                    //$sqlConnect = "select t2.id,t2.cm_id,t1.process,t1.sub_process,t2.amount,t2.ApplicableFrom,t2.ApplicableTo,t2.1st_pay,t2.2nd_pay,t2.window_month,emp_type,t2.createdby,t2.createdon,t1.location,t3.client_name,t4.location as locname,flag from new_client_master t1 join client_ref_master t2 on t1.cm_id= t2.cm_id join client_master t3 on t1.client_name= t3.client_id join location_master t4 on t4.id=t1.location where vh='" . $_SESSION['__user_logid'] . "' and flag is not null and t1.cm_id not in (select cm_id from client_status_master) ";

                    $empID = clean($_SESSION['__user_logid']);
                    $sqlConnect = "select t2.id,t2.cm_id,t1.process,t1.sub_process,t2.amount,t2.ApplicableFrom,t2.ApplicableTo,t2.1st_pay,t2.2nd_pay,t2.window_month,emp_type,t2.createdby,t2.createdon,t1.location,t3.client_name,t4.location as locname,flag from new_client_master t1 join client_ref_master t2 on t1.cm_id= t2.cm_id join client_master t3 on t1.client_name= t3.client_id join location_master t4 on t4.id=t1.location where vh=? and flag is not null and t1.cm_id not in (select cm_id from client_status_master)";
    ...
    $(document).ready(function() {
        ...
        $('#btn_RefMaster_Edit,#btn_RefMaster_Save').on('click', function() {
            ...
            if ($('#txt_1st_PayAmt').val() == '') {
                ...
                if ($('#spantxt_1st_PayAmt').length == 0) {
            ...
            // 	if ($('#spantxt_2nd_PayAmt').length() == 0) {
            ...
            if ($('#txt_WindowMonth').val() == '') {
                ...
                if ($('#spantxt_WindowMonth').length == 0) {
            ...
            $.ajax({
                ...
                success: function(result) {
                    ...
                    if (data[0] == 'Done') {
                        ...
                        window.location.replace("../View/client_ref_VH.php");
                    }
                }
            });
        }
