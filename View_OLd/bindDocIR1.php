...
if (isset($empid)) {
	//Echo 'Document <br/>';
	$sqlConnect = "SELECT i.id,ah_Datetime,ah_remark,ah_status,qh_modifiedon as `L1 DateTime`,qh_remark as 'L1 Remark',qh_status as 'L1 Status', group_concat(Document)`File` FROM warning_rth i inner join warning_rth_documents d on i.EmployeeID=d.EmployeeID
		...
		<table id="myTable1" class="data dataTable no-footer" cellspacing="0" width="100%">
			...
			<tbody>
				...
				foreach ($result as $key => $value) {
					...
					echo '<td>' . $value['L1 Remark'] . '</td>';
					echo '<td>' . $value['L1 DateTime'] . '</td>';
					echo '<td>' . $value['L1 Status'] . '</td>';
					echo '<td>' . $z . '</td>';
					echo '</tr>';
				}
				?>
