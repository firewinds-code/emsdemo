...
if (isset($_SESSION)) {
	if (!isset($user_logid)) {
		...
		if ($user_logid == '' || $user_logid == null) {
			...
			exit();
		} else if (!($user_logid == 'CE12102224' || $user_logid == 'CE03070003' || $user_logid == 'CE01145570' || $user_logid == 'CE0820912500' || $user_logid == 'MU03200198' || $user_logid == 'CE04159316' || $user_logid == 'CEK07120002' || $user_logid == 'CFK08190181' || $user_logid == 'CMK11171884' || $user_logid == 'CEV102073966' || $user_logid == 'CMK112073699' || $user_logid == 'CEK031925550')) {
...
$btn_ref_edit = isset($_POST['btn_ref_Edit']);
if (($btn_ref_edit)) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		//die;
		$DataID = cleanUserInput($_POST['hid_ref_ID']);
		$ConsultID = cleanUserInput($_POST['hid_consultID']);
		$locID = cleanUserInput($_POST['hid_locid']);
		$createdon = cleanUserInput($_POST['hid_createdon']);
		// 
		$hid_payout = cleanUserInput($_POST['hid_payout']);
		$hid_tenaure = cleanUserInput($_POST['hid_tenaure']);
		$txt_name = cleanUserInput($_POST['txt_name']);
		$txt_mobile = cleanUserInput($_POST['txt_mobile']);
		$txt_email = cleanUserInput($_POST['txt_email']);
		$txt_status = cleanUserInput($_POST['txt_status']);
		$txt_remarks = cleanUserInput($_POST['txt_remarks']);
		// $hid_payout = cleanUserInput($_POST['hid_payout']);


		// 

		$hid_payout = (isset($hid_payout) ? $hid_payout : null);
		$hid_tenaure = (isset($hid_tenaure) ? $hid_tenaure : null);

		$txt_empmap_subprocess = "0";
		$filename = "";
		$txt_name = (isset($txt_name) ? $txt_name : null);
		$txt_mobile = (isset($txt_mobile) ? $txt_mobile : null);
		$txt_email = (isset($txt_email) ? $txt_email : null);
		$txt_status = (isset($txt_status) ? $txt_status : null);
		$txt_remarks = (isset($txt_remarks) ? $txt_remarks : null);
		...
		if ($txt_status == "1") {
			$txt_empmap_subprocess = cleanUserInput($_POST['txt_empmap_subprocess']);
			$txt_empmap_subprocess = (isset($txt_empmap_subprocess) ? $txt_empmap_subprocess : null);
		...
		if ($validate == "0") {
			$Update = 'update consultancy_data set flag =1 where  id = ?';
			$up = $conn->prepare($Update);
			$up->bind_param("i", $DataID);
			$up->execute();
			...
			if ($up->affected_rows === 1) {
				$Update = 'insert into consultancy_candidate_status (consult_id,location,cm_id,candidate_name,mobile,candidate_email,`status`,mail_attached,remarks,createdby,mis_uploaddate,payout,tenuare) values(?,?,?,?,?,?,?,?,?,?,?,?,?) ';
				$ups = $conn->prepare($Update);
				$ups->bind_param("iiisisissssii", $ConsultID, $locID, $txt_empmap_subprocess, $txt_name, $txt_mobile, $txt_email, $txt_status, $filename, $txt_remarks, $ModifiedBy, $createdon, $hid_payout, $hid_tenaure,);
				$ups->execute();
				$result = $ups->get_result();
				// $result = $myDB->rawQuery($Update);
				// $mysql_error = $myDB->getLastError();
				// if (empty($mysql_error)) {
				if ($ups->affected_rows === 1) {
					echo "<script>$(function(){ toastr.success('Record has updated.... ') }); </script>";
				} else {
					echo "<script>$(function(){ toastr.error('Sorry, Record not updated....  ') }); </script>";
				}
			}
		}
					...
					<div class="modal-content">
						...
						<div class="modal-body" style="max-height: 100%;float:left;overflow: auto;">
							<div class="col s12 m12">
								...
								<div class="input-field col s6 m6">
									<input type="text" class="form-control" id="txt_location" name="txt_location" required />
