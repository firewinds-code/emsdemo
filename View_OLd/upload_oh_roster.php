		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				?>
					...
					<div class="col s12 m12" id="main_container_upload">
						<div class="input-field col s12 m12">
							<Select name="txt_dept" id="txt_dept">
								<?php
								$myDB = new MysqliDb();
								// $rowData = $myDB->query('select distinct Process,sub_process from new_client_master where oh="'.$clean_user_logid.'"');
								// if(count($rowData) > 0)
								// {
								// 	if($dept == 'ALL Process')	
								// 	{
								...
								$stmt->bind_param("s", $clean_user_logid);
								$stmt->execute();
						...
						<div class="input-field col s12 m12 right-align">
							...
							<!--<input  type="button" value="Upload Again" name="UploadAgain" id="UploadAgain" value="Upload Again" class="btn waves-effect waves-green close-btn hidden "/>-->
						</div>
					    ...
					    $uploadOk = 0;
								...
								if ($uploadOk == 0) {
									...
									if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
										...
										$query = "SELECT distinct DateOn FROM ho_list_admin";
										...
										$clean_txt_upload =  cleanUserInput($_POST['txt_Type_Upload']);

										if ($clean_txt_upload == 'Next Week') // && (date('l',time()) =='Friday' or date('l',time()) =='Saturday'))
										{
											...
											foreach ($activeSheetData as $row) {
												...
												if ($row_counter > 0 && !empty($row['A']) && $row['A'] != '') {
													...
													$stmt = $conn->prepare($query);
													$stmt->bind_param("ss", $row['A'], $sql_weekno);
													$stmt->execute();
													$result1234 = $stmt->get_result();
													$count = $result1234->num_rows;
													$weekoff_pref = $result1234->fetch_row();
													$weekoff_pref_1 = '';
													...
													$stmt = $conn->prepare($query);
													$stmt->bind_param("s", $row['A']);
													$stmt->execute();
													$result1234 = $stmt->get_result();
													$check_roster = $result1234->fetch_row();
													$check_roster = $check_roster[0]['rt_type'];
													$check_roster_temp = $check_roster;
													if ($check_roster == 3) {
														$check_roster = 1;
													...
													$query = "select Gender from personal_details where EmployeeID =? limit 1";

													$stmt = $conn->prepare($query);
													$stmt->bind_param("s", $row['A']);
													$stmt->execute();
													$result1234 = $stmt->get_result();
													$checkGender = $result1234->fetch_row();
													$gender = $checkGender[0]['Gender'];
													if (empty($gender)) {
														$gender = 'Male';
													}
													...
													$myDB = new MysqliDb();
													// $checkAccount = $myDB->query("select oh,Process,sub_process from employee_map inner join new_client_master on new_client_master.cm_id = employee_map.cm_id where EmployeeID ='".$row['A']."' limit 1");
													// $oh='';
													// $process_macth='';
													// if(count($checkAccount) > 0 )
													// {

													// 	$oh = $checkAccount[0]['oh'];
													...
													// }
													///
													$query = "select oh,Process,sub_process from employee_map inner join new_client_master on new_client_master.cm_id = employee_map.cm_id where EmployeeID =? limit 1";

													$stmt = $conn->prepare($query);
													$stmt->bind_param("s", $$row['A']);
													$stmt->execute();
													$result1234 = $stmt->get_result();
													$checkAccount = $result1234->fetch_row();
													$oh = '';
													$process_macth = '';
													if ($result1234->num_rows > 0) {
													...
													if ($oh == $uploader && ($process_macth == $dept || $dept == 'ALL Process')) {
														...
														for ($i = $begin; $begin <= $end; $i->modify('+1 day')) {
															...
															$roster_type = 1;
															/*$str = 'call InsertRoaster_New("'.strtoupper($row['A']).'","'.$col.'","'.$val.'","'.$clean_user_logid.'","'.date('Y-m-d').'","'.$month_row['month'].'","'.$row['J'].'")';*/
																	...
																	if ($Time_inout[0][0] == "0" || $Time_inout[0][0] == "1" || $Time_inout[0][0] == "2" || $Time_inout[0][0] == "3" || $Time_inout[0][0] == "4" || $Time_inout[0][0] == "5" || $Time_inout[0][0] == "6" || $Time_inout[0][0] == "7" || $Time_inout[0][0] == "8" || $Time_inout[0][0] == "9" || $Time_inout[0]  == 'WO|WO') {
																		if ($Time_inout[0] == 'WO|WO' && $Time_inout[1] == 'WO|WO') {
																			if ($weekoff_pref_1 == 'No Weekoff Required' && $weekOFF < 1) {
																				...
																				$noData_Uploadfor .= '<tr><td>' . strtoupper($row['A']) . '</td><td>' . $dateT_ins . '</td><td>' . $i->format('l') . '</><td class="textDanger">Employee haven`t selected "NWR" hence align Week Off</td></tr>';
																				$validate = 1;
																			}
																		} elseif (strlen($Time_inout[0]) == 11 && $Time_inout[0] != 'WO|WO' && strlen($Time_inout[1]) == 11 && $Time_inout[1] != 'WO|WO') {
														...
														$noData_Uploadfor_down .= '<p class="msgFile text-danger" style="font-weight:normal">Wrong Employee Data for upload. Employee ' . strtoupper($row['A']) . ' Not exists in your process list</p>';
														$validate = 1;
													}
												}
												$row_counter++;
											...
											$myDB = new MysqliDb();
											$rst_emp = $myDB->query("call activeFor_opshead('" . $uploader . "','" . $dept . "')");
											if ($EmployeeCounter != ($row_counter - 1)) {

												$noData_Uploadfor_down .= '<p class="msgFile text-danger" style="font-weight:normal"><b>Count mismatch error </b>, Wrong Employee Data to upload may be some Employee not in your account</p>';
												$validate = 1;
											} elseif (($row_counter - 1) != intval(count($rst_emp))) {

												$noData_Uploadfor_down .= '<p class="msgFile text-danger" style="font-weight:normal"><b>Count mismatch error </b>, File not contain Employee you downloaded.Download and try agian</p>';
												$validate = 1;
											}
											if ($validate == 0) {
																...
																/*if($val == 'WO-WO' || $val == 'WO|WO-WO|WO')
				                            	{
													...
													{
														$insert_exp =  'call sp_InsertException("'.strtoupper($row['A']).'","","Working on Leave","Insert By Server","'.$i->format('Y-m-d').'","'.$i->format('Y-m-d').'","Approve","Approve","NA","NA","NA","NA","NA","NA")';
														$myDB = new mysql();
										...
										if ($count > 0 && $validate == 0) {
											...
											$msgFile = $msgFile . "<p  class='msgFile text-danger'>Total " . $count . " Record  for " . ($row_counter - 1) . " Employees are Updated Sucessfully.</p>";
											if (file_exists($target_dir . basename($_FILES["fileToUpload"]["name"]))) {
												$ext = pathinfo($target_file, PATHINFO_EXTENSION);
												rename($target_file, $target_dir . time() . '_' . $uploader . "_Roster_OpsHead." . $ext);
											}
										} else
											$msgFile = $msgFile . "<p  class='msgFile text-danger'>Sorry, there was an error uploading your file " . $mysql_error . "</p>";
									} else {
					...
					<?php
					$idate = time();
					$newdate  = strtotime(date('Y-m-d', time()) . ' 14:00:00');
					if ($idate <= $newdate) {
					?>
								...
								clock = $('.clock').FlipClock(diff, {
									...
									callbacks: {
										stop: function() {
											...
											$('#count_container').html('Countdown complete !<a href=""> Click here </a> next window').addClass('animated slideInLeft').attr('style', 'position: initial;bottom: 30px;color: #d66708;font-size: 16px;text-shadow: 1px 1px 1px #d6a98e;font-weight: bold;');
										}
									}
								});
							});
