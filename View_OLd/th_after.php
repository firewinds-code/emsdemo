		...
		if (isset($_POST['cb'])) {
			...
			if ($count_check > 0) {
				...
				foreach ($cb as $val) {
					...
					$save = 'call manage_status_th_after("' . $empID . '","' . $txt_Remark . '","' . $createBy . '","TH AFTER")';
					//die;
...
if (isset($_POST['btn_refer'])) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		...
		if (isset($_POST['cb'])) {
			...
			$count_check = count($checked_arr);
			if ($count_check > 0) {
				$cb = cleanUserInput($_POST['cb']);
				foreach ($_POST['cb'] as $val) {
					$empID = cleanUserInput($val);
					$myDB = new MysqliDb();
					$txt_Remark = cleanUserInput($_POST['txt_Remark_' . $empID]);
					$save = 'call manage_refer_hr("' . $empID . '","' . $createBy . '","' . $txt_Remark . '","TH AFTER RETRAIN")';
		...
		if ($status != '' && $status != 'NA') {
			if (isset($_POST['cb'])) {
				...
				if ($count_check > 0) {
					...
					if (cleanUserInput($_POST['text_trcheck_Batch']) != 'New' && !empty($_POST['text_trcheck_Batch']) && cleanUserInput($_POST['text_trcheck_Batch']) != 'NA') {
						...
						foreach ($cb as $val) {
							...
							if ($check_rtr_q->num_rows > 0 && $check_rtr_q) {
								...
								$save = 'call manage_status_th("' . $empID . '","' . $status . '","' . $createBy . '",' . $batch_id_new . ',"' . addslashes($_POST['txt_Remark_' . $empID]) . '","' . cleanUserInput($_POST['txt_thcheck_crNo']) . '",' . $date_1_crt . ',' . $date_2_crt . ',' . $date_3_crt . ',' . $date_4_crt . ',' . $date_5_crt . ',"' . intval($txt_Day_crt_1) . '","' . intval($txt_Day_crt_2) . '","' . intval($txt_Day_crt_3) . '","' . intval($txt_Day_crt_4) . '","' . intval($txt_Day_crt_5) . '","' . $roster_log . '")';
...
if (isset($_POST['cS_ctatus'])) {
	$c_sstaus = cleanUserInput($_POST['cS_ctatus']);
} else {
	$c_sstaus = 'YES';
}
?>
				...
				<div class="form-inline">
					...
					<div id="pnlTable" class="col s12 m12 card">
						...
						if ($batch_id != 'NA' && !empty($batch_id)) {
							...
							echo $sqlConnect = 'call get_thchecklist_after("' . $userlogid . '","' . $c_sstaus . '","' . $batch_id . '")';
							$myDB = new MysqliDb();
	...
	$(document).ready(function() {
		...
		$('#btnSave,#btn_refer,#btn_retrain').click(function() {
			...
			$('input.cb_child:checkbox:checked').each(function() {
				if ($(this).parent('td').closest('tr').find('textarea[id^="txt_Remark_"]').val() == '' || $(this).parent('td').closest('tr').find('textarea[id^="txt_Remark_"]').val().length < 200) {
					validate = 1;
					toastr.info('Remark Can\'t be Empty or not less than 200 For any Checked Employee.');
				}
