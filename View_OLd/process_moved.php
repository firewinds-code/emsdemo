				...
				if ($rowCount > 0) {
					...
					if ($rowCount > 0) {
						foreach ($select_data_from_move_table as $key => $data_array) {
							...
							$selectQury = $conn->prepare($selectth);
							$selectQury->bind_param("i", $new_cm_id);
							$selectQury->execute();
							$rowCount = $selectQury->get_result();
							$select_th = $rowCount->fetch_row();
							// $mysql_error = $myDB->getLastError();
							// $rowCount = $myDB->count;
							...
							if ($rowCount > 0) {
								...
								// $myDB = new MysqliDb();
								$update_statustable = "Update status_table set ReportTo=?,Qa_ops='', $movedate_string BatchID='0',createdon=?  where  EmployeeID=?";
								$upQu = $conn->prepare($update_statustable);
								$upQu->bind_param("sss", $training_head, $date, $EmployeeID);
								$upQu->execute();
								...
								if ($rowCount > 0) {
									...
									$update_empmap = "UPDATE  employee_map set cm_id=?,modifiedon=?,modifiedby='Server' where EmployeeID=?  ";
									$upQuy = $conn->prepare($update_empmap);
									$upQuy->bind_param("sss", $new_cm_id, $date, $EmployeeID);
									$upQuy->execute();
									$update_emp_map = $upQuy->get_result();
