...
$question_num = "";
$id = isset($_GET['id']);
$id = cleanUserInput($id);
$delid = isset($_GET['delid']);
$delid = cleanUserInput($_GET['delid']);
if ($delid && $delid != "") {


	$delete_query = "DELETE from add_email_address  where ID=?";
	...
	$delete_query = "DELETE from manage_module_email  where emailID=?";
	$del = $conn->prepare($delete_query);
	$del->bind_param("i", $delid);
	$del->execute();
	$resultBy = $del->get_result();
...
$addbriefing = ($_POST['addbriefing']);
if (isset($email_address, $addbriefing)) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		$date = date('Y-m-d');
		$createdBy = $_SESSION['__user_logid'];
		$email_address = cleanUserInput($_POST['email_address']);
		$regex = '/^[_a-zA-Z0-9-]+(\.[_a-zA-Z0-9-]+)*@[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*(\.[a-z]{2,3})$/';
		if (!preg_match($regex, $email_address)) {
			$email_address = "";
		}
		if ($email_address != "") {
			$select_query = "select id from add_email_address where email_address=? ";
			$sel = $conn->prepare($select_query);
			$sel->bind_param("s", $email_address);
			$sel->execute();
			...
			if ($resultBy->num_rows > 0) {
				...
				$insertQuery = "INSERT INTO add_email_address set email_address=?,created_on=? ";
				$insert = $conn->prepare($insertQuery);
				$insert->bind_param("ss", $email_address, $date);
				$insert->execute();
...
//print_r($_POST);
$savebriefing = ($_POST['savebriefing']);

// print_r($_POST);
$idd = cleanUserInput($_POST['id']);
if (isset($savebriefing) && $idd != "") {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		$email_address = cleanUserInput($_POST['email_address']);
		...
		if ($email_address != "") {
			$updateQuery = "Update  add_email_address set email_address=? where ID=?";
			$update = $conn->prepare($updateQuery);
			$update->bind_param("si", $email_address, $idd);
			$update->execute();
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				if ($idd and $idd != "") {
					...
					$sqlConnect2 = 'select  * from  add_email_address  where ID=? ';
							...
							<tbody>
								...
								?>
									<td class="manage_item">
										<a onclick="editData('<?php echo $ID; ?>','<?php echo $add_email_address; ?>');"><i class="material-icons edit_item imgBtn imgBtnEdit tooltipped" data-position="left" data-tooltip="Edit">ohrm_edit</i></a>
									</td>

									<td class="manage_item">
										<a onclick="return confirm('Do you want to detete it?');" href="manage_email.php?delid=<?php echo $ID; ?>"><i class="material-icons delete_item imgBtn imgBtnDelete tooltipped" data-position="left" data-tooltip="Delete">ohrm_delete</i></a>
									</td>
