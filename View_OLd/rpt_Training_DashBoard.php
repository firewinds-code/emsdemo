...
if (isset($_SESSION)) {
	...
	if (!isset($clean_user_logid)) {
		...
		exit();
	} elseif (empty($clean_user_logid) || $clean_user_logid == '') {
...
<?php
	...
	?>	</select>
				...
				if (isset($_POST['btn_view'])) {
					...
					if ($clean_user_type == 'ADMINISTRATOR' || $clean_user_type == 'CENTRAL MIS') {
						$select = "select bm.BacthID,bm.BacthName,bm.client,bm.process,bm.subprocess,bm.clientid,new_client_master.cm_id,downtime_time_master.training_days,downtime_time_master.ojt_days,account_head,th from batch_master bm inner join new_client_master on new_client_master.client_name = bm.clientid and new_client_master.process = bm.process and new_client_master.sub_process = bm.subprocess inner join downtime_time_master on new_client_master.cm_id = downtime_time_master.cm_id where bm.createdon >=  ? and bm.BacthName not like '%_retrain' ";
						$selectQury = $conn->prepare($select);
						$selectQury->bind_param("s", $DateTo);
						$selectQury->execute();
						$chk_task = $selectQury->get_result();
					} else {

						$select = "select bm.BacthID,bm.BacthName,bm.client,bm.process,bm.subprocess,bm.clientid,new_client_master.cm_id,downtime_time_master.training_days,downtime_time_master.ojt_days,account_head,th from batch_master bm inner join new_client_master on new_client_master.client_name = bm.clientid and new_client_master.process = bm.process and new_client_master.sub_process = bm.subprocess inner join downtime_time_master on new_client_master.cm_id = downtime_time_master.cm_id where bm.createdon >=  ? and bm.BacthID in (select distinct BatchID from whole_details_peremp where ? in (account_head,th,qh,Trainer,Quality))   and bm.BacthName not like '%_retrain'";
						$selectQury = $conn->prepare($select);
						$selectQury->bind_param("ss", $DateTo, $clean_user_logid);
						$selectQury->execute();
						$chk_task = $selectQury->get_result();
					}
					//print_r($chk_task);
					...
					if ($chk_task->num_rows > 0) {
						...
						foreach ($chk_task as $key => $value) {
							...
							$select_emp = 'select BacthID from batch_master where BacthName  = ? ';
							$selectQy = $conn->prepare($select_emp);
							$selectQy->bind_param("s", $expected_batch_name);
							$selectQy->execute();
							$dt_retrain_tr = $selectQy->get_result();
							$dt_retrain = $dt_retrain_tr->fetch_row();
							if ($dt_retrain_tr && $dt_retrain_tr->num_rows > 0) {
								$retrain_batch_id = $dt_retrain[0];
							}
							//echo $retrain_batch_id.','; // Check for Retrain ID

							$selects = "select distinct Trainer,count(*) Count,EmployeeName as TrainerName from ( select max(id),EmployeeID,Trainer from status_training where BatchID = ?  group by EmployeeID union select max(id),EmployeeID,Trainer from status_training_log where BatchID = ?  group by EmployeeID) st inner join personal_details where st.Trainer = personal_details.EmployeeID ";
							$selectQ = $conn->prepare($selects);
							$selectQ->bind_param("ii", $value['BacthID'], $value['BacthID']);
							$selectQ->execute();
							$result = $selectQ->get_result();
							$tr_count = $result->fetch_row();
							$count_emplopyee = 0;
							if ($result->num_rows == 1 && $tr_count) {
								$table_data_per_td['Trainer Name'] = $tr_count[2];
								$table_data_per_td['Total Count'] = $count_emplopyee =  $tr_count[1];
							} else if ($result->num_rows == 1 && $tr_count) {
								$count_emplopyee = 0;
								foreach ($result as $k_tr => $val_tr) {
									$table_data_per_td['Trainer Name'] = $tr_count[2];
									$count_emplopyee += $tr_count[1];
								}
							...
							$sel = "select EmployeeID, BatchID, Trainer, `Status` , retrain_flag, Certification_1, c_status, date_cer_1, day_cer_1,`Table Status` from ( select max(id), EmployeeID, BatchID, Trainer, `Status` , retrain_flag, Certification_1, c_status, date_cer_1, day_cer_1,'Current' `Table Status` from status_training where BatchID = ?  group by EmployeeID union select max(id), EmployeeID, BatchID, Trainer, `Status` , retrain_flag, Certification_1, c_status, date_cer_1, day_cer_1,'Log' `Table Status` from status_training_log where BatchID = ?  group by EmployeeID) st ";
							$selectQury = $conn->prepare($sel);
							$selectQury->bind_param("ii",  $value['BacthID'], $value['BacthID']);
							$selectQury->execute();
							$tr1_count = $selectQury->get_result();
							if ($tr1_count->num_rows > 0 && $tr1_count) {
							...
							if ($retrain_batch_id > 0) {
								...
								$Getinfo = "select EmployeeID, BatchID, Trainer, `Status` , retrain_flag, Certification_1, c_status, date_cer_1, day_cer_1,`Table Status` from ( select max(id), EmployeeID, BatchID, Trainer, `Status` , retrain_flag, Certification_1, c_status, date_cer_1, day_cer_1,'Current' `Table Status` from status_training where BatchID = ?  group by EmployeeID union select max(id), EmployeeID, BatchID, Trainer, `Status` , retrain_flag, Certification_1, c_status, date_cer_1, day_cer_1,'Log' `Table Status` from status_training_log where BatchID = ?  group by EmployeeID) st ";
								$selectQr = $conn->prepare($Getinfo);
								$selectQr->bind_param("ii", $retrain_batch_id, $retrain_batch_id);
								$selectQr->execute();
								$tr1_rt_count = $selectQr->get_result();

								if ($tr1_rt_count->num_rows > 0 && $tr1_rt_count) {
							...
							$Getdata = "select EmployeeID, `Status`,InTraining, InOJT, OnFloor, OutTraining, InQAOJT, OutOJTQA, RetrainTime,reOJT,`Table Status` from ( select max(id),EmployeeID, `Status`,InTraining, InOJT, OnFloor, OutTraining, InQAOJT, OutOJTQA, RetrainTime,reOJT,'Current' `Table Status` from status_table where BatchID = ?  group by EmployeeID union  select max(id),EmployeeID, `Status`,InTraining, InOJT, OnFloor, OutTraining, InQAOJT, OutOJTQA, RetrainTime,reOJT,'Log' `Table Status` from status_table_log where BatchID = ?  group by EmployeeID ) st ";
							$selectQy = $conn->prepare($Getdata);
							$selectQy->bind_param("ii", $value['BacthID'], $value['BacthID']);
							$selectQy->execute();
							$tr2_count = $selectQ->get_result();

							if ($tr2_count->num_rows > 0 && $tr2_count) {
							...
							if ($retrain_batch_id > 0) {
								...
								$Get = "select EmployeeID, `Status`,InTraining, InOJT, OnFloor, OutTraining, InQAOJT, OutOJTQA, RetrainTime,reOJT,`Table Status` from ( select max(id),EmployeeID, `Status`,InTraining, InOJT, OnFloor, OutTraining, InQAOJT, OutOJTQA, RetrainTime,reOJT,'Current' `Table Status` from status_table where BatchID = ?  group by EmployeeID union  select max(id),EmployeeID, `Status`,InTraining, InOJT, OnFloor, OutTraining, InQAOJT, OutOJTQA, RetrainTime,reOJT,'Log' `Table Status` from status_table_log where BatchID = ?  group by EmployeeID ) st ";
								$selectQu = $conn->prepare($Get);
								$selectQu->bind_param("ii", $retrain_batch_id, $retrain_batch_id);
								$selectQu->execute();
								$tr2_rt_count = $selectQu->get_result();

								if ($tr2_rt_count->num_rows > 0 && $tr2_rt_count) {
							...
							if (count($Employee_data['Employee']) > 0 && $Employee_data['Employee']) {
								foreach ($Employee_data['Employee'] as $Employee) {
									$myDB = new MysqliDb();
									$conn = $myDB->dbConnect();
									$select_emp = 'select employee_map.emp_status from employee_map where EmployeeID =?';

									$selectQurys = $conn->prepare($select_emp);
									$selectQurys->bind_param("s", $Employee);
									// $selectQurys->bind_param("s", $Employee);
									$selectQurys->execute();
									$data_check_active = $selectQurys->get_result();
									$data_check_actives = $data_check_active->fetch_row();
									if ($data_check_active->num_rows > 0 && $data_check_active) {
										if ($data_check_actives[0] == 'Active') {
											$active_employee_current++;
											$Employee_data[$Employee]['Active'] = 'Active';
										} elseif ($data_check_actives[0] == 'InActive') {
						...
						{echo '<pre>'; var_dump($Employee_data['Employee']);echo '</pre>'; }*/
							...
							if ($retrain_count > 0) {
								if ($retrain_batch_id > 0) {
									...
									$status = 'select max(date_cer_1) as RT_End,(select min(RetrainTime) from status_table where BatchID = status_training.BatchID) RT_Start from status_training where BatchID = ? ';
									$selectQury = $conn->prepare($status);
									$selectQury->bind_param("i", $retrain_batch_id);
									$selectQury->execute();
									$res = $selectQury->get_result();
									$status_retrain_dt = $res->fetch_row();
									//Check for Retrain Data
									//var_dump($status_retrain_dt);
									if ($res->num_rows > 0 && $res) {
										$table_data_per_td['Re-Training Start Date'] = ((strtotime($status_retrain_dt[1])) ? date('Y-m-d', strtotime($status_retrain_dt[1])) : '');
										$table_data_per_td['Re-Training End Date'] = ((strtotime($status_retrain_dt[0])) ? date('Y-m-d', strtotime($status_retrain_dt[0])) : '');
									}
								}
							}
							...
							$ojt_count_data_fqa  = "";
							if ($retrain_batch_id > 0) {
								$ojt_count_data_fqa = "select distinct Quality,count(*) Count,EmployeeName as QualityName from ( select max(id),EmployeeID,Quality from status_quality where BatchID =? or BatchID =? group by EmployeeID union select max(id),EmployeeID,Quality from status_quality_log where BatchID =?  or BatchID =? group by EmployeeID) st inner join personal_details where st.Quality = personal_details.EmployeeID ";
								$selectQury = $conn->prepare($ojt_count_data_fqa);
								$selectQury->bind_param("iiii", $value['BacthID'], $retrain_batch_id, $value['BacthID'], $retrain_batch_id);
								$selectQury->execute();
								$resu = $selectQury->get_result();
								$qr_count = $resu->fetch_row();
							} else {
								$ojt_count_data_fqa = "select distinct Quality,count(*) Count,EmployeeName as QualityName from ( select max(id),EmployeeID,Quality from status_quality where BatchID = ? group by EmployeeID union select max(id),EmployeeID,Quality from status_quality_log where BatchID = ? group by EmployeeID) st inner join personal_details where st.Quality = personal_details.EmployeeID ";
								$selectQury = $conn->prepare($ojt_count_data_fqa);
								$selectQury->bind_param("ii", $value['BacthID'],  $value['BacthID']);
								$selectQury->execute();
								$resu = $selectQury->get_result();
							...
							if ($resu->num_rows == 1 && $resu) {
								$table_data_per_td['OJT QA name'] = $qr_count[2];
								$table_data_per_td['Total Count OJT'] = $qr_count[1];
								$count_emplopyee_qr = $qr_count[1];
							} else if ($resu->num_rows == 1 && $resu) {
								$count_emplopyee_qr = 0;
							...
							if ($retrain_batch_id > 0) {
								$Select = "select EmployeeID, BatchID, Quality, `Status` , ojt_status, Final_OJT_date,`Table Status` from ( select max(id), EmployeeID, BatchID, Quality, `Status` , ojt_status, case when ojt_status > 0 && ojt_duration > 0 and exists(select reOJT from ( select reOJT,EmployeeID from status_table where (BatchID ='1296' or BatchID ='1310' and reOJT is not null) union select reOJT,EmployeeID from status_table_log where (BatchID ='1296' or BatchID ='1310' and reOJT is not null) ) st where st.EmployeeID = status_quality.EmployeeID) then DATE_SUB(Final_OJT_date, INTERVAL ojt_duration DAY) else  Final_OJT_date end  Final_OJT_date,'Current' `Table Status` from status_quality where BatchID =? group by EmployeeID union   select max(id), EmployeeID, BatchID, Quality, `Status` , ojt_status, case when ojt_status > 0 && ojt_duration > 0  then DATE_SUB(Final_OJT_date, INTERVAL ojt_duration DAY) else  Final_OJT_date end  Final_OJT_date,'Log'   `Table Status` from status_quality_log where BatchID =?  group by EmployeeID) st  ";
								$selectQry = $conn->prepare($Select);
								$selectQry->bind_param("ii", $retrain_batch_id, $retrain_batch_id);
								$selectQry->execute();
								$qr1_rt_count = $selectQry->get_result();

								if ($qr1_rt_count->num_rows > 0 && $qr1_rt_count) {
							...
							if ($reojt_count > 0) {
								...
								$statusdt = 'select min(reOJT) as `Start_ReOJT`,max(OutOJTQA) as `End_ReOJT` from status_table where BatchID = ? or  BatchID = ? ';
								$selectQuy = $conn->prepare($statusdt);
								$selectQuy->bind_param("i", $value['BacthID'], $retrain_batch_id);
								$selectQuy->execute();
								$result = $selectQuy->get_result();
								$status_reOJT_dt = $result->fetch_row();
								//Kent|Kent Inbound|Inbound|35_retrain
								if ($result->num_rows > 0 && $result) {
									$table_data_per_td['Re-OJT Start Date'] = ((strtotime($status_reOJT_dt[0])) ? date('Y-m-d', strtotime($status_reOJT_dt[0])) : '');
									$table_data_per_td['Re-OJT End Date'] = ((strtotime($status_reOJT_dt[1])) ? date('Y-m-d', strtotime($status_reOJT_dt[1])) : '');
								}
							}
							...
							if ($inactive_employee_InM_qr > 0) {
								...
								if (count($Employee_data[$Employee]['Status']) > 0 and $Employee_data[$Employee]['Status']) {
									...
									foreach ($Employee_data['Employee'] as $Employee) {
										...
										if ($Employee_data[$Employee]['Status'] == 5) {
											$query_exit = 'select * from exit_emp where EmployeeID = ?';
							...
							$myDB = new MysqliDb(); //
							$get_client = "select bm.BacthID,bm.BacthName,bm.client,bm.process,bm.subprocess,bm.clientid,account_head,oh,qh,th,DateCreated,bm.createdon from batch_master bm join (select * from ( select account_head,oh,qh,th,client_name,process,sub_process,case when modifiedon is null then createdon else modifiedon end as 'DateCreated' from new_client_master union select account_head,oh,qh,th,client_name,process,sub_process,case when modifiedon is null then createdon else modifiedon end as 'DateCreated' from new_client_master_log ) t1) new_client_master on new_client_master.client_name = bm.clientid and new_client_master.process = bm.process and new_client_master.sub_process = bm.subprocess where BacthID = ? and cast(DateCreated as date) <= cast(bm.createdon as date) order by DateCreated desc limit 1";
							$selectQury = $conn->prepare($get_client);
							$selectQury->bind_param("i", $value['BacthID']);
							$selectQury->execute();
							$resultss = $selectQury->get_result();
							$get_client_detials = $resultss->fetch_row();
							if ($resultss->num_rows > 0 && $resultss) {

								$dt = "select EmployeeName from personal_details where EmployeeID =?;";
								$selectQury = $conn->prepare($dt);
								$selectQury->bind_param("s", $get_client_detials[9]);
								$selectQury->execute();
								$res = $selectQury->get_result();
								$dt_head_details = $res->fetch_row();

								$table_data_per_td['Training Head'] = $dt_head_details[0];

								$dt = "select EmployeeName from personal_details where EmployeeID =?;";
								$selectQury = $conn->prepare($dt);
								$selectQury->bind_param("s", $get_client_detials[6]);
								$selectQury->execute();
								$res = $selectQury->get_result();
								$dt_head_details = $res->fetch_row();

								$table_data_per_td['Account Head'] = $dt_head_details[0];
							...
							if (count($Employee_data['Employee']) > 0 && $Employee_data['Employee']) {
								...
								foreach ($Employee_data['Employee'] as $Employee) {
									...
									if ($Employee_data[$Employee]['Active'] == 'InActive') {
										...
										$query_exit = "select distinct EmployeeID from exit_emp where disposition='ABSC' and EmployeeID = ? and dol between ? and ?";
										$selectQry = $conn->prepare($query_exit);
										$selectQry->bind_param("sss", $Employee, $Start, $End);
										$selectQry->execute();
										$exit_data_check_peremp = $selectQry->get_result();


										// $exit_data_check_peremp = $myDB->query($query_exit_pemp);
										if ($exit_data_check_peremp->num_rows > 0 && $exit_data_check_peremp) {
											$table_data_per_td['3 day Attrition Abscond']++;
										}
									}


									$query_hr_pemp = "select EmployeeID from hr_refered where EmployeeID = ? and createdon between ? and ?";
									$selectQry = $conn->prepare($query_hr_pemp);
									$selectQry->bind_param("sss", $Employee, $Start, $End);
									$selectQry->execute();
									$ht_data_check_peremp = $selectQry->get_result();

									// $ht_data_check_peremp = $myDB->query($query_hr_pemp);
									if ($ht_data_check_peremp->num_rows > 0 && $ht_data_check_peremp) {
										$table_data_per_td['3 Days RHR']++;
									}

									$query_hr_pemp = "select EmployeeID from hr_refered where EmployeeID = ? and ref_level like 'QH%' and createdon between ? and ? ";
									$selectQry = $conn->prepare($query_hr_pemp);
									$selectQry->bind_param("sss", $Employee, $Start, $data_ojt_ActualEnd);
									$selectQry->execute();
									$ht_data_check_peremp = $selectQry->get_result();
									...
									$query_des_id_pemp = "select des_id from whole_dump_emp_data where EmployeeID = ?";
									$selectQry = $conn->prepare($query_des_id_pemp);
									$selectQry->bind_param("s", $Employee);
									$selectQry->execute();
									$data_des_id = $selectQry->get_result();
									$des_id = $data_des_id->fetch_row();
									// $data_des_id = $myDB->query($query_des_id_pemp);
									if ($data_des_id->num_rows > 0 && $data_des_id) {
