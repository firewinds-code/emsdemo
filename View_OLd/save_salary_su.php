...
if (strtoupper($_SERVER['REQUEST_METHOD']) == 'POST') {
	...
	$emp_loc = cleanUserInput($_POST['hidden_loc']);
...
if (isset($_REQUEST['empid']) && $EmployeeID == '' && !isset($_POST['EmployeeID'])) {
	...
	$getDetails = 'select t2.client_name from employee_map t1 inner join new_client_master t2 on t1.cm_id=t2.cm_id where t1.EmployeeID= ?';
	$selectQury = $conn->prepare($getDetails);
	$selectQury->bind_param("s", $EmployeeID);
	$selectQury->execute();
	$res = $selectQury->get_result();
	$result_all = $res->fetch_row();
	// $myDB = new MysqliDb();
	// $result_all = $myDB->rawQuery($getDetails);
	$clientid = $result_all[0];
...
<script>
	$(document).ready(function() {
		...
		if ((usrtype === 'ADMINISTRATOR' && usrtype_tmp === 'ADMINISTRATOR') || usrtype === 'HR') {} else if (usrtype === 'AUDIT' || (usrtype === 'ADMINISTRATOR' && usrtype_tmp === 'SITEADMIN')) {
			...
			<div class="schema-form-section row">
				...
				<div class="col s12 m12">
					...
					<div class="input-field col s4 m4">
						...
						<label for="txt_sal_type" class="active-drop-down active">Sal Type *</label>
					</div>
					<div class="input-field col s4 m4">
					...
					$(document).ready(function() {
						...
						var check_validate_sal = function() {
							...
							if ($('#txt_sal_type').val() == 'NA') {
								...
								validate = 1;
								alert_msg += '<li> Sal Type can not be Empty </li>';
							}
							/*if($('#txt_sal_pf').val()=='')
	        {
							...
							if (!isNaN($('#txt_sal_ctc').val())) {
								if ($('#txt_sal_emptype').val() == 'FT') {
									if ($('#txt_sal_paytype').val() == 'RT') {
										...
										var arr = [14100, 14500, 15000, 15500, 15800, 16000, 16500, 17000, 17500, 18000, 18500, 19000, 19500, 20000, 20500, 21000];


										if (parseInt($('#txt_sal_ctc').val()) < 14100 || parseInt($('#txt_sal_ctc').val()) > 21000) {
											validate = 1;
											//alert_msg+='<li> CTC  can not be Less Than 14100 and Greater Than  21000 for this Pay-Roll Type </li>';
										}
								...
								if ($('#txt_sal_emptype').val() == 'FT') {
									if ($('#txt_sal_paytype').val() == 'RT') {
										...
										if (hidden_loc == 7) {
											...
											if (parseInt($('#txt_sal_ctc').val()) < 10856 || parseInt($('#txt_sal_ctc').val()) > 21000) {
												validate = 1;
												alert_msg += '<li> CTC  can not be Less Than 10856 and Greater Than  21000 for this Pay-Roll Type </li>';
											}
										}
