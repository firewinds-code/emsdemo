 <?php
	...
	if ($submit) {
		if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
			$bank = cleanUserInput($_POST['txt_bank_name']);
			$q = "select lower(BankName)as BankName from bank_master where BankName=lower(?);";
			// $myDB = new MysqliDb();
			// $result_q = $myDB->query($q);
			$selectQ = $conn->prepare($q);
			$selectQ->bind_param("s", $bank);
			$selectQ->execute();
			...
			if ($result_q->num_rows > 0) {
				...
				$bankname = trim($_POST['txt_bank_name']);
				$insert = "insert into bank_master (`BankName`)values (upper(?));";
				//insert upper case approval of rinku mam 2021-01-01
				// $insert="insert into bank_master (`BankName`)values ('".$_POST['txt_bank_name']."');";
				// $myDB = new MysqliDb();
				// $result = $myDB->rawQuery($insert);
				$ins = $conn->prepare($insert);
				$ins->bind_param("s", $bankname);
				$ins->execute();
	...
	if ($update) {
		if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
			$bank_name = cleanUserInput($_POST['txt_bank_name']);
			$q = "select lower(BankName)as BankName from bank_master where BankName=lower(?);";
			// $myDB = new MysqliDb();
			// $result_q = $myDB->query($q);
			$selectQ = $conn->prepare($q);
			$selectQ->bind_param("s", $bank_name);
			$selectQ->execute();
			$result_q = $selectQ->get_result();
			...
			if ($result_q->num_rows > 0) {
				...
				$BANK = trim($_POST['txt_bank_name']);
				$ID = cleanUserInput($_POST['UpdateId']);
				$update = "update  bank_master set BankName =upper(?) where id=?;"; //update upper case approval of rinku mam 2021-01-01
				...
				$updates->bind_param("si", $BANK, $ID);
				$updates->execute();
				$result = $updates->get_result();
