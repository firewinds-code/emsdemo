			...
			<div class="schema-form-section row">
				...
				<div class="input-field col s6 m6 left-align">
					...
					<Select name="emp_status" style="min-width: 200px;" id="status">
						<option value='Active' <?php if (isset($empStatus) && $empStatus == 'Active') {
													echo "selected";
												} ?>>Active</option>
						<option value='InActive' <?php if (isset($empStatus) && $empStatus == 'InActive') {
														echo "selected";
													} ?>>InActive</option>
					</Select>
				</div>
				...
				if (isset($_POST['btn_view'])) {
					...
					$chk_task = $myDB->query('call sp_getAPRReport("' . $__user_logid  . '","' . $month . '","' . $year . '","' . $empStatus . '","' . $process . '")');
					$my_error = $myDB->getLastError();
					if (empty($my_error)) {
