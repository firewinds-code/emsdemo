...
if (isset($_POST['btnSave'])) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		...
		$status = trim(cleanUserInput($_POST['txt_thcheck_Trainer']));
		$createBy = clean($_SESSION['__user_logid']);

		if ($status != "" && $status != "NA") {

			$querycheck = 'SELECT  distinct status_training.Trainer,status_training.BatchID FROM  status_training inner join  status_table on status_table.EmployeeID = status_training.EmployeeID inner join  employee_map on employee_map.EmployeeID = status_training.EmployeeID  where status_table.Status="3"  and status_training.`Status`="NO" and employee_map.emp_status ="Active" and status_training.Trainer = ?';
			$selectQury = $conn->prepare($querycheck);
			$selectQury->bind_param("s", $status);
			$selectQury->execute();
			$query_check = $selectQury->get_result();
			$vaidate = 0;

			if ($query_check->num_rows > 0 && $query_check) {
				$vaidate = 1;
			...
			if (isset($_POST['cb']) && $vaidate == 0) {
				...
				if ($count_check > 0) {
					...
					if ($_POST['text_trcheck_Batch'] != 'New' && !empty($_POST['text_trcheck_Batch']) && $_POST['text_trcheck_Batch'] != 'NA') {
						$batch_id_tr = cleanUserInput($_POST['text_trcheck_Batch']);
						...
						foreach ($_POST['cb'] as $val) {
							...
							$myDB = new MysqliDb();
							$txt_Date_crt_1 = cleanUserInput($_POST['txt_Date_crt_1']);
							$date_1_crt = (!empty($txt_Date_crt_1) ? '"' . $txt_Date_crt_1 . '"' : "NULL");
							$txt_Date_crt_2 = cleanUserInput($_POST['txt_Date_crt_2']);
							$date_2_crt = (!empty($txt_Date_crt_2) ? '"' . $txt_Date_crt_2 . '"' : "NULL");
							$txt_Date_crt_3 = cleanUserInput($_POST['txt_Date_crt_3']);
							$date_3_crt = (!empty($txt_Date_crt_3) ? '"' . $txt_Date_crt_3 . '"' : "NULL");
							$txt_Date_crt_4 = cleanUserInput($_POST['txt_Date_crt_4']);
							$date_4_crt = (!empty($txt_Date_crt_4) ? '"' . $txt_Date_crt_4 . '"' : "NULL");
							$txt_Date_crt_5 = cleanUserInput($_POST['txt_Date_crt_5']);
							$date_5_crt = (!empty($txt_Date_crt_5) ? '"' . $txt_Date_crt_5 . '"' : "NULL");
							$txt_thcheck_crNo = cleanUserInput($_POST['txt_thcheck_crNo']);

							$txt_Day_crt_1 = cleanUserInput($_POST['txt_Day_crt_1']);
							$txt_Day_crt_2 = cleanUserInput($_POST['txt_Day_crt_2']);
							$txt_Day_crt_3 = cleanUserInput($_POST['txt_Day_crt_3']);
							$txt_Day_crt_4 = cleanUserInput($_POST['txt_Day_crt_4']);
							$txt_Day_crt_5 = cleanUserInput($_POST['txt_Day_crt_5']);
							$save = 'call manage_status_th("' . $empID . '","' . $status . '","' . $createBy . '",' . $batch_id_tr . ',"' . $_POST['txt_Remark_' . $empID] . '","' . $txt_thcheck_crNo . '",' . $date_1_crt . ',' . $date_2_crt . ',' . $date_3_crt . ',' . $date_4_crt . ',' . $date_5_crt . ',"' . intval($txt_Day_crt_1) . '","' . intval($txt_Day_crt_2) . '","' . intval($txt_Day_crt_3) . '","' . intval($txt_Day_crt_4) . '","' . intval($txt_Day_crt_5) . '","' . $roster_log . '")';
		...
		if (isset($_POST['cb'])) {
			...
			if ($count_check > 0) {
				...
				foreach ($_POST['cb'] as $val) {
					...
					$empID = cleanUserInput($val);
					$txt_Remark_ = cleanUserInput($_POST['txt_Remark_' . $empID]);
					$myDB = new MysqliDb();
					$save = 'call manage_refer_hr("' . $empID . '","' . $createBy . '","' . $txt_Remark_ . '","th")';
					$resulti = $myDB->query($save);
					$mysql_error = $myDB->getLastError();
					if (empty($mysql_error)) {
				...
				<div class="">
					...
					<div class="input-field col s6 s6 statuscheck">
						...
						<select id="text_trcheck_Batch" name="text_trcheck_Batch" onchange="javascript:return getBatch_state();">
							...
							$sqlBy = 'SELECT distinct batch_master.BacthID, batch_master.BacthName,createdon FROM batch_master where batch_master.createdby=? and datediff(curdate(),createdon) < 5 having datediff(curdate(),createdon) < (3 + (select count(DateOn) from roster_temp where EmployeeID = batch_master.createdby and roster_temp.InTime like "%WO%" and DateOn between cast(batch_master.createdon as date) and curdate())) order by batch_master.BacthName';
							$batch_id = 0;
							// $myDB = new MysqliDb();
							// $resultBy = $myDB->query($sqlBy);
							// $error  = $myDB->getLastError();
							$selectQury = $conn->prepare($sqlBy);
							$selectQury->bind_param("s", $createdby);
							$selectQury->execute();
					...
					<div id="pnlTable" class="col s12 m12 card">
						...
						$sqlConnect = 'call get_thchecklist("' . $userLOg . '")';
						$myDB = new MysqliDb();
...
<script>
	...
	function getBatch_state() {
		...
		$.ajax("../Controller/getBatchState.php?type=th&id=" + batchid + "&loc=" + location).done(function(result) {
