	...
	<div class="pim-container row" id="div_main">
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				$roster_account = $myDB->query("call rosterFor_accounthead_between('" . $user_logid . "','" . date('Y-m-d', strtotime($dt_First . ' -7days')) . "','" . date('Y-m-d', strtotime($dt_First . ' -1days')) . "','" . $dept . "')");
				//echo "<br>";
				$myDB = new mysqliDb();
				$rst_emp = $myDB->query("call activeFor_accounthead('" . $user_logid . "','" . $dept . "')");
				...
				if (count($rst_emp) > 0) {
					...
					foreach ($rst_emp as $key => $values) {
						...
						$OutOJT = "";
						if (count($extraDate) > 0 && $extraDate) {
							$OutOJT = $extraDates[0];
						}
