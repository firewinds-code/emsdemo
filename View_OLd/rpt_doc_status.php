	...
	if (!isset($user_logid)) {
		...
		if ($isPostBack && isset($_POST['txt_docStaus'])) {
			if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
				$doctype = cleanUserInput($_POST['txt_doctype']);
				$docStatus = cleanUserInput($_POST['txt_docStaus']);
			}
		}
	}
} else {
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				if (isset($_POST['btn_view'])) {
					...
					$conn = $myDB->dbConnect();
					$chk_task  = array();
					if (($docStatus == 'Having') && ($doctype == 'Adhar Card')) {
						// $doc = (($doctype == 'Adhar Card') ? 'Aadhar Card' : $doctype);
						...
						// $sqlBy = "select distinct t1.EmployeeID, EmployeeName, t2.dov_value as 'Aadhar Card', t2.doc_file, Process, sub_process, designation from whole_details_peremp t1 join doc_details t2 on t1.EmployeeID=t2.EmployeeID where doc_stype='Aadhar Card' and dov_value is not null and dov_value!='' and doc_file is not null and doc_file!='';";
						// $selectQury = $conn->prepare($sqlBy);
						// $selectQury->bind_param("", );
						// $selectQury->execute();
