			...
			if ($count_check > 0) {
				...
				foreach ($checked_arr as $val) {
					...
					if ($Status != 'NA' || $Status != 'Na') {
						...
						$sql = 'update resign_details set status_ah = "1" where EmployeeID = ?';
						...
						if ($resulti->num_rows > 0) {
							...
							$sql1 = 'INSERT INTO resign_comment_log(`EmployeeID`,`CreatedBy`,`CreatedOn`,`Comment`,`type`)VALUES(?,?,now(),?,"AH")';
							$stmt1 = $conn->prepare($sql1);
							$stmt->bind_param("sss", $EmpID, $createBy, $Remark);
							$stmt1->execute();
							$rstdt = $stmt1->get_result();
							// $rstdt = $myDB->rawQuery($sql1);
						} else {
							echo "<script>$(function(){ toastr.error('Request Not Updated ::Error'); }); </script>";
						}
					}
				}
			} else {
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				<div id="pnlTable">
					...
					$chk_taskQry = 'SELECT whole_details_peremp.EmployeeID,whole_details_peremp.EmployeeName,resign_details.remark,resign_details.nt_start,resign_details.nt_end,whole_details_peremp.Process,whole_details_peremp.sub_process,whole_details_peremp.clientname from resign_details inner join whole_details_peremp on whole_details_peremp.EmployeeID = resign_details.EmployeeID left outer join personal_details pd1 on pd1.EmployeeID = resign_details.EmployeeID left outer join personal_details pd on pd.EmployeeID = ReportTo where (curdate() between cast(DATE_SUB(resign_details.nt_end, INTERVAL 3 DAY) as date)  and cast(resign_details.nt_end as date)) and  (account_head = ?) and (status_ah  is null or status_ah = 0) and nt_end >= curdate() and rg_status < 9';
