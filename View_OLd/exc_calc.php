 <?php
	...
	$month = $year = $day = 0;
	if (isset($_SESSION)) {
		if (!isset($EmployeeID) || 	$EmployeeID != 'CE03070003') {
			$location = URL . 'Login';
			header("Location: $location");
		} else {
			$txtMonth = isset($_POST['txtMonth']);
			if ($txtMonth) {
				$month = cleanUserInput($_POST['txtMonth']);
				$year = cleanUserInput($_POST['txtYear']);
				$day = cleanUserInput($_POST['txtdtc']);
			} else {
				$month = date('m', time());
				$year = date('Y', time());
				$day = 10;
	...
	$alert_msg = '';
	$dateF = cleanUserInput($_POST['dateFrom']);
	$DateFrom = (isset($dateF)) ? $dateF : date('Y-m-d', strtotime('-10 days'));
	$btn_ExcCalculation_Save = isset($_POST['btn_ExcCalculation_Save']);
	if (isset($btn_ExcCalculation_Save)) {
			...
			if (count($res_ext) > 0 && $res_ext) {
				foreach ($res_ext as $key => $value) {
					if (!empty($_POST['dateFrom'])) {
						...
						$rstcmid = 'select cm_id from employee_map where EmployeeID= ?';
						$selectQ = $conn->prepare($rstcmid);
						$selectQ->bind_param("s", $empid);
						$selectQ->execute();
						$results = $selectQ->get_result();
						$rst_cmid = $results->fetch_row();
						$cmid = $rst_cmid[0];

						$DateFrom = cleanUserInput($_POST['dateFrom']);
 		...
 		<div class="form-div">
 			...
 			<div class="schema-form-section row">
 				...
 				<div class="input-field col s10 m10">
 					...
 					<label for="fileToUpload" class="active-drop-down active" style="margin-top: -15px;">Select File</label>
 				</div>
 				...
 				<?php
					...
					if (isset($UploadBtn)) {
						...
						if ($_FILES["fileToUpload"]["size"] > 5000000) {
							$msgFile = $msgFile . "<p  class='msgFile text-danger'>Sorry, your file is too large of Size.</p>";
							echo "<script>$(function(){ toastr.error('" . clean($msgFile) . "'); }); </script>";
							$uploadOk = 0;
						}
						...
						if ($uploadOk == 0) {
							...
							if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
								$msgFile = $msgFile . "<p  class='msgFile text-success'>The file  has been uploaded. </p>";

								echo "<script>$(function(){ toastr.success('" . clean($msgFile) . "'); }); </script>";
								...
								if ((count($activeSheetData) - 1) < 11) {
									...
									if ($count > 0) {
										...
										$msgFile = $msgFile . "<p  class='msgFile text-danger'>No Data Updated</p>";
										echo "<script>$(function(){ toastr.error('" . clean($msgFile) . "'); }); </script>";

										if (file_exists($target_dir . basename($_FILES["fileToUpload"]["name"]))) {
											$ext = pathinfo($target_file, PATHINFO_EXTENSION);
											if (!empty($_POST['fileToUpload'])) {
												$fileToUpload = cleanUserInput($_POST['fileToUpload']);
												rename($target_file, $target_dir . time() . '_' . $uploader . "_ExcAttendance_" . $fileToUpload . "." . $ext);
											}
										}
									}
								} else {
