	...
	<div class="pim-container row" id="div_main">
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				if ($type == 'CO Data') {
					//echo 'call mis_report_ComboOff("'.$date_by.'","'.$__user_logid.'","'.$__user_type.'")';
					$chk_task = $myDB->query('call mis_report_ComboOff("' . $date_by . '","' . $__user_logid . '","' . $__user_type . '")');
					...
					'call mis_report_paidleave("' . $date_From . '","' . $date_To . '","' . $__user_logid . '","' . $__user_type . '")';
					$chk_task = $myDB->query('call mis_report_paidleave("' . $date_From . '","' . $date_To . '","' . $__user_logid . '","' . $__user_type . '")');
					$my_error = $myDB->getLastError();;
					...
					//echo 'call mis_SignInOut_Report("'.$date_on.'","'.$__user_logid.'","'.$__user_type.'")';
					$chk_task = $myDB->query('call mis_SignInOut_Report("' . $date_on . '","' . $__user_logid . '","' . $__user_type . '")');
					$my_error = $myDB->getLastError();;
					...
					//echo 'call mis_RowPunch_Report("'.$date_on.'","'.$__user_logid.'","'.$__user_type.'")';
					$chk_task = $myDB->query('call mis_RowPunch_Report("' . $date_on . '","' . $__user_logid . '","' . $__user_type . '")');
					$my_error = $myDB->getLastError();;
					...
					//echo 'call mis_RowPunchWFH_Report("'.$date_on.'","'.$__user_logid.'","'.$__user_type.'")';
					$chk_task = $myDB->query('call mis_RowPunchWFH_Report("' . $date_on . '","' . $__user_logid . '","' . $__user_type . '")');
					$my_error = $myDB->getLastError();;
					...
					if ($__user_type == "ADMINISTRATOR" || $__user_type == "CENTRAL MIS") {
						...
						$select = "SELECT t1.EmployeeName PendingWith,t2.Exception,count(t2.EmployeeID) Count,date_format(t2.createdon,'%d %M,%Y') PendingSince,t2.MgrStatus,t4.location FROM exception t2 inner join whole_details_peremp t3 on t2.EmployeeID=t3.EmployeeID inner join personal_details t1 on t3.account_head=t1.EmployeeID join location_master t4 on t4.id = t1.location where t2.MgrStatus='Pending' and t3.account_head!=t2.EmployeeID and t3.account_head=? group by t1.EmployeeName,t2.Exception,cast(t2.createdon as date) ,t2.MgrStatus union SELECT 'NITIN SAHNI' Name,t2.Exception,count(t2.EmployeeID) Count,cast(t2.createdon as date) PendingSince,t2.MgrStatus,t4.location FROM exception t2 inner join whole_details_peremp t3 on t2.EmployeeID=t3.EmployeeID join location_master t4 on t4.id = t3.location  where t2.MgrStatus='Pending' and t3.ReportTo='CE07147134' and t3.account_head=? group by t2.Exception,cast(t2.createdon as date) ,t2.MgrStatus;";
...
<script>
	...
	$(document).ready(function() {
		...
		$('#btn_view').on('click', function() {
			...
			var usrid = <?php echo "'" . $__user_logid . "'"; ?>;
			var usrtype = <?php echo "'" . $__user_type . "'"; ?>;
			var admin = <?php echo "'" . $__user_type  . "'"; ?>;
			// alert(codata)
			if (codata == 'CO Data') {
				// $("#preloader").show();
				...
				if (admin == "ADMINISTRATOR" || admin == "CENTRAL MIS") {
