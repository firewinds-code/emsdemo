...
if (isset($_SESSION)) {
	...
	if (!isset($clean_u_logid)) {
		...
		$clean_date_month = cleanUserInput($_POST['txt_dateMonth']);
		if ($isPostBack && isset($clean_date_month)) {
			if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
				$date_To = cleanUserInput($_POST['txt_dateMonth']);
				$date_From = cleanUserInput($_POST['txt_dateYear']);
				$clean_dept = cleanUserInput($_POST['txt_dept']);
	...
	<div class="pim-container row" id="div_main">
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				$myDB = new MysqliDb();
				$type = '';
				$type = clean($_SESSION['__user_type']);
				$chk_task = $myDB->query('call sp_get_atnd_team_report("' . $clean_u_logid . '","' . $date_To . '","' . $date_From . '")');
				...
				if (count($chk_task) > 0 && $chk_task) {
					...
					foreach ($chk_task as $key => $value) {
						...
						$clean_emidd = clean($value['EmployeeID']);
						$selectQury->bind_param("s", $clean_emidd);
						$selectQury->execute();
						$res = $selectQury->get_result();
						$temp_result = $res->fetch_row();
						if (!empty(clean($temp_result[0])) && $value['emp_status'] == 'InActive' && !empty(clean($temp_result[1]))) {
							$i = 1;
							while ($i <= 31) {
								$date_1 = date('Y-m-d', strtotime(clean($temp_result[1])));
								$date_2 = date('Y-m-d', strtotime($value['Year'] . '/' . $value['Month'] . '/' . $i));


								if ($date_2 == $date_1) {
									if (strtoupper(clean($temp_result[2])) == 'RES' || strtoupper(clean($temp_result[0])) == 'RES') {
										$table .= '<td>' . $value['D' . $i] . '</td>';
									} else if (!empty(clean($temp_result[2]))) {
										$table .= '<td>' . clean($temp_result[2]) . '</td>';
									} else {
										$table .= '<td>' . clean($temp_result[0]) . '</td>';
									}
								} else if ($date_2 > $date_1) {
									if (!empty(clean($temp_result[2]))) {
										$table .= '<td>' . clean($temp_result[2]) . '</td>';
									} else {
										$table .= '<td>' . clean($temp_result[0]) . '</td>';
									}
						...
						if (!empty(clean($temp_result[0])) && $value['emp_status'] == 'InActive' && !empty(clean($temp_result[1]))) {
							$table .= '<td>' . date('Y-m-d', strtotime(clean($temp_result[1]))) . '</td>';
						} else {
							$table .= '<td></td>';
						}
