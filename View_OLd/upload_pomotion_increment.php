...
$btnupload = isset($_POST['UploadBtn']);
if ($btnupload) {
				...
				if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
					...
					foreach ($activeSheetData as $row) {
						if ($row_counter > 0 && !empty($row['A']) && $row['A'] != '') {
							...
							@$sql_p_inert = 'CALL insert_promotion("' . strtoupper($row['A']) . '","' . $row['B'] . '","' . $wef . '","' . $row['D'] . '","' . $user_logid . '");';
				...
				if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
					...
					foreach ($activeSheetData as $row) {
						if ($row_counter > 0 && !empty($row['A']) && $row['A'] != '') {
							...
							@$sql_p_inert = 'CALL insert_increment("' . strtoupper($row['A']) . '","' . $row['B'] . '","' . $wef . '","' . $row['D'] . '","' . $row['E'] . '","' . $row['F'] . '","' . $userid . '");';
	...
	<div class="pim-container row" id="div_main">
		...
		<div class="form-div">
			...
			<h4>Upload Promotion/ Increment
				...
				<a class="btn-floating btn-large waves-effect waves-light modal-trigger tooltipped" href="../FileContainer/upload_format_increment.xlsx" data-position="bottom" data-tooltip="Download Formate For Increment"><i class="material-icons">file_download</i></a>
			</h4>
