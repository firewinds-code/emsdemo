...
if ($result_all) {
	...
	if ($loc == "1" || $loc == "2") {
		...
		$loc_dir = 'Banglore_Fk/Docs/BankDocs/';
	}
}
...
if (strtoupper($_SERVER['REQUEST_METHOD']) == 'POST') {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		...
		$txt_name_asper_bank = cleanUserInput($_POST['txt_name_asper_bank']);
		$txt_name_asper_bank = (isset($txt_name_asper_bank) ? $txt_name_asper_bank : null);
		$txt_bank_ifsc = cleanUserInput($_POST['txt_bank_ifsc']);
		$txt_bank_ifsc = (isset($txt_bank_ifsc) ? $txt_bank_ifsc : null);
		...
		if (isset($_FILES["File3"]["name"]) and $_FILES["File3"]["name"] != "") {
			...
			$CancelledCheque = cleanUserInput($_POST['EmployeeID']) . '_CancelledCheque.' . $FileType;
...
$btn_experice_Add = isset($_POST['btn_experice_Add']);
if ($btn_experice_Add && $EmployeeID != '') {
	$myDB = new MysqliDb();
	...
	$selectBankQry = "select EmployeeID from bank_details where EmployeeID=? ";
	$stmt = $conn->prepare($selectBankQry);
	$stmt->bind_param("s", $EmployeeID);
	$stmt->execute();
	$selectBank = $stmt->get_result();
...
if ($btn_experice_Save && cleanUserInput($_POST['bankSelect']) != '') {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		$myDB = new MysqliDb();
		$createBy = clean($_SESSION['__user_logid']);
	...
	<div class="pim-container row" id="div_main">
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				$stmt1 = $conn->prepare($sqlBankQuery);
				...
				if ($data_array->num_rows > 0) {
					...
					$bankid = clean($data_arrayRow[0]); //['bank_id'];
				}
