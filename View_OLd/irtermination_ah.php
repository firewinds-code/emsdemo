...
if (($btn_issue_save)) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		$statusAH = cleanUserInput($_POST['StatusAH']);
		...
		$AccountHead = cleanUserInput($_POST['AccountHead']);
		$Clientname = cleanUserInput($_POST['Clientname']);
		$Designation = cleanUserInput($_POST['Designation']);
		$ReportTo = cleanUserInput($_POST['ReportTo']);
		$remarks = cleanUserInput($_POST['remark']);
		$TerminationDate = cleanUserInput($_POST['TerminationDate']);
		$DOJ = cleanUserInput($_POST['DOJ']);
		$p_Empid = cleanUserInput($_POST['p_EmpID']);

		$SubType = ($_POST['SubType']);
		if ($SubType != "" && $statusAH != "" && $TerminationDate != "") {
					...
					if (is_uploaded_file($_FILES['txt_doc_name_']['tmp_name'][$name])) {
						...
						if ($validation_check === 0) {
							if (move_uploaded_file($sourcePath, $targetPath)) {
								...
								$filename = $EmployeeID . '_' . preg_replace('/\s+/', '', $_POST['txt_doc_value_' . $count]) . '_' . date("dmYhis") . '.' . $ext;
								//$file=rename($targetPath,$_SERVER['DOCUMENT_ROOT'].'ems/wt_docs/'.$filename);
								$file = rename($targetPath, ROOT_PATH . 'wt_docs/' . $filename);


								$Insert = "Call insert_TerminationData('" . $EmployeeID . "','" . $EmployeeName . "','" . $process . "','" . $subProcess . "','" . $AccountHead . "','" . $Clientname . "','" . $Designation . "','" . $ReportTo . "','" . $remarks . "','" . $TerminationDate . "','" . $statusAH . "','" . $DOJ . "','" . $SubType . "',@SRID_)";
								$myDB = new MysqliDb();
								...
								if (empty($mysql_error) && $LastID != '') {
									//////////////////////////////////////////////////////////////////////////////////
									$file_counter = 0;
									// $sqlInsertDoc = "insert into warning_rth_documents(EmployeeID,DataId,Document,Title,UploadedBy,`By`,UploadedDate)values('" . $EmployeeID . "','" . $LastID . "','" . $filename . "','" . $_POST['txt_doc_value_' . $count] . "','" . $_SESSION['__user_logid'] . "','AH',Now());";
									$emID = $EmployeeID;
									$docVal = cleanUserInput($_POST['txt_doc_value_' . $count]);
									$sqlInsertDoc = "insert into warning_rth_documents(EmployeeID,DataId,Document,Title,UploadedBy,`By`,UploadedDate)values(?,?,?,?,?,'AH',Now());";
									$stmt = $conn->prepare($sqlInsertDoc);
									$stmt->bind_param("sisss", $emID, $LastID, $filename, $docVal, $userID);
									$stmt->execute();
									...
									echo "<script>$(function(){ toastr.success('Request not Accepted.') }); </script>";
								}
							}
						}
					}
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				<form method="post">
					...
					<div id="pnlTable">
						<?php
						$btnsearch = isset($_POST['btnSearch']);
						if ($btnsearch) {
							// $p_Empid = cleanUserInput($_POST['p_EmpID']);

							$sqlConnect = " call `getEmpForTermination`('" . $p_Empid . "','" . $userID . "')";
							$myDB = new MysqliDb();
...
<script>
	...
	function EditData(el) {
		...
		$('select').formSelect();
		$.ajax({
			url: <?php echo '"' . URL . '"'; ?> + "View/BingPreviousHistoryForTermination.php?empid=" + EmployeeID_
		}).done(function(data) {
