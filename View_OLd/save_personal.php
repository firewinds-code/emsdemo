...
$clean_loc = cleanUserInput($_POST['location']);
if (isset($clean_loc) && $clean_loc != "") {
	$ofc_loc = $clean_loc;
...
if (strtoupper($_SERVER['REQUEST_METHOD']) == 'POST') {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		$clean_text_personal = cleanUserInput($_POST['txt_Personal_First']);
		$First = (isset($clean_text_personal) ? $clean_text_personal : null);
		$clean_txt_middle = cleanUserInput($_POST['txt_Personal_Middle']);
		$Middle = (isset($clean_txt_middle) ? $clean_txt_middle : null);
		$clean_last = cleanUserInput($_POST['txt_Personal_Last']);
		$Last = (isset($clean_last) ? $clean_last : null);
		$clean_txt_dob = cleanUserInput($_POST['txt_Personal_DOB']);
		$DOB = (isset($clean_txt_dob) ? $clean_txt_dob : null);
		$clean_txt_father = cleanUserInput($_POST['txt_Personal_Father']);
		$Father = (isset($clean_txt_father) ? $clean_txt_father : null);
		$clean_txt_mother = cleanUserInput($_POST['txt_Personal_Mother']);
		$Mother = (isset($clean_txt_mother) ? $clean_txt_mother : null);
		$clean_txt_gender = cleanUserInput($_POST['txt_Personal_Gender']);
		$Gender = (isset($clean_txt_gender) ? $clean_txt_gender : null);
		$clean_exp = cleanUserInput($_POST['total_experience']);
		$total_experience = (isset($clean_exp) ? $clean_exp : null);
		$clean_plang = cleanUserInput($_POST['primaryLanguage']);
		if (isset($clean_plang) && $clean_plang != "") {
			$primaryLanguage = $clean_plang;
		} else {
			$primaryLanguage = "";
		}
		if (isset($_POST['secondaryLanguage'])) {
			...
			$secondaryLanguage = cleanUserInput($secondaryLanguage);
		} else {
			$secondaryLanguage = "";
		}
		$clean_persblood = cleanUserInput($_POST['txt_Personal_Blood']);
		$Blood = (isset($clean_persblood) ? $clean_persblood : null);
		$clean_marr_status = cleanUserInput($_POST['txt_Personal_Marriage_Status']);
		$Marriage_Status = (isset($clean_marr_status) ? $clean_marr_status : null);
		$clean_marr_spouse = cleanUserInput($_POST['txt_Personal_Marriage_Spouse']);
		$Marriage_Spouse = (isset($clean_marr_spouse) ? $clean_marr_spouse : null);
		$clean_per_marr_date = cleanUserInput($_POST['txt_Personal_Marriage_Date']);
		$Marriage_Date = (isset($clean_per_marr_date) ? $clean_per_marr_date : null);
		$clean_per_mar_cstatus  = cleanUserInput($_POST['txt_Personal_Marriage_CStatus']);
		$Marriage_CStatus = (isset($clean_per_mar_cstatus) ? $clean_per_mar_cstatus : null);
		$clean_nomiee_name = (cleanUserInput($_POST['txt_Nominee_Name']));
		$Nominee_Name = (isset($clean_nomiee_name) ? $clean_nomiee_name : null);
		$clean_txt_nom = cleanUserInput($_POST['txt_Nominee_Relation']);
		$Nominee_Relation = (isset($clean_txt_nom) ? $clean_txt_nom : null);
		$clean_father_dob = cleanUserInput($_POST['txt_Father_DOB']);
		$Father_DOB = (isset($clean_father_dob) ? $clean_father_dob : null);
		$clean_mother_dob = cleanUserInput($_POST['txt_Mother_DOB']);
		$Mother_DOB = (isset($clean_mother_dob) ? $clean_mother_dob : null);
		$clean_dstatus = cleanUserInput($_POST['txt_dstatus']);
		$dstatus = (isset($clean_dstatus) ? $clean_dstatus : null);
		...
		$adharNum = cleanUserInput($_POST['txt_adhar_card_number']);
	}
}
//Check Employee is exist or not
$clean_empid = cleanUserInput($_REQUEST['empid']);
$clean_txt_first = cleanUserInput($_POST['txt_Personal_First']);
$clean_empidd = cleanUserInput($_POST['EmployeeID']);
if (isset($clean_empid) && $EmployeeID == '' && !isset($clean_txt_first)) {
	$myDB = new MysqliDb();
	...
	if (empty($mysql_error)) {
		if (isset($result[0]['FirstName']) && $result[0]['FirstName'] != "") {
			$clean_intid = cleanUserInput($_REQUEST['intid']);
			if (isset($clean_intid) && $clean_intid != "") {
				$intid = $clean_intid;
				// $myDB = new MysqliDb();
				// $update_aaharVerification = "Update aadhar_verifiaction set EmployeeID='" . $EmployeeID . "' where INTID='" . $intid . "'";
				$update_aaharVerification = "Update aadhar_verifiaction set EmployeeID=? where INTID=?";
				// $updateresult = $myDB->rawQuery($update_aaharVerification);
				$stmt = $conn->prepare($update_aaharVerification);
				$stmt->bind_param("ss", $EmployeeID, $intid);
				$stmt->execute();
...
} elseif (isset($clean_empidd) && $clean_empidd != '') {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		$EmployeeID = $clean_empidd;
	}
}
...
$adharQry = "select dov_value,doc_file from doc_details where doc_stype='Aadhar Card' and EmployeeID=?";
$stmtAdhr = $conn->prepare($adharQry);
$stmtAdhr->bind_param("s", $EmployeeID);
$stmtAdhr->execute();
$selectQuery = $stmtAdhr->get_result();
$rowAdhar = $selectQuery->fetch_row();
// print_r($rowAdhar);
// die;
if ($selectQuery->num_rows > 0) {
	$adharNum = clean($rowAdhar[0]);
	$doc_file = clean($rowAdhar[1]);
}
	...
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		...
		if ($EmployeeID) {
			...
			if (isset($_FILES["FileUpload1"]) && !empty($_FILES["FileUpload1"]["name"])) {
				//$target_dir = URL.$locationdir.'Images/';
				$target_dir = '../' . $locationdir . 'Images/';
				//$target_dir = '../Images/';
				...
				if ($uploadOk == 1) {
					$txt_child_docarr1 = $_FILES['FileUpload1']['tmp_name'];
					...
					if (is_uploaded_file($_FILES['FileUpload1']['tmp_name'])) {
						...
						if (move_uploaded_file($_FILES['FileUpload1']['tmp_name'], $target_file)) {
							...
							$filename = $EmployeeID . '_ProfilePic.' . $ext;
			...
			$modifyby = clean($_SESSION['__user_logid']);
			$rf_id = 0;
			...
			if ($primaryLanguage != ""  && $adharNum != "") {
				...
				if (empty($mysql_error)) {
					...
					if ($Marriage_CStatus == 'Yes') {
						$count = cleanUserInput($_POST['count_child']);
						$target_dir = ROOT_PATH . $locationdir . 'childDoc/';
						for ($i = 1; $i <= $count; $i++) {
							$child_name = cleanUserInput($_POST['txt_child_name_' . $i]);
							$child_dob = cleanUserInput($_POST['txt_child_dob_' . $i]);
							$child_blood = cleanUserInput($_POST['txt_child_blood_' . $i]);
							$child_gender = cleanUserInput($_POST['txt_child_gen_' . $i]);
							...
							if ($child_name != '' && $child_dob != '') {
								...
								$child_insert = 'call add_child("' . $EmployeeID . '","' . addslashes($child_name) . '","' . $child_dob . '","' . $child_gender . '","' . $child_blood . '","' . $clean_userid . '","' . addslashes($basef1) . '")';
					...
					if ($dstatus == 'Yes') {
						$count = cleanUserInput($_POST['count_depend']);
						for ($i = 1; $i <= $count; $i++) {
							$dependent_name = cleanUserInput($_POST['txt_dependent_name_' . $i]);
							$dependent_dob = cleanUserInput($_POST['txt_dependent_dob_' . $i]);
							$dependent_relation = cleanUserInput($_POST['txt_dependent_relation_' . $i]);

							if ($dependent_name != '' && $dependent_dob != '') {
								$clean_userid = clean($_SESSION["__user_logid"]);
								$child_insert = 'call add_dependent("' . $dependent_name . '","' . $dependent_dob . '","' . $dependent_relation . '","' . $EmployeeID . '","' . $clean_userid . '")';
								$cresult = $myDB->query($child_insert);
								$mysql_error = $myDB->getLastError();
								if ($cresult) {
									echo "<script>$(function(){ toastr.success('Child added " . $i . "'); }); </script>";
...
<script>
	$(document).ready(function() {
		...
		if (usrtype === 'ADMINISTRATOR' || usrtype === 'HR' || usrid == 'CE12102224' || usrid == 'CE121622565') {} else if (usrtype === 'AUDIT') {
		...
		$('#txt_Personal_DOB').datetimepicker({
			format: 'Y-m-d',
			timepicker: false,
			maxDate: minDates,
			yearEnd: get_Year,
		...
		$('#txt_Personal_Marriage_Date').datetimepicker({
			format: 'Y-m-d',
			timepicker: false,
			maxDate: '0',
			scrollMonth: false
		});
		$('#txt_Father_DOB').datetimepicker({
			format: 'Y-m-d',
			timepicker: false,
			maxDate: '0',
			scrollMonth: false
		});
		$('#txt_Mother_DOB').datetimepicker({
			format: 'Y-m-d',
			timepicker: false,
			maxDate: '0',
			scrollMonth: false
		});
		$('#txt_Personal_DOJ').datetimepicker({
			format: 'Y-m-d',
			timepicker: false,
			scrollMonth: false
		});
		$('#txt_child_dob_1').datetimepicker({
			format: 'Y-m-d',
			timepicker: false,
			maxDate: '0',
			scrollMonth: false
		});
		$('#txt_dependent_dob_1').datetimepicker({
			format: 'Y-m-d',
			timepicker: false,
			maxDate: '0',
			scrollMonth: false
	...
	<div class="pim-container row" id="div_main">
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				<div class="input-field col s6 m6">
					<input type="text" value="<?php echo ($Father_DOB); ?>" id="txt_Father_DOB" name="txt_Father_DOB" />
					<label for="txt_Father_DOB">Father DOB</label>
				</div>

				<div class="input-field col s6 m6">
					<input type="text" value="<?php echo ($Mother_DOB); ?>" id="txt_Mother_DOB" name="txt_Mother_DOB" />
				...
				<div class="col s12 m12 no-padding">
					...
					?><div class="col s2 m2">
								<input type='checkbox' name="secondaryLanguage[]" id="secondaryLanguage<?php echo $i; ?>" value='<?php echo $value['Language']; ?>' <?php if (in_array($value['Language'], $newarray)) {
				...
				<div class="container col s12 m12" id="childtables">
					...
					$stmtChild = $conn->prepare($sqlConnect);
					$stmtChild->bind_param("s", $EmployeeID);
					$stmtChild->execute();
					$result = $stmtChild->get_result();
						...
						<table id="myTable1" class="data dataTable no-footer" cellspacing="0" width="100%">
							...
							<tbody>
								...
								foreach ($result as $key => $value) {
									...
									echo '<td class="manage_item" ><i class="material-icons delete_item imgBtn imgBtnDelete tooltipped" onclick="javascript:return ChildDelete(this);" id="' . base64_encode($value['id']) . '" data-position="left" data-tooltip="Delete">ohrm_delete</i> </td>';
									echo '</tr>';
								}
				...
				<div class="container col s12 m12" id="dependtables">
					...
					$sqlConnect1 = "select * from dependent_details where EmployeeID=? ";
					$stmtDpnd = $conn->prepare($sqlConnect1);
					$stmtDpnd->bind_param("s", $EmployeeID);
					$stmtDpnd->execute();
					$result1 = $stmtDpnd->get_result();
					// $myDB = new MysqliDb();
					// $result = $myDB->query($sqlConnect);
					if ($result1) { ?>
						<table id="myTable1" class="data dataTable no-footer" cellspacing="0" width="100%">
				...
				<?php if (clean($_SESSION["__user_logid"]) == ADMINISTRATORID || clean($_SESSION["__user_logid"]) == 'CE061930039') { ?>
...
<script>
	$(document).ready(function() {
		...
		$('#btn_childAdd').click(function() {
			...
			$tr.children("td:nth-child(2)").children("input").attr({
				"id": "txt_child_name_" + parseInt($count + 1),
				"name": "txt_child_name_" + parseInt($count + 1)
			}).val('');

			$tr.children("td:nth-child(3)").children("input").attr({
				"id": "txt_child_dob_" + parseInt($count + 1),
				"name": "txt_child_dob_" + parseInt($count + 1)
			}).datetimepicker({
				format: 'Y-m-d',
				timepicker: false
			}).val('');
			...
			$trSelect_n4 = $tr.children("td:nth-child(4)").find("select").clone().attr({
				"id": "txt_child_blood_" + parseInt($count + 1),
				"name": "txt_child_blood_" + parseInt($count + 1)
			}).empty().append('><option value="NA">---Select---</option><option value="A+">A+</option><option value="A-">A-</option><option value="B+">B+</option><option value="B-">B-</option><option value="AB+">AB+</option><option value="AB-">AB-</option><option value="O+">O+</option><option value="O-">O-</option>');
			...
			$trSelect_n5 = $tr.children("td:nth-child(5)").find("select").clone().attr({
				"id": "txt_child_gen_" + parseInt($count + 1),
				"name": "txt_child_gen_" + parseInt($count + 1)
			}).empty().append('><option>Male</option><option>Female</option>');
			...
			$tr.children("td:nth-child(6)").children("input").attr({
				"id": "txt_child_doc_" + parseInt($count + 1),
				"name": "txt_child_doc_" + parseInt($count + 1)
			}).val('');
		...
		$('#btn_dependAdd').click(function() {
			...
			$tr.children("td:first-child").html(parseInt($count + 1));
			$tr.children("td:nth-child(2)").children("input").attr({
				"id": "txt_dependent_name_" + parseInt($count + 1),
				"name": "txt_dependent_name_" + parseInt($count + 1)
			}).val('');

			$tr.children("td:nth-child(3)").children("input").attr({
				"id": "txt_dependent_dob_" + parseInt($count + 1),
				"name": "txt_dependent_dob_" + parseInt($count + 1)
			}).datetimepicker({
				format: 'Y-m-d',
				timepicker: false,
				scrollInput: false,
			...
			$tr.children("td:nth-child(4)").children("input").attr({
				"id": "txt_dependent_relation_" + parseInt($count + 1),
				"name": "txt_dependent_relation_" + parseInt($count + 1)
			}).val('');
		...
		$('.imgChildDelete').click(function() {
			if (confirm('you want to delete child ')) {
				$item = $(this);
				$.ajax({
					url: "../Controller/deleteChild.php?ID=" + $item.attr("id"),
	...
	function DepentDelete(el) {
		if (confirm('you want to delete Dependent ')) {
			$item = $(el);
			$.ajax({
				url: "../Controller/deleteDependent.php?ID=" + $item.attr("id"),
				success: function(result) {
					$var = result.split('|');
					if ($var[0] == "Done") {
						$item.closest("tr").remove();
