...
if (isset($_POST['btn_Verifier_Save'])) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		$txt_EmployeeID = cleanUserInput($_POST['txt_EmployeeID']);
		$EmpId = (isset($txt_EmployeeID) ? $txt_EmployeeID : null);
		$txt_EmployeeName = cleanUserInput($_POST['txt_EmployeeName']);
		$EmpName = (isset($txt_EmployeeName) ? $txt_EmployeeName : null);
		$txt_status = cleanUserInput($_POST['txt_status']);
		$EmployeeStatus = (isset($txt_status) ? $txt_status : null);
...
if (isset($_POST['btn_Verifier_Edit'])) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		$DataID = cleanUserInput($_POST['txt_Verifier_id']);
		$txt_EmployeeID = cleanUserInput($_POST['txt_EmployeeID']);
		$EmpId = (isset($txt_EmployeeID) ? $txt_EmployeeID : null);
		$txt_EmployeeName = cleanUserInput($_POST['txt_EmployeeName']);
		$EmpName = (isset($txt_EmployeeName) ? $txt_EmployeeName : null);
		...
		$ModifiedBy = $user_logid;
		if ($DataID != null &&   $DataID != "" && $EmpId != "" && $EmpId != null && $EmpName != "" && $EmpName != null && $EmployeeStatus != "" && $EmpName != null) {

			$Update = "UPDATE `asset_verifier` SET `EmployeeName` = ?, `status` = ? , `updatedBy` = ? , `updatedDate` = '" . date('Y-m-d h:i:s') . "' WHERE (`id` = ? and `EmployeeID` = ?);";
			$myDB = new MysqliDb();
			$conn = $myDB->dbConnect();
			...
			if (!empty($DataID) && $DataID != '') {
				...
				$up->bind_param("sisis", $EmpName, $EmployeeStatus, $ModifiedBy, $DataID, $EmpId);
				$up->execute();
				$result_whole = $up->get_result();
				if ($up->affected_rows === 1) {
					echo "<script>$(function(){ toastr.success('Verifier Updated Successfully'); }); </script>";
				} else {
					echo "<script>$(function(){ toastr.error('Verifier not updated ::); }); </script>";
				}
			} else {
	...
	$(document).ready(function() {
		...
		$('#btn_Verifier_Edit,#btn_Verifier_Save').on('click', function() {
			...
			$("input,select,textarea").each(function() {
				...
				if (($(this).val().replace(/^\s+|\s+$/g() == '') || $(this).val() == 'NA' || $(this).val() == undefined) && (typeof attr_req !== typeof undefined && attr_req !== false) && !$(this).hasClass('.select-dropdown')) {
					validate = 1;
		...
		if (Cnfm) {
			...
			xmlhttp.onreadystatechange = function() {
				if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
					...
					window.location.replace("../View/asset_verifiers_add_edit_delete.php");
