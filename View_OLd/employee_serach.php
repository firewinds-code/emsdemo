...
if (isset($_POST['btn_ED_Search'])) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		$txt_ED_joindate_to = cleanUserInput($_POST['txt_ED_joindate_to']);
		$last_from = (isset($txt_ED_joindate_to) ? $txt_ED_joindate_to : null);
		$txt_ED_joindate_from = cleanUserInput($_POST['txt_ED_joindate_from']);
		$last_to = (isset($txt_ED_joindate_from) ? $txt_ED_joindate_from : null);
		$txt_ED_Dept = cleanUserInput($_POST['txt_ED_Dept']);
		$dept = (isset($txt_ED_Dept) ? $txt_ED_Dept : null);
		$ddl_ED_Emp_Name = cleanUserInput($_POST['ddl_ED_Emp_Name']);
		$emp_nam = (isset($ddl_ED_Emp_Name) ? $ddl_ED_Emp_Name : null);
		$ddl_ED_Emp_Adhar = cleanUserInput($_POST['ddl_ED_Emp_Adhar']);
		$emp_adhar = (isset($ddl_ED_Emp_Adhar) ? $ddl_ED_Emp_Adhar : null);
		$ddl_ED_Emp_Emer = cleanUserInput($_POST['ddl_ED_Emp_Emer']);
		$emp_emer = (isset($ddl_ED_Emp_Emer) ? $ddl_ED_Emp_Emer : null);
		$ddl_ED_Emp_alt = cleanUserInput($_POST['ddl_ED_Emp_alt']);
		$emp_alt = (isset($ddl_ED_Emp_alt) ? $ddl_ED_Emp_alt : null);
		$ddl_ED_Emp_ofcmail = cleanUserInput($_POST['ddl_ED_Emp_ofcmail']);
		$emp_ofcmail = (isset($ddl_ED_Emp_ofcmail) ? $ddl_ED_Emp_ofcmail : null);
		$ddl_ED_Emp_Mobile = cleanUserInput($_POST['ddl_ED_Emp_Mobile']);
		$emp_mobile = (isset($ddl_ED_Emp_Mobile) ? $ddl_ED_Emp_Mobile : null);
		$ddl_ED_Emp_Email = cleanUserInput($_POST['ddl_ED_Emp_Email']);
		$emp_mail = (isset($ddl_ED_Emp_Email) ? $ddl_ED_Emp_Email : null);
		$ddl_ED_Emp_EmpName = cleanUserInput($_POST['ddl_ED_Emp_EmpName']);
		$emp_empname = (isset($ddl_ED_Emp_EmpName) ? $ddl_ED_Emp_EmpName : null);
		$proc = cleanUserInput($_POST['txt_ED_Proc']);
		$searchBy = cleanUserInput($_POST['searchBy']);
		$txt_ED_Status = cleanUserInput($_POST['txt_ED_Status']);
		$status = (isset($txt_ED_Status) ? $txt_ED_Status : null);
		$searchBy = cleanUserInput($_POST['searchBy']);
		if ($searchBy == 'By ID') {
			$classvarr = "'.byID'";
		} else if ($searchBy == 'By Adhar') {
			$classvarr = "'.byAdhar'";
		} else if ($searchBy == 'By Emergency') {
			$classvarr = "'.byEmergency'";
		} else if ($searchBy == 'By OfcMail') {
			$classvarr = "'.byOfcMail'";
		} else if ($searchBy == 'By Alternate') {
			$classvarr = "'.byAlternate'";
		} else if ($searchBy == 'By Mobile') {
			$classvarr = "'.byMobile'";
		} else if ($searchBy == 'By Mail') {
			$classvarr = "'.byMail'";
		} else if ($searchBy == 'By Name') {
			$classvarr = "'.byName'";
		} else if ($searchBy == 'By Date') {

			$classvarr = "'.byDate,.byStatus'";
		} else if ($searchBy == 'By Dept') {
			$classvarr = "'.byDept,.byStatus'";
		} else if ($searchBy == 'By Process') {
			$classvarr = "'.byProc,.byStatus'";
		} else if ($searchBy == 'By Reporting') {
...
if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
	$doj1 = cleanUserInput($_POST['txt_ED_joindate_to']);
	$doj2 = cleanUserInput($_POST['txt_ED_joindate_from']);
	$name = cleanUserInput($_POST['ddl_ED_Emp_Name']);
	$adhar = cleanUserInput($_POST['ddl_ED_Emp_Adhar']);
	$emer = cleanUserInput($_POST['ddl_ED_Emp_Emer']);
	$alt = cleanUserInput($_POST['ddl_ED_Emp_alt']);
	$ofcmail = cleanUserInput($_POST['ddl_ED_Emp_ofcmail']);
	$mobile = cleanUserInput($_POST['ddl_ED_Emp_Mobile']);
	$mail = cleanUserInput($_POST['ddl_ED_Emp_Email']);
	$emp_empname = cleanUserInput($_POST['ddl_ED_Emp_EmpName']);
	$searchBy = cleanUserInput($_POST['searchBy']);
	$dept = cleanUserInput($_POST['txt_ED_Dept']);
	$proc = cleanUserInput($_POST['txt_ED_Proc']);
}
?>
<script>
			...
			<div class="schema-form-section row">
				...
				<div class="">
					<div class="input-field col s6 m6 8">
						<select name="searchBy" id="searchBy" class="input-field col s12 m12 l6" title="Select Search Option">
							<option <?php if ($searchBy == 'By ID') {
										echo 'selected';
									} ?> value="By ID">Employee ID</option>
							<option <?php if ($searchBy == 'By Name') {
										echo 'selected';
									} ?> value="By Name">Employee Name</option>
							<option <?php if ($searchBy == 'By Date') {
										echo 'selected';
									} ?> value="By Date">Joinning Date</option>
							<option <?php if ($searchBy == 'By Dept') {
										echo 'selected';
									} ?> value="By Dept">Department</option>
							<option <?php if ($searchBy == 'By Process') {
										echo 'selected';
									} ?> value="By Process">Process</option>
							<option <?php if ($searchBy == 'By Adhar') {
										echo 'selected';
									} ?> value="By Adhar">Adhar Card Number</option>
							<option <?php if ($searchBy == 'By Mobile') {
										echo 'selected';
									} ?> value="By Mobile">Mobile Number</option>
							<option <?php if ($searchBy == 'By Alternate') {
										echo 'selected';
									} ?> value="By Alternate">Alternate Number</option>
							<option <?php if ($searchBy == 'By Emergency') {
										echo 'selected';
									} ?> value="By Emergency">Emergency Contact Number</option>
							<option <?php if ($searchBy == 'By Mail') {
										echo 'selected';
									} ?> value="By Mail">Email ID</option>
							<option <?php if ($searchBy == 'By OfcMail') {
										echo 'selected';
									} ?> value="By OfcMail">Office Email ID</option>
							<!--<option <?php if ($searchBy == 'By Reporting') {
											echo 'selected';
										} ?> value="By Reporting">Reporting</option>-->
				...
				<div class=" byDept">
					...
					<div class="input-field col s6 m6 8">
						<select class="" id="txt_ED_Dept" name="txt_ED_Dept" title="Select Department  Name">
							...
							$sqlBy = 'call get_dept_by_UserType("' . $userLOgid . '","' . $userType . '")';
							$myDB = new MysqliDb();
				...
				<div class=" byProc">
					...
					<div class="input-field col s6 m6 8">
						<select id="txt_ED_Proc" name="txt_ED_Proc" title="Enter Process Name">
							...
							<?php
							$sqlBy = 'call get_proccess_by_UserType("' . clean($_SESSION['__user_logid']) . '","' . clean($_SESSION['__user_type']) . '","' . clean($_SESSION["__location"]) . '")';
							$myDB = new MysqliDb();
							$resultBy = $myDB->rawQuery($sqlBy);
							$mysql_error = $myDB->getLastError();
							if (empty($mysql_error)) {
				...
				<div id="pnlTable">
					...
					if (isset($_POST['btn_ED_Search'])) {
						...
						$status = cleanUserInput($_POST['txt_ED_Status']);
						if (clean($_SESSION['__user_type']) == 'AUDIT') {
							$_user_Type = 'ADMINISTRATOR';
						} else {
							$_user_Type = clean($_SESSION['__user_type']);
						}


						if ($searchBy == 'By ID') {
							$sqlConnect = 'call getemp_serach_byID("' . $name . '","' . $_user_Type . '","' . clean($_SESSION['__user_logid']) . '","' . clean($_SESSION["__location"]) . '")';
						} else if ($searchBy == 'By Adhar') {
							$sqlConnect = 'call getemp_serach_byAdhar("' . $adhar . '","' . $_user_Type . '","' . clean($_SESSION['__user_logid']) . '","' . clean($_SESSION["__location"]) . '")';
						} else if ($searchBy == 'By Emergency') {
							$sqlConnect = 'call getemp_serach_byEmergency("' . $emer . '","' . $_user_Type . '","' . clean($_SESSION['__user_logid']) . '","' . clean($_SESSION["__location"]) . '")';
						} else if ($searchBy == 'By Alternate') {
							$sqlConnect = 'call getemp_serach_byAlt("' . $alt . '","' . $_user_Type . '","' . clean($_SESSION['__user_logid']) . '","' . clean($_SESSION["__location"]) . '")';
						} else if ($searchBy == 'By Mobile') {
							$sqlConnect = 'call getemp_serach_byMobile("' . $mobile . '","' . $_user_Type . '","' . clean($_SESSION['__user_logid']) . '","' . clean($_SESSION["__location"]) . '")';
						} else if ($searchBy == 'By Mail') {
							$sqlConnect = 'call getemp_serach_byMail("' . $mail . '","' . $_user_Type . '","' . clean($_SESSION['__user_logid']) . '","' . clean($_SESSION["__location"]) . '")';
						} else if ($searchBy == 'By OfcMail') {
							$sqlConnect = 'call getemp_serach_byOfcmail("' . $ofcmail . '","' . $_user_Type . '","' . clean($_SESSION['__user_logid']) . '","' . clean($_SESSION["__location"]) . '")';
						} else if ($searchBy == 'By Name') {
							$sqlConnect = 'call getemp_serach_byName("' . $emp_empname . '","' . $_user_Type . '","' . clean($_SESSION['__user_logid']) . '","' . clean($_SESSION["__location"]) . '")';
						} else if ($searchBy == 'By Date') {
							$sqlConnect = 'call getemp_serach_byDate("' . $doj2 . '","' . $doj1 . '","' . $_user_Type . '","' . clean($_SESSION['__user_logid']) . '","' . $status . '","' . clean($_SESSION["__location"]) . '")';
						} else if ($searchBy == 'By Dept') {
							$sqlConnect = 'call getemp_serach_byDept("' . $dept . '","' . $_user_Type . '","' . clean($_SESSION['__user_logid']) . '","' . $status . '","' . clean($_SESSION["__location"]) . '")';
						} else if ($searchBy == 'Unmapped') {
							$sqlConnect = 'call get_Unmaped("' . clean($_SESSION["__location"]) . '","' . $_user_Type . '")';
						} else if ($searchBy == 'By Process') {
							$sqlConnect = 'call getemp_serach_byProc("' . $proc . '","' . $_user_Type . '","' . clean($_SESSION['__user_logid']) . '","' . $status . '","' . clean($_SESSION["__location"]) . '")';
						} else if ($searchBy == 'By Reporting') {
								...
								<div class="">
									<table id="myTable" class="data dataTable no-footer row-border" cellspacing="0" width="100%">
										<thead>
											<tr>
												<?php
												if (cleanUserInput($_POST['searchBy']) == 'Unmapped') {
												?>
													<th> Employee ID </th>
													<th> Name </th>
													<th> Father Name </th>
										...
										<tbody>
											...
											foreach ($result as $key => $value) {
												echo '<tr>';
												if (cleanUserInput($_POST['searchBy']) == 'Unmapped') {
													if (clean($_SESSION['__user_type']) == 'ADMINISTRATOR' || clean($_SESSION['__user_type']) == 'HR' || clean($_SESSION['__user_type']) == 'CENTRAL MIS' || clean($_SESSION['__user_type']) == 'AUDIT') {
