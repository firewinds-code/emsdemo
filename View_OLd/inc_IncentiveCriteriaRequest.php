...
if (isset($user_logid) &&  ($status_ah != 'No' && $status_ah == $user_logid)) {
	$userProcess = clean($_SESSION['__user_process']);
	$userClientId = clean($_SESSION['__user_client_ID']);

	// $delid = base64_decode($_GET['delid']);
	if (isset($_GET['delid'])) {
		$delid = cleanUserInput($_GET['delid']);
		// echo $delid = base64_decode($_GET['delid']);
		if ($delid != "") {
			// $myDB->rawQuery("delete from inc_incentive_criteria where id=$delid");
			$sql = "delete from inc_incentive_criteria where id=?";
	...
	if ($btnSave) {
		if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
			...
			if ($createdBy != "" && $Incentive_Type != "" && $StartDate != "" && $EndDate != "" && $Rate != "" && $BaseCriteria != "" && $criteria1 != "" && $criteria2 != "" && $cm_id != "" && $process != "" && $Request_Status != "" && $ApplicableFor != "") {
				...
				// $myDB = new MysqliDb();
				// $conn = $myDB->dbConnect();
				$selectquery = "select id,EndDate,incentiveStatus from inc_incentive_criteria where Incentive_Type=? and CreatedBy=? and incentiveStatus=1 and Request_Status!='Decline' and cm_id=? and ApplicableFor=? and ((month(EndDate)>= month(?)) and (year(EndDate)=year(?))  || (month(EndDate)< month(?) and year(EndDate)>year(?)))order by id desc ";
				$selQ = $conn->prepare($selectquery);
				$selQ->bind_param("ssisssss", $Incentive_Type, $createdBy, $cm_id, $ApplicableFor, $StartDate, $StartDate, $StartDate, $StartDate);
				$selQ->execute();
				$select_query = $selQ->get_result();
	...
	if (isset($_POST['btnEdit'])) {
		if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
			...
			$cm_id = cleanUserInput($_POST['cm_id']);
			$process = cleanUserInput($_POST['userProcess2']);
			$Request_Status = cleanUserInput($_POST['Request_Status']);
			$BaseCriteria = cleanUserInput($_POST['BaseCriteria2']);
			...
			if ($Id != ""  && $createdBy != "" && $Incentive_Type != "" && $StartDate != "" && $EndDate != "" && $Rate != "" && $BaseCriteria != "" && $criteria1 != "" && $criteria2 != "" && $cm_id != "" && $process != "" && $Request_Status != "") {
				//$empID=$val;
				$myDB = new MysqliDb();
				$select_query = $myDB->rawQuery("select id,EndDate,incentiveStatus from inc_incentive_criteria where Incentive_Type=? and CreatedBy=? and incentiveStatus=1  and cm_id=? and ApplicableFor=? and ((month(EndDate)>= month(?)) and (year(EndDate)=year(?))  || (month(EndDate)< month(?) and year(EndDate)>year(?))) and id!=$Id ");
				$selQ = $conn->prepare($selectquery);
				$selQ->bind_param("ssisssss", $Incentive_Type, $createdBy, $cm_id, $ApplicableFor, $StartDate, $StartDate, $StartDate, $StartDate);
				$selQ->execute();
				$select_query = $selQ->get_result();
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				<div class="input-field col s12 m12 ">
					...
					<div class="input-field col s4 m4 ">
						<?php $processQuery = "call inc_ProcessForAH('" . $user_logid . "')";
						$myDB = new MysqliDb();
				...
				<div id="pnlTable">
					<?php
					$sqlConnect = "call  inc_GetIncentiveCriteria('" . $user_logid . "','" . $sachinSirId . "')";
					$myDB = new MysqliDb();
								...
								<table id="myTable" class="data dataTable no-footer row-border" cellspacing="0" width="100%">
									...
									<tbody>
										...
										foreach ($result as $key => $value) {
											...
											if ($value['incentiveStatus'] == '1') echo 'Active';
											else echo 'Inactive';
											echo '</td>';
										?>
