...
if (count($chk_task) > 0 && $chk_task) {
	foreach ($chk_task as $key => $value) {
		...
		$mob = $value['mobile'];
		$sql = 'insert into onfloor_smsmail set employeeid=?, smsstatus=?,sms_text=? ,createdBy= "Server",mobile=? ';
		$insert = $conn->prepare($sql);
		$insert->bind_param("sssi", $emp, $smsstatus, $sms_text, $mob);
		$insert->execute();
		$sms_status = $insert->get_result();
	}
}
