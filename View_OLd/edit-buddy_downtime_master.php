...
if ($btn_df_Save) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		...
		// $myDB = new MysqliDb();
		$result_check = 'SELECT * FROM buddy_dtmatrix where cm_id = ?';
		$stmt = $conn->prepare($result_check);
		...
		if ($result1234->num_rows > 0) {
			...
			$Insert = 'INSERT INTO buddy_dtmatrix(cm_id,Min_Time,Max_Time,CreatedBy) VALUES(?,?,?,?);';

			$stmt1 = $conn->prepare($Insert);
			$stmt1->bind_param("ssss", $_cm_id, $txt_min_time, $txt_max_time, $createBy);
...
if ($btn_df_Edit) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		...
		if (empty($txtEditID) && !is_numeric($txtEditID)) {
			echo "<script>$(function(){ toastr.error('No data found to update'); }); </script>";
		} else {
			$createBy = clean($_SESSION['__user_logid']);
			$txt_min_time = cleanUserInput($_POST['txt_min_time']);
			$txt_max_time = cleanUserInput($_POST['txt_max_time']);
			$Update = 'UPDATE buddy_dtmatrix set Min_Time = ?,Max_Time =? where cm_id = ? and id = ?;';
			// $myDB = new MysqliDb();
			// $result = $myDB->query($Update);
			$stmt2 = $conn->prepare($Update);
			$stmt2->bind_param("ssii", $txt_min_time, $txt_max_time, $_cm_id, $txtEditID);
			$stmt2->execute();
