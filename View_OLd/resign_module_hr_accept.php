			...
			if ($count_check > 0) {
				...
				foreach ($_POST['cb'] as $val) {
					...
					if ($Status != 'NA' || $Status != 'Na') {
						...
						if ($Status == 'Accept') {
							$Status = 1;
							$sql = 'update resign_details set accept = ? ,accept_time = now(),rg_status=1,accepter_remark =?,accepter =? where EmployeeID = ? and rg_status = 0';
							$upQ = $conn->prepare($sql);
							$upQ->bind_param("isss", $Status, $Remark, $createBy, $EmpID);
							$upQ->execute();
							$resulti = $upQ->get_result();
						} else {
							$Status = 2;
							$sql = 'update resign_details set accept = ? ,accept_time = now(),rg_status=9,accepter_remark =?,accepter =? where EmployeeID = ? and rg_status = 0';
							$upQ = $conn->prepare($sql);
							$upQ->bind_param("isss", $Status, $Remark, $createBy, $EmpID);
							$upQ->execute();
							$resulti = $upQ->get_result();
						...
						if ($upQ->affected_rows === 1) {
							...
							if ($Status == 1) {
								$Qury = 'select nt_start,nt_end  from resign_details where EmployeeID = ? and rg_status = 1 and accept = 1 order by accept_time desc limit 1;';
								...
								$request_for = 'RESIGN Accept|' . $result2[0] . '|' . $result2[1];
								// $myDB = new MysqliDb();
								$alert_start = $result2[0];
								$alert_end = $result2[1];
								$insert = 'INSERT INTO alert_details(EmployeeID,alert_start,alert_end,type,createdon,createdby)VALUES(?,?,?,?,"' . date('Y-m-d H:i:s', time()) . '",?)';
								$insertQr = $conn->prepare($insert);
								$insertQr->bind_param("sssss", $EmpID, $alert_start, $alert_end, $request_for, $createBy);
								$insertQr->execute();
								$result3 = $insertQr->get_result();
							} else {
								$Qury = 'select nt_start,nt_end  from resign_details where EmployeeID = ? and rg_status = 9 and accept = 2 order by accept_time desc limit 1;';
								$selectQ = $conn->prepare($Qury);
								$selectQ->bind_param("s", $EmpID);
								$selectQ->execute();
								$result = $selectQ->get_result();
								...
								$request_for = 'RESIGN Reject|' . $result2[0] . '|' . $result2[0];
								// $myDB = new MysqliDb();
								$insert = 'INSERT INTO alert_details(EmployeeID,alert_start,alert_end,type,createdon,createdby)VALUES(?,"' . date('Y-m-d', time()) . '","' . date('Y-m-d', strtotime('+3 days')) . '",?,"' . date('Y-m-d H:i:s', time()) . '",?)';
								$insertQr = $conn->prepare($insert);
								$insertQr->bind_param("sss", $EmpID,  $request_for, $createBy);
								$insertQr->execute();
								$result3 = $insertQr->get_result();
							}
							...
							echo "<script>$(function(){ toastr.error('Request Not Updated ::Error :- <code></code>'); }); </script>";
						}
					}
				}
			} else {
			...
			<div class="schema-form-section row">
				...
				<div id="pnlTable">
					...
					if ($location == "1") {
						...
						$sqlConnect = 'select  wh.EmployeeID,wh.EmployeeName,rs.remark,rs.nt_start,rs.rg_status ,rs.nt_end,wh.Process,wh.sub_process,wh.clientname,pd1.img from resign_details rs inner join whole_details_peremp  wh on wh.EmployeeID = rs.EmployeeID left outer join personal_details pd1 on pd1.EmployeeID =rs.EmployeeID left outer join personal_details pd on pd.EmployeeID = ReportTo where (rs.accept = 0 or rs.accept is null) and rg_status = 0 and wh.location in (?)';
						$selectQ = $conn->prepare($sqlConnect);
