		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				<?php
					...
					$dt_info = '';
					// $myDB  = new MysqliDb();
					$datainfo  = "select count(EmpID) count from downtime where FAID =? and FAStatus = 'Pending' and CreatedOn >= ?";
					$selectQu = $conn->prepare($datainfo);
					$selectQu->bind_param("ss", $EmployeeID, $date_From);
					$selectQu->execute();
					$data_info = $selectQu->get_result();
					// print_r($data_info);
					// die;
					$total = 0;

					if ($data_info->num_rows > 0 && $data_info) {
					...
					// $myDB  = new MysqliDb();
					$datainfo  = "select count(EmpID) count from downtime where RTID =? and (RTStatus = 'Pending' and FAStatus = 'Approve') and CreatedOn >= ?";
					$selectQu = $conn->prepare($datainfo);
					$selectQu->bind_param("ss", $EmployeeID, $date_From);
					$selectQu->execute();
					$data_info = $selectQu->get_result();
					...
					$datainfo  = "SELECT Exception,count(*) count FROM exception inner join module_master_new t1 on t1.EmployeeID = exception.EmployeeID where MgrStatus='Pending' and t1.l1empid = ? and t1.module_name='Exception' and exception.CreatedOn >= ? group by Exception";
					$selectQu = $conn->prepare($datainfo);
					$selectQu->bind_param("ss", $EmployeeID, $date_From);
					$selectQu->execute();
					$data_info = $selectQu->get_result();
					...
					$datainfo  = "select LeaveType,sum(count) count from (select LeaveType,count(*) count from leavehistry t2 inner join module_master_new t3 on t2.EmployeeID=t3.EmployeeID where  (t2.HRStatusID='Approve' or t2.HRStatusID = 'Decline') and (EmployeeComment = 'Pending' and (MngrStatusID is null or MngrStatusID ='Pending')) and t3.l2empid=? and t3.module_name='Leave'  and cast(t2.DateCreated as date)>cast(? as date) and   ReasonofLeave !='Back Dated Leave'  group by LeaveType union all  select LeaveType,count(*) count from leavehistry t2  inner join module_master_new t3 on t2.EmployeeID=t3.EmployeeID where  ((t2.HRStatusID='Pending' or t2.HRStatusID is null)  and EmployeeComment = 'Pending' and (MngrStatusID is null or MngrStatusID ='Pending')) and cast(t2.DateCreated as date)>cast(? as date)  and ReasonofLeave !='Back Dated Leave' and t3.l1empid = ? and t3.module_name='Leave'  group by LeaveType)t1 group by LeaveType;";
					$selectQu = $conn->prepare($datainfo);
					$selectQu->bind_param("ssss", $EmployeeID, $date_From, $date_From, $EmployeeID);
					$selectQu->execute();
					$data_info = $selectQu->get_result();
