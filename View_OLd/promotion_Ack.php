...
$GetData = "select distinct t1.EmployeeId,t1.EmployeeName,t1.WEF,t1.Designation,t3.emailid,t4.location,t4.gender from tbl_promotion t1 left outer join tbl_increment t2 on t1.EmployeeId = t2.EmployeeId join contact_details t3 on t1.EmployeeId = t3.EmployeeID join personal_details t4 on t1.EmployeeID = t4.EmployeeID join employee_map t5 on t5.EmployeeID=t1.EmployeeID where t1.Flag=0 and emp_status='Active' and df_id !=77  and case when t2.EmployeeId is null then 1=1 else   t2.Flag!=0 end limit 210";
//Flag=0";
		...
		<p>Consequent to the performance review during the ";
			...
			if ($emailid != "") {
				...
				if (!$mail->send()) {
					...
					$upQ = $conn->prepare($updateFlag);
					$upQ->bind_param("s", $EmployeeID);
					$upQ->execute();
					$resu = $upQ->get_result();
					// $myDB = new MysqliDb();
