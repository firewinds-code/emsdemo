...
$GetData = "select t2.EmployeeId, t1.EmployeeId,t1.EmployeeName,t1.WEF,t1.PLIMode,t1.NewCTC,t1.PLIPercent,t3.emailid,t4.location,t4.gender from tbl_increment t1 left outer join tbl_promotion t2 on t1.EmployeeId = t2.EmployeeId join contact_details t3 on t1.EmployeeId = t3.EmployeeID join personal_details t4 on t1.EmployeeID = t4.EmployeeID where t1.Flag=0 and case when t2.EmployeeId is null then 1=1 else   t2.Flag!=0 end";
//Flag=0";
$myDB = new MysqliDb();
		...
		<p>Yours truly,</p>
			...
			if ($emailid != "") {
				...
				if (!$mail->send()) {
					...
					$up = $conn->prepare($updateFlag);
					$up->bind_param("s", $EmployeeID);
					$up->execute();
					$resu = $up->get_result();
					// if (empty($error)) {
