		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				if (empty($my_error)) {
					...
					$table .= '<th>Cosmo ID</th>';
					$table .= '<th>Employee ID</th><thead><tbody>';
						...
						if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
							...
							foreach ($activeSheetData as $row) {
								...
								if ($row_counter > 0 && !empty($row['A']) && $row['A'] != '') {
									$sql = 'select cm_id from employee_map where EmployeeID=?;';
									$selectQ = $conn->prepare($sql);
									$selectQ->bind_param("s", strtoupper($row['B']));
									$selectQ->execute();
									$ds_cmid = $selectQ->get_result();
									$dscmid = $ds_cmid->fetch_row();
									// $mysql_error = $myDB->getLastError();
									if (($ds_cmid) && $ds_cmid->num_rows > 0) {
										$cm_id = $dscmid[0];
										if ($cm_id == '27' || $cm_id == '58' || $cm_id == '83' || $cm_id == '45' || $cm_id == '46' || $cm_id == '126' || $cm_id == '90') {
											...
											if (empty($mysql_error)) {
												...
												$query = 'SELECT distinct cast(LoggedIn as date) as LoggedIn  FROM CCSPAPR_final where Agent_ID=? order by LoggedIn;';
												$sel = $conn->prepare($query);
												$sel->bind_param("s", strtoupper($row['A']));
												$sel->execute();
												$ds_rawapr = $sel->get_result();
												$ds_raw = $ds_rawapr->fetch_row();
												// $ds_rawapr = $myDB->rawQuery($query);
												// $mysql_error = $myDB->getLastError();

												$d2 = date('Y-m-d', strtotime("-20 days"));
												if (($ds_rawapr) && $ds_rawapr->num_rows > 0) {
													// for ($i = 0; $i < $ds_rawapr->num_rows; $i++) {
													while ($ds_rawaprr = mysqli_fetch_assoc($ds_rawapr)) {
														$d1 = date('Y-m-d', strtotime($ds_rawaprr['LoggedIn']));
														if (strtotime($d1) >= strtotime($d2)) {
															$url = URL . 'View/calc_apr_one.php?empid=' . strtoupper($row['B']) . '&type=one&date=' . date('Y-m-d', strtotime($ds_rawaprr['LoggedIn']));
															$curl = curl_init();
												...
												if (strtotime(date('Y-m-d', strtotime($ds_raw[0]))) >= strtotime($d2)) {
													$url = URL . 'View/calcRange_apr.php?empid=' . strtoupper($row['B']) . '&type=one&from=' . date('Y-m-d', strtotime($ds_raw[0]));
												} else {
