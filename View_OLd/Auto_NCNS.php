...
$m	= $date1->format('n');
$y  = $date1->format('Y');
$sqlstr = "select w.EmployeeID from ActiveEmpID w inner join (select EmployeeID  from calc_atnd_master where month=? and year =? and concat($d1,$d2,$d3)='AAA' )cal on w.EmployeeID = cal.EmployeeID  left join excp_emp_ncns_msg ex on w.EmployeeID = ex.EmpID where ex. EmpID is null";


$selectQ = $conn->prepare($sqlstr);
$selectQ->bind_param("ii", $m, $y);
$selectQ->execute();
...
if ($chk_task->num_rows > 0 && $chk_task) {
	foreach ($chk_task as $key => $value) {
		...
		if ($ncns_count == 3) {
			...
			if (!empty($rst_contact[1])) {
				$msg = "Dear " . $rst_contact[0] . ", You are not reporting to office since (Date : " . date('d/m/Y', strtotime("-3 days")) . ") without any information to us. To ensure that your employment status continues, You are advised to contact HR department in person by " . date('d/m/Y', strtotime("+1 days")) . " - Cogent E Services";
			...
			$qury = 'insert into login_ncns_smsmail set employeeid=?, smsstatus=?,sms_text=?,EmailAddress=?,emailStatus=? ,createdBy= "Server",type= "NCNS",mobile=? ';
			$insQr = $conn->prepare($qury);
			$insQr->bind_param("sssssi", $value['EmployeeID'], $smsstatus, $sms_text, $EmailAddress, $emailStatus, $Mobile);
			$insQr->execute();
			$sms_status = $insQr->get_result();
		}
	}
}
