...
<div class="" style="top: 100px;right: 35px;height: 47px;float:right;">
	...
	<p>
		...
		<?php
		$getDetails_sc = 'select cm_id from employee_map where EmployeeID=?';
		$selectQury = $conn->prepare($getDetails_sc);
		$selectQury->bind_param("s", $EmployeeID);
		$selectQury->execute();
		$res = $selectQury->get_result();
		$result_all_sc = $res->fetch_row();
		// $result_all_sc = $myDB->query($getDetails_sc);
		if ($res) {
			if ($result_all_sc[0] == '531') {
		...
		<?php
		$getDetails_sc = 'select location from personal_details where EmployeeID =?';
		$selectQury = $conn->prepare($getDetails_sc);
		$selectQury->bind_param("s", $EmployeeID);
		$selectQury->execute();
		$result_all_sc = $selectQury->get_result();
