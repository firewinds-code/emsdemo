...
$specialization = cleanUserInput($_POST['specialization']);

if (isset($_POST['specialization'], $_POST['addmodule']) && trim($_POST['specialization']) != "") {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		...
		if ($_POST['specialization'] != "") {
			...
			$select_query = "select id from education_specilization where specilization=? ";
			...
			if ($resultBy->num_rows > 0) {
				...
				$insertQuery = "INSERT INTO  education_specilization set specilization=?,createdby=?";
				$stmt = $conn->prepare($insertQuery);
				$stmt->bind_param("ss", $specialization, $ceatedBy);
				$stmt->execute();
				$resultBy = $stmt->get_result();
				// print_r($resultBy);
				// die;
...
if (isset($_POST['savemodule']) && $_POST['id']  != "") {
	$id = cleanUserInput($_POST['id']);
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		$specialization = trim(addslashes($specialization));
		if ($specialization != "") {
			$myDB = new MysqliDb();
			$conn = $myDB->dbConnect();

			// echo $updateQuery = "Update  education_specilization set specilization='" . $specialization . "',updatedon='" . $date . "',updatedby='" . $ceatedBy . "'  where id='" . $id . "'";
			// die;
			$updateQuery = "Update  education_specilization set specilization=?,updatedon=?,updatedby=? where id=?";
			$stmt = $conn->prepare($updateQuery);
			$stmt->bind_param("sssi", $specialization, $date, $ceatedBy, $id);
			$stmt->execute();
			$resultBy = $stmt->get_result();
						...
						<tbody>
							...
							?>
								...
								<td class="manage_item">
									<a onclick="editData('<?php echo $ID; ?>','<?php echo $val['specilization']; ?>');"><i class="material-icons edit_item imgBtn imgBtnEdit tooltipped" data-position="left" data-tooltip="Edit">ohrm_edit</i></a>
								</td>
