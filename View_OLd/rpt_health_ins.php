...
if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
	if (isset($_POST['txt_dateFrom']) && $_POST['txt_dateFrom'] != "" && isset($_POST['txt_dateTo']) && $_POST['txt_dateTo'] != "") {
		...
		$sqlConnect = 'select * from health_ins_mail_log where  (cast(created_at as date) between cast(? as date) and cast(? as date))';
		$selectQ = $conn->prepare($sqlConnect);
		$selectQ->bind_param("ss", $fromdate, $todate);
		$selectQ->execute();
		$result = $selectQ->get_result();
	} else {
		$sqlConnect = 'select * from health_ins_mail_log where (cast(created_at as date) between cast(? as date) and cast(? as date))';
		$selectQ = $conn->prepare($sqlConnect);
		$selectQ->bind_param("ss", date('Y-m-d'), date('Y-m-d'));
		$selectQ->execute();
		$result = $selectQ->get_result();
	}
}
			...
			<div class="schema-form-section row">
				...
				<div class="input-field col s12 m12" id="rpt_container">
					<div class="input-field col s3 m3">
						...
						<input type="text" name="txt_dateFrom" id="txt_dateFrom" required value="<?php $status = cleanUserInput($_POST['status']);
																									$txt_dateFrom = cleanUserInput($_POST['txt_dateFrom']);
																									echo ((isset($status)) ? $txt_dateFrom : '') ?>" autocomplete="off" placeholder="From Date" />
					</div>
					<div class="input-field col s3 m3">

						<input type="text" name="txt_dateTo" id="txt_dateTo" required value="<?php
																								$status = cleanUserInput($_POST['status']);
																								$txt_dateTo = cleanUserInput($_POST['txt_dateTo']);
																								echo ((isset($status)) ? $txt_dateTo : '') ?>" autocomplete="off" placeholder="To Date" />
					</div>
