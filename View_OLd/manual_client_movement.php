...
$hiddentype = isset($_POST['hiddentype']);
$hiddentypes = cleanUserInput($_POST['hiddentype']);
$type = ($hiddentype ? $hiddentypes : null);
...
if (isset($_POST['btn_Assign_Batch'])) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		...
		if (isset($_POST['cb'])) {
			...
			$txt_batch = cleanUserInput($_POST['txt_batch']);
			$cm_id = (isset($txt_batch) ? $txt_batch : null);
			$training_head = '';
			$operation_head = '';
			$createBy = clean($_SESSION['__user_logid']);
			$date = date("Y-m-d H:i:s");

			$selectth = "Select th,oh from new_client_master where cm_id=?";
			$stmt1 = $conn->prepare($selectth);
			$stmt1->bind_param("i", $cm_id);
			$stmt1->execute();
			$result = $stmt1->get_result();
			$select_th = $result->fetch_row();
			// $mysql_error = $myDB->getLastError();
			// $rowCount = $myDB->count;
			if ($result->num_rows > 0) {
				$training_head = $select_th[0];
				$operation_head = $select_th[1];
			}
			...
			if ($count_check > 0) {
				...
				foreach ($cb as $val) {
					...
					$mysquery = "select ctc,cm_id,designation,id from salary_details s inner join whole_details_peremp w on w.EmployeeID=s.EmployeeID where s.EmployeeID=? and (CAST(SUBSTRING_INDEX(ctc, '-', -1) AS UNSIGNED)  <= (select CAST(SUBSTRING_INDEX(max_lim, '-', -1) AS UNSIGNED) as maxslab from tbl_salary_slab_by_cps  where cm_id=?) or w.des_id not  in (9,12) )";
					$sel = $conn->prepare($mysquery);
					$sel->bind_param("si", $empID, $cm_id);
					$sel->execute();
					$results = $sel->get_result();
					$resultBy = $results->fetch_row();
					// $myDB = new MysqliDb();
					...
					if (($results) && $results->num_rows > 0) {
						...
						if ($versantflag == 1) {
							...
							if ($resultBy[2] == 'CSA') {
								$query = "SELECT * FROM bgv_matrix where cm_id = ? and desig = (select case when df_id=74 then 'CSA' else 'Support' end as desig from employee_map where EmployeeID=?) and (Addr='Yes' or Edu='Yes' or Emp='Yes' or Crim='Yes');";
								$sel = $conn->prepare($query);
								$sel->bind_param("is", $cm_id, $empID);
								$sel->execute();
								$res = $sel->get_result();
								if ($res->num_rows > 0) {
									$query = "select doc_file from doc_details where employeeid=? and doc_stype='BG verification';";
									$sel = $conn->prepare($query);
									$sel->bind_param("s", $empID);
									$sel->execute();
									$resu = $sel->get_result();
							...
							if ($bgvflag == 1) {
								...
								$old_cmid = $resultBy[1];
								$insSQl = "insert into tbl_client_toclient_move (EmployeeID, move_date, new_cm_id, old_cm_id, flag, status, updated_on, transfer_by, hr_comment, ah_comment, HR_updated_by, AH_updated_on, HR_updated_on, AH_updated_by, Updated_by) values(?,now(),?,?,'FM','transfered',now(),?,'Manual Client Movement','Manual Client Movement',?,now(),now(),?,'Manual Client Movement'); ";
								$selectQu = $conn->prepare($insSQl);
								$selectQu->bind_param("siisss", $empID, $cm_id, $old_cmid, $createBy, $createBy, $createBy);
								$selectQu->execute();
								$data_status_query = $selectQu->get_result();
								// $mysql_error = $myDB->getLastError();
								// $rowCount = $myDB->count;
								// if ($rowCount > 0) {
								if ($selectQu->affected_rows === 1) {
									...
									$rowCount = $myDB->count;
									if ($rowCount > 0) {
										if (($resultBy[3] == "7" || $resultBy[3] == "8" || $resultBy[3] == "10") && ($resultBy[2] == "Executive" || $resultBy[2] == "Team Leader" || $resultBy[2] == "Senior Executive" || $resultBy[2] == "Assistant Team Leader" || $resultBy[2] == "CSA" || $resultBy[2] == "Group Team Leader" || $resultBy[2] == "Senior CSA")) {

											// $myDB = new MysqliDb();
											$up = "Update status_table set Status='2',ReportTo=?,Qa_ops='',BatchID='0',createdon=?,InTraining=NULL,InOJT=NULL,OnFloor=NULL,OutTraining=NULL,InQAOJT=NULL,OutOJTQA=NULL,RetrainTime=NULL,roster=NULL,reOJT=NULL,mapped_date=?  where  EmployeeID=?";
											$upQu = $conn->prepare($up);
											$upQu->bind_param("ssss", $training_head, $date, $date, $empID);
											$upQu->execute();
											$update_status_table = $upQu->get_result();
											// $mysql_error = $myDB->getLastError();
											// $rowCount = $myDB->count;
										} else {
											// echo "Update status_table set  ReportTo='" . $operation_head . "',createdon='" . $date . "'  where  EmployeeID='" . $empID . "' ";
											// $myDB = new MysqliDb();
											$update = "Update status_table set  ReportTo=?,createdon=?  where  EmployeeID=? ";
											$upQu = $conn->prepare($update);
											$upQu->bind_param("sss", $operation_head, $date, $empID);
											$upQu->execute();
											$update_status_table = $upQu->get_result();
										}
										...
										// $myDB = new MysqliDb();
										$update_statustable = "insert into employee_map_updates ( select * from employee_map  where EmployeeID=?)";
										$ins = $conn->prepare($update_statustable);
										$ins->bind_param("s", $empID);
										$ins->execute();
										$update_status_table = $ins->get_result();
										// $myDB = new MysqliDb();
										$update = "UPDATE  employee_map set cm_id=?,modifiedon=?,modifiedby='Server' where EmployeeID = ?";
										$upQu = $conn->prepare($update);
										$upQu->bind_param("iss", $cm_id, $date, $empID);
										$upQu->execute();
										$update_status_table = $upQu->get_result();
										// $myDB = new MysqliDb();
...
if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
	$txt_subProcess = cleanUserInput($_POST['txt_subProcess']);
	$txt_type = cleanUserInput($_POST['txt_type']);
}
?>
<script>
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
					...
					<div class="batchdetails">
						<div class="input-field col s12 m12">
							<select id="txt_batch" name="txt_batch">
								...
								$loc = (isset($txt_location) ? $txt_location : null);
								$txt_subProcess = cleanUserInput($_POST['txt_subProcess']);
								$cmid = (isset($txt_subProcess) ? $txt_subProcess : null);
								$sqlBy = '';
								if ($loc != "") {
									$myDB = new MysqliDb();
									$conn = $myDB->dbConnect();
									$sqlBy = "select distinct concat(clientname,'|',Process,'|',sub_process) as process,cm_id from whole_details_peremp where location=? and cm_id not in (select cm_id from client_status_master) and cm_id!=? and id in (7,8,10) order by clientname;";
									$stmt = $conn->prepare($sqlBy);
									$stmt->bind_param("ii", $loc, $cmid);
									$stmt->execute();
				...
				if (isset($_POST['btn_Department_Save'])) {
					...
					$emptype = (isset($txt_type) ? $txt_type : null);
					$myDB = new MysqliDb();
					$conn = $myDB->dbConnect();
					$sqlBy = 'select EmployeeID,EmployeeName,designation,DATE_FORMAT(doj, "%d-%M-%Y") as DOJ,clientname,Process,sub_process,location_master.location from whole_details_peremp join location_master on whole_details_peremp.location=location_master.id where cm_id=? and status=6 and whole_details_peremp.id in (7,8,10) order by EmployeeName;';
					$stmt = $conn->prepare($sqlBy);
	...
	$(document).ready(function() {
		...
		$('#btn_Department_Save').on('click', function() {
			...
			if ($('#txt_subProcess').val() == 'NA' && $('#txt_type').val() == 'Offline') {
				...
				if ($('#spantxt_subProcess').length == 0) {
