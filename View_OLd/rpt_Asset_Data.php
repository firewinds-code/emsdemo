...
if (isset($_SESSION)) {
	...
	if ($user_type != 'ADMINISTRATOR' &&  $user_logid != 'CE10091236') {
		$location = URL . 'Login';
		header("Location: $location");
	}
} else {
...
if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
	if (isset($_POST['report_for'])) {
		$ReportType = cleanUserInput($_POST['report_for']);
	}
}

if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
	$ReportDate = cleanUserInput($_POST['txt_report_date']);
}
?>
			...
			<div class="schema-form-section row">
				...
				if (isset($_POST['btn_view'])) {
					...
					if ($ReportType == 'Verified') {
						...
						$qq = "SELECT  ac.EmployeeID, ac.empImage, ac.assetImgComplete, ac.home_img, ac.emp_signature_img, ac.asset_lat, ac.asset_lng, ac.verified_By, ac.verifiedDate,  ai.assetName, ai.assetImg, ai.isCollected, ai.isDamaged, ai.assetRemark, ai.asset_flag, ai.assignedDate, t4.EmployeeName,t3.client_name, process,sub_process,mobile,altmobile,address, address_p FROM ems.asset_coordinates ac left join asset_info ai on ai.EmployeeID = ac.EmployeeID left join employee_map t1 on  ai.EmployeeID = t1.EmployeeID left join new_client_master t2 on t1.cm_id = t2.cm_id left join client_master t3 on t2.client_name=t3.client_id left join personal_details t4 on t1.EmployeeID = t4.EmployeeID left join contact_details t5 on t1.EmployeeID=t5.EmployeeID left join address_details t6 on t1.EmployeeID=t6.EmployeeID where cast(ac.createdDate as date) = ? and cast(ac.verifiedDate as date) =cast(ai.verifyDate as date) ;";
						$SelecT = $conn->prepare($qq);
						$SelecT->bind_param("s", $ReportDate);
						$SelecT->execute();
						...
						$sql = "select distinct t1.EmployeeID,( ACOS( COS( RADIANS( t2.lat  ) ) * COS( RADIANS(t1.asset_lat ) ) * COS( RADIANS( t1.asset_lng ) - RADIANS( t2.lng ) ) + SIN( RADIANS( t2.lat  ) )* SIN( RADIANS( t1.asset_lat ) )) * 6371) AS distance_in_km  from ((select * from asset_coordinates )t1  left join (select * from login_lat_lng  where  cast(login_lat_lng.created_on as date) = ?)t2 on t1.EmployeeID = t2.EmployeeID ) HAVING distance_in_km > 1";
						$SelecT = $conn->prepare($sql);
						$SelecT->bind_param("s", $day_before);
						$SelecT->execute();
						$empListQResult = $SelecT->get_result();

						if ($empListQResult && $empListQResult->num_rows > 0) {
						...
						$sql = "select t1.EmployeeID  from ((select * from roster_temp as r  where  r.work_from = 'WFH' and r.DateOn = '" . $day_before . "')t1  inner join (select * from calc_atnd_master as c where  c.Month =? and c.Year = ?  and  c.d" . $day . " = 'A' )t2 on t1.EmployeeID = t2.EmployeeID inner join (select * from asset_info where asset_info.asset_flag = 0 )t3 on t1.EmployeeID = t3.EmployeeID )";
								...
								<tbody>
									...
									foreach ($chk_task as $key => $value) {
										...
										if ($ReportType == 'Verified') {
											...
											echo '<td><a href="https://demo.cogentlab.com/erpm/assetSingle/' . $value['empImage'] . '" target="_blank">View Image</a></td>';
											echo '<td><a href="https://demo.cogentlab.com/erpm/assetSingle/' . $value['home_img'] . '" target="_blank">View Image</a></td>';
											echo '<td><a href="https://demo.cogentlab.com/erpm/assetSingle/' . $value['assetImgComplete'] . '" target="_blank">View Image</a></td>';
											echo '<td><a href="https://demo.cogentlab.com/erpm/assetSingle/' . $value['emp_signature_img'] . '" target="_blank">View Image</a></td>';
											echo '<td>' . $value['assetName'] . '</td>';
											echo '<td><a href="https://demo.cogentlab.com/erpm/assetSingle/' . $value['assetImg'] . '" target="_blank">View Image</a></td>';
