...
if (isset($_POST['btn_ED_Search'])) {
  ...
  $emp_nam = (isset($clean_emp_name) ? $clean_emp_name : null);
  $searchBy = $clean_emp_name;
}
...
if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
  ...
  $month = cleanUserInput($_POST['month']);
  $year = cleanUserInput($_POST['Years']);
}
?>
<style>
      ...
      <div class="schema-form-section row">
          ...
          <div class="input-field col s4 m4 4">
            <div class="form-group">
              ...
              for ($i_month = 1; $i_month <= 12; $i_month++) {
                ...
                $selected = ($clean_month  == $i_month ? ' selected' : '');
          ...
          <div class="input-field col s4 m4 4">
            <div class="form-group">
              ...
              for ($i_year = $year_start; $i_year <= $year_end; $i_year++) {
                $clean_year = cleanUserInput($_POST['Years']);
                $selected = ($clean_year == $i_year ? ' selected' : '');
        ...
        <div id="pnlTable">
          ...
          if (isset($_POST['btn_ED_Search'])) {
            ...
            $location = 'select location from personal_details where EmployeeID=?';
            $selectQury = $connn->prepare($location);
            ...
            if ($res->num_rows > 0) {
              ...
              $sql = "select location from salary_master where EmpID=? and location like '%" . $loc . "%'";
              $Sel = $connn->prepare($sql);
              $Sel->bind_param("s", $user);
              $Sel->execute();
              $results = $Sel->get_result();
              if ($results->num_rows > 0) {
                $CSA = "CSA";
                $sqlConnect = 'select * from salary_certificate where Empid=? and month=? and year=? and designation=?';
                //$sqlConnect = 'select * from salary_certificate where Empid="' . $EmpID . '" and month="' . $month . '" and year="' . $year . '" ';
                $SelQury = $connn->prepare($sqlConnect);
                $SelQury->bind_param("siis", $EmpID, $month, $year, $CSA);
                $SelQury->execute();
