...
if (isset($_POST['btnSave'])) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		$status = cleanUserInput($_POST['txt_thcheck_Quality']);
			...
			if ($count_check > 0) {
				...
				if ($_POST['batch_id'] > 0 || !empty($_POST['batch_id'])) {
					...
					foreach ($cb as $val) {
						...
						$save = 'call manage_status_qh_after("' . $empID . '","' . $status . '","' . $txt_Remark_ . '","' . $createBy . '")';
		...
		if (isset($_POST['cb'])) {
			...
			if ($count_check > 0) {
				...
				foreach ($cb as $val) {
					...
					$myDB = new MysqliDb();
					$txt_Remark_ = cleanUserInput($_POST['txt_Remark_' . $empID]);
					$save = 'call manage_refer_hr("' . $empID . '","' . $createBy . '","' . $txt_Remark_ . '","QH AFTER")';
					$resulti = $myDB->query($save);
					$mysql_error = $myDB->getLastError();
					if (empty($mysql_error)) {
...
if (isset($_POST['btn_retrain'])) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		$status = cleanUserInput($_POST['txt_thcheck_Quality1']);
		$createBy = clean($_SESSION['__user_logid']);
		if ($status != "" && $status != "NA") {
			if (isset($_POST['cb'])) {
				$checked_arr = $_POST['cb'];
				...
				if ($count_check > 0) {
					...
					foreach ($cb as $val) {
						...
						$myDB = new MysqliDb();
						$conn = $myDB->dbConnect();
						$checkrtr_q = 'select EmployeeID from status_table where EmployeeID =? and reOJT is not null limit 1';
						...
						if ($check_rtr_q->num_rows > 0 && $check_rtr_q) {
							...
							$myDB = new MysqliDb();
							$batch_id = cleanUserInput($_POST['batch_id']);
							$txt_Remark_ = cleanUserInput($_POST['txt_Remark_' . $empID]);
							$txt_Date_crt_1 = cleanUserInput(cleanUserInput($_POST['txt_Date_crt_1']));
							$txt_Day_crt_1 = cleanUserInput(cleanUserInput($_POST['txt_Day_crt_1']));

							$save = 'call manage_status_qh("' . $empID . '","' . $status . '","' . $createBy . '",' . $batch_id . ',"' . $txt_Remark_ . '","QH","' . $txt_Date_crt_1 . '","' . $txt_Day_crt_1 . '","' . $roster_log . '")';
							$resulti = $myDB->query($save);
							$mysql_error = $myDB->getLastError();
							if (empty($mysql_error)) {
								$myDB = new MysqliDb();
								$txt_Remark_ = cleanUserInput($_POST['txt_Remark_' . $empID]);
								$savea = 'call manage_qh_over("' . $empID . '","' . $txt_Remark_ . '","' . $createBy . '","QH AFTER RE-OJT")';
				...
				<div class="form-inline">
					<div class="col s12 m12 no-padding">
						<div class="input-field col s6 s6">
							...
							<select id="text_trcheck_Batch" name="text_trcheck_Batch">
								...
								$selectQ->bind_param("ss", $empid, $empid);
								$selectQ->execute();
					...
					<div class="input-field col s6 m6 statuscheck">
						...
						<select id="txt_thcheck_Quality" name="txt_thcheck_Quality">
							...
							$sqlBy = 'select personal_details.EmployeeID,personal_details.EmployeeName,Designation from df_master inner join employee_map on employee_map.df_id=df_master.df_id inner join designation_master on designation_master.ID=df_master.des_id inner join personal_details on personal_details.EmployeeID=employee_map.EmployeeID inner join status_table on status_table.EmployeeID=employee_map.EmployeeID where (Designation not in ("Senior CSA","CSA") and function_id in (8,10) and employee_map.emp_status="Active" and personal_details.EmployeeID is not null) or (status_table.ReportTo =? and Designation not in ("Senior CSA","CSA")) order by personal_details.EmployeeName';
							$selectQ = $conn->prepare($sqlBy);
					...
					<div id="pnlTable" class="col s12 m12 card">
						...
						if ($batch_id != 'NA') {
							...
							$Empid = clean($_SESSION['__user_logid']);
							$sqlConnect = 'call get_qhchecklist_after("' . $Empid . '","' . $batch_id . '")';
							$myDB = new MysqliDb();
