...
if (strtoupper($_SERVER['REQUEST_METHOD']) == 'POST') {
	...
	$pli_percent = filter_var($_POST['txt_pli_percent'], FILTER_SANITIZE_NUMBER_INT);
	if ($pli_percent == '')
		$pli_percent = 0;
...
if (isset($_POST['btn_sal_Save']) && $EmployeeID != '') {
	...
	if ($emptype == '' || $paytype  == '' || $newctc  == '' || $MinWages  == '' || $basic   == '' || $hra   == '' || $convence   == '' || $sp_allow   == '' || $gorss_sal  == '' || $pf_count  == '' || $pf_empl_count  == '' || $esis_count  == '' || $esis_empl_count   == '' || $professional_tex  == '' || $takehome  == '' || $pli_percent  == '' || $pli_ammount  == '' || $nt_takehome  == '' || $pli_status  == '' || $pf_status  == '' || $pli_mode  == '' || $rt_type == '' || $sal_type == '') {
		$validate = 1;
	}
					...
					<div class="input-field col s4 m4">
						<select id="txt_pli_deduct" name="txt_pli_deduct">
							...
							if ($emptype == "AT") { ?>
								...
								<?php } else if (clean($_SESSION["__user_type"]) == 'HR' && ($takehome == '')) {
						...
						$('#btn_sal_Save').on('click', function() {
							...
							if ($('#txt_sal_type').val() == 'NA') {
								...
								if ($('#stxt_sal_type').size() == 0) {
									$('<span id="stxt_sal_type" class="help-block">Sal Type can not be Empty.</span>').insertAfter('#txt_sal_type');
								}
								validate = 1;
							}
							/*if($('#txt_sal_pf').val()=='')
