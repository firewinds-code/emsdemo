...
$btn_Department_Save = isset($_POST['btn_Department_Save']);
if ($btn_Department_Save) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		$txt_Department_Name = cleanUserInput($_POST['txt_Department_Name']);
		$_Name = (isset($txt_Department_Name) ? $txt_Department_Name : null);
		$createBy = clean($_SESSION['__user_logid']);

		// if (!preg_match("/^[a-zA-Z0-9]+$/", $_Name)) {
		if (!preg_match('/^[a-z0-9 _]+$/', $_Name)) {
			echo "<script>$(function(){ toastr.error('Only alphabets and whitespace are allowed'); }); </script>";
		} else {
			...
			if (empty($mysql_error)) {
				...
				echo "<script>$(function(){ toastr.error('Department not Added'); }); </script>";
			}
		}
	}
}
// Trigger Button-Edit Click Event and Perform DB Action
$btn_Department_Edit = isset($_POST['btn_Department_Edit']);
if ($btn_Department_Edit) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		$DataID = cleanUserInput($_POST['hid_Department_ID']);
		$txt_Department_Name = cleanUserInput($_POST['txt_Department_Name']);
		$_Name = (isset($txt_Department_Name) ? $txt_Department_Name : null);
...
<script>
	...
	// $(document).ready(function() {
	// 	$("#txt_Department_Name").keydown(function(event) {
	// 		var inputValue = event.which;
	// 		// allow letters and whitespaces only.
	// 		if (!(inputValue >= 65 && inputValue <= 120) && (inputValue != 32 && inputValue != 0)) {
	// 			event.preventDefault();
	// 			alert("Please type Alphabates only");
	// 		}
	// 	});
	// });
