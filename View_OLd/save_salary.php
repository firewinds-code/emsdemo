  <?php
	...
	$alert_msg = $EmployeeID = $btnShow = $emptype = $paytype = $sal_type = $newctc = $pfaccount = $esiNo = $file_esi = $MinWages = $basic = $hra = $convence = $sp_allow = $gorss_sal = $pf_count = $pf_empl_count = $esis_count = $esis_empl_count = $professional_tex =  $takehome = $pli = $pli_percent = $pli_ammount = $nt_takehome = $pli_status = $pf_status = $pli_mode = $uino = $pli_effected = $pli = $bonus = $gratuity = $medical_allowance = '';
	$professional_tex = 0;
	...
	if (strtoupper($_SERVER['REQUEST_METHOD']) == 'POST') {
		...
		$emp_type = cleanUserInput($_POST['emp_type']);
		/*if(substr($EmployeeID,0,2) != 'TE')
	{
		if(substr($EmployeeID,0,3) == 'CCE')
		{
			$emp_type = "CCE";
		}
	}*/
	...
	if (isset($_REQUEST['empid']) && $EmployeeID == '' && !isset($_POST['EmployeeID'])) {
		...
		if ($result_all) {
			...
			if ($result_all[0]['location'] == "5" || $result_all[0]['location'] == "2" || $result_all[0]['location'] == "8") {
	...
	if (isset($_POST['btn_sal_Save']) && $EmployeeID != '') {
		...
		// if ($emptype == '' || $paytype  == '' || $sal_type == '' || $newctc  == '' || $MinWages  == '' || $basic   == '' || $hra   == '' || $convence   == '' || $sp_allow   == '' || $gorss_sal  == '' || $pf_count  == '' || $pf_empl_count  == '' || $esis_count  == '' || $esis_empl_count   == '' || $takehome  == '' || $pli_ammount  == '' || $nt_takehome  == '' || $pli_status  == '' || $pf_status  == '' || $pli_mode  == '' || $rt_type == '' || $professional_tex == '') {
		if ($emptype == '' || $paytype  == '' || $sal_type == '' || $newctc  == '' || $MinWages  == '' || $basic   == '' || $hra   == '' || $convence   == '' || $sp_allow   == '' || $gorss_sal  == '' || $takehome  == '' || $nt_takehome  == '' || $pli_status  == '' || $pf_status  == '' || $rt_type == '' || $professional_tex == '') {
			$validate = 1;
		}
		...
		if (substr($EmployeeID, 0, 2) != 'TE') {
			...
			if ($validate == 0) {
				...
				$sqlInsertDoc = 'call manage_salary("' . $EmployeeID . '","' . $emptype . '","' . $paytype . '","' . $sal_type . '","' . $ppf . '","' . $newctc . '","' . $takehome . '","' . $hra . '","' . $convence . '","' . $bonus . '","' . $sp_allow . '","' . $gorss_sal . '","' . $pf_count . '","' . $esis_count . '","' . $createBy . '","' . $pfaccount . '","' . $esiNo . '","' . $basic . '","' . $pf_empl_count . '","' . $esis_empl_count . '","' . $file_esi . '","' . $professional_tex . '","' . $MinWages . '","' . $pf_status . '","' . $pli_status . '","' . $pli_mode . '","' . $pli_percent . '","' . $pli_ammount . '","' . $nt_takehome . '","' . $uino . '","' . $rt_type . '","' . $pli_effected . '","' . $gratuity . '","' . $medical_allowance . '")';

				$result = $myDB->query($sqlInsertDoc);
				$mysql_error = $myDB->getLastError();
				if (empty($mysql_error)) {
					echo "<script>$(function(){ toastr.success('Data Saved Successfully  Provident Fund = <b>" . $pf_count . " <i class=\'fa fa-inr\'></i></b> ESIS = <b>" . $esis_count . "  <i class=\'fa fa-inr\'></i></b>  Take HOME = <b>" . $takehome . "  <i class=\'fa fa-inr\'></i></b>') });</script>";
				} else {
	...
	// $myDB = new  MysqliDb();
	$sqlBy = "select des_id,dateofjoin from employee_map inner join df_master on  employee_map.df_id = df_master.df_id inner join designation_master on designation_master.ID = df_master.des_id where EmployeeID = ?";
	// $myDB = new MysqliDb();
  	...
  	<div class="pim-container row" id="div_main">
  		...
  		<div class="form-div">
  			...
  			<div class="schema-form-section row">
  				...
  				<input type="hidden" name="emp_type" id="emp_type" value="<?php echo $emp_type; ?>" />
  				...
  				<div class="col s12 m12">
  					...
  					<div class="input-field col s4 m4 hidden pf_deduct col s4 m4">
  						...
  						<select id="txt_pf_deduct" name="txt_pf_deduct">
  							<?php //if ($sal_type == 'NAPS' || $sal_type == 'NATS') { 
								?>
  						...
  						<select id="txt_pli_deduct" name="txt_pli_deduct">
  							<?php
								...
								?>
  								...
  								<?php
								} else if (clean($_SESSION["__user_type"]) == 'HR' && ($takehome == '')) {
									if ($des_id != 9 && $des_id != 12 && $des_id != 30) {
									?>
  								...
  								<?php
									...
									?>
  								<option>No</option>
  								<?php if ($des_id != 9 && $des_id != 12 && $des_id != 30) { ?>
  					...
  					<div class="input-field col s4 m4">
  						<?php if (($_SESSION["__user_type"]) == 'ADMINISTRATOR') { ?>
  				...
  				<script>
  					$(document).ready(function() {
  							...
  							if (!isNaN($('#txt_sal_ctc').val())) {
  								if ($('#txt_sal_emptype').val() == 'FT') {
  									if ($('#txt_sal_paytype').val() == 'RT') {
  										if (parseInt($('#txt_sal_ctc').val()) >= 8100 || parseInt($('#txt_sal_ctc').val()) < 5000) {
  											validate = 1;
  										...
  										var arr = [8100, 8500, 9000, 9500, 10000, 10500, 11000, 11500, 12000, 12500, 13000, 13500, 14000, 14500, 15000, 15500, 15800, 16000, 16500, 17000, 17500, 18000, 18500, 19000, 19500, 20000, 20500, 21000];


  										if (parseInt($('#txt_sal_ctc').val()) < 8100 || parseInt($('#txt_sal_ctc').val()) > 21000) {
  											validate = 1;
  											alert_msg += '<li> CTC  can not be Less Than 8100 and Greater Than  21000 for this Pay-Roll Type </li>';
  										}
  						...
  						var calculate = function() {
  							...
  							if (emptype == 'CCE' && parseInt($('#txt_sal_ctc').val()) > 10000) {
  							...
  							if (parseInt($("#txt_sal_ctc").val()) > 0) {
  								...
  								if ($('#txt_sal_type').val() == "NAPS" || $('#txt_sal_type').val() == "NATS") {
  									...
  									//alert(PLI_percent)
  									PLI_ammount = (CTC * PLI_percent) / 100;
  									EmployeePF = EmployerPF = EmployerESIC = EmployeeESIC = 0;
  									TkHome_1 = CTC - (EmployeePF + EmployerPF + EmployerESIC + EmployeeESIC + ProfessionalTax);
  									// alert(TkHome_1)
  									NetTakeHome = TkHome_1 - PLI_ammount;
  									$('#txt_pf_count').val(0);
  								...
  								$("#txt_nt_takehome").val(NetTakeHome.toFixed(0));
  								$("#txt_takehome").text("  " + NetTakeHome.toFixed(0));
