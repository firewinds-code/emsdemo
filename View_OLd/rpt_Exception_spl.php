...
<div id="content" class="content">
	...
	<span id="PageTittle_span" class="hidden">New Exception Report</span>
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				<div class="input-field col s12 m12" id="rpt_container">
					...
					<div class="input-field col s4 m4">
						<Select class="form-control" name="emp_status" id="status">
					...
					<div class="input-field col s6 m6 l6">
						<select id="clientName" name="clientName">
							...
							<?php
							// $myDB = new MysqliDb();
							// $result = $myDB->query('select distinct t1.client_name ID,t3.client_name from new_client_master t1 join report_map t2 on t1.cm_id=t2.processID join client_master t3 on t1.client_name=t3.client_id where t2.EmpID="' . $_SESSION['__user_logid'] . '" and reportID=3');

							$empid = clean($_SESSION['__user_logid']);
							$Query = 'select distinct t1.client_name ID,t3.client_name from new_client_master t1 join report_map t2 on t1.cm_id=t2.processID join client_master t3 on t1.client_name=t3.client_id where t2.EmpID=? and reportID=3';
				...
				if (isset($_POST['btn_view'])) {
					...
					// echo 'call sp_getExceptionReport_new("' . $userid . '","' . $date_From . '","' . $date_To . '","' . $empStatus . '","' . $clientname . '")';
					// die;
					$chk_task = $myDB->query('call sp_getExceptionReport_new("' . $userid . '","' . $date_From . '","' . $date_To . '","' . $empStatus . '","' . $clientname . '")');
					// print_r($chk_task);
					// die;
					// echo 'call sp_getExceptionReport_new("' . $_SESSION['__user_logid'] . '","' . $date_From . '","' . $date_To . '","' . $empStatus . '","' . $clientname . '")';
					$my_error = $myDB->getLastError();
					if (empty($my_error)) {
...
<script>
	$('#btn_view').on('click', function() {
		...
		if (dept == 'ALL') {
			...
			var sp = "call sp_getExceptionReport_new('" + usrid + "','" + date_from + "','" + date_to + "','" + status + "','" + dept + "')";
