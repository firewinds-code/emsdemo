...
if (isset($_POST['btn_login'])) {
	...
	$sqlConnect = 'call temp_user_login("' . $txt_usrId . '","' . $txt_usr_pwd . '")';
	$myDB = new MysqliDb();
	//$myDB = new MysqliDb ();
	...
	if ($result) {
		...
		$rstlogin = 'INSERT INTO login_history(EmployeeID,IP) VALUES(?,?)';
		$stmt = $conn->prepare($rstlogin);
		$stmt->bind_param("ss", $user_logid, $clientip1);
		$rst_login = $stmt->execute();
