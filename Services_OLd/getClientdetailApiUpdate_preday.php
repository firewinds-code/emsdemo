...
$today = cleanUserInput(date('Y-m-d'));
...
left join 
(
select count(t.EmployeeID) rostered,e.cm_id from roster_temp t inner join ActiveEmpID e on t.EmployeeID=e.EmployeeID left join status_table s on e.EmployeeID=s.EmployeeID  where DateOn=cast(? as  date)  and InTime not in('HO','WO') and status in(4,5,6)  and df_id=74 group by cm_id
) t3 on t1.cm_id=t3.cm_id 
left join 
(

select count(t.EmployeeID) ttrostered,e.cm_id from roster_temp t inner join ActiveEmpID e on t.EmployeeID=e.EmployeeID left join status_table s on e.EmployeeID=s.EmployeeID  where DateOn=cast(? as  date) and  cast(InTime as time) <=cast(? as time ) and InTime not in('HO','WO') and status in(4,5,6) and df_id=74  group by cm_id
) tr3 on t1.cm_id=tr3.cm_id 
left join 
(
select ifnull(count(" . $day . "),0) present,b.cm_id from calc_atnd_master a left Join employee_map b  on b.EmployeeID=a.EmployeeID left join status_table s on a.EmployeeID=s.EmployeeID where status in(4,5,6) and ? like 'P%' and a.Month=Month(?) and a.Year=Year(?)  and df_id=74  group by cm_id 
) t6 on t1.cm_id=t6.cm_id 
left join 
(
select ifnull(count(" . $day . "),0) absent,b.cm_id from calc_atnd_master a left Join employee_map b  on b.EmployeeID=a.EmployeeID left join status_table s on a.EmployeeID=s.EmployeeID where status in(4,5,6)  and (?='A' || ?='LANA' ) and a.Month=Month(?) and a.Year=Year(?) and df_id=74  group by cm_id
) t6a on t1.cm_id=t6a.cm_id 
left join
(
select count(t1.EmployeeID) active_ojt,t2.cm_id from status_table t1 inner join ActiveEmpID t2 on t1.EmployeeID=t2.EmployeeID  where status in (4,5) and df_id=74  group by cm_id
...
left Join 
(
select count(t1.EmployeeID) on_floor_attrition,e.cm_id from exit_emp t1 Inner join ActiveEmpID e on t1.EmployeeID=e.EmployeeID left join status_table s on t1.EmployeeID=s.EmployeeID where status in (4,5,6) and month(t1.dol)=month(?) and year(t1.dol)=year(?) and df_id=74  group by cm_id
)t7 on t1.cm_id=t7.cm_id 
left join
(
select ifnull(count(" . $day . "),0)hd,b.cm_id from calc_atnd_master a left Join employee_map b  on b.EmployeeID=a.EmployeeID left join status_table s on a.EmployeeID=s.EmployeeID where status in(4,5,6) and ? like 'H%' and ? !='HO' and a.Month=Month(?) and a.Year=Year(?) and df_id=74  group by cm_id 
)t8 on t1.cm_id=t8.cm_id 
left join
(
select ifnull(count(" . $day . "),0) leaves,b.cm_id from calc_atnd_master a left Join employee_map b  on b.EmployeeID=a.EmployeeID left join status_table s on a.EmployeeID=s.EmployeeID where status in(4,5,6) and ((? like 'L%' and ?!='LANA')|| ?='CO' ) and a.Month=Month(?) and a.Year=Year(?) and df_id=74  group by cm_id 
)t9 on t1.cm_id=t9.cm_id 
left join
(
select ifnull(count(" . $day . "),0) wo,b.cm_id from calc_atnd_master a left Join employee_map b  on b.EmployeeID=a.EmployeeID left join status_table s on a.EmployeeID=s.EmployeeID where status in(4,5,6) and ? in('WO','WONA','HO') and a.Month=Month(?) and a.Year=Year(?)  and df_id=74 group by cm_id 
)t11 on t1.cm_id=t11.cm_id  
left join
(
select ifnull(count(t.EmployeeID),0) ncns,e.cm_id from login_ncns_smsmail t inner join ActiveEmpID e on t.EmployeeID=e.EmployeeID left join status_table s on e.EmployeeID=s.EmployeeID where status in (4,5,6) and cast(t.createdOn as date)=?  and type='NCNS'  and df_id=74 group by cm_id
)t12 on t1.cm_id=t12.cm_id
left join
(
select ifnull(count(t.EmployeeID),0) notice,e.cm_id from resign_details t inner join ActiveEmpID e on t.EmployeeID=e.EmployeeID left join status_table s on e.EmployeeID=s.EmployeeID where status in (4,5,6) and ? between nt_start and nt_end  and df_id=74 group by cm_id
)t13 on t1.cm_id=t13.cm_id inner join  process_apiid_log on t1.cm_id=process_apiid_log.cm_id where `date`=? ";

	$stmt = $conn->prepare($query);
	$stmt->bind_param("sssssssssssssssssssssssssss", $date, $date, $date, $day, $date, $date, $day, $day, $date, $date, $date, $date, $day, $day, $date, $date, $day, $day, $day, $date, $date, $day, $date, $date, $date, $date, $date);
	if (!$stmt) {
		echo "failed to run";
		die;
	}
