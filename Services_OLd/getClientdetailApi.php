...
from 
(
select cm.client_id,ncm.cm_id,cm.client_name,process_id, process,ncm.sub_process,ncm.location,ncm.locationName from client_master cm inner join (select Distinct pm.process_id, a.process as process, a.sub_process, a.client_name,a.cm_id,a.location,lm.location as locationName from new_client_master a inner Join location_master lm on lm.id=a.location left join process_map pm on a.process=pm.process where a.cm_id Not IN(select cm_id from client_status_master) and process_id is not null) ncm on cm.client_id=ncm.client_name
)t1 left join 
(
	...
	foreach ($ClientD as $val) {
		...
		if ($err) {
			...
			if ($ddArray->_id != "") {
				...
				$APIID = clean($APIID);
				$cmid = clean($val['cm_id']);
				$date = clean($date);

				$insertQuery = "Insert into process_apiid_log set api_id=? ,cm_id=?,date=?";

				$stmt = $conn->prepare($insertQuery);
				$stmt->bind_param("sss", $APIID, $cmid, $date);
				if (!$stmt) {
					echo "failed to run";
					die;
				}
