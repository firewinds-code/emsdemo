...
if(isset($Data['Exception']) && $Data['ExpceptionID']!="" && ($Data['Exception']=='Working on WeekOff' || $Data['Exception']=='Working on Leave') && trim($Data['ShiftOut'])!="" && trim($Data['ShiftIn'])!="" ) 
{
					$cm_id=$Data['cm_id'];
	...
	$sqlInsertException = 'call Aap_UpdateRequestDetailsManager("'.$ExpID.'","'.$DateFrom.'","'.$DateTo.'","'.$Exception.'","'.$comment.'","'.$MngrStatus.'","'.$ApproveStatus.'","'.$approvedBy.'","'.$DateModified.'","'.$IssueType.'","'.$CurrAtt.'","'.$UpdateAtt.'","'.$ShiftIn.'","'.$ShiftOut.'","'.$LeaveType.'","App")';
					...
					if($rowCount>0)
					{
						...
						{
							 if (($Exception == "Working on Holiday" || $Exception == "Working on WeekOff" || $Exception == "Working on Leave") && $ShiftIn!='NA' && $ShiftOut!='NA' )
					        {  
					        $count=0;
