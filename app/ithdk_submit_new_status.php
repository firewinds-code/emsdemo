	...
	if($isNewStatusInProg == 1){
		...
		$result=$myDB->rawQuery($sql);
				...
				if(empty($myDB->getLastError())){
					...
					if(strtoupper($newStatus) == 'INPROGRESS'){
						$msg ="Dear <b>" .$empname.",</b><br/><br/>Greetings for the day!<br/><br/><br/>Update Reference <b>Ticket ID - ".$ticketId."</b> <br/><br/>TAT :<b> ".$totalTat."Hour </b> <br/><br/>Handler Remark :<b> ".$handlerRemark."</b><br/><br/>";
					}else{
						$msg ="Dear <b>" .$empname.",</b><br/><br/>Greetings for the day!<br/><br/><br/><b>Ticket ID - ".$ticketId."</b> Has been closed please find the closer remark mentioned below. <br/><br/>Handler Remark :<b> ".$handlerRemark."</b><br/><br/>";
					}
					...
					if (empty($myDB->getLastError()) && $mail->send())
					{
						...
						$response['status']=0;
						$response['msg']='Status not updated please try later.';
					}
				}else{
					$response['status']=0;
