 ...
 <?php
    ...
    if (count($dataArray) > 0) {
        if ($dataArray[0]['status'] == 1) {
            ...
            $result1['msg'] = "You have already received your Appointment Letter on your email " . $dataArray[0]['fetcheEmail'];
            $result1['status'] = 0;
    ...
    if ($disable == "") {
        ...
        if (isset($EmployeeID) && $EmployeeID != "" && isset($Data['appkey']) && $Data['appkey'] == 'al' && $loc != "" && $Data['emailAddress'] != "") {
            ...
            $GetData = "select whole_details_peremp.EmployeeID,EmployeeName,location,DOJ,designation,ctc,status_table.onFloor,des_id,function,Gender,cm_id,tehsil from whole_details_peremp left outer join salary_details on salary_details.EmployeeID = whole_details_peremp.EmployeeID left outer join status_table on whole_details_peremp.EmployeeID = status_table.EmployeeID left join address_details on whole_details_peremp.EmployeeID = address_details.EmployeeID where whole_details_peremp.EmployeeID='" . $EmployeeID . "'";
            ...
            $flag = 0;
	   		if(($resultsE[0]['cm_id']=="520" || $resultsE[0]['cm_id']=="521") && round((time() - strtotime($resultsE[0]['DOJ'])) / (60 * 60 * 24))<=20)
	   		{
	   			$flag = 1;			
			}	
            ...
            if (is_array($resultsE) > 0) {
                foreach ($resultsE as $val) {
                    ...
                    <dt><b>10. Termination on account fraud, misconduct or ZTP:</b></dt>
                        ...
                        <br><br><br>
                        <p>Yours truly,<br><b>For " . $company1 . "</b></p></br><br><br>";
					...
					else if (in_array($var_desg_id, array(30)))  //for field executive
                {
                    ...
                    $pdfaddress = '<p style="color:#958d8d;text-align:center;font-size:13px;">' . $location . '</p>';
                    ...
                    } else if (in_array($var_desg_id, array(5, 7, 8, 10, 13, 15, 16, 22, 23))) //For AM & Above
                    {
                        $tcpdf->Image('../Style/img/sk_sign.jpeg', 15, 165, 30, 15, 'JPEG'); //sig 
                    ...
                    if (file_exists($path)) 
                    {
                        ...
                        if (!$mail->send()) {
                            ...
                            $myDB = new MysqliDb();
                            $myDB->rawQuery("update appointmentlonline set status=1,source_from='Web-146byemp',fetcheEmail='" . addslashes($Data['emailAddress']) . "',ReceivedDate=now() where EmployeeID='" . $EmployeeID . "'");
