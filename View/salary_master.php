			...
			<div class="schema-form-section row">
				...
				<div class="form-div">
					...
					<div class="col s12 m12 no-padding">
						...
						<?php
						// $employee_id=$_POST['ddl_ED_Emp_Name'];
				...
				if (isset($_POST['submit'])) {
					...
					$employee_id = $_POST['ddl_ED_Emp_Name'];
					if (isset($_POST['ddl_ED_Emp_Name'])) {
						$sql =  "select EmpID from salary_master where EmpID='" . $employee_id . "'";
						$myDB = new MysqliDb();
						$res = $myDB->query($sql);

						if (count($res) > 0) {
							$SQL = "UPDATE salary_master SET location = '" . $location . "', created_at='" . $created_at . "' WHERE EmpID ='" . $employee_id . "'";
							$myDB = new MysqliDb();
							$results = $myDB->query($SQL);
							$my_error = $myDB->getLastError();
							echo "<script>$(function(){ toastr.success('Data Updated Successfully.') }); </script>";
						} else {
							$sqlConnect =  "insert into salary_master (EmpID,location, created_at)values('" . $employee_id . "','" . $location . "','" . $created_at . "');";
							$myDB = new MysqliDb();
