...
if(isset($_POST['btn_initiate']))
{
	if(count($_POST['cb']) > 0 && $_POST['cb'])
	{
		...
		foreach($_POST['cb'] as $EmployeeID)
		{
			...
			"Status" => 1,
			"Remark" => $_SESSION['__user_Name'].'>'.$_SESSION['__user_logid'].'>'.date('Y-m-d H:i:s').'>'.$_POST['txt_remark_qa2qa'].'|',
			...
			else
			{
				...
				{
						$insert_flag++;
						$mySQLError_log .= $EmployeeID.' not initiated. Error :'.$mySQLError;
			...
			<?php 
				if(!empty($process) && $process != 'NA')
				{
					...
					<div  class="hidden" id="div_btn">
						...
						echo "<script>$(function(){ toastr.success('Total".count($rst_qa2qa)."Employee found for ".$process."'); }); </script>";
