...
//echo $EmpID=$EmployeeID=$_REQUEST['EmpID'];
...
if (isset($_SESSION['__user_logid'])) {
	...
	$name = '';
	$target_dir = '';
	/*if(isset($_REQUEST['txtEmployeeName'])){
	...
	if (isset($_REQUEST['dirloc'])) {
		$dirLoc = $_REQUEST['dirloc'];
	}
}
					...
					$.ajax({
						...
						success: function(data) {
							if (data == 1) {
								...
								$(function() {
									toastr.success('Appintment letter not Created, please check this EmployeeID');
								});
							}
...
<body>
	...
	if (count($result) && $result && $EmployeeID != "") {
		...
		if (intval($result[0]['des_id']) == 9 || intval($result[0]['des_id']) == 12 || intval($result[0]['des_id']) == 33 || intval($result[0]['des_id']) == 34 || intval($result[0]['des_id']) == 35 || intval($result[0]['des_id']) == 36) {
  ...
  <strong>Reference ID: <label>';
			...
			if (count($Locationresult) > 0) {
				foreach ($Locationresult as $val) {
					if ($locationid == "1" || $locationid == "2") {
						...
						//echo $varhtnl .=substr($EmployeeID, 0, 3);
			...
			if ($des_id == '9') {
				...
				$varhtnl .= 'Customer Support Associate - Technical Assistant';
			} elseif ($des_id == '34') {
				$varhtnl .= 'Customer Support Associate - Inside Sales';
			} elseif ($des_id == '35') {
				$varhtnl .= 'Customer Support Associate  Other1';
			} elseif ($des_id == '36') {
				$varhtnl .= 'Customer Support Associate  Other2';
			} else {
...
<p>Yours truly,<br />';
			...
			if ($myDB->count > 0) {
				...
				if ($myDB->count < 1) {
					...
					$myDB = new MysqliDb();
					$insert_Query = $myDB->rawQuery("Insert Into appointmentlonline set EmployeeID='" . $EmployeeID . "',EmpName='" . $name . "',cm_id='" . $cm_id . "',CreateDate='" . date('Y-m-d') . "',status='0',source_from='web427'");
				}
			}
		} else {
