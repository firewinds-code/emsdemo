...
if ($_SESSION["__status_ah"] == 'No' && $_SESSION["__status_ah"] != $_SESSION['__user_logid']) {
    $location = URL . 'Error';
    header("Location: $location");
    exit();
}

$processQRY = "select cm_id, concat(t2.client_name,'|',t1.process,'|',t1.sub_process) as Process from new_client_master t1 join client_master t2 on t1.client_name=t2.client_id where account_head='" . $_SESSION["__user_logid"] . "' order by process;";
$myDB = new MysqliDb();
...
if (isset($_POST['btn_text_send'])) {
    ...
    $created_by = $_SESSION["__user_logid"];


    //select EmployeeID from ActiveEmpID where cm_id=56 and df_id=74;

    $insert_msg = 'INSERT INTO desimination_matrix(empid,process,created_by) values("' . $empid . '","' . $cm_id . '","' . $created_by . '")'; //"'.$cmid.'"  and df_id=74
    $myDB = new MysqliDb();
...
if (isset($_GET['delete_id'])) {
    ...
    $deleteQRY = "delete from desimination_matrix where id='" . $delete_id . "'";
    $myDB = new MysqliDb();
