							...
							if (!isNaN($('#txt_sal_ctc').val())) {
								if ($('#txt_sal_emptype').val() == 'FT') {
									if ($('#txt_sal_paytype').val() == 'RT') {
										...
										if (parseInt($('#txt_sal_ctc').val()) < 12193 || parseInt($('#txt_sal_ctc').val()) > 21050) {
											validate = 1;
											alert_msg += '<li> CTC  can not be Less Than 12193 and Greater Than  21050 for this Pay-Roll Type </li>';
										}
						...
						var calculate = function() {
							...
							if (parseInt($("#txt_sal_ctc").val()) > 0) {
								...
								if (CTC > 21000 && $('#txt_pf_deduct').val() == 'Yes') {
									...
									OtherAllownces = (CTC - 12193) / 1.0325;
								} else if (CTC <= 21000) {
