...
<span id="PageTittle_span" class="hidden">Employee Cycle Log Report</span>
...
<!-- Header for Form If any -->
	 <h4>Employee Cycle Log Report</h4>				
...
<div id="pnlTable">	
		...
		{
			$myDB=new MysqliDb();
			$sqlConnect='call getReport_ECLog("'.$date_From.'","'.$date_To.'")';
			...
			if(empty($my_error)){  
				...
				$table .='<th>Common Comments</th>';
				$table .='<th>Trainer Remarks</th>';
				$table .='<th>QA OJT Remarks</th>';
				...
				foreach($chk_task as $key=>$value)
				{
					...
					$table .='<td>'.$value['Common Comments'].'</td>';
					$table .='<td>'.$value['Trainer Remarks'].'</td>';
					$table .='<td>'.$value['QA OJT Remarks'].'</td>';
					...
					$table .='<td>'.$value['Qa OJT'].'</td>';
					$table .='<td>'.$value['Qa OPS'].'</td>';
