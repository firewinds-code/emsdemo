...
if (isset($_SESSION)) {
	...
	if (!(($_SESSION['__status_th'] != 'No'  && $_SESSION['__status_th'] == $_SESSION['__user_logid']) || ($_SESSION['__status_qh'] != 'No'  && $_SESSION['__status_qh'] == $_SESSION['__user_logid']) || ($_SESSION['__status_oh'] != 'No'  && $_SESSION['__status_oh'] == $_SESSION['__user_logid']) || ($_SESSION['__status_ah'] != 'No'  && $_SESSION['__status_ah'] == $_SESSION['__user_logid']) || $_SESSION["__user_type"] == 'ADMINISTRATOR')) {
...
if (isset($_SESSION['__user_logid']) && $_SESSION['__user_logid'] != "") {
	if (isset($_GET['delid']) && $_GET['delid'] != "") {
		$myDB = new MysqliDb();
		$delete_query_acknowledge = $myDB->rawQuery("DELETE from brf_briefing where id='" . $_GET['delid'] . "'");
		@$delete_query = $myDB->rawQuery("DELETE from brf_question where BriefingId='" . $_GET['delid'] . "'");
		@$delete_query = $myDB->rawQuery("DELETE from brf_acknowledge where BriefingId='" . $_GET['delid'] . "'");
		@$delete_query = $myDB->rawQuery("DELETE from brf_quiz_attempted where BriefingId='" . $_GET['delid'] . "'");
		if ($delete_query) {
	...
	if (isset($_POST['addbriefing'])) {
		...
		if ($clientID != "NA"  && $bheading != "" && $remark1 != "" && $from_date != "" && $createdBy != "") {
			...
			if ($select_query && count($select_query) > 0) {
			...
			if (isset($_FILES['upload_file']['name']) && $_FILES['upload_file']['name'] != "") {
				...
				if ($fsize > 0 && $fsize <= 2097152) {
					...
					if (!move_uploaded_file($_FILES["upload_file"]["tmp_name"], $target_file)) {
						...
						$hidden_file_name = $_POST['hidden_filename'];
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				<div class="input-field col s6 m6 ">
					<?php
					if ($_SESSION["__login_type"] == "Briefing") {
						$sqlBy = 'call  get_client_master_briefing("' . $_SESSION['__user_client_ID'] . '")';
					} else {
					$sqlBy = 'call  get_client_master("' . $_SESSION['__user_logid'] . '")';
					}
					$myDB = new MysqliDb();
				...
				function getAckBriefing($bid, $table)
				{
					...
					if ($table == 'brf_quiz_attempted') {
						$string = "SELECT distinct EmployeeID AS total_count  FROM $table where BriefingId='" . $bid . "' ";
					}
					$myDB = new MysqliDb();
					$ack_query = $myDB->rawQuery($string);
					if ($ack_query) {
						return  count($ack_query);
					} else
						return 0;
				}
				$todate = "";
				...
				<div class='input-field col s6 m6'>
					<input type='text' name='from_date2' id='from_date2' <?php if (isset($_GET['from_date2'])) { ?> value='<?php echo $_GET['from_date2']; ?>' <?php } else { ?> value='<?php echo date('Y-m-d'); ?>' <?php } ?>>
					<label for="from_date2" class='active'>From Date</label>
				</div>
				<div class='input-field col s6 m6'>
					<input type='text' name='to_date' id='to_date' <?php if (isset($_GET['to_date'])) { ?> value='<?php echo $_GET['to_date']; ?>' <?php } else { ?> value='<?php echo date('Y-m-d'); ?>' <?php } ?>>
