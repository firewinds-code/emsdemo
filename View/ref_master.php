...
if (isset($_POST['btn_ref_Save'])) {
	$_type = (isset($_POST['txt_ref_Type']) ? $_POST['txt_ref_Type'] : null);
	$_cp = (isset($_POST['txt_ref_cp']) ? $_POST['txt_ref_cp'] : null);
	$_cn = (isset($_POST['txt_ref_cn']) ? $_POST['txt_ref_cn'] : null);
	$_rn = (isset($_POST['txt_ref_rn']) ? $_POST['txt_ref_rn'] : null);
	$_paytm = (isset($_POST['txt_ref_payout']) ? $_POST['txt_ref_payout'] : null);
...
<div id="content" class="content">
    ...
    <div class="pim-container row" id="div_main">
        ...
        <div class="form-div">
            ...
            <h4>Reference Master Details <a
                    class="btn-floating btn-large waves-effect waves-light modal-trigger tooltipped" id="ContentAdd"
                    href="#myModal_content" data-position="bottom" data-tooltip="Add Reference"><i
                        ...
                        <div class="modal-body" style="max-height: 100%;float:left;overflow: auto;">
                            <div class="col s12 m12">
                                ...
                                <div class="input-field col s6 m6 hidden">
                                    <input type="text" class="form-control" readonly="true" value="Consultancy"
                                        id="txt_ref_Type" name="txt_ref_Type" />
                                    <label for="txt_ref_Type">Source Of Recruitment</label>
                                </div>
                                <div class="input-field col s6 m6">
                                    <input type="text" class="form-control" id="txt_ref_rn" name="txt_ref_rn"
                                        required />
                                    <label for="txt_ref_rn">Consultancy Name</label>
                                </div>
                                <div class="input-field col s6 m6">
                                    <input type="text" class="form-control" id="txt_ref_cp" name="txt_ref_cp"
                                        required />
                                    <label for="txt_ref_cp">Contact Person</label>
                                </div>
                                <div class="input-field col s6 m6">
                                    <input type="text" class="form-control" id="txt_ref_cn" name="txt_ref_cn"
                                        required />
                                    <label for="txt_ref_cn">Contact No</label>
                                </div>
                                <div class="input-field col s6 m6">
                                    <input type="text" class="form-control" id="txt_ref_payout" name="txt_ref_payout"
                                        required />
                            ...
                            <div class="input-field col s12 m12 right-align">
                                ...
                                <button type="submit" name="btn_ref_Save" id="btn_ref_Save"
                                    class="btn waves-effect waves-green ">Add</button>
                                <button type="submit" name="btn_ref_Edit" id="btn_ref_Edit"
                                    class="btn waves-effect waves-green hidden">Save</button>
                                <button type="button" name="btn_ref_Can" id="btn_ref_Can"
                                    class="btn waves-effect modal-action modal-close waves-red close-btn">Cancel</button>
                            </div>
                        </div>
                    </div>
                </div>
...
$(document).ready(function() {
    ...
    $('#btn_ref_Edit,#btn_ref_Save').on('click', function() {
        ...
        $("input,select,textarea").each(function() {
            ...
            $(this).removeClass('has-error');
            if ($(this).is('select')) {
                $(this).parent('.select-wrapper').find('.select-dropdown').removeClass(
                    "has-error");
            }
            var attr_req = $(this).attr('required');
            if (($(this).val() == '' || $(this).val() == 'NA' || $(this).val() == undefined) &&
                (typeof attr_req !== typeof undefined && attr_req !== false) && !$(this)
                .hasClass('.select-dropdown')) {
                validate = 1;
                $(this).addClass('has-error');
                if ($(this).is('select')) {
                    $(this).parent('.select-wrapper').find('.select-dropdown').addClass(
                        "has-error");
                }
                if ($('#' + spanID).size() == 0) {
                    $('<span id="' + spanID + '" class="help-block"></span>').insertAfter('#' +
                        $(this).attr('id'));
                }
