...
<body>
	...
	<form name="indexForm" id="indexForm" method="post" action="<?php echo ($_SERVER['REQUEST_URI']); ?>" enctype="multipart/form-data">
					...
					<div class="schema-form-section row">
						...
						<div class="scheduler-border">
							<div id="secques" class="input-field col s12 m12   hidden">
								<?php $validate_no = rand(1231, 7879); ?>
								<input type="hidden" id="code" name="code" value="<?php echo  $validate_no; ?>" />
								...
								<div class="input-field col s12 m12 rigth-align hidden" id="btndiv1">
									<button type="button" name="btn_save" id="btn_save" class="btn waves-effect waves-green">Submit</button>
								</div>
							</div>
							...
							<div id="sendotp">
								<div class="input-field col s6 m6 ">
									<input type="password" class="otp clschangepwd" id="txt_otp" name="txt_otp" placeholder="Please Enter OTP" autocomplete="off" />
									<label for="txt_otp">Verification Code, you have recieved in phone</label>
								</div>
								<div class="input-field col s12 m12 rigth-align">
									<button type="button" name="btn_save1" id="btn_save1" class="btn waves-effect waves-green">Submit</button>
								</div>
							</div>

							<div id="div_error1" class="col s12">
							</div>
							...
							<div id="loginhere" style="border: 1px solid #bfbfbf;margin-bottom:  20px;padding: 10px;border-radius: 10px;box-shadow: 1px 1px 1px 1px #d0d0d0;"></div>
							...
							<div id="checkotp">
								...
								<input type="hidden" id="txt_pwd_empid" name="txt_pwd_empid" />
								...
								<div class="input-field col s12 m12 right-align ">
									<button type="button" name="btn_change_password" id="btn_change_password" class="btn waves-effect waves-green">Change Password</button>
								</div>
							</div>
						</div>
		...
		<script>
			$('#sendotp').hide();
			$('#checkotp').hide();
			$('#loginhere').addClass('hidden').html('<code><a style="font-size:18px;"  href="View/Login">LogIn Here</a></code>');
			$(function() {
				...
				$('#btn_change_password').on('click', function() {
					...
					var chg_pwd = $('#txt_chg_pwd').val();
					// alert(chg_pwd)
					// return false;
					var txt_empid = $('#txt_pwd_empid').val();
					// alert(txt_empid);
					$.ajax({
						url: "../Controller/change_pass.php",
						data: {
							id: txt_empid,
							chg_pwd: chg_pwd
						},
						success: function(result) {
							...
							if (result == 0) {
								...
								$('#sendotp').hide();
								// return false;
								toastr.info("New password can not be same as last one.");
							} else {
								// alert(empid);
								$('#div1').hide();
								$('#checkotp').hide();
								...
								$('#loginhere').removeClass('hidden').html('<code><a style="font-size:18px;"  href="Login">LogIn Here</a></code>');
				...
				$('#btn_save').on('click', function() {
					// alert('dsfgsdf');
					var empid = $('#txt_empid').val();
					var seqans = $('#txtans').val();

					// alert(empid);
					$.ajax({
						url: "../Controller/sendotp.php?id=" + empid + "&seqans=" + seqans + "&code=" + code,
						...
						success: function(result) {
							...
							if (result == 0) {
								...
								$('#btndiv1').removeClass('hidden');
								$('#sendotp').hide();
								$('#div_error1').removeClass('hidden').html('<p class="text-danger">Wrong Answer Of Security Question Try Again.</p>');
								$('#checkotp').hide();
								// return false;
								...
								$('#secques').hide();
								$('#btndiv1').addClass('hidden');
								$('#div_error1').addClass('hidden').html('<p class="text-danger">Wrong Answer Of Security Question Try Again.</p>');
							}
						}
					});
				});
				...
				$('#btn_save1').on('click', function() {
					...
					$('#txt_pwd_empid').val(empid)
					// alert(empid);
					$.ajax({
						url: "../Controller/checkotp.php?id=" + empid + "&txt_otp=" + txt_otp + "&reqtype=" + "OTPSubmit" + "&code=" + code,
						success: function(result) {
							// alert(result);
							// alert(empid);
							if (result == 0) {
								...
								$('#checkotp').hide();
								// return false;
								$('#div_error1').removeClass('hidden').html('<p class="text-danger">Wrong OTP Please Try Again.</p>');
								...
								$('#secques').hide();
								$('#btndiv1').addClass('hidden');
								$('#div_error1').addClass('hidden').html('<p class="text-danger">Wrong OTP Please Try Again.</p>');
							}
						}
					});
				});
