	...
	<div class="schema-form-section row" >
		...
		<?php
			if(isset($_POST['btn_add']))
			{
				...
				if($_POST['btn_add']  == '9' || $_POST['btn_add'] == '12')
				{	
					$str = 'call atnddna1("'.$_POST['txtEmployeeID'].'","'.$_POST['txtMonth'].'","'.$_POST['txtYear'].'")';
					$myDB=new MysqliDb();
					...
					{  
						$table='<div class="had-container pull-left row card" style="margin-top: 10px;width: 100%;padding: 15px;">
							...
							{
								$str = 'call atnddna2("'.$_POST['txtEmployeeID'].'","'.$_POST['txtMonth'].'","'.$_POST['txtYear'].'")';
								$myDB=new MysqliDb();
								$chk_task=$myDB->query($str);
								$my_error= $myDB->getLastError();			
								if(count($chk_task) > 0 && $chk_task)
