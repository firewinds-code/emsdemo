    ...
    <div class="pim-container row" id="div_main">
        ...
        <div class="form-div">
            ...
            <div class="schema-form-section row">
                ...
                $select_emp = "select b.id, b.EmployeeID,c.client_name,w.Process,w.sub_process,w.cm_id,case when w.df_id in (74,77,146, 147,148,149) then 'CSA' else 'Support' end as des, w.EmployeeName,w.designation,l1.location,w.DOJ from whole_details_peremp as w  join client_master as c on w.client_name=c.client_id join bgv as b on w.EmployeeID=b.EmployeeID join location_master l1 on l1.id=w.location where b.flag=0 and w.DOJ=?";
                ...
                if ($btn_view) {
                    if ($result->num_rows > 0) {
                        ...
                        foreach ($result as $key => $value) {
                            ...
                            if (($cmid == '484' && $designation == 'CSA')  || ($cmid == '494' && $designation == 'CSA')) {
                                $educations = "select * from education_details where EmployeeID=? and edu_level='Graduation'";
                            ...
                            } else if (($cmid == '495' && $designation == 'CSA')  || ($cmid == '501' && $designation == 'CSA') || ($cmid == '563' && $designation == 'CSA') || ($cmid == '565' && $designation == 'CSA') || ($cmid == '597' && $designation == 'CSA') || ($cmid == '659' && $designation == 'CSA')) {
                                $educations = "select * from education_details where EmployeeID=? and edu_level='Graduation'";
                                $selQury = $conn->prepare($educations);
                                $selQury->bind_param("s", $employee);
                                $selQury->execute();
                ...
                if ($submit) {
                    ...
                    if ($tcid) {
                        ...
                        foreach ($emp_id as $key => $value) {
                            ...
                            // print_r($emp);
                            // die;
                            $select = "select FirstName, LastName, FatherName,dob, Gender,mobile,emailid, location,cm_id,case when df_id in (74,77,146, 147,148,149) then 'CSA' else 'Support' end as des from personal_details t1 left join contact_details t2 on t1.employeeid=t2.EmployeeID join employee_map t3 on t1.EmployeeID=t3.EmployeeID where t1.EmployeeID=? and emp_status='Active'";
                            ...
                            if ($results->num_rows > 0) {
                                ...
                                if (($cm_id == '484' && $des == 'CSA')  || ($cm_id == '494' && $des == 'CSA')) {
                                    $educations = "select * from education_details where EmployeeID=? and edu_level='Graduation'";
                                    $selQury = $conn->prepare($educations);
                                ...
                                } else if (($cm_id == '495' && $des == 'CSA')  || ($cm_id == '501' && $des == 'CSA') || ($cm_id == '563' && $des == 'CSA') || ($cm_id == '565' && $des == 'CSA') || ($cm_id == '597' && $des == 'CSA') || ($cm_id == '659' && $des == 'CSA')) {
                                    $educations = "select * from education_details where EmployeeID=? and edu_level='Graduation'";
                                    $selQury = $conn->prepare($educations);
                                    $selQury->bind_param("s", $emp);
                                    $selQury->execute();
                    ...
                    foreach ($_SESSION['tempemp'] as $key => $value) {
                        ...
                        $table .= '<td style="font-size:medium" class="red-text text-darken">' . $value . '---SENT DOCUMENT</td>';
                ...
                ?>
                    <div class="form-group">
                        <div class="input-field col s8 m8">
                            <select class="form-control" name="bgv" id="bgv" required>
                                ...
                                <?php
                                $sqlBy = 'select id,bgv_vender_name from bgv_vender where id not in (2);';
                                $myDB = new MysqliDb();
                    ...
                    <div class="input-field col s12 m12 right-align">
                        <button type="submit" name="send" id="send" class="btn waves-effect waves-green">SENT</button>
                    </div>
                <?php
                }
                ...
                if ($send) {
                    ...
                    foreach ($_SESSION['tempemp'] as $key => $value) {
                        $emp = $value;
                        $chkflag = '';
                        $select = "select FirstName,MiddleName, LastName, FatherName,dob, Gender,mobile,emailid, t7.location,t3.cm_id,case when df_id in (74,77,146, 147,148,149) then 'CSA' else 'Support' end as des,EmployeeName,dateofjoin,address_p,t6.client_name,altmobile,em_contact from personal_details t1 left join contact_details t2 on t1.employeeid=t2.EmployeeID join employee_map t3 on t1.EmployeeID=t3.EmployeeID left join address_details t4 on t1.EmployeeID=t4.EmployeeID join new_client_master t5 on t3.cm_id=t5.cm_id join client_master t6 on t5.client_name=t6.client_id join location_master t7 on t1.location=t7.id where t1.EmployeeID=? and emp_status='Active'";
                        $htmlresp = $htmlresp  . $emp;
                        ...
                        if ($_POST['bgv'] == '1') {
                            ...
                            if ($res->status == 'True') {
                                ...
                                $htmlresp = $htmlresp . '---' . $res->candidate_id . ' Successfully Sent Document' . '<br/>';
                                $candi_id = $res->candidate_id;
                                $bgv_vender = cleanUserInput($_POST['bgv']);
                                $updateres = 'update bgv set Candidate_id=?, flag=1, Modifiedon = now(),ModifiedBy= ?,check_flag= ?,bgv_vender=? , sla_id=? where EmployeeID=? ';
                                $update = $conn->prepare($updateres);
                                $update->bind_param("sssiis", $candi_id, $userID, $chkflag, $bgv_vender, $sla_id, $emp);
                                $update->execute();
                            ...
                            if ($EmployeeName &&  $dob &&  $doj && $father_name && $location && $sla_id != '') {
                                ...
                                // echo $EmployeeName;
                                curl_setopt_array($curl, array(
                                    CURLOPT_URL => 'http://bgverp.foxivision.com:3000/api/v1/insertApplicationWithAuth',
                                    CURLOPT_RETURNTRANSFER => true,
                                    CURLOPT_ENCODING => '',
                                    CURLOPT_MAXREDIRS => 10,
                                    CURLOPT_TIMEOUT => 0,
                                    ...
                                    CURLOPT_POSTFIELDS =>
                                    '{
                                        "client":"6507fabbeeb43e213ff1b7a3",
                                        ...
                                        "location":"' . $location . '",
                                        "slaId":"' . $sla_id . '",
                                        "zipDoc":"https://ems.cogentlab.com/erpm/Services/Docs_Inactivezip/' . $emp . '.zip",
                                        "emergencyContact":"' . $em_contact . '",
                                        "alternateContact":"' . $alt_mobile . '",
                                        "address":"' . $Address . '",
                                        "process":"' . $client_name . '"
                                    }',
                                    CURLOPT_HTTPHEADER => array(
                                ...
                                if ($res->status == 'ok') {
                                    ...
                                    // die;
                                    $userID = clean($_SESSION['__user_logid']);
                                    $htmlresp = $htmlresp . ' Successfully Sent Document' . '<br/>';

                                    $bgv_vender = cleanUserInput($_POST['bgv']);
                                    $updateres = 'update bgv set Candidate_id=?, flag=1, Modifiedon = now(),ModifiedBy= ?,check_flag= ?,bgv_vender=?,  sla_id=? where EmployeeID=? ';
                                    $update = $conn->prepare($updateres);
                                    $update->bind_param("sssiis", $emp, $userID, $chkflag, $bgv_vender, $sla_id, $emp);
                                    $update->execute();
                                    $resultBy = $update->get_result();
                                } else {
                                    // echo '2222222';
                            ...
                            $data['candid_code'] = $emp;
                            $data['gender'] = $gender;
                            $data['father_name'] = $father_name;
                            $data['contact_no'] = $phone;
                            $data['contact_no_1'] = $alt_mobile;
                            $data['contact_no_2'] = $em_contact;
                            ...
                            $data['pan_number'] = '';
                            $data['aadhar_number'] = '';
                            //$data['PRODUCTION CODE'] = 'COGENTMISPL';
                            $data['package_id'] = $sla_id;
                            $zipcreated = "/var/www/html/erpm/Services/Docs_Inactivezip/$emp.zip";
                            $data['attchments[ ]'] = new CURLFILE($zipcreated);
                            // print_r($data);
                            // die;
                            curl_setopt_array($curl, array(
                                CURLOPT_URL => 'https://mistitservices.in/mist-it/api/resource/candid_process',
                                CURLOPT_RETURNTRANSFER => true,
                                CURLOPT_ENCODING => '',
                                CURLOPT_MAXREDIRS => 10,
                                CURLOPT_TIMEOUT => 0,
                            ...
                            $res = json_decode($response);
                            // print_r($res);
                            if ($res->error_sts == '200') {
                                // print_r($res->error_sts);
                                // die;
                                $userID = clean($_SESSION['__user_logid']);
                                $htmlresp = $htmlresp . '---' . $res->data_list->candid_id . ' Successfully Send Document' . '<br/>';
                                $candi_id = $res->data_list->candid_id;
                                $bgv_vender = cleanUserInput($_POST['bgv']);
                                $updateres = 'update bgv set Candidate_id=?, flag=1, Modifiedon = now(),ModifiedBy= ?,check_flag= ?,bgv_vender=? , sla_id=? where EmployeeID=? ';
                                $update = $conn->prepare($updateres);
                                $update->bind_param("sssiis", $candi_id, $userID, $chkflag, $bgv_vender, $sla_id, $emp);
                                ...
                                // echo $res->error_sts;
                                $userID = clean($_SESSION['__user_logid']);
                                $htmlresp = $htmlresp . '---' . $res->error_msg . '<br/>';
                                $respon = $res->error_msg;
                                $update = 'Insert into bgv_api_log (Emp_id,response,createdby)values(?,?,?)';
                                $ins = $conn->prepare($update);
                                $ins->bind_param("sss", $emp, $respon, $userID);
                                $ins->execute();
