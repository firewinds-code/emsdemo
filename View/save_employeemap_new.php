...
if(isset($_REQUEST['empid']))
{
	...
	if($loc=="7" || $loc=="1" || $loc=="2" || $loc=="3" || $loc=="4")
	{
		if(substr($EmployeeID,0,2) != 'TE')
		{
			...
			{
				if(substr($EmployeeID,0,2) == 'CE' || substr($EmployeeID,0,2) == 'AE' || substr($EmployeeID,0,2) == 'MU' || substr($EmployeeID,0,2) == 'OC')
				{
					$emptype = "Cogent";		
				}
				else if(substr($EmployeeID,0,3) == 'CCE')
					...
					if(substr($EmployeeID,0,2) == 'TE')
					{
						...
						if($_POST['employment_type']=="1")
						{
							...
							else
							{
								if($desg == 9 || $desg == 12)
								{
									...
									{
										$orderID = $myDB->query('select ifnull((max(cast(RIGHT(EmployeeID,(length(EmployeeID) - 6)) as unsigned)) + 1),1) EMPID from personal_details where EmployeeID like "AE%" and substring(Employeeid,1,3)!="AEN"');
										$orderID = $orderID[0]['EMPID'];
										$emp_alias ='AE';	
									}
								}
						...
						else if($_POST['employment_type']=="3")
						{
							...
							else
							{
								if($desg == 9 || $desg == 12)
								{
									$orderID = $myDB->query('select ifnull((max(cast(RIGHT(EmployeeID,(length(EmployeeID) - 7)) as unsigned)) + 1),1) EMPID from personal_details where EmployeeID like "OCM%"');
									$orderID = $orderID[0]['EMPID'];
									$emp_alias ='OCM';
								}
								else
						...
						if($emp_alias != '')
						{
							if($orderID)
							{
								...
								if($EmployeeID !='')
								{
									...
									$myDB=new MysqliDb();
									$sql_updateper = $myDB->query('call update_mapedemp_new("'.$EmployeeID.'","'.$emp_IDorg.'","'.$rt_type.'")');				
									$mysql_error = $myDB->getLastError();
...
<script>
	$(document).ready(function(){
		...
		if(usrtype === 'ADMINISTRATOR'||usrtype === 'HR' || usrid=='CE121622565')
		{
		}
...
<!-- Form container if any -->
	...
	?>
           ...
           <input type="hidden" name="emp_salary" id="emp_salary" value="<?php echo $emp_salary;?>"/>
			...
			{
				$sqlBy="select distinct client_id,t1.client_name FROM client_master t1 join new_client_master t2 on t1.client_id=t2.client_name where location in (1,2) "; 
			}
			else
			{
				$sqlBy="select distinct client_id,t1.client_name FROM client_master t1 join new_client_master t2 on t1.client_id=t2.client_name where location='".$_SESSION["__location"]."'"; 
			}
	       ...
	       <?php  } ?>
			...
			if($loc=="1" || $loc=="2")
			{
				if((substr($EmployeeID,0,2) == 'CE' || substr($EmployeeID,0,2) == 'AE' || substr($EmployeeID,0,2) == 'MU') && $dept !="" &&  $sub_process1 != "" && $desg != "")
				{
			...
			{
				if((substr($EmployeeID,0,2) == 'CE' || substr($EmployeeID,0,2) == 'OC') && $dept !="" &&  $sub_process1 != "" && $desg != "")
				{
					echo '<div id="div_tempCard"><a href="#" data_empID="'.$EmployeeID.'" id="a_print_card">
					      <i class="fa fa-link"></i> Print ID Card</a>
					      </div>';
...
<script>
	$(document).ready(function(){
	    ...
	    $('#txt_empmap_client').change(function(){
	    		...
	    		value_click_pro =2;
			    $.ajax({
				  url: <?php echo '"'.URL.'"';?>+"Controller/getprocess.php?id="+clientId+"&loc="+location
	     ...
	     $('#txt_empmap_process').change(function(){
	    		...
	    		//alert(location);
			    $.ajax({
				  url: <?php echo '"'.URL.'"';?>+"Controller/getsubprocess_map.php?id="+id+"&proc="+tval+"&dept="+dept+"&loc="+location
				}).done(function(data) { // data what is sent back by the php page
	...
	$('#btn_empmap_Save').on('click', function(){
	     ...
	     $('#txt_empmap_dept').change(function(){
	    		...
	    		value_click_cl =2;
			    $.ajax({
				  url: <?php echo '"'.URL.'"';?>+"Controller/get_client_bydept.php?id="+tval
				}).done(function(data) { // data what is sent back by the php page
		...
		$('#txt_empmap_subprocess').click(function(){
	    		if(value_click_spro == 1)
	    		{
		    		...
		    		var location = <?php echo $_SESSION["__location"] ?>;
				    $.ajax({
					  url: <?php echo '"'.URL.'"';?>+"Controller/getsubprocess_map.php?id="+id+"&proc="+tval+"&loc="+location
					}).done(function(data) { // data what is sent back by the php page
						$('#txt_empmap_subprocess').html(data);
							$(".schema-form-section input,.schema-form-section textarea").each(function(index, element) {
						     if($(element).val().length > 0) {
