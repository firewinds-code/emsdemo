...
{
	$_CMID = $_POST['txt_Client_Name'];
	
	$myDB = new MysqliDb();
	$data_validate = $myDB->query("select cm_id from client_status_master where cm_id = '".$_CMID."'");
	//print_r($data_validate);
	$dataID = $dataIDs = '';
	
	
	
	if(count($data_validate) > 0  && $data_validate && $_POST['txt_status'] == 'Active')
	{
		
		$sql_delete = "delete FROM client_status_master where cm_id in ($_CMID)";
		$myDB = new MysqliDb();
	...
	elseif($_POST['txt_status'] == 'Inactive' && empty($data_validate))
	{
