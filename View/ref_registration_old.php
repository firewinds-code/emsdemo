...
{
	$mySql = new MysqliDb();
	$result_cliproc = $mySql->query("SELECT clientname,Process,sub_process FROM whole_details_peremp where EmployeeID = '".$_SESSION["__user_logid"]."'");
...
<!-- Form container if any -->
	<div class="schema-form-section row" >
			...
			if(false){
				$myDB=new MysqliDb();
				$chk_task=$myDB->query("SELECT CandidateName, CandidateNumber, CandidateAddress, createdon FROM tbl_reference_reg_detail where EmployeeID = '".$_SESSION['__user_logid']."';");
				$counter = 0;
...
<div id="pnlTable">
		    <?php 
				$sqlConnect = 'call sp_rpt_refschemebyid("'.$_SESSION['__user_logid'].'")';
				$myDB=new MysqliDb();
				$result=$myDB->rawQuery($sqlConnect);
				$mysql_error = $myDB->getLastError();
		    ...
		    <thead>
		        <tr>
					...
					<th>Walkin(Y/N)</th>
					<th>Walkin Date</th>
					<th>Interview clered (Y/N)</th>
					<th>Prcess Seleceted</th>
					<th>Joining Date</th>
					<!--<th>INTID</th>-->
					<th>Joined Y/N</th>
					<th>Current Active (Y/N)</th>
					<th>Total Amount</th>
					<th>1st Pay Amount</th>
					<th>1st Pay Date</th>
					<th>2nd Pay Amount</th>
					<th>2nd Pay Date</th>
		    ...
		    <tbody>					        
		       <?php
				...
				echo '<td>'.$value['CandidateNumber'].'</td>';	
				//echo '<td>'.$value['EmployeeID'].'</td>';	
				echo '<td>'.$value['Walkin (Y/N)'].'</td>';	
				echo '<td>'.$value['Walkin Date'].'</td>';
				echo '<td>'.$value['Interview clered (Y/N)'].'</td>';	
				echo '<td>'.$value['Prcess Seleceted'].'</td>';	
				echo '<td>'.$value['Joining Date'].'</td>';	
				//echo '<td>'.$value['INTID'].'</td>';
				echo '<td>'.$value['Joined Y/N'].'</td>';
				echo '<td>'.$value['Current Active (Y/N)'].'</td>';
				echo '<td>'.$value['Total Amount'].'</td>';
				echo '<td>'.$value['1st_Pay_Amount'].'</td>';
				echo '<td>'.$value['1st_PayDate'].'</td>';
				echo '<td>'.$value['2nd_Pay_Amount'].'</td>';
				echo '<td>'.$value['2nd_PayDate'].'</td>';
			?>
		...
		$('#txtCandidateNumber').keydown(function(event) {
		    ...
		    $('#txtCandidateNumber').keyup(function () {
		    	if($('#txtCandidateNumber').val().length == 10)
		    	{
		    		...
		    		//alert(number);
					jQuery.ajax({url: <?php echo '"'.URL.'"';?>+"Controller/number_validation.php?number="+number}).done(function(data) {
	        	//alert(data);
				if(data==0){
