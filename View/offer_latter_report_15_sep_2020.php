...
if(isset($_SESSION))
{
	...
	else
	{
		...
		{
			if($_SESSION['__user_logid'] == "CE05070035" || $_SESSION['__user_logid'] == "CE03070003"  || $_SESSION['__user_logid'] == "CE10091236" || $_SESSION['__user_logid'] == "CE061930039")
			{
				$isPostBack = false;
				$referer = "";
				$alert_msg="";
		...
		{
			if($_SESSION['__user_logid'] == "CEM061912447" || $_SESSION['__user_logid'] == "CEM071712012"  || $_SESSION['__user_logid'] == "CEM06191244")
			{
				$isPostBack = false;
				$referer = "";
				$alert_msg="";
		...
		else if($_SESSION["__location"]=="4" || $_SESSION["__location"]=="5")
		{
			
		}
		else if($_SESSION["__location"]=="6" || $_SESSION["__location"]=="7")
		{
			if($_SESSION['__user_logid'] == "CE05070035" || $_SESSION['__user_logid'] == "CE03070003"  || $_SESSION['__user_logid'] == "CEK061927543")
			{
				$isPostBack = false;
				$referer = "";
				$alert_msg="";
		...
		{
			if($_SESSION['__user_logid'] == "CE05070035" || $_SESSION['__user_logid'] == "CE03070003"  || $_SESSION['__user_logid'] == "CE071829466"|| $_SESSION['__user_logid'] == "CFK09190785")
			{
				$isPostBack = false;
				$referer = "";
				$alert_msg="";
...
<script>
	$(function(){
		...
		$('#btn_view').click(function(){
			...
			$.ajax({url: "../Controller/OLReportIssueALetter.php?EmpID="+txtEmployeeID, success: function(result){
		            if(result == 1)
					{	
...
<!--Form element start-->
	<input type='hidden' name='loginuser' id='loginuser' value='<?php echo $_SESSION["__user_Name"]; ?>'/>
...
<!--Reprot / Data Table start -->
    <div id="pnlTable">
		<?php
			$myDB=new MysqliDb();
			$sqlConnect='call get_new_emp_forOfferLatter("'.$_SESSION["__location"].'")';
...
<script>
	$(document).ready(function(){
		$('#btn_hold').click(function(){
			...
			var txtEmployeeID= $('#txtEmployeeID').val();
			$.ajax({url: "../Controller/OfferLetterReportAction.php?EmpID="+txtEmployeeID+'&Comment='+Comment, success: function(result){
		            if(result)
					{
						$("#preloader").hide();
		...
		$('#btn_view2').click(function(){
			//$('#btn_view2').hide();
			var txtEmployeeID= $('#txtEmployeeID').val();
			$.ajax({url: "../Controller/OLReportIssueRetainership.php?EmpID="+txtEmployeeID, success: function(result){
				//alert(result);
		            if(result)
		...
		$('#btn_valid').click(function(){
			...
			var i=0;
			var txtEmployeeID= $('#txtEmployeeID').val();
			$.ajax({url:"../Controller/getValidatedoc.php?EmpID="+txtEmployeeID,success: function(result)
				{
					if(result==1){
						...
						$(function(){ toastr.success(alert_msg); });
						window.open('ALetter_download_multipdf2.php?EmpID='+txtEmployeeID+'&Comment='+Comment+'&txtEmployeeName='+txtEmployeeName, '_blank');
					}else{
						$(function(){ toastr.error('document details or education details are not valid'); });
					}
