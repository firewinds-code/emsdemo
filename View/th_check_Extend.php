...
if (isset($_POST['btnSave'])) {
	$status = $_POST['txt_thcheck_Trainer'];
		...
		if ($count_check > 0) {
			...
			if ($_POST['text_trcheck_Batch'] != 'New' && !empty($_POST['text_trcheck_Batch']) && $_POST['text_trcheck_Batch'] != 'NA') {
				...
				foreach ($_POST['cb'] as $val) {
					...
					$save = 'call manage_status_th_extended("' . $empID . '","' . $status . '","' . $createBy . '","' . $_POST['txt_Remark_' . $empID] . '",' . $date_1_crt . ',"' . intval($_POST['txt_Day_crt_1']) . '","' . $roster_log . '")';
				...
				$myDB = new MysqliDb();
				$dt_mail = $myDB->query("select distinct ofc_emailid,contact_details.EmployeeID from contact_details inner join ( select distinct account_head EmployeeID from whole_details_peremp where BatchID = '" . $_POST['text_trcheck_Batch'] . "' union all  select distinct th from whole_details_peremp where BatchID = '" . $_POST['text_trcheck_Batch'] . "' union all  select distinct oh from whole_details_peremp where BatchID = '" . $_POST['text_trcheck_Batch'] . "') td  on td.EmployeeID = contact_details.EmployeeID where  contact_details.EmployeeID  != 'CE03070003' and contact_details.EmployeeID  != 'CE07147134' and ofc_emailid != ''");
				...
				$count = 0;
				$myDB = new MysqliDb();
				$dt_batch = $myDB->query("select BacthName,process,subprocess from batch_master where BacthID = '" . $_POST['text_trcheck_Batch'] . "'");
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				<div class="col s12 m12">
					...
					<div id="pnlTable">
						<?php
						if (!empty($_POST['text_trcheck_Batch'])) {
							$sqlConnect = 'call get_thcheck_extendedlist("' . $_SESSION['__user_logid'] . '","' . $_POST['text_trcheck_Batch'] . '")';
							//echo $sqlConnect;
							$myDB = new MysqliDb();
