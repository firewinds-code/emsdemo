...
$GetData = "select pr.EmployeeId ,pr.EmployeeName ,pr.Designation, des.id,contact_details.emailid,p.location, p.Gender,i.WEF, i.NewCtc from tbl_promotion as pr  left join contact_details on  pr.EmployeeId=contact_details.EmployeeId left join personal_details p on pr.EmployeeId=p.EmployeeId left join tbl_increment i on pr.EmployeeID=i.EmployeeId join designation_master des on pr.Designation=des.Designation  where pr.flag=0 and i.flag=0 limit 100; ";
//Flag=0";
$myDB = new MysqliDb();
		...
		if (substr($EmployeeID, 0, 3) != 'CCE' && substr($EmployeeID, 0, 3) != 'OCM' && substr($EmployeeID, 0, 2) != 'AE' && substr($EmployeeID, 0, 2) != 'RS' && substr($EmployeeID, 0, 3) != 'RSM') {
			...
			if (in_array($var_desg_id, array(2, 3, 4, 6, 11, 18, 19, 20, 21, 25, 26, 27, 28))) {
				...
				if ($loc == "3") {
					...
					$target_dir = ROOT_PATH . "Gurgaon/Promotion_increment_pdf/";
				} else if ($loc == "11") {
					$target_dir = ROOT_PATH . "Hyderabad/Promotion_increment_pdf/";
				}
