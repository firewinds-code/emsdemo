		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				if ($_SESSION['__user_type'] == 'HR' && (($_SESSION['__status_ah'] != 'No' && $_SESSION['__status_ah'] == $_SESSION['__user_logid']) && $_SESSION['__status_ah'] != '')) {
					...
					$sqlstr = "select w.EmployeeID,t1.EmployeeName,date_format(w.DOJ,'%D %M %Y') as DOJ,t2.mobile,t2.altmobile,t4.client_name,t3.process,t3.sub_process from ActiveEmpID w inner join (select EmployeeID  from calc_atnd_master where month=$m and year =$y and concat($d1,$d2)='AA' )cal on w.EmployeeID = cal.EmployeeID  left join excp_emp_ncns_msg ex on w.EmployeeID = ex.EmpID join personal_details t1 on t1.EmployeeID=cal.EmployeeID join contact_details t2 on cal.EmployeeID=t2.EmployeeID join new_client_master t3 on w.cm_id=t3.cm_id join client_master t4 on t3.client_name=t4.client_id where ex. EmpID is null and t1.location=$loc and t3.er_scop='" . $_SESSION['__user_logid'] . "'";
				}
