...
$sql = "select a.EmployeeID, a.EmployeeName, a.DOB, a.DOJ, concat(a.clientname,' | ',a.Process,' | ',a.sub_process) as newprocess,  a.account_head,a.designation, a.ReportTo ,b.id, b.empid, b.ins_request, b.policy_number, b.policy_provider, b.document, b.policy_validupto, b.avail_insurance, b.createdOn from whole_details_peremp a  inner JOIN  health_insurance b on a.EmployeeID=b.Empid where  cast(b.createdOn as date) between '" . $date_From . "' and '" . $date_To . "' ";
?>
<script>
						...
						<tbody>
							...
							if (count($result) > 0) {
								foreach ($result as $key => $value) {
									if ($value['document'] != "") {
										$filePath = REQUEST_SCHEME . '://' . $_SERVER['HTTP_HOST'] . "/hid/UploadedDoc/" . $value['document'];
