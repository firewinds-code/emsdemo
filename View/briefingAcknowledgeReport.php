	...
	<div class="pim-container ">
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				$select_briefing = "select a.heading,a.id,a.cm_id, a.CreatedOn,b.sub_process from brf_briefing  a Inner Join  new_client_master b ON a.cm_id=b.cm_id ";
				if ($_SESSION["__user_type"] != 'ADMINISTRATOR' and $_SESSION["__user_type"] != 'CENTRAL MIS') {
					$select_briefing .= " where a.CreatedBy='" . $_SESSION['__user_logid'] . "' ";
				}
				$select_briefing .= " order by a.id desc";
				$myDB = new MysqliDb();
				...
				$sqlConnect = "select a.*,b.EmployeeName,c.quiz,c.TotalQuestionNum from brf_acknowledge a   INNER JOIN  brf_briefing c ON a.BriefingId=c.id INNER JOIN personal_details b on a.EmployeeID=b.EmployeeID where a.BriefingId='" . $bf_id . "' order by a.id desc";
				} else {
					$sqlConnect = "select a.*,b.EmployeeName,c.quiz,c.TotalQuestionNum from brf_acknowledge a   INNER JOIN  brf_briefing c ON a.BriefingId=c.id INNER JOIN personal_details b on a.EmployeeID=b.EmployeeID where a.BriefingId='" . $bf_id . "' order by a.id desc";
				}
				//echo $sqlConnect;
								...
								<tbody>
									...
									foreach ($result as $key => $value) {
										...
										if ($QrowCount > 0) {
											...
											$select_attempted = $myDB->rawQuery("select a.*,b.Answer as ActualAnswer from brf_quiz_attempted a INNER JOIN brf_question b ON a.QuestionId=b.QuestionId where a.BriefingId='" . $bf_id . "' and a.EmployeeId='" . $value['EmployeeID'] . "'");
											$error = $myDB->getLastError();
											$rowCount = $myDB->count;
											if ($rowCount > 0) {
												$attempt_quiz = 'Yes';
