...
else
{
	...
	if($dshr > 0)
	{
		...
		if($_SESSION["__user_Desg"]=="CSA" || $_SESSION["__user_Desg"]=="Senior CSA")
		{
			/////////////////  Calculate DOD ///////////////////////////////
			...
			foreach($value as $k=>$val)
			{
				if($counter <= $date_counter)
				{
					if(strtoupper($val)  =='P' ||strtoupper($val) =='L' ||strtoupper($val) =='HO' ||strtoupper($val) =='CO' ||strtoupper($val) =='WO')
					{
						...
						{
							if(($_SESSION["__user_Desg"]=="CSA" || $_SESSION["__user_Desg"]=="Senior CSA") && $flag == 1)
							{
...
<!-- Header for Form If any -->
	...
	<?php } ?>
		...
		<span><h4 style="margin-top: -69px; margin-left: 47px; font-size: 20;"><?php echo date('F', mktime(0, 0, 0, $month, 10)) . ' - ' . $year ?> Salary Details</h4></span>
