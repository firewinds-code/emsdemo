...
if (isset($_SESSION)) {
	if (!isset($_SESSION['__user_logid'])) {
		...
		if ($isPostBack) {
			if (isset($_POST['txt_dateMonth'])) {
				$date_To = $_POST['txt_dateMonth'];
			} else {
				$date_To = date('M', time());
			}
			if (isset($_POST['txt_dateYear'])) {
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				<div class="input-field col s12 m12">
					<?php
					$myDB = new MysqliDb();
					//echo 'call get_performance_MIS("' . $_SESSION['__user_logid'] . '","' . $date_From . '","' . $date_To . '")';
					$rst_mis = $myDB->query('call get_performance_MIS("' . $_SESSION['__user_logid'] . '","' . $date_From . '","' . $date_To . '")');
					if ($rst_mis) {
						echo '<div class="row" >';
				...
				<div class="input-field col s12 m12">
					<?php
					$myDB = new MysqliDb();
					//echo 'call get_performance_Daily("' . $_SESSION['__user_logid'] . '","' . $date_From . '","' . $date_To . '")';
					$rst_daily = $myDB->query('call get_performance_Daily("' . $_SESSION['__user_logid'] . '","' . $date_From . '","' . $date_To . '")');
