...
date_default_timezone_set('Asia/Kolkata');
		...
		{
			$result=$myDB->rawQuery("SELECT AssignTo,cast(EndDate as date) Date,count(*) as total FROM modulemaster.activitylog where Status = 'Completed' and month(EndDate) = month(curdate())  group by AssignTo,cast(EndDate as date) order by EndDate ; ");
		}
		echo "SELECT AssignTo,cast(EndDate as date) Date,count(*) as total FROM modulemaster.activitylog where Status = 'Completed' and month(EndDate) = month(curdate())  group by AssignTo,cast(EndDate as date) order by EndDate ; ";
		...
		print_r($value_login);
		$array_data = array();
	////foreach(array_unique($label_login) as $val )
		//{
			foreach($value_login as $k=>$v)
			{
				//$array_data[$k][$val] = $value_login[$k][$val];
$array_data[$k][$v] = $value_login[$k][$v];
			}
