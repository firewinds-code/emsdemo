...
$myDB=new MysqliDb();
$data_exp = $myDB->query('
select t2.EmployeeID, t2.EmployeeName, location_master.location,t1.dol, t1.rsnofleaving,t1.disposition, t2.designation,clientname, t2.Process, t2.sub_process, t2.ReportTo, t2.account_head from exit_emp t1 inner join whole_dump_emp_data t2 on t2.EmployeeID = t1.EmployeeID    inner join location_master on location_master.id=t2.location where   cast(t1.createdon as date) = curdate() and emp_status = "Inactive" ');
$table = '';
if(count($data_exp) > 0 && $data_exp)
{
	$count= 0;
	$table = '<table border="1" colspacing=0><htead><tr><th>Sr No.</th><th>EmployeeID</th><th>EmployeeName</th><th>Location</th><th>Date Of Leaving</th><th>Reason Of Leaving</th><th>Disposition</th><th>Designation</th><th>Client</th><th>Process</th><th>Sub Process</th><th>Report To</th><th>Account Head</th></tr></thead><tbody>';
