...
if(isset($_POST['btn_add_new']))
{
	 ...
	 $fileFor = (isset($_POST['fileFor'])? $_POST['fileFor'] : null);
	 $processesSelected = (isset($_POST['subProcess'])? $_POST['subProcess'] : null);
	 $clientSelected = (isset($_POST['Client'])? $_POST['Client'] : null);
	 $LocationIDSelected = (isset($_POST['LocationID'])? $_POST['LocationID'] : null);
	 	
	$fileType=(isset($_POST['fileType'])? $_POST['fileType'] : null); //Banner Type
	$BannerType=(isset($_POST['BannerType'])? $_POST['BannerType'] : null);
	...
	if($filesCount > 0 && $fileType!="" && $fileType!=null &&  $BannerType!="" && $BannerType!=null && $fileFor!=null ){
		if(($fileFor == "Client" && $clientSelected!=null ) ||($fileFor == "LocationID" && $LocationIDSelected!=null )||  ($fileFor == "SubProcess" && $processesSelected!=null ) || $fileFor == "All" ){
			...
			if($fileFor == "Client" && $clientSelected!=null){				
				...
				$myDB=new MysqliDb();
				$sqlBy="select process, sub_process ,cm_id from new_client_master where client_name in ('".implode("','",$clientSelected)."');";
											...
											for($j=0 ; $j < count($processesSelected) ; $j++){
												...
												if($isUploaded == 1){
													...
													if(count($processSetailArray) == 3){
														...
														$myDB=new MysqliDb();
														$sqlBy="INSERT INTO ipp_details(cmid, process_name, banner_type, platform, file_name) VALUES ('".$processSetailArray[2]."', '".$processSetailArray[0].'|'.$processSetailArray[1]."', '".$BannerType."', 'WEB' , '".$fName."');";
...
<!-- Header for Form If any -->
	  <h4>Handle Photos
	  <a class="btn-floating btn-large waves-effect waves-light modal-trigger tooltipped" id="ContentAdd"  onclick="javascript:return openModelToAdd();" data-position="bottom" data-tooltip="Add Photo"><i class="material-icons">add</i></a></h4>	
