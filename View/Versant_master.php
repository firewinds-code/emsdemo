...
if(isset($_POST['cm_id'],$_POST['cert_name']) and trim($_POST['filename'])!="" and trim($_POST['cert_name'])!="" and $_POST['id']=='' )
{	
		...
		if($cm_id!=""  && $cert_name!="" && $filename!="" ){
			$select_query="select id from certification_require_by_cmid where cm_id='".$cm_id."' and cert_name='".$cert_name."' ";
			...
			{
				$insertQuery="INSERT INTO  certification_require_by_cmid set cert_name='".$cert_name."',filename='".$filename."',cm_id='".$cm_id."',createdby='".$ceatedBy."'";
...
{  
	...
	$myDB=new MysqliDb();				
	if($cm_id!=""  && $cert_name!="" && $filename!="" ){
		$updateQuery="Update  certification_require_by_cmid set cert_name='".$cert_name."',filename='".$filename."',cm_id='".$cm_id."',updatedby='".$ceatedBy."'  where id='".$id."'";
					    ...
					    <tbody>					        
					       ...
					       $i=1;
					        foreach($result as $key=>$val){
								...
								<a onclick="editData('<?php echo $ID; ?>','<?php echo $val['cert_name']; ?>','<?php echo $val['cm_id']; ?>','<?php echo $val['filename']; ?>','<?php echo $val['id']; ?>');" ><i class="material-icons edit_item imgBtn imgBtnEdit tooltipped" data-position="left" data-tooltip="Edit">ohrm_edit</i></a></td>
