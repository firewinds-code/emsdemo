...
if(isset($_POST['btn_Issue_Save']))
{
	...
	$Insert="update warning_rth set qh_status='".$_POST['IRTStatusHR']."',`Termination Date`='".$TerDate."',`qh_remark`='".addslashes($_POST['remark'])."',`qh_modifiedon`=now() where id='".$_POST['DataID']."';";
	$myDB=new MysqliDb();
	...
	<div class="schema-form-section row" >
	         ...
	         <h4 class="col s12 m12 model-h4">Refer to HR Requests</h4>
		      <div class="modal-body">
			   ...
			   <!-- <div class="input-field col s12 m12">
				...
				<div class="input-field col s12 m12">
					<textarea name="remark" id="remark" required maxlength="250" class="materialize-textarea"></textarea>
			    ...
			    <div class="input-field col s4 m4">
				            ...
				            <label for="IRTStatusAH"  class="active-drop-down active">Status</label>
			    </div>
        ...
        <form method="post">
		     ...
		     <div id="pnlTable">
				...
				{
					$sqlConnect = "SELECT * FROM warning_rth where `ah_status`='".$_POST['SearchBy']."'and qh_status is null;";
					/*if($_SESSION["__location"]=="1")
					{
						 $sqlConnect = "SELECT wr.* FROM warning_rth wr inner join personal_details pd on wr.EmployeeID = pd.EmployeeID where wr.ah_status = '".$_POST['SearchBy']."' and wr.hr_status is null and pd.location in (1,2);";
					}
					else
					{
						 $sqlConnect = "SELECT wr.* FROM warning_rth wr inner join personal_details pd on wr.EmployeeID = pd.EmployeeID where wr.ah_status = '".$_POST['SearchBy']."' and wr.hr_status is null and pd.location in ('".$_SESSION["__location"]."');";
					}*/
					//echo $sqlConnect;
...
$('#IRTStatusHR').change(function()
	{
		...
		{
			$("#remark").attr('minlength','10');
		}
		else
		{
			$("#remark").attr('minlength','250');
		}
	});
...
function EditData(el)
{
	...
	$('#myModal_content').modal('open');
		...
		$('select').formSelect();
		 $.ajax({ url: <?php echo '"'.URL.'"';?>+"View/bindDocIR.php?empid="+EmployeeID_ }).done(function(data) {$('#AppendDoc').html(data)});
