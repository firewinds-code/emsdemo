...
if(isset($_POST['btnSave']))
{
	...
	$disposition=trim($_POST['txt_Disposition']);
	
	if($disposition=='Better career prospect')
	{
		$para1=trim($_POST['text_company']);
		$para2=trim($_POST['text_location']);
		$para3=trim($_POST['text_level']);
		$para4=trim($_POST['bc_date']);
		$para5=trim($_POST['text_ctc']);
	...
	{
		$para1=$fdid_apply_leave=trim($_POST['resion_apply_leave']);
		$para2=trim($_POST['whome_did_you_apply']);
		$para3=trim($_POST['issue_resolve_rejoin']);
		$para4=trim($_POST['family_date']);
		if($para1=="" )
		{
			echo "<script>$(function(){ toastr.info('Please fill all the fields') }); </script>";
		}		
	...
	{
		$para1=trim($_POST['learning']);
		$para2=trim($_POST['joinback']);
		$para3=trim($_POST['join_date']);
		if($para1=="")
		{
			echo "<script>$(function(){ toastr.info('Please fill all the fields') }); </script>";
		}	
	...
	{
		$para1=trim($_POST['willing_work']);
		$para2=trim($_POST['mpref_location']);
		$para3=trim($_POST['when_date']);
		if($para1=="")
		{
			echo "<script>$(function(){ toastr.info('Please fill all the fields') }); </script>";
	...
	{
		$para1=trim($_POST['pref_location']);
		$para2=trim($_POST['pref_shift']);
		$para3=trim($_POST['distance_join']);
		$para4=trim($_POST['joinback_date']);
	...
	{
		$para1=trim($_POST['oprocess_offered']);
		$para2=trim($_POST['decertify_joinback']);
		$para3=trim($_POST['dcerti_join']);
	...
	$contact_status=($_POST['txt_ContactStatus']);
...
<div id="search">
		...
		<div class="input-field col s2 m2">            
		    ...
		    <input type="hidden" name="flag" id="flg" value="<?php echo $_GET['flg']; ?>" ?>
		</div>
 </div>  
...
<!--Reprot / Data Table start -->
    ...
    if($sdate!="" && $edate!="" )
    {
    	$sqlConnect="call  get_inactive_emp_list('".$_SESSION['__user_logid']."','".$_GET['flg']."','".$sdate."','".$edate."')";
    	$myDB=new MysqliDb();
