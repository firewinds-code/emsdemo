...
<span id="PageTittle_span" class="hidden">Upload IOS User List</span>
...
<!-- Header for Form If any -->
	 <h4>Upload IOS User<!--<a class="btn-floating btn-large waves-effect waves-light modal-trigger tooltipped"  href="#myModal_content" data-position="bottom" data-tooltip="Download Formate"><i class="material-icons">file_download</i></a>--></h4>				
	...
	<div class="schema-form-section row" >		
				   	...
				   	<div class="input-field col s5 m5">
			            <select class="form-control" id="txt_Type_Upload"  name="txt_Type_Upload" >
			            	<option value="Hours">IOS User</option>
			            </select>
					...
					// if everything is ok, try to upload file
					    ...
					    if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file))
					    {
					     echo "<script>$(function(){ toastr.success('The file ".basename( $_FILES["fileToUpload"]["name"])." has been uploaded'); }); </script>";					        $document = PHPExcel_IOFactory::load($target_file);
							...
							{
								$myDB=new MysqliDb();
								$myDB->query("Truncate table ios_user;");
								$row_counter = 0;
							...
							{							 	
								 	...
								 	{	
								 			//$date_month = date_parse($row['AG']);
 								 			$sql_p_inert='CALL insert_ios("'.strtoupper($row['A']).'");';
