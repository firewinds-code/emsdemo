			...
			<div class="schema-form-section row">
				...
				if (!empty($_POST['txt_Process']) && $_POST['txt_Process'] != 'NA') {
					...
					if ($_SESSION['__user_type'] == 'ADMINISTRATOR' || $_SESSION['__user_type'] == 'CENTRAL MIS') {
						...
						if ($_POST['txt_Process'] == 'ALL') {
							$query = "select status_table.EmployeeID ,wh.des_id,
case 
...
where  wh.cm_id ='" . $_POST['txt_Process'] . "'";
						}
					} else {
						$query = "select status_table.EmployeeID ,wh.des_id,
case 
...
left outer join personal_details pdrt on  wh.ReportTo = pdrt.EmployeeID 
left outer join personal_details pdqaops on  wh.Qa_ops = pdqaops.EmployeeID
 where ( wh.ReportTo = '" . $_SESSION['__user_logid'] . "' or wh.EmployeeID = '" . $_SESSION['__user_logid'] . "' or wh.account_head = '" . $_SESSION['__user_logid'] . "' or wh.oh = '" . $_SESSION['__user_logid'] . "'  or wh.qh = '" . $_SESSION['__user_logid'] . "' or wh.th = '" . $_SESSION['__user_logid'] . "' ) and wh.cm_id ='" . $_POST['txt_Process'] . "'";
					}
					...
					if (count($chk_task) > 0 && $chk_task) {
						...
						foreach ($chk_task as $key => $value) {
							...
							if ($value['des_id'] != '9' && $value['des_id'] != '12' && $value['EmployeeID'] != $_SESSION['__user_logid'] && $_POST['txt_Process'] != 'ALL') {
