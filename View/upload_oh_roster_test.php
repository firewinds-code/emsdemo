...
<script>
	$(function(){
		$('.textDanger').css({ 'color': 'rgb(187, 15, 15)','font-weight': 'bold'});	
		$('#myTable').DataTable({
				        dom: 'Bfrtip',
				        lengthMenu: [
				            [ 5,10, 25, 50, -1 ],
...
?>
			...
			<div class="col s12 m12" id="main_container_upload">
			 		       <div class="input-field col s12 m12">
								...
								<?php 
									$myDB =new MysqliDb();
									$rowData = $myDB->query('select distinct Process,sub_process from new_client_master where oh="'.$_SESSION['__user_logid'].'"');
									if(count($rowData) > 0)
									{
										if($dept == 'ALL Process')	
										{
...
else
{
	?>
		...
		<div class="col s12 m12" id="count_container">
		<h1 style="text-shadow: 2px 2px 1px rgb(183, 183, 183);font-size: 30px;border: 1px solid #0c7696;border-radius: 3px;padding: 5px;float: left;margin-right: 20px;margin-top: 50px;color: #0c7696;">Time left </h1><div class="clock" style="margin:2em;"></div>
		</div>
