            ...
            <div class="schema-form-section row">
                ...
                if ($send) {
                    ...
                    foreach ($_SESSION['tempemp'] as $key => $value) {
                        $emp = $value;
                        $chkflag = '';
                        $select = "select FirstName,MiddleName, LastName, FatherName,dob, Gender,mobile,emailid, location,cm_id,case when df_id in (74,77,146, 147,148,149) then 'CSA' else 'Support' end as des,EmployeeName,dateofjoin,address_p from personal_details t1 left join contact_details t2 on t1.employeeid=t2.EmployeeID join employee_map t3 on t1.EmployeeID=t3.EmployeeID left join address_details t4 on t1.EmployeeID=t4.EmployeeID where t1.EmployeeID=? and emp_status='Active'";
                        $htmlresp = $htmlresp  . $emp;
                        ...
                        if ($_POST['bgv'] == '1') {
                            ...
                            if ($EmployeeName &&  $dob &&  $doj && $father_name && $location && $sla_id != '') {
                                ...
                                $data['sla_id'] = $sla_id;
                                $data['filelink'] = "https://ems.cogentlab.com/erpm/Services/Docs_Inactivezip/$emp.zip";
