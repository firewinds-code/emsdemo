	...
	{	$date=date("Y-m-d h:i:s");
		...
		{	
			...
			else
			{
				...
				echo "<script>$(function(){ toastr.error('Data Not Updated ::Error :- ".$mysql_error."'); }); </script>";
			}
	...
	<div class="schema-form-section row" >
			 ...
			 <div id="pnlTable">
			    ...
			    if(isset($_GET['process']) && $_GET['process']!=""){
					...
					//$new_client_name='11';
			     	$old_client_name=$_GET['process'];
				 	$sqlConnect =" select a.EmployeeID,b.id as moveid , a.EmployeeName,a.designation,a.emp_level,concat(a.clientname,' | ',a.Process,' | ',a.sub_process) as Client,DATE_FORMAT(b.move_date,'%Y-%m-%d') AS move_date,DATE_FORMAT(b.created_on,'%Y-%m-%d') AS created_on,b.status,b.AH_comment,b.tr_required from whole_details_peremp a,tbl_oh_tooh_move b where a.EmployeeID=b.EmployeeID and b.new_cm_id='".$old_client_name."' and b.flag='toAH' and b.status='Pending' and b.move_location='".$Loggin_location."'";
			    	$myDB=new MysqliDb();
					      	...
					      	<?php
									...
									foreach($resultBy as $key=>$data_array){
										...
										if($data_array['AH_comment']!=""){
											$ahComment=addcslashes($data_array['AH_comment']);
										}
										echo '<tr>';	
										echo "<td >".$j."</td>";						
