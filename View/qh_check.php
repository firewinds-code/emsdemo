...
if (isset($_POST['btnSave'])) {
	...
	$status = trim($_POST['txt_thcheck_Quality']);
			...
			if ($count_check > 0) {
				...
				if ($_POST['batch_id'] > 0 || !empty($_POST['batch_id'])) {
					foreach ($_POST['cb'] as $val) {
						...
						$myDB = new MysqliDb();
						$save = 'call manage_status_qh("' . $empID . '","' . $status . '","' . $createBy . '",' . $_POST['batch_id'] . ',"' . $_POST['txt_Remark_' . $empID] . '","QH","' . $_POST['txt_Date_crt_1'] . '","' . $_POST['txt_Day_crt_1'] . '","' . $roster_log . '")';
	...
	if (isset($_POST['cb'])) {
		...
		if ($count_check > 0) {
			...
			foreach ($_POST['cb'] as $val) {
				$empID = $val;
				$myDB = new MysqliDb();
				$save = 'call manage_refer_hr("' . $empID . '","' . $createBy . '","' . $_POST['txt_Remark_' . $empID] . '","QH REFER")';
				$resulti = $myDB->query($save);
				$mysql_error = $myDB->getLastError();
				if (empty($resulti)) {
		...
		if ($count_check > 0) {
			...
			foreach ($_POST['cb'] as $val) {
				...
				if (count($check_rtr_q) > 0 && $check_rtr_q) {
					...
					$myDB = new MysqliDb();
					$save = 'call manage_retrain("' . $empID . '","' . $createBy . '","' . $_POST['txt_Remark_' . $empID] . '","QH RETRAIN")';
					$resulti = $myDB->query($save);
					$mysql_error = $myDB->getLastError();
					if (empty($mysql_error)) {
			...
			<div class="schema-form-section row">
				<div class="form-inline">
					...
					<div class="input-field col s6 s6">
						<select id="text_trcheck_Batch" name="text_trcheck_Batch">
							...
							if ($_SESSION['training_approver'] != 'No') {
								$sqlBy = 'SELECT  distinct  batch_master.BacthID,batch_master.BacthName,batch_master.cm_id FROM personal_details left outer join status_table on status_table.EmployeeID=personal_details.EmployeeID  left outer join batch_master on batch_master.BacthID=status_table.BatchID join employee_map t1 on t1.EmployeeID=personal_details.EmployeeID join new_client_master t2 on t1.cm_id=t2.cm_id where  status_table.EmployeeID !="' . $_SESSION['__user_logid'] . '" and status_table.Status=4 and t1.emp_status="Active"  and BacthName is not null and t2.client_name in (select client_id from training_master where Approver_id="' . $_SESSION['__user_logid'] . '") order by BacthName';
							} else {
								$sqlBy = 'SELECT  distinct  batch_master.BacthID,batch_master.BacthName,batch_master.cm_id FROM personal_details left outer join status_table on status_table.EmployeeID=personal_details.EmployeeID  left outer join batch_master on batch_master.BacthID=status_table.BatchID join employee_map t1 on t1.EmployeeID=personal_details.EmployeeID join new_client_master t2 on t1.cm_id=t2.cm_id where t2.qh="' . $_SESSION['__user_logid'] . '" and  status_table.EmployeeID !="' . $_SESSION['__user_logid'] . '" and status_table.Status=4 and t1.emp_status="Active"  and BacthName is not null and t2.client_name not in (select client_id from training_master) order by BacthName';
							}

							//$sqlBy = 'SELECT  distinct  batch_master.BacthID,batch_master.BacthName FROM personal_details left outer join status_table on status_table.EmployeeID=personal_details.EmployeeID  left outer join batch_master on batch_master.BacthID=status_table.BatchID where  status_table.EmployeeID !="' . $_SESSION['__user_logid'] . '" and status_table.Status=4 and personal_details.location="' . $_SESSION['__location'] . '" and BacthName is not null order by BacthName;';
							$batch_id = 0;
							$myDB = new MysqliDb();
					...
					<div id="pnlTable" class="col s12 m12 card">
						...
						$sqlConnect = 'call get_qhchecklist("' . $_SESSION['__user_logid'] . '",' . $batch_id . ')';
						$myDB = new MysqliDb();
...
<script>
	...
	function getBatch_state() {
		...
		//alert(location);
		var userid = <?php echo "'" . $_SESSION["__user_logid"] . "'"; ?>;

		$.ajax("../Controller/getBatchState.php?type=ojt&id=" + batchid + "&loc=" + location + "&userid=" + userid + "&lvl=" + approver).done(function(result) {
