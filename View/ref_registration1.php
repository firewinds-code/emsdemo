...
if (isset($_POST['btn_df_Save'])) {
	...
	$query_insert = 'INSERT INTO tbl_reference_reg_detail(`EmployeeID`,`EmployeeName`,`Designation`,`Client`,`Process`,`SubProcess`,`CandidateName`,`CandidateNumber`,`CandidateAddress`,`Remark`,`CandidateLevel`,`createdby`,`RefID`,`Agreed`,source) VALUES("' . $EmployeeID . '","' . $EmployeeName . '","' . $Designation . '","' . $Client . '","' . $Process . '","' . $SubProcess . '","' . addslashes($CandidateName) . '","' . $CandidateNumber . '",(select location from location_master where id="' . $loc . '"),"' . $Remark . '","' . $CandidateLevel . '","' . $createdby . '","' . $ID . '","' . $Agreed . '","Web");';
	$myDB = new MysqliDb();
	...
	if (empty($myError)) {
		...
		echo "<script>$(function(){ toastr.error('Reference not registered. Some error occurred...'" . $myError . ") }); </script>";
	}
}
?>
<script>
