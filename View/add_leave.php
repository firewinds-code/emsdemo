...
if (strtoupper($_SERVER['REQUEST_METHOD']) != 'POST' && isset($_REQUEST['id']) && $_REQUEST['req'] == 'HOD') {
	...
	if (count($result) > 0 && $result) {
		...
		$DateFrom = $result[0]['DateFrom'];
		$HRStatusID = $result[0]['HRStatusID'];
		$MngrStatusID = $result[0]['MngrStatusID'];
		$aaprovedbyoh = $result[0]['HR'];
		$emp_dept = $result[0]['sub_process'];
		$Level = $result[0]['level'];
		$l1empid = $result[0]['l1empid'];
		$l2empid = $result[0]['l2empid'];
		$Reportto = $result[0]['ReportTo'];
		$_ohcomment = '';
		$disable = 'disabled="true"';
		$hd_comment_hidden = '';
		$hod_level = $btnAdd = $btnOPS = $btnSave = 'hidden';

		if ($HRStatusID == "Pending" && $Reportto == "CE07147134" && $Reportto == $_SESSION['__user_logid']) {
			$hod_level = $btnSave = '';
			$hd_comment_hidden = 'hidden';
			$leave_Level = 'hidden';
			$aaprovedby = $_SESSION['__user_logid'];
		} else if ($HRStatusID == "Pending" && $Level == "1" && $l1empid == $_SESSION['__user_logid']) {
			...
			$leave_Level = 'hidden';
			$aaprovedby = $_SESSION['__user_logid'];
		} else if ($HRStatusID == "Pending" && $Level == "2" && $l1empid == $l2empid && $l1empid == $_SESSION['__user_logid']) {
			if ($Reportto == "CE07147134") {
				$hod_level = $btnSave = 'hidden';
				$hd_comment_hidden = '';
			} else {
			...
			$leave_Level = 'hidden';
			$aaprovedby = $_SESSION['__user_logid'];
		} else if ($HRStatusID == "Pending" && $Level == "2" && $l1empid == $_SESSION['__user_logid']) {
			$aaprovedby = $_SESSION['__user_logid'];
			...
			$leave_Level = 'hidden';
		} else if (($HRStatusID == 'Decline' || $HRStatusID == 'Approve') && $Level == "2" && $l2empid == $_SESSION['__user_logid']) {
			if ($Reportto == "CE07147134") {
				$hod_level = $btnSave = 'hidden';
				$hd_comment_hidden = '';
			} else {
		...
		} else if (($HRStatusID == 'Decline' || $HRStatusID == 'Approve') && $Level == "2" && $Reportto == "CE07147134" && $Reportto == $_SESSION['__user_logid']) {
			$hod_level = $btnSave = '';
			$hd_comment_hidden = 'hidden';
			$leave_Level = 'hidden';
			$aaprovedby = $_SESSION['__user_logid'];
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				<div id="pnlTable">
					...
					<?php
					$sqlConnect = 'call get_pendingleave_new("' . $_SESSION['__user_logid'] . '")';
					$myDB = new MysqliDb();
