			...
			<div class="schema-form-section row">
				...
				if (isset($_POST['btn_view'])) {
					...
					if ($_POST['ddl_clfs_Process'] == 'ALL') {
						$chk_task = $myDB->query("select rpt_payroll_locked.* from rpt_payroll_locked where  rpt_payroll_locked.`Salary Month` = '" . date("F Y", strtotime($DateTo)) . "';");
					} else {

						$chk_task = $myDB->query("select rpt_payroll_locked.* from rpt_payroll_locked where  (select cm_id from new_client_master nt inner join client_master on client_master.client_id = nt.client_name where rpt_payroll_locked.`Sub Process` = nt.sub_process and rpt_payroll_locked.`process` = nt.`process` and client_master.client_name = rpt_payroll_locked.`Client`)= '" . $_POST['ddl_clfs_Process'] . "' and rpt_payroll_locked.`Salary Month` = '" . date("F Y", strtotime($DateTo)) . "' ;");
					}
					echo "select rpt_payroll_locked.* from rpt_payroll_locked where  (select cm_id from new_client_master nt inner join client_master on client_master.client_id = nt.client_name where rpt_payroll_locked.`Sub Process` = nt.sub_process and rpt_payroll_locked.`process` = nt.`process` and client_master.client_name = rpt_payroll_locked.`Client`)= '" . $_POST['ddl_clfs_Process'] . "' and rpt_payroll_locked.`Salary Month` = '" . date("F Y", strtotime($DateTo)) . "' ;";
					$counter = 0;
