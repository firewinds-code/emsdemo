	...
	function getEmailID($empid){
		if($empid!=""){
			...
			if($email_array && $rowCount>0){
				if($email_array[0]['ofc_emailid']!=""){
					$employeeEmail=$email_array[0]['ofc_emailid'];
				}
				return $employeeEmail;
			}
		}
	}
	$AH_ID=$_SESSION['__user_logid'];
	if(isset($_POST['cb']))
	{
		...
		{	
			...
			for($p=$max_key;$p>=$min_key; $p--)
			{
				if(isset($checked_arr[$p])){
					...
					$dateofjoin=$_POST['dateofjoin'][$p];
					$AH_status=$_POST['AH_status'][$p];
					$client_name=$_POST['client_name'][$p];
					$process=$_POST['process'][$p];
					$subprocess=$_POST['subprocess'][$p];
					$employee_name=$_POST['empname'][$p];
					$empID=$checked_arr[$p];
					if(	$empID!="" ){
						$myDB=new MysqliDb();
						...
						if($rowCount2>0 && $AH_status=='Approve'){
							...
							$OH_ARRAY[]=$_POST['oh'][$p];
							$AH_ARRAY[]=$_POST['account_head'][$p];
							$HR_ARRAY[]=$_POST['HR_ID'][$p];
						}
