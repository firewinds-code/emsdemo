				...
				for ($p = $max_key; $p >= $min_key; $p--) {
					if (isset($checked_arr[$p]) && $checked_arr[$p] != "") {
						...
						if (empty($mysql_error) && $rowCount > 0) {
							...
							if (empty($mysql_error) && $rowCount > 0) {
								$mysquery = "select cert_name,cm_id from certification_require_by_cmid where cm_id='" . $new_client . "'";
								$versantflag = 0;
								$myDB = new MysqliDb();
								...
								if ($versantflag == 1) {
									...
									//$myDB = new MysqliDb();
									//$testname = $myDB->rawQuery($mysquery);
									$query = $myDB->rawQuery("SELECT * FROM bgv_matrix where cm_id = '" . $new_client . "' and desig = (select case when df_id in (74,77,146, 147,148,149) then 'CSA' else 'Support' end as desig from employee_map where EmployeeID='" . $empID . "') and (Addr='Yes' or Edu='Yes' or Emp='Yes' or Crim='Yes');");
					...
					?>
						<div class="input-field col s6 m6 8">
							<select id="client_to" name="new_client" required>
								...
								$sqlBy2 = "select ncm.cm_id,concat(cm.client_name,' | ',ncm.tprocess,' | ',ncm.sub_process) as Client from client_master cm inner join 
(select Distinct process as tprocess,sub_process, client_name,cm_id from new_client_master  where cm_id Not IN(select  cm_id  from client_status_master) and location='" . $Loggin_location . "' and cm_id!='" . $cm_id . "') ncm on cm.client_id=ncm.client_name order by cm.client_name";
								$myDB = new MysqliDb();
