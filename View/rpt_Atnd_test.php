	...
	<div class="pim-container row" id="div_main">
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
						...
						<div class="modal-body">
							...
							<div class="input-field col s4 m4">
								<Select name="txt_dept" id="txt_dept" required>
									<?php
									if ($_SESSION['__user_type'] == 'ADMINISTRATOR' || $_SESSION['__user_type'] == 'CENTRAL MIS' || $_SESSION['__user_type'] == 'HR' || ($_SESSION['__user_type'] == 'HR' &&  $_SESSION['__status_ah'] == $_SESSION['__user_logid']) || $_SESSION['__user_logid'] == 'CE081930104' || $_SESSION['__user_logid'] == 'CE12102224') {
										...
										$myDB = new MysqliDb1();
										$rowData = $myDB->query('select distinct Process from new_client_master where account_head="' . $_SESSION['__user_logid'] . '" or th="' . $_SESSION['__user_logid'] . '" or qh="' . $_SESSION['__user_logid'] . '" or oh="' . $_SESSION['__user_logid'] . '"  or VH="' . $_SESSION['__user_logid'] . '" ');
							...
							<div class="input-field col s12 m12 right-align">
								...
								<?php if ($_SESSION['__user_type'] == 'HR' || $_SESSION['__user_type'] == 'ADMINISTRATOR' || $_SESSION['__user_type'] == 'CENTRAL MIS' || $_SESSION['__user_logid'] == 'CE12102224') {  ?>
				...
				$myDB = new MysqliDb1();
				$type = '';
				if (($_SESSION['__user_type'] == 'HR' &&  ($_SESSION['__status_ah'] == $_SESSION['__user_logid'] || $_SESSION['__user_type'] == 'HR'))) {
					$type = 'ADMINISTRATOR';
				} else {
					$type = $_SESSION['__user_type'];
				}
				//echo 'call sp_get_atnd_Report("'.$_SESSION['__user_logid'].'","'.$date_To.'","'.$date_From.'","'.$dept.'","'.$type.'","'.$_SESSION["__location"].'")'; 
				$loc = '';
				if (isset($_SESSION["__location"])) {
					$loc = $_SESSION["__location"];
				}

				$chk_task = $myDB->query('call sp_get_atnd_Report_test("' . $_SESSION['__user_logid'] . '","' . $date_To . '","' . $date_From . '","' . $dept . '","' . $type . '","' . $loc . '")');


				//echo 'call sp_get_atnd_Report_test("' . $_SESSION['__user_logid'] . '","' . $date_To . '","' . $date_From . '","' . $dept . '","' . $type . '","' . $loc . '")';
				...
				<tr>';
					...
					foreach ($chk_task as $key => $value) {
						...
						$temp_result  = $myBD_tmp->query('select rsnofleaving,dol,disposition,createdon from exit_emp where EmployeeID  = "' . $value['EmployeeID'] . '" order by id desc limit 1');
						...
						if (!empty($temp_result[0]['rsnofleaving']) && $value['emp_status'] == 'InActive' && !empty($temp_result[0]['dol'])) {
							if ($temp_result[0]['rsnofleaving'] == 'RES') {
								$table .= '<td>' . date('Y-m-d', strtotime($temp_result[0]['createdon'])) . '</td>';
							} else {
