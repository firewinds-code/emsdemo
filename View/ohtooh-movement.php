		...
		if ($old_process != "") {
			for ($i = $max_key; $i >= $min_key; $i--) {
				...
				if (isset($checked_arr[$i]) && $checked_arr[$i] != "" && $_POST['oh_comment'][$i] != "") {
					...
					$empID = $checked_arr[$i];
					$oh_comment = $_POST['oh_comment'][$i];
					if ($flag != "") {
						$save = "UPDATE tbl_oh_tooh_move set status='" . $status[$i] . "',flag='" . $flag . "',OH_updated_on='" . $date . "',oh_comment='" . $oh_comment . "',OH_updated_by='" . $updatedBy . "' , Updated_by='" . $updatedBy . "',updated_on='" . $date . "' where  EmployeeID='" . $checked_arr[$i] . "' and id='" . $moveid . "' and move_location='" . $Loggin_location . "'";
						$myDB = new MysqliDb();
						$resulti = $myDB->rawQuery($save);
						$mysql_error = $myDB->getLastError();
						$rowCount = $myDB->count;
						if ($rowCount > 0) $counttol++;
					}
				}
			}
			...
			if ($old_process != "") {
				...
				for ($p = $max_key; $p >= $min_key; $p--) {
					if (isset($_POST['tcid'][$p]) && $_POST['tcid'][$p] != "") {
						...
						$mysquery = "select ctc,designation,des_id from salary_details s inner join whole_details_peremp w on w.EmployeeID=s.EmployeeID where s.EmployeeID='" . $empID . "' and ( CAST(SUBSTRING_INDEX(ctc, '-', -1) AS UNSIGNED)  <= (select CAST(SUBSTRING_INDEX(max_lim, '-', -1) AS UNSIGNED) as maxslab from tbl_salary_slab_by_cps  where cm_id='" . $new_cm_id . "') or w.des_id not  in (9,12,33,34,35,36) )";

						$myDB = new MysqliDb();
						$salary_array = $myDB->query($mysquery);
						if (count($salary_array) > 0) {
							$versantflag = 0;
							if ($salary_array[0]['des_id'] == '9' || $salary_array[0]['des_id'] == '12' || $salary_array[0]['des_id'] == '33' || $salary_array[0]['des_id'] == '34' || $salary_array[0]['des_id'] == '35' || $salary_array[0]['des_id'] == '36') {
								...
								if (empty($mysql_error) && $versantrowCount > 0) {
									...
									if (empty($mysql_error) && $testrowCount > 0) {
										if ($versantrowCount == $testrowCount) {
											$flag1 = '';
							...
							if ($versantflag == 1) {
								...
								$bgvflag = 0;
								$query = $myDB->rawQuery("SELECT * FROM bgv_matrix where cm_id = '" . $new_cm_id . "' and desig = (select case when df_id in (74,77,146, 147,148,149) then 'CSA' else 'Support' end as desig from employee_map where EmployeeID='" . $empID . "') and (Addr='Yes' or Edu='Yes' or Emp='Yes' or Crim='Yes');");
								if ($myDB->count > 0) {
				...
				if (($counttol > 0) && empty($mysql_error)) {
					...
					if ($notSalaryslab != "") {
						...
						echo "<script>$(function(){ toastr.error('Employee Not Transfter ::Error :- <code>'" . $mysql_error . "'</code>'); }); </script>";
					}
				}
			} else {
				...
				<div id="pnlTable">
					...
					if (isset($_GET['process']) && $_GET['process'] != "") {
						...
						if ($action == 'tr') {
							...
							$sqlConnect = "select id,cm_id,EmployeeID,EmployeeName,client_name,clientname,Process,sub_process,designation,emp_level,emp_status from whole_details_peremp where 	process='" . $process . "'and cm_id='" . $cm_id . "' and status='6' and location='" . $Loggin_location . "' and EmployeeID NOT IN(select EmployeeID from tbl_oh_tooh_move where old_process='" . $process . "' and cm_id='" . $cm_id . "' and(flag<>'AHR' and flag<>'NPR' and flag<>'FM')) and  EmployeeID NOT IN(select resign_details.EmployeeID from resign_details inner join employee_map on employee_map.EmployeeID = resign_details.EmployeeID where rg_status > 0 and rg_status < 9 and accept = 1 and employee_map.emp_status = 'Active' and employee_map.cm_id = '" . $cm_id . "' and final_acceptance is null) ";
					...
					if ($action == 'acc') {
							...
							$sqlConnect = "select b.AH_comment, b.oh_comment,b.id as moveid,a.id,a.EmployeeID,a.EmployeeName,b.status,a.client_name,a.clientname,a.cm_id,a.Process,a.sub_process ,a.designation,a.emp_level,a.emp_status from whole_details_peremp a, tbl_oh_tooh_move b where b.EmployeeID=a.EmployeeID and	b.new_cm_id='" . $cm_id . "' and b.flag='toOH' and b.status='AHApprove' and a.location='" . $Loggin_location . "'  ";
						}
						$myDB = new MysqliDb();
