	...
	if(isset($_POST['cb']))
	{
		...
		{		
				foreach($_POST['cb'] as $val)
				{
					...
					$save='call insert_tr_dailyrpt("'.$EmpID.'","'.$createBy.'","'.$Remark.'","'.$batch_id.'")';	
					...
					if(empty($mysql_error))
					{
						...
						{
							$myDB = new MysqliDb();
							$insert1='call manage_status_tr1("'.$EmpID.'","NO","'.$Remark.'","0","'.$createBy.'","")';
							$myDB->query($insert1);
							$myDB = new MysqliDb();
							$insert2='call manage_status_tr("'.$EmpID.'","NO")';
							$myDB->query($insert2);
						}
...
<span id="PageTittle_span" class="hidden">Manage Employee Trainer Daily LOG</span>
	...
	<div class="schema-form-section row" >
			 ...
			 <div class="form-inline" >
			    ...
			    <?php 
					...
					$sqlConnect='call get_trchecklist_daily("'.$_SESSION['__user_logid'].'",'.$batch_id.')';
					//echo $sqlConnect;
			    	$myDB=new MysqliDb();
			   		...
			   		<div class="flow-x-scroll">
				        ...
				        <thead>
				          <tr>
				            ...
				            <th >Not Trainable (To TH)</th>
				            <th class="hidden">Employee ID</th>
				            
				            <th class="hidden">Employee ID</th>
				            <th class="">Employee Name</th>
				            <th class="">Training Status</th>						            
				            <th class="hidden">Certification Level</th>
				            <th class="hidden">Total Certifications Levels</th>
					    ...
					    <tbody>					        
					       ...
					       $count=0;
							...
							{
								echo '<td class="retrain_flag">Re- Training</td>';		
							}				
							else
							{
								echo '<td class="retrain_flag">Training</td>';	
							}
							
							echo '<td class="cirtification_level  hidden">'.$value['cirtification_level'].'</td>';			
							echo '<td class="levelno  hidden">'.$value['no_of_Certification'].'</td>';					
							echo '<td class="date_cer_1 ">'.date('d M,Y',strtotime($value['date_cer_1'])).'</td>';
						
							echo '<td class="hidden input-field" style="padding:0px;"><input type="input" name="txt_EmployeeID_'.$count.'" id="txt_EmployeeID_'.$count.'" readonly="true"  value="'.$value['EmployeeID'].'" /></td>';
							
							echo '<td class="edit_info_body hidden input-field"  style="padding:0px;"><textarea name="txt_Remark_'.$count.'" class="materialize-textarea" id="txt_Remark_'.$count.'"  title="Remark">NA</textarea></td>';
