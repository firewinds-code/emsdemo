...
$statusdate=date('d');
//if(!($statusdate >='23'  && $statusdate<='25' &&  $_SESSION['__status_oh']==$_SESSION['__user_logid'])){
if(!($statusdate >='16'  && $statusdate<='20' &&  $_SESSION['__status_oh']==$_SESSION['__user_logid'])){
	$location= URL.'Login'; 
...
$operation_head= $_SESSION['__user_logid'];
//$operation_head='CE121621933';
...
<span id="PageTittle_span" class="hidden">RYG Status Operation Head</span>
...
<!-- Header for Form If any -->
	 <h4>RYG Status Operation Head </h4>	
	...
	<div class="schema-form-section row" >
		<div id="pnlTable">
			  <div class="had-container pull-left row card" >
				<div class=""  >
			    <?php 
			     $query="SELECT distinct(ep.EmployeeID),ncm.oh,pd.EmployeeName,ryg.ryg_status,ryg_substatus,ryg_remark,rstatus,rsubstatus,rremark,pd.location,cm.client_name,ncm.`process`,ncm.`sub_process`  from employee_map ep  inner join personal_details pd on ep.EmployeeID=pd.EmployeeID inner join new_client_master ncm on ncm.cm_id=ep.cm_id inner Join client_master cm on cm.client_id=ncm.client_name  left join ( select * from ryg_oh    where   Month(created_on)=MONTH(CURRENT_DATE()) and YEAR(created_on)=YEAR(CURRENT_DATE()) ) ryg on ep.EmployeeID=ryg.EmployeeID left JOIN  ( select EmployeeID,ryg_status as rstatus,ryg_substatus as rsubstatus,ryg_remark  as rremark from ryg_reportto where   Month(created_on)=MONTH(CURRENT_DATE()) and YEAR(created_on)=YEAR(CURRENT_DATE()) ) ryg_reportto  on ep.EmployeeID=ryg_reportto.EmployeeID where ep.emp_status='Active' and ncm.oh='".$operation_head."' and ep.EmployeeID!='".$operation_head."'";
					$myDB=new MysqliDb();
					...
					if(empty($mysql_error) && $rowCount>0){ ?>	
						  ...
						  <table id="myTable" class="data dataTable no-footer row-border" cellspacing="0" width="100%">
						    <thead >
						        <tr>
						            ...
						            <th>OH Remarks</th>      
						           	<th>TL Status</th>
						            <th class="ttttt">TL Sub Status</th> 
						            <!--<th> TL Remarks</th> -->
						           <!-- <th>Client</th>-->
						            <th>Process</th>
						            <th >Sub-Process</th>
					    ...
					    <input class='empclass hidden ' type='text' name='OHid'  id="OHid" value="<?php echo $operation_head; ?>" >				        
					      	<?php
									$i=0;
									$j=1;
									foreach($result as $key=>$data_array){
										...
										<!--<a ><i class="fa fa-lg fa-save"></i></a>-->
											<!--<button type='button' name='save'  class=" waves-green rygsave <?php if($data_array['ryg_remark']!=""){ ?>   savedclass <?php } ?> " style="min-width:15px !important;"  onclick="rygSave(<?php echo  $j; ?>)" id="sid<?php echo  $j; ?>" ><i class="fa fa-lg fa-save"></i></button>-->
									...
									<?php	}
										?>	
											...
											<td class="rstatus"><span class="<?php echo $colorclass; ?>" <?php if($data_array['rremark']!=""){ ?> title="<?php echo  $data_array['rremark'];?>" <?php }else{ ?> title="Pending" <?php } ?>><i class="fa fa-lg fa-circle"></i></span></td>
											<td class="rsubstatus ttttt"><?php echo  $rsubstatus;?></td>
...
<script>
	...
	function rygSave(id){
		...
		if(sval!="" && substatus!="" && OHid!=""){
			if(remarks!="")
			{
	   			$.ajax({
				  method: "POST",
				  url: '../Controller/saveRYGOHstatus.php',
				  data: {'saves':'saves','rygstatus':sval,'substatus':substatus,'rygsr':remarks,'empid':empid,'oh':OHid}
				}).done(function( msg ) {
