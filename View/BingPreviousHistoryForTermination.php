...
date_default_timezone_set('Asia/Kolkata');
	...
	{
 /*echo $sqlConnect="SELECT i.id,concat('On ',ah_Datetime,', ',ah_remark)ah_remark,concat('On',hr_Datetime,', 
',hr_remark)hr_remark, group_concat(Document)`File` FROM irtable i inner join ir_termination_documents d on i.EmployeeID=d.EmployeeID
 and i.id=d.DataId where i.EmployeeID='".$_REQUEST['empid']."' group by d.DataId";*/
  $sqlConnect="SELECT i.id,ah_Datetime,ah_remark,ah_status,hr_Datetime,hr_remark,hr_status, group_concat(Document)`File` FROM warning_rth i inner join warning_rth_documents d on i.EmployeeID=d.EmployeeID
 and i.id=d.DataId where i.EmployeeID='".$_REQUEST['empid']."' group by d.DataId";
		$myDB=new MysqliDb();
