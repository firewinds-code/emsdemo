...
if(isset($_POST['btnSave']))
{
	...
	{
		$myDB = new MysqliDb();
		$insert = "insert into tbl_bqm (EmployeeID) values ('".$EmployeeID."')";
...
if(!empty($_GET['action']))
{
	if($_GET['action'] == "delete" && !empty($_GET['empid']))
	{
		...
		{
			$myDB = new MysqliDb();
			$delete = "delete from tbl_bqm where EmployeeID = '".$EmployeeID."'";
			...
			{
				echo "<script>$(function(){ toastr.success('Request not completed ".$my_error." '); }); </script>";
			}
		}	
		else
		{
...
<!-- Form container if any -->
	<div class="schema-form-section row">
			...
			<?php if(!empty($_GET['pid']) and $_GET['pid']!='NA'){ ?>
				  	<div id="pnlTable">
			    <?php 
			    	$myDB=new MysqliDb();
					$chk_task=$myDB->query('select * from whole_details_peremp inner join tbl_bqm on tbl_bqm.EmployeeID = whole_details_peremp.EmployeeID where des_id in (9,12) and  cm_id ='.$_GET['pid']); 
