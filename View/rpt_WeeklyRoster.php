							...
							<div class="col s12 m12" id="rpt_container">
								...
								<div class="input-field col s6 m6">
									<select name="txt_Process" id="txt_Process" required>
										...
										<?php
										if ($_SESSION['__user_logid'] == 'CEK082281380') {
											$sqlBy = "select distinct Process,clientname,sub_process,cm_id from whole_details_peremp  where (ReportTo = '" . $_SESSION['__user_logid'] . "' or whole_details_peremp.EmployeeID = '" . $_SESSION['__user_logid'] . "' or whole_details_peremp.account_head = '" . $_SESSION['__user_logid'] . "' or whole_details_peremp.oh = '" . $_SESSION['__user_logid'] . "' or whole_details_peremp.qh = '" . $_SESSION['__user_logid'] . "' or whole_details_peremp.th = '" . $_SESSION['__user_logid'] . "') or (location=7 and client_name=82) order by clientname";
										} else {
											$sqlBy = "select distinct Process,clientname,sub_process,cm_id from whole_details_peremp  where (ReportTo = '" . $_SESSION['__user_logid'] . "' or whole_details_peremp.EmployeeID = '" . $_SESSION['__user_logid'] . "' or whole_details_peremp.account_head = '" . $_SESSION['__user_logid'] . "' or whole_details_peremp.oh = '" . $_SESSION['__user_logid'] . "' or whole_details_peremp.qh = '" . $_SESSION['__user_logid'] . "' or whole_details_peremp.th = '" . $_SESSION['__user_logid'] . "') order by clientname";
										}
				...
				if (!empty($_POST['txt_Process']) && $_POST['txt_Process'] != 'NA') {
					...
					$table .='<th rowspan="2">Account Head</th>';*/
						...
						foreach ($chk_task as $key => $value) {
							...
							if ($value['des_id'] != '9' && $value['des_id'] != '12' && $value['EmployeeID'] != $_SESSION['__user_logid']) {
								$table .= '<td><a onclick="javascript:return calc_Team(this);" data="' . $value['EmployeeID'] . '" date="' . $DateTo . '" cm_id="' . $_POST['txt_Process'] . '" class="btn"><i class="fa fa-plus"></i> ' . $value['EmployeeID'] . '</a></td>';
							} else {
							...
							$myDB = new MysqliDb();
							$ds_roster = $myDB->query('select DateOn,InTime,OutTime,type_ from roster_temp where EmployeeID ="' . $EmployeeID . '" and DateOn between "' . $DateFrom . '" and "' . $last_date . '"');
							$RosterIn = array();
							$RosterOut = array();
							$Roster_type = array();
							...
							for ($i = $begin; $begin <= $end; $i->modify('+1 day')) {
								...
								if ($i->format('Y-m-d') == date('Y-m-d', time())) {
									$myDB = new MysqliDb();
									$leaveCheck = $myDB->query("select LeaveID from leavehistry where EmployeeID = '" . $EmployeeID . "' and MngrStatusID='Approve' and LeaveType = 'Leave' and '" . date('Y-m-d', time()) . "' between DateFrom and DateTo limit 1;");
									if (count($leaveCheck) > 0 && $leaveCheck) {
