...
if (isset($_POST['addbriefing'])) {
    $reportid = explode(",", $_POST['hidden_reportid']);
    $processid = explode(",", $_POST['hidden_processid']);
    //print_r($reportid);	print_r($processid);exit;
    $EmpID = substr($_POST['txt_empid'], strpos($_POST['txt_empid'], "(") + 1, (strpos($_POST['txt_empid'], ")")) - (strpos($_POST['txt_empid'], "(") + 1));

    $myDB = new MysqliDb();
    $query = "delete from report_map where EmpID='" . $EmpID . "';";
    $resultBy = $myDB->rawQuery($query);

    foreach ($reportid as $rvar) {
        foreach ($processid as $pvar) {
            $myDB = new MysqliDb();
            $query = "insert into report_map set EmpID='" . $EmpID . "',reportID='" . $rvar . "', processID='" . $pvar . "', CreatedBy='" . $_SESSION['__user_logid'] . "'; ";
            $resultBy = $myDB->rawQuery($query);
            $mysql_error = $myDB->getLastError();

            //echo $_POST['txt_empid'].' - '.$rvar.'-'.$pvar.'<br/>';
        }
    }
                ...
                <div class="form-inline" id="ajaxid">
                    ...
                    <div class="input-field col s12 m12">
                        ...
                        ?><div class="col s4 m4 l4">
                                    <input type='checkbox' name="report[]" id="<?php echo  $value2['id']; ?>" value="<?php echo $value2['id']; ?>">
                    ...
                    <div class="col s12 m12">
                        ...
                        <br />
                        <iframe name="irsc" src="dropdown.php" style="width: 100%;height: 370px;" frameborder="0"></iframe>
...
<script>
    $(document).ready(function() {
        ...
        /*$("#txt_empid").keyup(function(){
        	...
        	$('select').formSelect();CE0821939495
        });	*/
    ...
    function editIT(moduleID, locid) {
        ...
        if (moduleID != "") {
            $.ajax({
                url: <?php echo '"' . URL . '"'; ?> + "Controller/getAssignedEmail.php?mid=" + moduleID + "&locid=" +
                    locid
            }).done(function(data) {
                $('#ajaxid').html(data);
                $('select').formSelect();
