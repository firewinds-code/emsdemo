...
$checkresign = "select EmployeeID from resign_details where EmployeeID=? and now() between nt_start and nt_end";
$stmts = $conn->prepare($checkresign);
$stmts->bind_param("s", $EmployeeID);
$stmts->execute();
$res_checkresign = $stmts->get_result();
$checkEmpID = $res_checkresign->fetch_row();
// print_r($checkEmpID);
// die;
if (!$checkEmpID[0]) {
} else {
...
if ($select_query->num_rows > 0) {
	foreach ($select_query as $key => $value) {
		...
		$tmpdate = $value['apr'];
		// echo "<br>";
		// $date = date_parse($value['apr']);
		// $aprmnth = $date['month'];
		// echo $month1 = date('Y-' . $aprmnth . '-01', strtotime('-1 month'));
		...
		//$dd = "Y-".$tmpdate."-05";
		$date = date_parse($tmpdate);
		// print_r($date);
		// echo $dd = "15-" . $tmpdate . '-' . date('Y');
		$formatdat = "15-" . $date['month'] . '-' . date('Y');
		// echo $prev_month_ts = strtotime($formatdat .  '-1 month');
		// echo $prev_month = date('Y-m-d', $prev_month_ts);
		$prev_month_ld = strtotime($formatdat .  'last day of previous month');
		// echo "<br>";
		$dd = date('Y-m-d', $prev_month_ld);
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				<div class="input-field col s12 m12 no-padding">
					...
					<div class="input-field col s12 m12" id="HRCommentShow" style="border: 1px solid #19aec4;display: none;"></div>
...
if($CurMonth==$UpdateMonth)
{
   echo '<button type="submit" value="Save" name="btnSave" id="btnSave" class="btn waves-effect waves-green">Save</button>';
					...
					if ($curdate >= $prev_month && $curdate <= $prev_month_ldat) {
						echo '<button type="submit" value="Save"btn name="btnSave" id="btnSave" class="btn waves-effect waves-green">Save</button>';
					}
					?>
		...
		$.ajax({
			...
			success: function(result) {
				if (result != '') {
					...
					if (HRStatus != 'Pending') {
						$.ajax({
							url: "../Controller/get_Comment_HR.php?ID=" + elID,
							success: function(result) {
								if (result != '') {
									$('#HRCommentShow').html(result);
									$('#HRCommentShow').show();
								}
								$('select').formSelect();
							}
