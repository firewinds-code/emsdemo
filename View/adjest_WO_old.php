...
$weekOFF_date = array();
$myDB = new MysqliDb();
$rs_emp =$myDB->query("select des_id,DOJ,whole_details_peremp.EmployeeID,salary_details.rt_type from whole_details_peremp inner join salary_details on salary_details.EmployeeID = whole_details_peremp.EmployeeID where  whole_details_peremp.EmployeeID='AE011913578'");
	...
	foreach($rs_emp as $rs_key=>$rs_val)
	{
		...
		if($day_array)
		{
			...
			elseif($day ==7)
			{
				...
				if($unvelidWeekOf == 'all' || $unvelidWeekOf == 0)
				{
					$myDB = new MysqliDb();
				    $myDB->query('call delete_CO("'.$EmpID.'","'.$monday_counter[1].'",1)');
				}
