...
settimestamp('Auto_Exit_Mail','Start');
			$myDB=new MysqliDb();
			$chk_task=$myDB->query('select EmployeeID,location from whole_dump_emp_data where employeeid="CE031929841" and des_id in (5,7,8,10,13,14,15,16,22) and disposition in ("IR","RES")');
		
			//$chk_task=$myDB->query('select EmployeeID,location from whole_dump_emp_data where EmployeeID="CE031829429";');
			...
			{  
				foreach($chk_task as $key=>$value)
				{
					...
					$myDB=new MysqliDb();
					$chk_task2=$myDB->query('select t1.EmployeeID,t2.ofc_emailid from whole_details_peremp t1 inner join contact_details t2 on t1.EmployeeID=t2.EmployeeID where t1.location!="'.$Joinee_loc.'" and t1.des_id in (1,5,7,8,10,13,14,15,16,22) and ofc_emailid is not null and ofc_emailid !="";');
