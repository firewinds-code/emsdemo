	...
	<div class="pim-container ">
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				<?php
				$view_for_query = "";
				if ($_SESSION['__user_status'] == 6 || $_SESSION['__user_status'] == 5 || $_SESSION['__user_status'] == 4) {
				...
				if (isset($_SESSION['__user_logid']) || $_SESSION['__user_logid'] != "") {
					...
					$myDB = new MysqliDb();
					$movedate = "";
					$q1 = "select EmployeeID,updated_on,new_cm_id from tbl_client_toclient_move where EmployeeID='" . $_SESSION['__user_logid'] . "' and  flag='FM' order by id desc";
					...
					$myDB = new MysqliDb();
					$processmove_query = $myDB->rawQuery("select EmployeeID,updated_on,new_cm_id from tbl_oh_tooh_move where EmployeeID='" . $_SESSION['__user_logid'] . "' and  flag='FM' order by id desc");
					$process_update_date = "";
			...
			if ($process_update_date != "") {
						...
						<ul class="collapsible">
							...
							if ($rowCount > 0) {
								...
								foreach ($result2 as $key => $val) {
									...
									$select_attempted = "select * from brf_quiz_attempted where BriefingID='" . $briefingId . "' and EmployeeID='" . $_SESSION['__user_logid'] . "'";
									$myDB = new MysqliDb();
							...
							?>
				     			 ...
				     			 <a   class="btn-floating  "  target='_blank' title='View Document' data-position="bottom" onclick="openWindow('<?php echo URL . 'briefingDoc/' . $file_name; ?>','acck<?php echo $val['id']; ?>')" >View</i></a> </label>-->
												...
												<a id="acck<?php echo $val['id']; ?>" style="cursor:pointer;font-weight: bold; color: #1c94c4; " title="Click me" onclick="acknowledge_me('<?php echo $val['id']; ?>','<?php echo $_SESSION['__user_logid']; ?>','<?php echo $total_question_num; ?>','<?php echo $fromdate; ?>','<?php echo $emp_status; ?>','<?php echo $cm_id; ?>','<?php echo $view_for; ?>')">Acknowledge</a>
											...
											<div id="divacck<?php echo $val['id']; ?>" style="<?php echo $style; ?>">
												...
												?>
														...
														?>
														<button type="button" value="Cancel" name="btnCan" id="btnCancel" class="btn waves-effect waves-green " title="Click me" onclick="quiz_attempt('<?php echo $val['id']; ?>','<?php echo $_SESSION['__user_logid']; ?>','<?php echo $total_question_num; ?>','<?php echo $ac_id; ?>')">Submit</button>
				...
				<?php } else { ?>
					<script>
						$('#alert_msg').html('<ul class='
							text - danger '>Session has exspired, Please login again</ul>')
					</script>
