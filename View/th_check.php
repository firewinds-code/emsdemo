				...
				<div class="">
					...
					<div class="input-field col s6 s6">
						<select id="text_trcheck_Batch" name="text_trcheck_Batch" onchange="javascript:return getBatch_state();">
							...
							$sqlBy = 'SELECT distinct batch_master.BacthID, batch_master.BacthName,createdon FROM batch_master where batch_master.createdby=? and datediff(curdate(),createdon) <= 10 order by batch_master.BacthName';
							$batch_id = 0;
					...
					<div id="pnlTable" class="col s12 m12 card">
						<!-- <div id="table1" class="col s12 m12 card"> -->
					</div>
				</div>
			</div>
			<!--Form container End -->
...
<script>
	$(document).ready(function() {
		...
		$('#btnCan').click(function() {
			...
			$('#text_trcheck_Batch').val('NA');
			$('.statuscheck').addClass('hidden');
			$('#pnlTable').addClass('hidden');
			$('#docTable').html('');
	...
	$('#text_trcheck_Batch').on('change', function() {
		var batchid = $("#text_trcheck_Batch").val();
		$.ajax({
			url: "../Controller/get_checkByID.php?batchID=" + batchid,
			success: function(result) {
				$('#pnlTable').html(result);
