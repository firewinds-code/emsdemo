	...
	$(function(){
		...
		$('#myTable').DataTable({
			...
			$('.buttons-copy').attr('id','buttons_copy');
		   	...
		   	$('#txt_dateOn').datepicker({ dateFormat: 'yy-mm-dd' });
	});
</script>
...
<!--Reprot / Data Table start -->		
		...
		<?php
			...
			{
				$query = 'select employeeid,mobile, EmailAddress,emailStatus,createdOn as DateTime from login_ncns_smsmail where type="Login" and cast(createdOn as date) between "'.$date_From.'" and "'.$date_To.'" ';
			}
			else if($type == 'NCNS Notification')
			{
				$query = 'select employeeid,mobile, EmailAddress,emailStatus,createdOn as DateTime from login_ncns_smsmail where type="NCNS" and cast(createdOn as date) between "'.$date_From.'" and "'.$date_To.'" ';
			}
			else if($type == 'Welcome Notification')
			{
				$query = 'select employeeid,mobile, EmailAddress,emailStatus,createdOn as DateTime from welcome_msg_smsmail where cast(createdOn as date) between "'.$date_From.'" and "'.$date_To.'" ';
			}
