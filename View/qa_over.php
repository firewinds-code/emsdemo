		...
		{	
			...
			if($_POST['batch_id']>0||!empty($_POST['batch_id']))
			{
				foreach($_POST['cb'] as $val)
				{
					...
					{
						$myDB=new MysqliDb();
						$save='call manage_qh_over("'.$empID.'","'.$_POST['txt_Remark_'.$empID].'","'.$createBy.'","QH OVER")';	
	...
	if(isset($_POST['cb']))
	{
		...
		{	
				...
				foreach($_POST['cb'] as $val)
				{
					$empID=$val;
					$myDB=new MysqliDb();
					$save='call manage_refer_hr("'.$empID.'","'.$createBy.'","'.$_POST['txt_Remark_'.$empID].'","QH OVER REFER")';	
					$resulti = $myDB->query($save);
					$mysql_error = $myDB->getLastError();	
					if(empty($resulti))
					{
	...
	<div class="schema-form-section row" >
			 <div class="form-inline" >
			    ...
			    <?php 
			    	...
			    	$sqlConnect='call get_qhchecklist_over("'.$_SESSION['__user_logid'].'",'.$batch_id.')';
			    	$myDB=new MysqliDb();
