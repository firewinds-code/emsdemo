...
function Calculaion_Biometric($empid, $date)
{
	...
	if (count($result_roster) > 0 /*&& count($result_bioinout) && $result_bioinout*/ && $result_roster) {
		$myDB = new MysqliDb();
		...
		$Shift1 = explode("|", $result_roster[0]['InTime']);
		$Shift2 = explode("|", $result_roster[0]['OutTime']);
...
function check_validate($Exception, $EmployeeID, $DateFrom, $DateTo, $hd_check)
{
	...
	$myDB = new MysqliDb();
	$check_count = $myDB->query('SELECT count(*) `Count` FROM exception where EmployeeID ="' . $EmployeeID . '" and Exception = "' . $Exception . '" and date_format(DateFrom,"%Y-%m") = "' . date('Y-m', strtotime($DateFrom)) . '" and MgrStatus != "Decline" and HeadStatus !="Decline";');
	if (count($check_count) > 0 && $check_count && $hd_check) {
		$exc_count = $check_count[0]['Count'];
		if (($_POST['txt_Request'] == "Shift Change") && $exc_count >= 2) {
			...
			return array(0 => 1, 1 => 'Request Not Submitted : The ' . $_POST['txt_Request'] . ' request count exceeded the maximum limit.');
		} else if (($_POST['txt_Request'] == "Working on Holiday" || $_POST['txt_Request'] == "Working on WeekOff") && $exc_count >= 2) {
			return array(0 => 1, 1 => 'Request Not Submitted : The ' . $_POST['txt_Request'] . ' request count exceeded the maximum limit.');
		}
	}
	...
	$myDB = new MysqliDb();
	$get_subProcess = $myDB->query("select sub_process from whole_dump_emp_data where EmployeeID = '" . $EmployeeID . "'");
	$get_subProcess = $get_subProcess[0]['sub_process'];


	if ($_POST['txt_Request'] == "Roster Change" || $_POST['txt_Request'] == "Shift Change") {
		$ShiftIn = $_POST['txt_ShiftIn'];
		$ShiftOut = $_POST['txt_ShiftOut'];
		$ShiftIn2 = $_POST['txt_ShiftIn2'];
		$ShiftOut2 = $_POST['txt_ShiftOut2'];
		$IssueType = "NA";
		$CurrAtt = "NA";
		$UpdateAtt = "NA";
		$LeaveType = "NA";
		...
		if (count($rst_shift) > 0 && $rst_shift) {
			...
			if (strtotime($InTime)) {
				...
				if (strtoupper($rst_gender[0]['gender']) == 'FEMALE') {
					...
					$roster_out = date('Y-m-d H:i:s', (strtotime($DateFrom . ' ' . $_POST['txt_ShiftOut'])));
			...
			if (strtotime($InTime)) {
				...
				if (strtoupper($rst_gender[0]['gender']) == 'FEMALE') {
					$roster_old = date('Y-m-d H:i:s', (strtotime($shift_temp_old)));
					$roster_cur = date('Y-m-d H:i:s', (strtotime($shift_temp_current)));
					$roster_out = date('Y-m-d H:i:s', (strtotime($DateFrom . ' ' . $_POST['txt_ShiftOut2'])));
					$roster_out_lim = date('Y-m-d H:i:s', (strtotime($DateFrom . ' 19:00')));
					$roster_diffrence = '';
		...
		$myDB = new MysqliDb();
		$Emd_des = $myDB->query('select designation,des_id from whole_dump_emp_data where EmployeeID ="' . $EmployeeID . '"');
		...
		if ($_POST['txt_Request'] == "Working on WeekOff") {
			...
			$myDB = new MysqliDb();
			$rstl = $myDB->query('select count(*) counts  from exception where month(DateFrom) =  ' . intval(date('m', strtotime($DateFrom))) . ' and year(DateFrom) =  ' . intval(date('Y', strtotime($DateFrom))) . ' and Exception = "Working on Weekoff" and EmployeeID = "' . $EmployeeID . '" and  "Decline"  not in(MgrStatus, HeadStatus)');
	...
	if ($len_check === 1) {
						...
						if ($Exception == "Roster Change" || $Exception == "Shift Change") {
							...
							if ($dt1 == $dt2) {
								...
								if ($rowCount > 0) {
									...
									$myDB = new MysqliDb();
									$logQuery = "INSERT INTO exception_log_error set exp_id='" . $ExpID . "' ,exp_error='Roster or shift change for split one day is not updated'";
								...
								for ($i = $begin; $begin <= $end; $i->modify('+1 day')) {
									...
									if ($rowCount > 0) {
										...
										$myDB = new MysqliDb();
										$logQuery = "INSERT INTO exception_log_error set exp_id='" . $ExpID . "' ,exp_error='Roster or shift change for split is not updated'";
										$myDB->rawQuery($logQuery);
									}
								}
								unset($begin);
							...
							if ($rowCount > 0) {
								...
								$myDB = new MysqliDb();
								$logQuery = "INSERT INTO exception_log_error set exp_id='" . $ExpID . "' ,exp_error='back dated leave split is not updated'";
								$myDB->rawQuery($logQuery);
							}
							...
							if ($dt1 == $dt2) {
								...
								if ($rowCount > 0) {
									...
									$myDB = new MysqliDb();
									$logQuery = "INSERT INTO exception_log_error set exp_id='" . $ExpID . "' ,exp_error='Working on Holiday split is not updated'";
									$myDB->rawQuery($logQuery);
								}
							}
		...
		if ($validation === 0 && $validate === 0) {
			...
			try {
					...
					if ($MngrStatusID == "Approve") {
						if ($Exception == "Roster Change" || $Exception == "Shift Change") {
							...
							if ($dt1 == $dt2) {
								...
								$year = intval(date('Y', strtotime($dt1)));
								$day  = intval(date('d', strtotime($dt1)));
								$query = "call sp_UpdateRoaster_Split('" . $_POST['txtEmpID'] . "','" . $month . "','" . $year . "','" . $day . "','" . $Shift1 . "','" . $Shift2 . "')";
								$myDB = new MysqliDb();
								$flag = $myDB->rawQuery($query);
								$error = $myDB->getLastError();
								$rowCount = $myDB->count;
								if ($rowCount > 0) {
									...
									$myDB = new MysqliDb();
									$logQuery = "INSERT INTO exception_log_error set exp_id='" . $ExpID . "' ,exp_error='Split " . $count . $error . "'";
									$myDB->rawQuery($logQuery);
								}
							} else {
								...
								for ($i = $begin; $begin <= $end; $i->modify('+1 day')) {
									...
									if ($rowCount > 0) {
										...
										$myDB = new MysqliDb();
										$logQuery = "INSERT INTO exception_log_error set exp_id='" . $ExpID . "' ,exp_error='split " . $count . "'";
										$myDB->rawQuery($logQuery);
									}
								}
								unset($begin);
							...
							if ($rowCount > 0) {
								...
								$myDB = new MysqliDb();
								$logQuery = "INSERT INTO exception_log_error set exp_id='" . $ExpID . "' ,exp_error='split" . $flag . "'";
								$myDB->rawQuery($logQuery);
							}
						} else if ($Exception == "Working on Holiday" || $Exception == "Working on WeekOff" || $Exception == "Working on Leave") {
							$EmployeeID = $_POST['txtEmpID'];
				...
				$myDB = new MysqliDb();
				$logQuery = "INSERT INTO exception_log_error set exp_id='" . $ExpID . "' ,exp_error='" . $ex->getMessage() . "'";
				$myDB->rawQuery($logQuery);
				$alert_msg = 'Request not submitted :' . $error;
							...
							<div class="col s12 m12 16" style="">
								...
								<div id="pnlTable">
									<div class="had-container pull-left card col s12 m12">
										<div class=" ">
											<?php
											$sqlConnect = 'call GetRequestDetails4("' . $_SESSION['__user_logid'] . '","' . $date_srch_from . '","' . $date_srch_to . '")';
											$myDB = new MysqliDb();
