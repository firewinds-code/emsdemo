...
if ($btnEdit) {
	...
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		if (cleanUserInput($_POST['HRStatus']) == 'Pending' || cleanUserInput($_POST['HRStatus']) == 'Hold' || cleanUserInput($_POST['HRStatus']) == 'Postpone') {
			$date = date('Y-m-d', strtotime(cleanUserInput($_POST['LastDate']) .  '+1 day'));
			$aprYear = date('Y', strtotime(cleanUserInput($_POST['LastDate']) .  '+1 day'));
			...
			if (empty($error)) {
				if (cleanUserInput($_POST['ApproverStatus']) == 'Approve') {
					...
					$i_letterSendDate = cleanUserInput($_POST['i_letterSendDate']);
					$createdby = clean($_SESSION['__user_logid']);
					$updateaprmonth = 0;
					$Update = 'call update_aprsalary("' . $EmpId  . '","' . $EmpName . '","' . cleanUserInput($_POST['NewCTCM']) . '","' . cleanUserInput($_POST['takehome']) . '","' . cleanUserInput($_POST['HRA']) . '","' . cleanUserInput($_POST['convence']) . '","' . cleanUserInput($_POST['sp_allow']) . '","' . cleanUserInput($_POST['gross_sal']) . '","' . cleanUserInput($_POST['pf']) . '","' . cleanUserInput($_POST['esis']) . '","' . $createdby . '","' . cleanUserInput($_POST['Basic']) . '","' . cleanUserInput($_POST['pf_employer']) . '","' . cleanUserInput($_POST['esi_employer']) . '","' . cleanUserInput($_POST['professional_tex']) . '","' . cleanUserInput($_POST['min_wages']) . '","' . cleanUserInput($_POST['PLI']) . '","' . cleanUserInput($_POST['net_takehome']) . '","' . cleanUserInput($_POST['i_letterSendDate']) . '","' . cleanUserInput($date) . '","' . cleanUserInput($_POST['NewPLIPercent']) . '","' . cleanUserInput($_POST['CurrentCTCM']) . '","' . cleanUserInput($_POST['PerAppro']) . '","' . cleanUserInput($_POST['AprMonth']) . '","' . cleanUserInput($aprYear) . '","' . $updateaprmonth . '")';
					...
					if (empty($error)) {
						if (cleanUserInput($_POST['PromotionRecommend']) == 'Yes') {
							...
							$Update = 'call apr_UpdatePromotion("' . cleanUserInput($_POST['DataId']) . '","' . cleanUserInput($_POST['AprMonth']) . '","' . cleanUserInput($_POST['PromotionRecommendApr']) . '","' . $newpost . '","' . $postid . '","' . $EmpId  . '","' . $EmpName . '","' . cleanUserInput($date) . '","' . cleanUserInput($_POST['p_letterSendDate']) . '","' . $createdby . '","' . cleanUserInput($_POST['AprMonth']) . '","' . cleanUserInput($aprYear) . '")';
							$myDB = new MysqliDb();
							$result = $myDB->rawQuery($Update);
					...
					$rowCount = $myDB->count;
					$error = $myDB->getLastError();
				} else if (cleanUserInput($_POST['ApproverStatus']) == 'Hold') {
					$AprMonth = cleanUserInput($_POST['AprMonth']);
					$Update = 'call apr_UpdatePendingAppraisal("' . cleanUserInput($_POST['EmpId']) . '","' . $AprMonth . '","' . cleanUserInput($_POST['HoldMonth']) . '")';
					$myDB = new MysqliDb();
					$result = $myDB->rawQuery($Update);
					$rowCount = $myDB->count;
					$error = $myDB->getLastError();
				...
				$Remarks = 'call apr_UpdateApproverRemark("' . $_POST['DataId'] . '","' . $_POST['Remarks1'] . '","' . $_POST['Remarks2'] . '","' . $_POST['Remarks3'] . '","' . $_POST['Remarks4'] . '","' . $_POST['Remarks5'] . '","' . $_POST['Remarks6'] . '","' . $_POST['Remarks7'] . '","' . $_POST['Remarks8'] . '","' . $_POST['Remarks9'] . '","' . $_POST['Remarks10'] . '")';
				$myDB = new MysqliDb();
					...
					<div class="input-field col s12 m12 no-padding">
						<div class="input-field col s12 m12">
							...
							<ul class="collapsible">
								...
								<!--<li>
			<div class="collapsible-header topic">Previous Appraisal Details</div>
			<div class="collapsible-body">
				<div class="input-field col s4 m4 ">
				 	<input type="text" id="EmpName1" name="EmpName1" readonly="true">  
				 	<label for="EmpName">Previous %</label>
				</div>
				<div class="input-field col s4 m4 ">
				 	<input type="text" id="EmpId2" name="EmpId2" readonly="true">  
				 	<label for="EmpId">Previous CTC</label>
				</div>
				<div class="input-field col s4 m4 ">
				 	<input type="text" id="doj3" name="doj3" readonly="true">  
				 	<label for="doj">Previous Date</label>
				</div>
			</div>
	    </li>-->
							</ul>
						...
						<div id="ApproverRec" style="display: none;">
							...
							<div class="input-field col s6 m6" id="p_letterSendDateDIV" style="display: none;">
								<input type="text" name="p_letterSendDate" id="p_letterSendDate">
								<label for="p_letterSendDate">Promotion Letter Send Date</label>
							</div>

							<div class="input-field col s6 m6" id="i_letterSendDateDIV" style="display: none;">
								<input type="text" name="i_letterSendDate" id="i_letterSendDate">
								<label for="i_letterSendDate">Increment Letter Send Date</label>
							</div>
						</div>
						...
						<div class="input-field col s12 m12" id="PerAppro1">
							<input type="text" name="PerAppro" id="PerAppro" onkeypress="javascript:return isNumberKey(event);" maxlength="5">
						...
						<div class="input-field col s12 m12" id="incr_trail">
							<ul class="collapsible">
								<li>
									<div class="collapsible-header topic">Increment Trail</div>
									<div class="collapsible-body">
						...
						<div class="input-field col s12 m12" id="promo_trail">
							<ul class="collapsible">
								<li>
									<div class="collapsible-header topic">Promotion Trail</div>
									<div class="collapsible-body">
...
<script>
		...
		$('#btnEdit').click(function() {
			...
			if ($('#ApproverStatus').val() == "Approve") {
				...
				if ($('#PromotionRecommendApr').val() == "Yes" && $('#PromotionPostApr').val() == "NA") {
					...
					if ($('#p_letterSendDate').val() == "") {
						validate = 1;
						$('#p_letterSendDate').addClass('has-error');
						$('#p_letterSendDate').parent('.select-wrapper').find('input.select-dropdown').addClass("has-error");
						if ($('#span_EvaluatorScore10').length == 0) {
							$('<span id="span_EvaluatorScore10" class="help-block">Require *</span>').insertAfter('#p_letterSendDate');
						}
					}
				}

				if ($('#i_letterSendDate').val() == "") {
					validate = 1;
					$('#i_letterSendDate').addClass('has-error');
					$('#i_letterSendDate').parent('.select-wrapper').find('input.select-dropdown').addClass("has-error");
					if ($('#span_EvaluatorScore10').length == 0) {
						$('<span id="span_EvaluatorScore10" class="help-block">Require *</span>').insertAfter('#i_letterSendDate');
					}
				}
	...
	$('#PromotionRecommendApr').change(function() {
		if ($('#PromotionRecommendApr').val() == 'Yes') {
			...
			$('#p_letterSendDateDIV').show();
			$('#i_letterSendDateDIV').show();
			$('#promo_trail').show();
			//getProcess();
		} else {
			$('#PromotionPostIdApr').hide();
			$('#p_letterSendDateDIV').hide();
			$('#promo_trail').hide();
			$('#i_letterSendDateDIV').show();
		}
	});
	...
	function EditData(el) {
		...
		$.ajax({
			url: "../Controller/get_incr_trail.php?ID=" + EmplID,
			success: function(result) {
				if (result != '') {
					$('#div_incr').html(result);
		...
		$.ajax({
			url: "../Controller/get_promotion_trail.php?ID=" + EmplID,
			success: function(result) {
				if (result != '') {
					$('#div_promo').html(result);
		...
		$.ajax({
			...
			success: function(result) {
				...
				$('#AppraisalPer').on("blur", appraisalcalc)
				appraisalcalc();
			}
		});
	...
	// CHECK FOR CALCULATOR ON KEYUP
	$('#PerAppro').on('change keyup check', function() {
		var aprper = $(this).val();
	...
	// $('#i_letterSendDate,#P_letterSendDate').datetimepicker({
	// 	format: 'Y-m-d',
