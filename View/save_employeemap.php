...
if (isset($_POST['employment_type']) && $_POST['employment_type'] != "") {
	...
	} else if ($_POST['employment_type'] == "10") {
		$locationRaw = rawurlencode('Gurgaon');
		$EMS_CenterName = 'Gurgaon';
		$dir_location = "Gurgaon/";
	} else if ($_POST['employment_type'] == "11") {
		$locationRaw = rawurlencode('Hyderabad');
						...
						if ($validate == 1 && $validate_date == 0) {
							if (substr($EmployeeID, 0, 2) == 'TE') {
								...
								if ($_POST['employment_type'] == "1") {
									...
									$orderID = $myDB->query('select ifnull((max(cast(RIGHT(EmployeeID,(length(EmployeeID) - 7)) as unsigned)) + 1),1) EMPID from personal_details where EmployeeID like "CEG%"');
									$orderID = $orderID[0]['EMPID'];
									$emp_alias = 'CEG';
								}
									...
									if ($orderID) {
										...
										if ($EmployeeID != '') {
											if ($cm_id == "570") {
												...
												$sql = "select EmployeeName from personal_details where EmployeeID='" . $emp_IDorg . "'";
												$result = $myDB->query($sql);
											...
											if (count($salary_array) > 0) {
												$insertData = "call manageOfferSalary('" . $salary_array->salary . "','" . $_POST['hiddenIntID'] . "','" . $emp_IDorg . "','" . $_SESSION['__user_logid'] . "','" . $salary_array->emp_type . "','" . $salary_array->stipend_days . "','" . $salary_array->stipent_amount . "')";
												$myDB = new MysqliDb();
...
?>
								...
								<?php
											...
											if ($_POST['employment_type'] == "1") {
												...
												$location = rawurlencode('Gurgaon');
											} else if ($_POST['employment_type'] == "11") {
												$location = rawurlencode('Hyderabad');
											}
										...
										if (($_POST['employment_type'] == "1" || $_POST['employment_type'] == "3" || $_POST['employment_type'] == "4" || $_POST['employment_type'] == "5" || $_POST['employment_type'] == "6" || $_POST['employment_type'] == "7" || $_POST['employment_type'] == "8" || $_POST['employment_type'] == "9" || $_POST['employment_type'] == "10") && $desg != 9 && $desg != 12 && $desg != 30 && $desg != '') {
	...
	<div class="pim-container row" id="div_main">
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				} else if ($loc == "4" || $loc == "5" || $loc == "6" || $loc == "7" || $loc == "8" || $loc == "9" || $loc == "10") {
