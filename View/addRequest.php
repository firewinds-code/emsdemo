...
$desArray = array(9, 12);
$myDB = new MysqliDb();
$sql_roster = "select t2.`des_id` from employee_map t1 join df_master t2 on t1.df_id=t2.df_id where EmployeeID = '" . $_SESSION['__user_logid'] . "'";
...
function get_apr_ATND($travelTime, $roster_type, $DateFrom)
{
	...
	if ($v1 > 24) {
		if ($_SESSION["__cm_id"] == "88" || $_SESSION["__cm_id"] == "239" || $_SESSION["__cm_id"] == "265" || $_SESSION["__cm_id"] == "270" || $_SESSION["__cm_id"] == "420" || $_SESSION["__cm_id"] == "444" || $_SESSION["__cm_id"] == "445") {
...
function check_validate($Exception, $EmployeeID, $DateFrom, $DateTo, $hd_check)
{
	...
	$day1 = $day2 = 0;
	if ($Exception == 'Roster Change' && ($_SESSION["__cm_id"] == "88" || $_SESSION["__cm_id"] == "239" || $_SESSION["__cm_id"] == "265" || $_SESSION["__cm_id"] == "270" || $_SESSION["__cm_id"] == "420" || $_SESSION["__cm_id"] == "444" || $_SESSION["__cm_id"] == "445")) {
	...
	$myDB = new MysqliDb();
	$getDesigID = '';
	$get_Query = $myDB->query("select t2.sub_process,t3.des_id from employee_map t1 join new_client_master t2 on t1.cm_id=t2.cm_id join df_master t3 on t1.df_id=t3.df_id where t1.EmployeeID = '" . $EmployeeID . "'");
	...
	if (count($check_count) > 0 && $check_count && $hd_check) {
		...
		$cm_idArray = array(192, 311, 487, 702, 312, 703, 488, 313, 194, 314, 440, 427, 455, 670, 196, 669);
		if (in_array($_SESSION["__cm_id"], $cm_idArray)) {
			if ($_POST['txt_Request'] == "Roster Change" && $exc_count >= 2) {
				return array(0 => 1, 1 => 'Request Not Submitted : The ' . $_POST['txt_Request'] . ' request count exceeded the maximum limit.');
			} else if (($_POST['txt_Request'] == "Biometric issue" || $_POST['txt_Request'] == "Working on Leave") && $exc_count >= 1) {
				return array(0 => 1, 1 => 'Request Not Submitted : The ' . $_POST['txt_Request'] . ' request count exceeded the maximum limit.');
			}
		}

		if (in_array($getDesigID, $sesArray) && ($_SESSION["__cm_id"] == "88" || $_SESSION["__cm_id"] == "239" || $_SESSION["__cm_id"] == "265" || $_SESSION["__cm_id"] == "270" || $_SESSION["__cm_id"] == "420" || $_SESSION["__cm_id"] == "444" || $_SESSION["__cm_id"] == "445") && $_POST['txt_Request'] == "Working on WeekOff") {
	...
	if (($_POST['txt_Request'] == "Roster Change" || $_POST['txt_Request'] == "Shift Change") && ($_POST['txt_ShiftIn'] != 'NA' && $_POST['txt_ShiftOut'] != 'NA')) {
		...
		if (trim($_POST['txt_ShiftIn']) == "" || trim($_POST['txt_ShiftOut']) == "") {
			$validation = 1;
		...
		$myDB = new MysqliDb();
		$Emd_des = $myDB->query('select t3.designation,t2.des_id from employee_map t1 join df_master t2 on t1.df_id=t2.df_id join designation_master t3 on t2.des_id=t3.id where EmployeeID ="' . $EmployeeID . '"');
		$Emd_des = $Emd_des[0]['des_id'];
...
$str = "select t1.l1empid,t2.EmployeeName from module_master_new t1 join personal_details t2 on t1.l1empid=t2.EmployeeID where t1.EmployeeID='" . $_SESSION['__user_logid'] . "' and module_name='Exception'";
$myDB = new MysqliDb();
...
if (isset($_POST['btn_Leave_Add'])) {
	...
	if ($validation === 0 && $validate === 0) {
		...
		if (count($rst) > 0) {
			...
			//{
			if (strtoupper((trim($rst[0]['gender']))) == strtoupper("Female") && $_SESSION["__cm_id"] != "88" && $_SESSION["__cm_id"] != "239" && $_SESSION["__cm_id"] != "265" && $_SESSION["__cm_id"] != "270" && $_SESSION["__cm_id"] != "420" && $_SESSION["__cm_id"] != "444" && $_SESSION["__cm_id"] != "445") {
...
if (isset($_POST['btn_Leave_Save'])) {
	...
	if ($_POST['txtID'] > 0 && $len_check === 0) {
		...
		if ($_POST['txt_Request'] == "Roster Change" || $_POST['txt_Request'] == "Shift Change") {
			...
			if (trim($_POST['txt_access_in']) == "" || trim($_POST['txt_ShiftOut']) == "") {
				echo "<script>$(function(){ toastr.info('Access in or access out should not be blank.') }); </script>";
				$validation = 1;
			} else {
				$ShiftIn = $_POST['txt_access_in'];
			...
			if ($rowCount > 0) {
				...
				if ($HeadStatusID == "Approve") {
					if ($Exception == "Roster Change" || $Exception == "Shift Change") {
						...
						if ($dt1 == $dt2) {
							...
							if ($rowCount > 0) {
								...
								if (strtotime($date1) > strtotime($date2)) {
									$ds_APR = $myDB->query('select t1.EmployeeID, designation,des_id, windowstart,windowend,t3.work_from from employee_map t1 inner join process_window t2 on t1.cm_id=t2.cm_id inner join roster_temp t3 on t1.EmployeeID=t3.EmployeeID join df_master t4 on t1.df_id=t4.df_id join designation_master t5 on t4.des_id=t5.id where t1.EmployeeID= "' . $_POST['txtEmpID'] . '" and t3.DateOn= "' . $date2 . '" ');
									...
									if (count($ds_APR) > 0 && $ds_APR) {
										if ($ds_APR[0]['des_id'] == '9' && $ds_APR[0]['des_id'] == '12' && $ds_APR[0]['des_id'] == '33' && $ds_APR[0]['des_id'] == '34' && $ds_APR[0]['des_id'] == '35' && $ds_APR[0]['des_id'] == '36' && $ds_APR[0]['work_from'] != '') {
											$url = 'http://10.147.20.14/apr_processing/user_apr.php?apr_date=' . $date2 . '&username=' . $_POST['txtEmpID'] . '&intime=' . $ShiftIn . '&outtime=' . $ShiftOut . '&WorkFrom=' . $ds_APR[0]['work_from'] . '&windowstart=' . $ds_APR[0]['windowstart'] . '&windowend=' . $ds_APR[0]['windowend'];
											//echo $url;
											$curl = curl_init();
						...
						$alert_msg = $Exception . ' Request for ' . $count . ' Day is ' . $HeadStatusID . ' <b> by ' . $_POST['txtApprovedBy_sh'] . '</b>';
						echo "<script>$(function(){ toastr.success('" . $alert_msg . "') }); </script>";
						...
						if ($rowCount > 0) {
							...
							$alert_msg = $Exception . " Request for " . $TotalLeaves1 . " day is " . $HeadStatusID . ' by ' . $_POST['txtApprovedBy_sh'] . ' </b>';
							echo "<script>$(function(){ toastr.success('" . $alert_msg . "') }); </script>";
						} else {
							$myDB = new MysqliDb();
						...
						$alert_msg = $Exception . " Request for " . $count . " day is " . $HeadStatusID . '</b>  by ' . $_POST['txtApprovedBy_sh'];
						echo "<script>$(function(){ toastr.success('" . $alert_msg . "') }); </script>";
					} else if ($Exception == "Biometric issue") {
						$alert_msg = $Exception . ' Request for 1 Day is ' . $HeadStatusID . ' <b> by ' . $_POST['txtApprovedBy_sh'] . '</b>';
						echo "<script>$(function(){ toastr.success('" . $alert_msg . "') }); </script>";
					}
					$url = '';
					...
					if ($_POST['txtcm_id'] == "88" || $_POST['txtcm_id'] == "239" || $_POST['txtcm_id'] == "265" || $_POST['txtcm_id'] == "270" || $_POST['txtcm_id'] == "420" || $_POST['txtcm_id'] == "444" || $_POST['txtcm_id'] == "445") {
					...
					if ($_SESSION['__user_logid'] == strtoupper($EmployeeID)) {
		...
		<div>
			...
			if (($_SESSION['__status_th'] == $_SESSION['__user_logid'] || $_SESSION['__status_oh'] == $_SESSION['__user_logid'] || $_SESSION['__status_qh'] == $_SESSION['__user_logid']) || ((($_SESSION['__status_ah'] != 'No' && $_SESSION['__status_ah'] == $_SESSION['__user_logid']) && $_SESSION['__status_ah'] != '') || $_SESSION['__user_type'] == 'ADMINISTRATOR' || $_SESSION['__user_type'] == 'CENTRAL MIS' || $_SESSION['__user_type'] == 'HR')) {
				...
				<div class="form-div">
					...
					<input type="hidden" name="txt_cur_cm_id" id="txt_cur_cm_id" value="<?php echo $_SESSION['__cm_id']; ?>" />
						...
						<div>
							...
							<div class="input-field col s6 m6 8 clsIDHome">
								...
								<select class="" id="txt_Request" name="txt_Request">
									...
									<?php
									if (($_SESSION["__cm_id"] == "88" || $_SESSION["__cm_id"] == "239" || $_SESSION["__cm_id"] == "265" || $_SESSION["__cm_id"] == "270" || $_SESSION["__cm_id"] == "420" || $_SESSION["__cm_id"] == "444" || $_SESSION["__cm_id"] == "445") && ($_SESSION["__df_id"] == '74' || $_SESSION["__df_id"] == '77' || $_SESSION["__df_id"] == '146' || $_SESSION["__df_id"] == '147' || $_SESSION["__df_id"] == '148' || $_SESSION["__df_id"] == '149')) {
									} else {
									?>
						...
						<div class="hid-container col s12 m12" style="">
							...
							<div id="pnlTable">
								<div class="had-container pull-left card col s12 m12">
									<div class="">
										<?php
										$sqlConnect = 'call GetRequestDetails3_new1("' . $_SESSION['__user_logid'] . '","' . $date_srch_from . '","' . $date_srch_to . '")';
										$myDB = new MysqliDb();
								...
								<input type="checkbox" class="check_val_" style="margin-left: 40%;" name="check_val_up[]" id="chkitem_' . $td_counter . '" value="' . $value['ID'] . '" onclick="checkAll();" /></td>';
														...
														if ($value['Level2'] == 'Pending' && $value['Level1'] == 'Pending' && $_SESSION['__user_logid'] == $value['EmployeeID']) {
...
<script>
	$(document).ready(function() {
							...
							success: function(result) {
								...
								if ($('#txt_ShiftIn').val() == "WO") {
									...
									if (parts) {
										...
										<?php
										if (($_SESSION["__cm_id"] == "88" || $_SESSION["__cm_id"] == "239" || $_SESSION["__cm_id"] == "265" || $_SESSION["__cm_id"] == "270" || $_SESSION["__cm_id"] == "420" || $_SESSION["__cm_id"] == "444" || $_SESSION["__cm_id"] == "445") && in_array($designationID, $desArray)) {
										?>
		...
		$('#txt_Request').change(function() {
			var cmid_arr = ['192', '311', '487', '702', '312', '703', '488', '313', '194', '314', '440', '427', '455', '670', '196', '669'];
			if (jQuery.inArray($('#txt_cur_cm_id').val(), cmid_arr) !== -1 && $(this).val() == 'Shift Change') {
				toastr.info('You are not allowed to raise shift change request');
					...
					if (Exception != '') {
						...
						if (EmployeeID == $('#txtEmpID1').val()) {
							...
							if (Level == '1' && l2id == '') {
								...
								if (MgnrStatus == 'Pending' && ReportTo == 'CE07147134' && ReportTo == <?php echo '"' . $_SESSION['__user_logid'] . '"'; ?>) {
							...
							if (Level == '1' && l2id == '') {
								...
								if (HeadStatus == 'Pending' && AccountHead == <?php echo '"' . $_SESSION['__user_logid'] . '"'; ?>) {
									$('#btn_Leave_Save').removeClass('hidden');
									...
									$('#txtApprovedBy_sh').val(l2name);
									$('#txtApprovedBy_shID').val(l2id);
								} else if (HeadStatus != 'Pending' && MgnrStatus == 'Pending' && l2id == <?php echo '"' . $_SESSION['__user_logid'] . '"'; ?>) {
