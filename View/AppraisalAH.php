...
if (isset($_POST['btnEdit'])) {
	...
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		...
		if (empty($Error)) {
			...
			echo "<script>$(function(){ toastr.error(Not Saved '); }); </script>";
		}
		// } else {
		// 	$EmployeeName = cleanUserInput($_POST['EmployeeName']);
		// 	$remaks = cleanUserInput($_POST['Remarks']);
		// 	$userType = cleanUserInput($_POST['UserType']);
		// 	echo $AHComment = 'call apr_addcomment("' . $DataId . '","' . $EmployeeId . '","' . $EmployeeName . '","' . $remaks . '","' . $userType . '")';
		// 	$myDB = new MysqliDb();
		// 	$result3 = $myDB->rawQuery($AHComment);
		// 	$error = $myDB->getLastError();
		// 	if (empty($error)) {
		...
		// 	} else {
		// 		echo "<script>$(function(){ toastr.error('Remarks Not Saved.''); }); </script>";
		// 	}
		// }
	}
}
			...
			<div class="schema-form-section row">
				...
				<div id="ComForm" style="display: none;">
					<div class="input-field col s12 m12 no-padding">
						...
						<div class="input-field col s12 m12">
							<input type="text" name="AppraisalPer" id="AppraisalPer" onkeypress="javascript:return isNumberKey(event);" maxlength="5">
...
<script>
	...
	$('#AppraisalPer').on('change keyup check', function() {
		var aprper = $(this).val();
		var NewCTC = Number(($("#CurrentCTCM").val() * aprper) / 100) + Number($("#CurrentCTCM").val());
		NewCTC = parseFloat(NewCTC).toFixed(0);
		if (isNaN(NewCTC)) {
