...
<span id="PageTittle_span" class="hidden">Upload Manual Performance</span>
...
<!-- Header for Form If any -->
	 <h4>Upload Manual Performance<!--<a class="btn-floating btn-large waves-effect waves-light modal-trigger tooltipped"  href="#myModal_content" data-position="bottom" data-tooltip="Download Formate"><i class="material-icons">file_download</i></a>--></h4>				
	...
	<div class="schema-form-section row" >		
				   	...
				   	<div class="input-field col s5 m5">
			            <select class="form-control" id="txt_Type_Upload"  name="txt_Type_Upload" >
			            	<option value="Hours">Manual Performance</option>
			            </select>
							...
							{							 	
								 	...
								 	{	
								 			//$date_month = date_parse($row['AG']);
 								 			$sql_p_inert='CALL insert_manual_performance("'.strtoupper($row['A']).'","'.$row['B'].'","'.$row['C'].'");';
						    	...
						    	if(file_exists($target_dir . basename($_FILES["fileToUpload"]["name"])))
									{
										...
										rename($target_file,$target_dir.time()."Performance.".$ext);
									}
						    }
						    else
						    {
