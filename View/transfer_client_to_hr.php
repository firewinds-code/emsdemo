...
$msg='';
$rowCount="";
if(isset($_POST['transfer_client']) && $_POST['transfer_client']=='Update')
{
	$status=$_POST['status'];	
			...
			for($i=$max_key;$i>=$min_key; $i--)
			{
				...
				if(isset($checked_arr[$i]) && $checked_arr[$i]!="" && $comment[$i]!=""){
					...
					if($status[$i]=='Pending'){
					 ...
					 $save="UPDATE tbl_client_toclient_move set hr_comment='".$comment[$i]."',status='".$status[$i]."',flag='".$flag."',HR_updated_on='".$date."',HR_updated_by='".$updatedBy."',Updated_by='".$updatedBy."',updated_on='".$date."',move_date='".$movedate."' where old_cm_id='".$old_client."' and EmployeeID='".$checked_arr[$i]."'  and id='".$moveid."' and move_location='".$Loggin_location."'";
				//echo "<br><br>";
	...
	<div class="schema-form-section row" >		
			    ...
			    if(isset($_GET['cm_id']) && $_GET['cm_id']!=""){
			    	...
			    	function getClientName($cmid){
			    		$myDB=new MysqliDb();
						$selecName=$myDB->rawQuery("Select concat(clientname,' | ',process,' | ',sub_process) AS newClient from whole_details_peremp where cm_id='".$cmid."' and location='".$_SESSION["__location"]."' " );
						$mysql_error = $myDB->getLastError();
						$rowCount = $myDB->count;
							
						if($rowCount>0){
							$name=$selecName[0]['newClient'];
							return $name;
						}
					}
