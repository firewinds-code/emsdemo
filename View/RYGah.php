...
$account_head= $_SESSION['__user_logid'];
					      	...
					      	<?php
								...
								foreach($result as $key=>$data_array){
									...
									<?php	}
										...
										<td class="rstatus"><span class="<?php echo $rcolorclass; ?>"  
										<?php if($data_array['rremark']!="") { 
										?> title="<?php echo  $data_array['rremark'];?>" <?php }else{
											 ?> title='Pending' <?php } ?>
										 ><i class="fa fa-lg fa-circle"></i></span><span style="padding-left:2px; " class="<?php echo $ohcolorclass; ?>" <?php if($data_array['ohremark']!="") { 
	...
	function rygSave(id){
		...
		if(sval!="" && substatus!="" && AHid!=""){
			if(remarks!="")
			{
				  ...
				  data: {'saves':'saves','rygstatus':sval,'substatus':substatus,'rygsr':remarks,'empid':empid,'ah':AHid}
				}).done(function( msg ) {
