			...
			<div class="schema-form-section row">
				...
				<div id="pnlTable">
					...
					if (isset($_POST['btn_ED_Search'])) {
						...
						$sqlConnect = 'select e.EmployeeID, e.EmployeeName, e.designation, e.process, e.subprocess, l.id,e.ReportTo,l.location from emp_details  as e join location_master as l on l.id=e.location where EmployeeId=?';
						...
						if ($result->num_rows > 0) { ?>
							<input type="hidden" name="old_locid" id="old_locid" value="<?php echo $old_loc; ?>" />
				...
				<?php if (isset($_POST['update'])) {
					$old_location = cleanUserInput($_POST['old_locid']);
					...
					$insert = 'insert into transfer_emp(EmployeeID,location,client_name,process,sub_process,reports_to,transfer_date,createdon,old_loc) values(?,?,?,?,?,?,?,?,?) ';
					...
					$stmt->bind_param("sissssssi", $EmpId, $location, $client_name, $process, $sub_process, $reports_to, $transfer_date, $date, $old_location);
