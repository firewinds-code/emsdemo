...
if (strtoupper($_SERVER['REQUEST_METHOD']) == 'POST') {
	...
	$uino =  $_POST['txt_uino'];
	$gratuity =  $_POST['txt_gratuity'];
	$medical_allowance =  $_POST['txt_medical_allow'];
...
if (isset($_REQUEST['empid']) && $EmployeeID == '' && !isset($_POST['EmployeeID'])) {
	...
	$getDetails = 'select t2.client_name from employee_map t1 inner join new_client_master t2 on t1.cm_id=t2.cm_id where t1.EmployeeID= "' . $EmployeeID . '"';
	$myDB = new MysqliDb();
	$result_all = $myDB->rawQuery($getDetails);
	$clientid = $result_all[0]["client_name"];
...
if (isset($_POST['btn_sal_Save']) && $EmployeeID != '') {
	...
	if ($emptype == '' || $paytype  == '' || $newctc  == '' || $MinWages  == '' || $basic   == '' || $hra   == '' || $convence   == '' || $sp_allow   == '' || $gorss_sal  == '' || $pf_count  == '' || $pf_empl_count  == '' || $esis_count  == '' || $esis_empl_count   == '' || $professional_tex  == '' || $takehome  == '' || $pli_percent  == '' || $pli_ammount  == '' || $nt_takehome  == '' || $pli_status  == '' || $pf_status  == '' || $pli_mode  == '' || $rt_type == '' || $gratuity == '' || $medical_allowance == '' || $bonus == '') {
		$validate = 1;
	}
					...
					<div class="input-field col s4 m4">
						<select id="txt_pli_deduct" name="txt_pli_deduct">
							...
							?>
								...
								<?php
							} else if ($_SESSION["__user_type"] == 'HR' && ($takehome == '')) {
