...
$GetData = "select c.emailid,w.EmployeeID , w.EmployeeName , w.location ,w.Gender ,w.MarriageStatus, w.designation ,e.rsnofleaving,DATE_FORMAT(e.dol,'%Y-%m-%d') as dol ,td.description,e.createdon,ad.address_p,w.ctc,w.FatherName,w.DOJ from whole_dump_emp_data as w inner join exit_emp e on w.EmployeeID=e.EmployeeID inner Join contact_details c on e.EmployeeID=c.EmployeeID left join termination_description td on  e.rsnofleaving=td.title left join address_details ad on w.EmployeeID = ad.EmployeeID where  cast(DATE_SUB(e.createdon, INTERVAL -11 DAY) as date)=cast(now() as date) and  e.disposition='TER' and w.EmployeeID='CMK082278807'";
...
if (count($resultsE) > 0) {
	foreach ($resultsE as $val) {
		...
		$Gender = $val['Gender'];
		$emailid = $val['emailid'];
		$FatherName = ucwords($val['FatherName']);
		...
		if ($loc != '') {
			if ($loc == "1" || $loc == "2") {
				$target_dir = ROOT_PATH . "experience_ter_emp_pdf/";
			}
			if ($loc == "3") {

				$target_dir = ROOT_PATH . "Meerut/experience_ter_emp_pdf/";
			} else if ($loc == "4") {

				$target_dir = ROOT_PATH . "Bareilly/experience_ter_emp_pdf/";
			} else if ($loc == "5") {
				$target_dir = ROOT_PATH . "Vadodara/experience_ter_emp_pdf/";
			} else if ($loc == "6") {
				$target_dir = ROOT_PATH . "Manglore/experience_ter_emp_pdf/";
			} else if ($loc == "7") {
				$target_dir = ROOT_PATH . "Bangalore/experience_ter_emp_pdf/";
			} else if ($loc == "8") {
				$target_dir = ROOT_PATH . "Nashik/experience_ter_emp_pdf/";
			} else if ($loc == "9") {
				$target_dir = ROOT_PATH . "Anantapur/experience_ter_emp_pdf/";
			} else if ($loc == "10") {
				$target_dir = ROOT_PATH . "Gurgaon/experience_ter_emp_pdf/";
			} else if ($loc == "11") {
				$target_dir = ROOT_PATH . "Hyderabad/experience_ter_emp_pdf/";
			}
		...
		///// pdf creation	
		$pdf = "<h4>EXPERIENCE LETTER</h4>";
		$pdfcenter = "<u><h4>TO WHOM IT MAY CONCERN</u></h4>";
		$pdf1 = "<table>
	             <tr><td>Date: " . dateFormatu($dateformat) . "</td></tr><br><br>
	            <br>
				<p>This is to state that Mr./Ms. " . $EmployeeName . ", S/D of Mr./Mrs. " . $FatherName . ", resident of " . $address . ", has been an employee of M/S Cogent E-services Limited. The details of " . $title1 . " employment as per our records are as under.</p>
				<br><br>
				 ...
				 <p>We wish " . $title2 . " success in their future endeavors.</p> <br /><br />";
		...
		$filename = $EmployeeID . "_Experience_letter.pdf";
		if ($target_dir != '') {
		...
		$tcpdf->SetTitle('Cogent| Experience Letter');
		...
		$tcpdf->writeHTMLCell($w = 0, $h = 0, $x = 0, $y = 70, $pdfcenter, $border = 0, $ln = 1, $fill = false, $reseth = true, $align = 'C');
		...
		if ($emailid != "") {
			...
			$mail->setFrom(EMAIL_FROM,  'Cogent | Experience Letter');

			$mail->AddAddress($emailid);
			$mail->addBCC('vijayram.yadav@cogenteservices.com');
			$mail->addBCC('sanoj.pandey@cogenteservices.com');
			$mail->Subject = "Candidate Experience Letter";
			$mail->isHTML(true);
			$msg2 = " Mr./Ms. " . $EmployeeName . ',<br/><br/>
		     Please find  attached your Experience letter which was processed on ' . $dol . '.</br><br/>
		     Yours Truly, <br/> Cogent ';
			$mail->Body = $msg2;
			$mymsg = '';
			$response = '';
		...
		<div class="form-div">
			<div class="schema-form-section ">
				<p style="text-align: right;">
					<?php
					if ($locationid == "1" || $locationid == "2" || $locationid == "4" || $locationid == "5" || $locationid == "6" || $locationid == "7" || $locationid == "8" || $locationid == "9" || $locationid == "10") {
						if ('AE' == substr($EmployeeID, 0, 2)) {
					?>
				...
				<h4 style="text-align: center"><u>EXPERIENCE LETTER</u></h4>

				<p><b>Date:</b><?php echo $dateformat; ?></p><br>
				<br>
				<p>This is to state that (Mr./Ms.) <?php echo $EmployeeName ?>, S/D of (Mr./Mrs.)
					<?php echo $EmployeeName ?>, resident of <?php echo $address ?>, has been an employee of M/S
					Cogent E-services Limited. The details of his/her employment as per our records are as under.</p>

				<p><b> Designation: </b><span><?php echo $designation; ?></span></p>
				<p><b> Date of Joining: </b><?php echo $doj; ?></p>
				<p><b> Last working day: </b><?php echo $dol; ?> </p>
				<p><b> Annual CTC: </b><?php echo $ctc; ?></p>

				<p>We wish him/her success in their future endeavors.</p><br><br>
