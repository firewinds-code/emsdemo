...
if (isset($_POST['btnSave'])) {
	...
	if (isset($_POST['change_report']) && $_POST['change_report'] == 'rea' && $_POST['emptype'] == 's') {
		...
		$employeeID = trim($_POST['employeeID']);
		$empstatus = "Active";
		//echo "call update_status_alteration('" . $employeeID . "','" . $empstatus . "','" . $createBy . "')";
	...
	if (isset($_POST['change_report']) && $_POST['change_report'] == 'rt' && $_POST['emptype'] == 's') {
		$myDB = new MysqliDb();
		$employeeID = trim($_POST['employeeID']);
		$rt = $_POST['rt_type'];
		$Update_rt = "call updatert_alteration('" . $employeeID . "','" . $rt . "','" . $createBy . "')";
		$result = $myDB->query($Update_rt);
		$error = $myDB->getLastError();
		$rowCount = $myDB->count;
		echo "<script>$(function(){ toastr.success('Roster type of Employee Updated Successfully'); }); </script>";
		// if ($rowCount > 0) {
		// 	echo "<script>$(function(){ toastr.success('Roster type of Employee Updated Successfully'); }); </script>";
		// } else {
	...
	if (isset($_POST['change_report']) && $_POST['change_report'] == 'rt' && $_POST['emptype'] == 'm') {
		$btnUploadCheck = 1;
		...
		if ($uploadOk == 0) {
			...
			if (move_uploaded_file($_FILES["empfile"]["tmp_name"], $target_file)) {
				...
				foreach ($activeSheetData as $row) {
					...
					if ($row_counter > 0 && !empty($row['A']) && $row['A'] != '') {
						...
						if ($rt == 'Full Time') {
							...
							$rt_type = '3';
						} elseif ($rt == 'Split Time') {
							$rt_type = '4';
						}
						$myDB = new MysqliDb();
						$update_rt = "call updatert_alteration('" . $row['A'] . "','" . $rt_type . "','" . $createBy . "')";
						$result = $myDB->query($update_rt);
				...
				if (file_exists($target_dir . basename($_FILES["empfile"]["name"]))) {
					...
					rename($target_file, $target_dir . time() . '_' . $uploader . "_alt_rosterType." . $ext);
				}
			} else {
		...
		if ($uploadOk == 0) {
			...
			if (move_uploaded_file($_FILES["empfile"]["tmp_name"], $target_file)) {
				...
				foreach ($activeSheetData as $row) {
					if ($row_counter > 0 && !empty($row['A']) && $row['A'] != '') {
						$myDB = new MysqliDb();
						$reactive_report = "call update_status_alteration('" . $row['A'] . "','" . $row['B'] . "','" . $createBy . "')";
						$result = $myDB->query($reactive_report);
						$mysql_error = $myDB->getLastError();
						if (empty($mysql_error)) {
							$count++;
				...
				if (file_exists($target_dir . basename($_FILES["empfile"]["name"]))) {
					$ext = pathinfo($target_file, PATHINFO_EXTENSION);
					rename($target_file, $target_dir . time() . '_' . $uploader . "_alt_reactive." . $ext);
				}
			} else {
				echo "<script>$(function(){ toastr.error('Sorry, there was an error uploading your file') }); </script>";
			}
	...
	<div class="pim-container row" id="div_main">
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				<a href="../FileContainer/upload_reactive.csv" id='change_Reactive' target="_blank" class="btn btn-default btn-danger">
					<i class="fa fa-download"></i> Upload Reactive Employee Format </a>
				...
				<a href="../FileContainer/upload_rt_type.csv" id='change_rtType' target="_blank" class="btn btn-default btn-danger">
					<i class="fa fa-download"></i> Upload Change Roster Type Format </a>
				<div class="input-field col s12 m12">
					...
					<select id="updateAction" name="change_report">
						...
						<option value='rt'>Change Roster Type</option>
					</select>
				...
				<div class="input-field col s12 m12" id='for_formattr'>
					...
					<div class="input-field col s6 m6 " id='statusdiv'>
						<input type="text" id="empstatus" name="empstatus" value="Active" placeholder="Active" disabled />
					...
					<div class="input-field col s6 m6" id='rt_type'>
						<select id="rt_type" name="rt_type">
