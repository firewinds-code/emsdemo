...
if ($btnsave) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		...
		$myDB = new MysqliDb();
		$query = 'call open_issueticket("' . $remark . '","' . $reqid . '");';
...
if ($btnclose) {
	if (isset($_POST["token"]) && isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]) {
		...
		$createdby = clean($_SESSION['__user_logid']);
		$myDB = new MysqliDb();
		$query = 'call close_issueticket("' . $remark . '","' . $reqid . '");';
		$result = $myDB->query($query);
		//echo $query;
		$mysql_error = $myDB->getLastError();
		$user_name = clean($_SESSION['__user_Name']);
				...
				if (!empty($reqid)) {
					...
					<div class="input-field col s9 m9"><b>' . $value['concern_off'] . '</b></div>';
						...
						<div align="right">';
								...
								$getOnlyReqDate = date('Y-m-d', strtotime($reqdate));
								$prev_month_ts = strtotime($getOnlyReqDate . '-1 month');
								$prev_month1st = date('Y-m-01', $prev_month_ts);
								// if ($getOnlyReqDate >= $prev_month1st && $getOnlyReqDate <= $currmonth) {
								// 	echo "11111111";
								// } else {
								// 	echo "22222222";
								// }
								if ($value['status'] == 'Resolve' && ($getOnlyReqDate >= $prev_month1st && $getOnlyReqDate <= $currmonth)) {
								...
								if ($value['status'] == 'Close') {
							      ...
							      <div class="input-field col s9 m9"><b> ' . $value['status'] . ' </b></div>';
									if ($value['rating'] == "" && $value['feedback'] == "") { ?>
										<div class="input-field col s3 m3"><b>How would you like to rate us?</b></div>
										<div class="input-field col s9 m9">
											<form action="" method="post">

												<!-- <span class="input-field col s3 m3">Ratinng
												...
												<span class="input-field col s5 m5" style="margin-top: 4rem;">
													<label style="margin-top: -3.6rem;" for="">Rating</label>
													<div class="radio-item">
														<input type="radio" id="likes" name="rating" value="10" class="ropt1">
														<label class="active" id="labelID" style="color: green;" for="likes"><i class="fa fa-thumbs-up fa-lg" style="font-size: 2rem;"></i></label>
													</div>

													<div class="radio-item">
														<input type="radio" id="dislikes" name="rating" value="1" class="ropt2">
														<label class="active" id="labelID" style="color: red;" for="dislikes"><i class="fa fa-thumbs-down fa-lg" style="font-size: 2rem;"></i></label>
													</div>
												</span>

												<span class="input-field col s5 m5">Feedback
													<textarea name="feedback" maxlength="250" id="feedback"></textarea>
												</span>
												<span class="input-field col s1 m1" style="margin-top:32px;">
													<button type="submit" id="feedSave" class="btn waves-effect waves-green" name="feedSave">Submit <i class="fa fa-send"></i></button>
												</span>
									...
									<?php
										...
										<div class="input-field col s9 m9"><b>
												<?php
												if ($value['rating'] < 10) {
													echo '<i class="fa fa-thumbs-down fa-lg" style="font-size: 2rem; color:red;"></i>';
												} else {
													echo '<i class="fa fa-thumbs-up fa-lg" style="font-size: 2rem; color:green"></i>';
												}
												?></b>
										</div>
				...
				<?php	}
						...
						if ($value['status'] == 'Refer') {
							...
							<div class="input-field col s9 m9"><b> Inprogress </b></div>';
								} else if ($value['status'] == 'Payout Request' || $value['status'] == 'Payout Reviewed') {
									echo '<div class="input-field col s3 m3">Request Status</div>
							<div class="input-field col s9 m9"><b> Inprogress </b></div>';
								} else {
									echo '<div class="input-field col s3 m3">Request Status</div>
	...
	$(function() {
		...
		$('#btnSave').click(function() {
			...
			if ($('#remark').val().length < 10) {
				$('#remark').addClass('has-error');
				validate = 1;
				$('<span id="spanremark" class="help-block">Remark should be greater than 10 character</span>').insertAfter('#remark');
				return false;
			}
