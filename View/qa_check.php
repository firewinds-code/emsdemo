			...
			<div class="schema-form-section row">
				<div class="form-inline">
					<div class="input-field col s6 s6">
						<select id="text_trcheck_Batch" name="text_trcheck_Batch">
							...
							if ($_SESSION['training_approver'] != 'No') {
								$sqlBy = 'SELECT  distinct  batch_master.BacthID,batch_master.BacthName FROM employee_map  left outer join status_table on status_table.EmployeeID=employee_map.EmployeeID  left outer join status_quality on employee_map.EmployeeID=status_quality.EmployeeID  left outer join batch_master on batch_master.BacthID=status_quality.BatchID join new_client_master on new_client_master.cm_id=employee_map.cm_id  where status_quality.EmployeeID !="' . $_SESSION['__user_logid'] . '" and status_table.Status=5 and ojt_status < 2 and employee_map.emp_status="Active" and new_client_master.client_name in (select client_id from training_master where Approver_id="' . $_SESSION['__user_logid'] . '") order by batch_master.BacthName';
							} else {
								$sqlBy = 'SELECT  distinct  batch_master.BacthID,batch_master.BacthName FROM employee_map  left outer join status_table on status_table.EmployeeID=employee_map.EmployeeID  left outer join status_quality on employee_map.EmployeeID=status_quality.EmployeeID  left outer join batch_master on batch_master.BacthID=status_quality.BatchID join new_client_master on new_client_master.cm_id=employee_map.cm_id  where status_quality.Quality="' . $_SESSION['__user_logid'] . '" and status_quality.EmployeeID !="' . $_SESSION['__user_logid'] . '" and status_table.Status=5 and ojt_status < 2 and employee_map.emp_status="Active" and new_client_master.client_name not in (select client_id from training_master) order by batch_master.BacthName;';
							}
					...
					<div id="pnlTable" class="col s12 m12 card">
						...
						$sqlConnect = 'call get_qachecklist("' . $_SESSION['__user_logid'] . '",' . $batch_id . ')';
						$myDB = new MysqliDb();
