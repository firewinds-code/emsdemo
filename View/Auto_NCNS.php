...
$m	= $date1->format('n');
$y  =$date1->format('Y');
$sqlstr= "select w.EmployeeID from ActiveEmpID w inner join (select EmployeeID  from calc_atnd_master where month=$m and year =$y and concat($d1,$d2,$d3)='AAA' )cal on w.EmployeeID = cal.EmployeeID  left join excp_emp_ncns_msg ex on w.EmployeeID = ex.EmpID where ex. EmpID is null";
