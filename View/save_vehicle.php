...
if ($result_all) {
	...
	if ($loc == "1" || $loc == "2") {
		...
		$loc_dir = 'Gurgaon/Docs/VehicleDocs/';
	} else if ($loc == "11") {
		$loc_dir = 'Hyderabad/Docs/VehicleDocs/';
	}
}
//-------------------------- Personal Details TextBox Details ----------------------------------------------//
if (strtoupper($_SERVER['REQUEST_METHOD']) == 'POST') {
	$dl_no = (isset($_POST['txt_dl_no']) ? $_POST['txt_dl_no'] : null);
	$dl_name = (isset($_POST['txt_dl_name']) ? $_POST['txt_dl_name'] : null);
	$rc_no = (isset($_POST['txt_rc_no']) ? $_POST['txt_rc_no'] : null);
	$rc_name = (isset($_POST['txt_rc_name']) ? $_POST['txt_rc_name'] : null);
	...
	if (isset($_FILES["File1"]["name"]) and $_FILES["File1"]["name"] != "") {
		...
		$CancelledCheque1 = $_POST['EmployeeID'] . '_dl_docs.' . $FileType;
	...
	if (isset($_FILES["File3"]["name"]) and $_FILES["File3"]["name"] != "") {
		...
		$CancelledCheque3 = $_POST['hiddenFile3'];
	}
} else {
...
if (isset($_POST['btn_experice_Add']) && $EmployeeID != '') {
	...
	$selectBank = $myDB->query("select EmployeeID from vehicle_details where EmployeeID='" . $EmployeeID . "' ");
...
if (isset($_POST['btn_experice_Save']) && $_POST['vehicleSelect'] != '') {
	$myDB = new MysqliDb();
	$createBy = $_SESSION['__user_logid'];
	$vehicle_id = $_POST['vehicleSelect'];
	...
	<div class="pim-container row" id="div_main">
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				<?php
				$sqlBankQuery = "select * from vehicle_details where EmployeeID='" . $EmployeeID . "' ";
				$myDB = new MysqliDb();
				...
				if (count($data_array) > 0) {
					$dl_no = $data_array[0]['dl_no'];
					$dl_name = $data_array[0]['dl_name'];
					$CancelledCheque1 = $data_array[0]['dl_docs'];
					$rc_no = $data_array[0]['rc_no'];
					$rc_name = $data_array[0]['rc_name'];
					$CancelledCheque2 = $data_array[0]['rc_docs'];
					$CancelledCheque3 = $data_array[0]['vehicle_docs'];
					$vehicelid = $data_array[0]['id'];
				}
