...
{
	$myDB=new MysqliDb();
	$delete_query="DELETE from module_manager  where ID='".$_GET['delid']."'";
	$delete_query="DELETE from manage_module_email  where moduleID='".$_GET['delid']."'";
...
if(isset($_POST['module_name'],$_POST['addmodule']))
{	$date=date('Y-m-d');
	$createdBy=$_SESSION['__user_logid'];
	$module_name=trim($_POST['module_name']);
			$myDB=new MysqliDb();		
		if($module_name!=""){
			$select_query="select ID from module_manager where module_name='".$module_name."' ";
			...
			{
				$insertQuery="INSERT INTO module_manager set module_name='".$module_name."',createdOn='".$date."' ";
...
{  //print_r($_POST);
	...
	$myDB=new MysqliDb();				
	if($module_name!=""){
			$updateQuery="Update  module_manager set module_name='".$module_name."' where ID='".$id."'";
		    ...
		    <div class="modal-content">
		      ...
		      <div class="modal-body">
						 <?php   	
							...
							$Id=$_GET['id'];
							$sqlConnect2 = 'select  * from  module_manager  where ID="'.$Id.'" '; 
							$module_name="";
							$myDB=new MysqliDb();
					    ...
					    <tbody>					        
					       ...
					       $i=1;
					        foreach($result as $key=>$val){
								...
								<a onclick="editData('<?php echo $ID; ?>','<?php echo $module_name; ?>');" ><i class="material-icons edit_item imgBtn imgBtnEdit tooltipped" data-position="left" data-tooltip="Edit">ohrm_edit</i></a></td>
