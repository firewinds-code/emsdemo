...
if (isset($_POST['transfer_client'])) {
	...
	//echo "<br><br><br>";
	$counttol = 0;
	$sub_process_info = $_POST['sub_process_info'];
	...
	if ($date >= 25 && $date <= 30) {
		...
		if (isset($_POST['tcid'])) {
			...
			if ($cm_id != "") {
				...
				if ($counttol > 0 && empty($mysql_error)) {
					$sender_data = "SELECT ofc_emailid,emailid  FROM contact_details  where EmployeeID  IN ('" . $oh . "' , '" . $updatedBy . "' )";
					$sender_AH = "";
					$sender_OH = "";
					$EmailTo = "";
					$email_array = array();
					...
					echo "<script>$(function(){ toastr.success('Sub-Process Transfter Request Proceeded Successfully.." . $lblMMAILmsg . "'); }); </script>";
				} else {
