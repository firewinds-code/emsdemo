			...
			<div class="schema-form-section row">
				...
				<div id="pnlTable">
					...
					if ($rpt_type == 'Incomplete Info') {
						...
						$myDB = new MysqliDb();
						$chk_task = $myDB->query("select personal_details.EmployeeID, EmployeeName,case when (FatherName='' or FatherName is null)  then 
'Not Updated' else FatherName end as FatherName , case when (DOB='' or DOB is null) then 
'Not Updated' 
else date_format(DOB,'%d %M,%Y')  end as DOB , case when (MotherName='' or MotherName is null)  
then 
'Not Updated' else MotherName end as MotherName , case when (Gender='' or Gender is null)  then
 'Not Updated' else Gender end as Gender , case when (BloodGroup='' or BloodGroup is null)  then
 'Not Updated' else BloodGroup end as BloodGroup , case when (MarriageStatus ='' or MarriageStatus 
 is null) then 'Not Updated' else MarriageStatus end as MarriageStatus,case when
 MarriageStatus ='Yes' and (Spouse='' or Spouse is null)  then 'Not Updated' else
 Spouse end as Spouse , case when MarriageStatus ='Yes' and (MarriageDate='' or
 MarriageDate is null)  then 'Not Updated' else MarriageDate end as MarriageDate , 
 case when MarriageStatus ='Yes' and (ChildStatus='' or ChildStatus is null)  then 
 'Not Updated' else ChildStatus end as ChildStatus from  personal_details 
  inner join employee_map on personal_details.EmployeeID=employee_map.EmployeeID
    WHERE (employee_map.emp_status='Active') and
 ((MarriageStatus ='' or MarriageStatus is null) or (MarriageStatus ='Yes' and
 (Spouse='' or Spouse is null)) or (FatherName='' or FatherName is null) or
 (DOB='' or DOB is null)  or (MotherName='' or MotherName is null) or 
 (Gender='' or Gender is null) or (BloodGroup='' or BloodGroup is null) 
 or (MarriageStatus ='Yes' and (MarriageDate='' or MarriageDate is null))
 or (MarriageStatus ='Yes' and (ChildStatus='' or ChildStatus is null) ))");
						$my_error = $myDB->getLastError();
						if (count($chk_task) > 0 && $chk_task) {
							$table = '<div class="had-container pull-left row card" style="margin-top: 10px;width: 100%;padding: 15px;"><table id="myTable" class="data dataTable no-footer row-border" cellspacing="0" width="100%"><thead><tr>';
							$table .= '<th>EmployeeID</th>';
						...
						$myDB = new MysqliDb();
						$chk_task = $myDB->query("select contact_details.EmployeeID, case when (mobile ='' or mobile is null) then 'Not Update' else 
mobile end as mobile,case when (altmobile ='' or altmobile is null) then 'Not Update' 
else altmobile end as altmobile,case when (em_contact ='' or em_contact is null) then 
'Not Update' else em_contact end as em_contact,case when (emailid ='' or emailid is null) 
then 'Not Update' else emailid end as emailid,case when (em_relation ='' or em_relation is null) 
then 'Not Update' else em_relation end as em_relation from contact_details 
 inner join employee_map on contact_details.EmployeeID=employee_map.EmployeeID
    WHERE (employee_map.emp_status='Active') and
 ((mobile ='' or mobile is null) or(altmobile ='' or altmobile is null) or(em_contact ='' or em_contact is null) 
or (emailid ='' or emailid is null) or (em_relation ='' or em_relation is null))");
						$my_error = $myDB->getLastError();
						if (count($chk_task) > 0 && $chk_task) {
							$table = '<div class="had-container pull-left row card" style="margin-top: 10px;width: 100%;padding: 15px;"><table id="myTable" class="data dataTable no-footer row-border" cellspacing="0" width="100%"><thead><tr>';
							$table .= '<th>EmployeeID</th>';
						...
						$chk_task = $myDB->query("select distinct whole_details_peremp.EmployeeID,EmployeeName, designation, Process, sub_process, ReportTo from whole_details_peremp left outer join doc_details on doc_details.EmployeeID=whole_details_peremp.EmployeeID
  where doc_file is not null");
						...
						$myDB = new MysqliDb();
						$chk_task = $myDB->query(" select distinct  whole_details_peremp.EmployeeID , case when edu_level='Post Graduation' then 'Done' else 'Not Done' end as PG,case when edu_level='Graduation' then 'Done' else 'Not Done' end as Graduation,case when edu_level 
in ('Basic','Diploma') then 'Done' else 'Not Done' end as Basic from education_details 
left outer join whole_details_peremp on education_details.EmployeeID=whole_details_peremp.EmployeeID where whole_details_peremp.EmployeeID is not null");
