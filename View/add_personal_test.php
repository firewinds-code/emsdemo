...
if (isset($_GET['id']) && trim($_GET['id']) != "") {
	...
	if (count($result2) > 0) {
		...
		echo $int_url = INTERVIEW_URL . "getSalary.php?intid=" . $intid;
		$curl = curl_init();
		...
		print_r($salary_array);
		if (count($salary_array) > 0) {
			$insertData = "call manageOfferSalary('" . $salary_array->salary . "','" . $intid . "','" . $EmployeeID . "','" . $_SESSION['__user_logid'] . "','" . $salary_array->emp_type . "')";
			$myDB = new MysqliDb();
