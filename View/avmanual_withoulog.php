...
$result = $myDB->query("SELECT aadhar_status FROM aadhar_verifiaction where EmployeeID = '".$userempid."' and aadhar_status ='verified' limit 1; ");
   ...
   $Adhar_DataEmployee=$myDB->rawQuery($getadharData);
	...
	$getEmployeeData="SELECT personal_details.EmployeeID,img,EmployeeName,FatherName,mobile,cast(DOB as date) as DOB FROM ems.personal_details  INNER JOIN contact_details ON personal_details.EmployeeID=contact_details.EmployeeID where personal_details.EmployeeID='".$userempid."';";
...
if(isset($_POST['btn_genrateOtpyes']))
{
	...
	$adhar_no=$_POST['aadhar_yes'];
	 $userempid=$_POST['empid'];
			...
			if($adhar_no !="")
			{
				$url = 'https://ems.cogentlab.com/erpm/Services/avotppro.php?aadhar='.$adhar_no;
...
if(isset($_POST['btn_verify_otp']))
{
	$userempid=$_POST['empidhnd'];
		...
		{ 
			...
			$checkData="SELECT FirstName,FatherName,cast(DOB as date) as DOB,EmployeeID FROM ems.personal_details where EmployeeID='".$userempid."' and FirstName='".$first_name[0]."' and DOB='".$resposne_data2->data->dob."'   limit 1;";
			...
			if(empty($mysql_error))
			{
				...
				{
					$remarks=" ";
					if(strpos(strtolower($FatherName),str_replace('  ',' ',trim(strtolower($verifyData[0]['FatherName'])))))					
					{
					...
					if(count($verifyEmpData) > 0 )
					{
						...
						$sql="UPDATE aadhar_verifiaction  SET aadhar_status = 'verified' , remarks='".$oldremark."' WHERE EmployeeID = '".$userempid."' ;";
					}
					else
					{						
					$fn=$resposne_data2->data->aadhaar_number.'.jpg';
						...
						$sql = "INSERT INTO aadhar_verifiaction (EmployeeID, adhar_no,created_by,aadhar_status,EmpName,FatherName,DOB,dist,loc,country,subdist,street,vtc,state,house,po,zip,image,remarks) VALUES ('".$userempid."', '".$aadhaarno."', '".$_SESSION['__user_logid']."' , 'verified' ,'".$empname."','".$aadhaar_FatherName."','".$aadhaardob."','".$resposne_data2->data->address->dist."','".$resposne_data2->data->address->loc."','".$resposne_data2->data->address->country."','".$resposne_data2->data->address->subdist."','".$resposne_data2->data->address->street."','".$resposne_data2->data->address->vtc."','".$resposne_data2->data->address->state."','".$resposne_data2->data->address->house."','".$resposne_data2->data->address->po."','".$resposne_data2->data->zip."','".$fn."','".$remarks."');";
				...
				$checkEmpData="SELECT remarks FROM ems.aadhar_verifiaction where EmployeeID='".$userempid."' limit 1;";
						...
						$checkData1 = "SELECT FirstName,FatherName,cast(DOB as date) as DOB FROM ems.personal_details where EmployeeID='".$userempid."' limit 1;";
				//echo	$checkData="SELECT * FROM ems.personal_details where EmployeeID='".$userempid."' and FirstName='".$first_name[0]."' and  FatherName='".$FatherName1[1].' '.$FatherName1[2]."' and DOB='".$resposne_data2->data->dob."'   limit 1;";
						$myDB=new MysqliDb();
					...
					if(strpos(strtolower($FatherName),str_replace('  ',' ',trim(strtolower($verifyData1[0]['FatherName'])))))
					//if(strtolower(trim($verifyData1[0]['FatherName'])) !=strtolower(trim($FatherName)))
					{
						$remarks .= "Father Name  ";
					}
					$remarks .= " not matched";
	...
	<div class="schema-form-section row" >
			...
			<div class="input-field col s5 m5 center-align">
				  <table>				
							...
							<tr><td><img src="<?php echo IMAGE.$EmployeeData[0]['img']?>" height="200px" width="200px"></td></tr>
							<tr><td>Name :<?php echo $EmployeeData[0]['EmployeeName'].' ('.$EmployeeData[0]['EmployeeID'].')' ?> </td></tr>
							<tr><td>Father Name : <?php echo $EmployeeData[0]['FatherName'] ?></td></tr>
							<tr><td>DOB : <?php echo $EmployeeData[0]['DOB'] ?></td></tr>
							<tr><td>Aadhar No. : <?php echo $Adhar_DataEmployee[0]['dov_value'] ?></td></tr>
							<tr><td>Contact no. : <?php echo $EmployeeData[0]['mobile'] ?></td></tr>
							<tr><td>Do you want to proceed with existing aadhar number ? </td></tr>
				...
				<div class="input-field col s5 m5 center-align">
				 <?php if(isset($resposne_data2->data->aadhaar_number) && $resposne_data2->data->aadhaar_number!="") {?>
					...
					<tr><td><img src="<?php echo '../aadharverification/'.$resposne_data2->data->aadhaar_number.'.jpg'; ?>" height="200px" width="200px">
						
					</td></tr>
							<tr><td>Name :<?php echo $resposne_data2->data->full_name ?>
							<?php if(strtolower(trim($resposne_data2->data->full_name))==strtolower(trim($EmployeeData[0]['EmployeeName']))){?>
							<i class="fa fa-check " style="color:green;font-size: 20px;" ></i>
							<?php }else{ ?>
							<i class="fa fa-times " style="color:red;font-size: 20px;" ></i>
							<?php } ?></td></tr>
							<tr><td>Father Name : <?php echo $resposne_data2->data->care_of ?>
							
					<?php if(strpos(strtolower($resposne_data2->data->care_of),str_replace('  ',' ',trim(strtolower($EmployeeData[0]['FatherName']))))){?>
								<i class="fa fa-check " style="color:green;font-size: 20px;" ></i>
							<?php }else{ ?>
								<i class="fa fa-times " style="color:red;font-size: 20px;" ></i>
							<?php } ?>
							</td></tr>
							<tr><td>DOB : <?php echo $resposne_data2->data->dob ?>
							...
							<tr><td>Aadhar no. : <?php echo $resposne_data2->data->aadhaar_number ?></td></tr>
				...
				<div class="schema-form-section row" s5 m5  >
					<div style="display:none" id="aadhar_idno">
						<form id="genrateotpno" method="post">
							...
							<div class="input-field col s5 m5 center-align">
							   <button name="btn_genrateOtpno" id="btn_genrateOtpno" class="btn waves-effect waves-green">Genrate OTP</button>
							</div>
						</form>		
					</div>		
					...
					<div style="display:none" id="aadhar_idyes">
						<form id="genrateotpyes" method="post">
							<div class="input-field col s5 m5 center-align" >
							   <input type="hidden"  name="aadhar_yes" id="aadhar_yes"  value="<?php echo $Adhar_DataEmployee[0]['dov_value'] ?>" /></div>
							   <input type="hidden"  name="empid"  value="<?php echo $EmployeeData[0]['EmployeeID'] ?>" />
							<div class="input-field s5 m5 center-align">
							   <button name="btn_genrateOtpyes" id="btn_genrateOtpyes" class="btn waves-effect waves-green">Genrate OTP</button>
							</div>
						</form>		
						</div>
				</div>
