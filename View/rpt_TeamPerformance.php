...
<!-- Form container if any -->
	<div class="schema-form-section row" >
	       ...
	       <div id="pnlTable">
				<?php 
					$myDB = new MysqliDb();
					$dt_Test = $myDB->query("select * from (select performance_data.* from performance_data inner join status_table on status_table.EmployeeID = performance_data.EmployeeID inner join employee_map on employee_map.EmployeeID = performance_data.EmployeeID  and emp_status = 'Active' where (status_table.ReportTo = '".$_SESSION['__user_logid']."' or status_table.Qa_ops = '".$_SESSION['__user_logid']."') and performance_data.Type ='Detail'   order by DateFor desc,EmployeeID ) performance_data group by EmployeeID");
