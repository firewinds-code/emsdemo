...
if (isset($_POST['btn_df_Save'])) {
	...
	$min_value  = 	$_POST['txt_minValue'];
	$max_value  = 	$_POST['txt_maxValue'];
	$avg_value  = 	$_POST['txt_AverageValue'];

	$myDB = new MysqliDb();
	$result_check = $myDB->query('SELECT * FROM tbl_salary_slab_by_cps where cm_id = "' . $_cm_id . '"');
	if (count($result_check) > 0 && $result_check) {
		...
		$Insert = 'insert into tbl_salary_slab_by_cps(cm_id, min_lim, max_lim,avg_sal) VALUES("' . $_cm_id . '","' . $min_value . '","' . $max_value . '","' . $avg_value . '")';
		$myDB = new MysqliDb();
...
if (isset($_POST['btn_df_Edit'])) {
	...
	if (empty($_POST['txtEditID'])) {
		...
		$Update = 'update tbl_salary_slab_by_cps set cm_id = "' . $_cm_id . '", min_lim = "' . $min_value . '", max_lim = "' . $max_value . '", avg_sal = "' . $avg_value . '", modifiedon=now() where id="' . $_POST['txtEditID'] . '"';
		$myDB = new MysqliDb();
	...
	<div class="pim-container row" id="div_main">
		...
		<div class="form-div">
			...
			<div class="schema-form-section row">
				...
				<div id="pnlTable">
					<?php
					$sqlConnect = 'SELECT t1.id, t1.cm_id, t1.min_lim, t1.max_lim, t1.avg_sal, t3.client_name, t2.process, t4.location, t2.sub_process,t1.createdon,t1.modifiedon
						...
						<table id="myTable" class="data dataTable no-footer row-border" cellspacing="0" width="100%">
							...
							<tbody>
								...
								foreach ($result as $key => $value) {
									...
									echo '<td class="modifiedon">' . $value['modifiedon'] . '</td>';
