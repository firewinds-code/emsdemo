...
$expLetterSearchID = (isset($_POST['searchID']) ? $_POST['searchID'] : null);
...
if ($_SESSION['__user_logid'] == 'CE09134997' || $_SESSION['__user_logid'] == 'CE12102224') {
	// proceed further
} else {
	$location = URL . 'error';
			...
			<div class="schema-form-section row">
				...
				<div id="pnlTable">
					...
					if (isset($_POST['btn_ID_search'])) {
						...
						$sqlConnect = "select * from releiving_experience_ack where EmployeeID='" . $_POST['searchID'] . "' ";
						$myDB = new MysqliDb();
						$result = $myDB->rawQuery($sqlConnect);
						$locate = isset($result[0]['location_id']) ? $result[0]['location_id'] : '';
						...
						if (count($resultsE) > 0) {
							foreach ($resultsE as $val) {
								...
								if ($emailid != "") {
									...
									$myDB = new MysqliDb();
									$Insertrelieving = "insert into releiving_experience_ack (EmployeeID, Mail_response,file_name, email_id ,Created_date,location_id)values('" . $EmployeeID . "','" . $response . "','" . $filename . "','" . $emailid . "','" . $dt . "','" . $locationid . "');";
									$myDB = new MysqliDb();
