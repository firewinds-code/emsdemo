...
if(isset($_SESSION))
{
	...
	else
	{
		...
		{
			$sql = "update resign_details set revoke_status = 1,revoke_on = '".date('Y-m-d H:i:s')."',revoke_comment='".$_POST['txtComent_agent']."'  where  EmployeeID = '".$_SESSION['__user_logid']."' and rg_status = 1 and accept = 1";
			$myDB = new MysqliDb();
			$myDB->query($sql);	
		}
...
<style>
	table td,td, table.dataTable tbody td, table.dataTable tbody td
	{
		line-height: 18px;
	}
</style>
...
<!-- Form container if any -->
	<div class="schema-form-section row">
		...
		<div class="flow-y-scroll">
		 <?php
			$myDB=new MysqliDb();
			$chk_task=$myDB->query('select w.DOJ,w.designation,w.clientname,w.Process,w.sub_process ,alert_details.*,pd1.EmployeeName,pd.EmployeeName as Suppervisor,pd1.img from alert_details left outer join whole_dump_emp_data w on w.EmployeeID = alert_details.EmployeeID left outer join personal_details pd1 on pd1.EmployeeID = alert_details.EmployeeID left outer join personal_details pd on pd.EmployeeID = ReportTo where (curdate() between alert_details.alert_start and alert_end) and (alert_details.EmployeeID = "'.$_SESSION['__user_logid'].'" or account_head = "'.$_SESSION['__user_logid'].'" or oh =  "'.$_SESSION['__user_logid'].'" or th = "'.$_SESSION['__user_logid'].'" or ReportTo = "'.$_SESSION['__user_logid'].'" or  qh = "'.$_SESSION['__user_logid'].'" or "CE03070003" =  "'.$_SESSION['__user_logid'].'" or (select true from whole_details_peremp where cm_id  = "37" and des_id in  (1,5,7,8,10) and EmployeeID = "'.$_SESSION['__user_logid'].'"))');
