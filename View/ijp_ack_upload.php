...
if ($EmployeeID == 'CE01145570' || $EmployeeID == 'CE03070003' || $EmployeeID == 'CE10091236' || $EmployeeID == 'CE0223950633') {
    // proceed further
} else {
    $location = URL . 'error';
...
<div id="content" class="content">
    ...
    <span id="PageTittle_span" class="hidden">Upload IJP Data</span>
        ...
        <div class="form-div">
            ...
            <div class="schema-form-section row">
                ...
                if (isset($_POST['UploadBtn'])) {
                    $btnUploadCheck = 1;
                    $target_dir = ROOT_PATH . 'IJP_upload/';
                        ...
                        if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
                            ...
                            foreach ($activeSheetData as $row) {
                                if ($row_counter > 0 && !empty($row['A']) && $row['A'] != '') {
                                    $myDB = new MysqliDb();
                                    $flag = $myDB->query('Insert into ijp_req (empid,Msg1,Manager,TeamLeader,QualityAnalyst,ProcessTrainer,BusinessCompliance,MIS_WFM,HR,IT)values ("' . $row['A'] . '","' . $row['B'] . '","' . $row['C'] . '","' . $row['D'] . '","' . $row['E'] . '","' . $row['F'] . '","' . $row['G'] . '","' . $row['H'] . '","' . $row['I'] . '","' . $row['J'] . '")');
