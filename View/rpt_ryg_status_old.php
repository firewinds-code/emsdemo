...
foreach($result as $lval){
	...
	$reporttsto='';
	$where="";
	if(isset($_SESSION['__status_oh']) && $_SESSION['__status_oh']==$_SESSION['__user_logid']){
		$ohloginid=$_SESSION['__status_oh'];
		$whereoh=" ncm.oh='".$ohloginid."' ";
		
	}	
	if(isset($_SESSION['__status_ah']) && $_SESSION['__status_ah']==$_SESSION['__user_logid']){
		$ahloginid=$_SESSION['__status_ah'];
		$whereah="  ncm.account_head='".$ahloginid."' "; 
	}
	if(isset($_SESSION['__user_logid']) && $_SESSION['__user_logid']!=""){
		//echo "select EmployeeID from status_table where ReportTo='".$_SESSION['__user_logid']."' ";
		$resultrrt=$myDB->query("select EmployeeID from status_table where ReportTo='".$_SESSION['__user_logid']."' ");
		if(count($resultrrt)>0)
		{ 
			if(isset($resultrrt[0]['EmployeeID']) && $resultrrt[0]['EmployeeID']!="")
			{
				$reporttsto=$_SESSION['__user_logid'];
			}
		}
	}
	if(isset($_SESSION['__user_type']) && $_SESSION['__user_type']=='ADMINISTRATOR' || $_SESSION['__user_type']=='CENTRAL MIS' ){
		$condition='';
	}else{
...
if( isset($_POST['newYear']))
{
  ...
  $sql="SELECT allEmp.EmployeeID,allEmp.emp_status,allEmp.account_head,allEmp.EmployeeName ,allEmp.location,allEmp.client_name,allEmp.process,allEmp.sub_process,ahstatus,ahsubstatus,ahremark,ahcreated_on,rstatus,rsubstatus,rremark,rcreated_on,ohstatus,ohsubstatus,ohsubstatus,ohremark,ohcreated_on from (SELECT distinct(ep.EmployeeID),ep.emp_status,ncm.account_head,pd.EmployeeName,pd.location,cm.client_name,ncm.`process`,ncm.`sub_process` from employee_map ep inner join personal_details pd on ep.EmployeeID=pd.EmployeeID inner join new_client_master ncm on ncm.cm_id=ep.cm_id inner Join client_master cm on cm.client_id=ncm.client_name  inner join status_table st  on st.EmployeeID=ep.EmployeeID $condition ) allEmp left join ( select EmployeeID,ryg_status as ahstatus ,ryg_substatus as ahsubstatus,ryg_remark as ahremark ,created_on as ahcreated_on from ryg_ah where Month(created_on)='".$Month."' and YEAR(created_on)='".$Year."' ) ryg on allEmp.EmployeeID=ryg.EmployeeID left JOIN ( select EmployeeID,ryg_status as rstatus,ryg_substatus as rsubstatus,ryg_remark as rremark,created_on as rcreated_on from ryg_reportto where Month(created_on)='".$Month."' and YEAR(created_on)='".$Year."') reportto on allEmp.EmployeeID=reportto.EmployeeID left JOIN ( select EmployeeID,ryg_status as ohstatus,ryg_substatus as ohsubstatus,ryg_remark as ohremark,created_on as ohcreated_on from ryg_oh where Month(created_on)='".$Month."' and YEAR(created_on)='".$Year."' ) rygoh on allEmp.EmployeeID=rygoh.EmployeeID left join exit_emp emp on emp.EmployeeID=allEmp.EmployeeID where allEmp.emp_status='Active' or ( Month(emp.createdon)='".$Month."' and YEAR(emp.createdon)='".$Year."' )  ";
...
<div class="input-field col s12 m12" id="rpt_container">
		...
		?>
			...
			<?php if(count($month_result)>0){
				foreach($month_result as $val){
				 ?>
					<option value="<?php echo $val['val']; ?>" <?php if($Month==$val['val']){ echo "selected"; } ?>><?php echo $val['month']; ?></option>
		...
		<div class="input-field col s3 m3">
			...
			<select name="newYear"  id="newYear"  >
				<option value="<?php echo ($Year2-1);?>" <?php if($Year==($Year2-1)){ echo "selected";}  ?> ><?php echo ($Year2-1);?></option>
				<option value="<?php echo $Year2;?>" <?php if($Year==($Year2)){ echo "selected";}  ?>  ><?php echo $Year2;?></option>
				<option value="<?php echo ($Year2+1);?>" <?php if($Year==($Year2+1)){ echo "selected";}  ?>   ><?php echo ($Year2+1);?></option>
			</select>
		</div>
