...
$GetData = "select distinct t2.EmployeeId, t1.EmployeeId,t1.EmployeeName,t1.WEF,t1.PLIMode,t1.NewCTC,t1.PLIPercent,t3.emailid,t4.location,t4.gender from tbl_increment t1 left outer join tbl_promotion t2 on t1.EmployeeId = t2.EmployeeId join contact_details t3 on t1.EmployeeId = t3.EmployeeID join personal_details t4 on t1.EmployeeID = t4.EmployeeID where t1.EmployeeId='CE12102224' and t1.Flag=0 and case when t2.EmployeeId is null then 1=1 else   t2.Flag!=0 end limit 100";
//Flag=0";
$myDB = new MysqliDb();
...
if (count($resultsE) > 0) {
	foreach ($resultsE as $val) {
		...
		<p>Congratulations ! <br/> <br/> Based on your performance the company has decided to accord you an increment in salary. ";
			$pdf = $pdf . " Henceforth, your revised remuneration w.e.f. ";
			$pdf = $pdf . dateFormatu1(date('j-F-Y', strtotime($WEF)));
			$pdf = $pdf . " will be as under:</p><p><b>Annual CTC: INR </b>";
			$pdf = $pdf . $NewCTC;
		...
		<p>Yours truly,</p>
			...
			$tcpdf->Image('../Style/img/sk_sign.jpeg', 10, 185, 30, 15, 'JPEG'); //SIG
			...
			$tcpdf->writeHTMLCell($w = 0, $h = 0, $x = 10, $y = 200, $pdf3, $border = 0, $ln = 1, $fill = false, $reseth = true, $align = '');
			...
			if ($emailid != "") {
			    ...
			    Please find  attached your Increment letter. <br/><br/> Yours truly <br/> HR Team <br/> Cogent E Services Limited.';
				$mail->Body = $msg2;
				$mymsg = '';
				$response = '';
