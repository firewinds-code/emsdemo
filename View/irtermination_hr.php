...
if(isset($_POST['btn_Issue_Save']))
{
	...
	{
		$TerDate=$_POST['TerminationDateHR'];
	}
	else
	{
		$TerDate=$_POST['TerminationDateAH'];
	}
	if(isset($_POST['PLIDeductionYN']))
	{
		$Deduction=$_POST['PLIDeductionYN'];
		$Months=$_POST['Months'];
	}
	$Insert="update warning_rth set hr_status='".$_POST['IRTStatusHR']."',`Termination Date`='".$TerDate."',`hr_remark`='".addslashes($_POST['remark'])."',`PLI_Deduction`='".$Deduction."',`Months`='".$Months."',hr_Datetime= now() where id='".$_POST['DataID']."';";
	$myDB=new MysqliDb();
		...
		if ($_FILES['txt_doc_name_']['size'][$name] > 400000) 
		{
			...
			if($validation_check === 0)
			{
				if(move_uploaded_file($sourcePath,$targetPath)) 
				{
					...
					$filename=$_POST['EmployeeID'].'_'.preg_replace('/\s+/','',$_POST['IRTStatusHR']).'_'.date("dmYhis").'.'.$ext;
					$file=rename($targetPath,ROOT_PATH.'wt_docs/'.$filename);
					//$file=rename($targetPath,$_SERVER['DOCUMENT_ROOT'].'ems/wt_docs/'.$filename);
							$myDB=new MysqliDb();
	$sqlInsertDoc="insert into warning_rth_documents(EmployeeID,DataId,Document,Title,UploadedBy,`By`,UploadedDate)values('".$_POST['EmployeeID']."','".$_POST['DataID']."','".$filename."','".$_POST['txt_doc_value_'.$count]."','".$_SESSION['__user_logid']."','HR',Now());";
...
//////////////////////////////////////////////////////////////////////////////////
		$dsfsdf=$_POST['IRTStatusHR'];
		if($_POST['IRTStatusHR']=='Approved')
		{	$ah_remark='';
			if(isset($_POST['ah_Remark'])){
				$ah_remark=$_POST['ah_Remark'];
			}
			$now='Now()';
			...
			/*$Insert="INSERT INTO exit_emp(EmployeeID,dol,rsnofleaving,hrcmt,optcmt,createdby,disposition)VALUES('".$_POST['EmployeeID']."',".$now.",'TER','".addslashes($_POST['remark'])."','".addslashes($ah_remark)."','".$_SESSION['__user_logid']."','TER');";*/
				...
				{
					$myDB=new MysqliDb();
			 	    $Insert='call exit_employee("'.$_POST['EmployeeID'].'",'.$now.',"'.$subtype.'","'.addslashes($_POST['remark']).'","'.addslashes($ah_remark).'","'.$_SESSION['__user_logid'].'","TER")';
	...
	{
		echo "<script>$(function(){ toastr.success('Error-'.$mysql_error) }); </script>";
	}
}
?>
...
<div class="form-div">
	 <h4>Refer to HR Requests<!--<a class="btn-floating btn-large waves-effect waves-light modal-trigger tooltipped" id="ContentAdd" href="#myModal_content" data-position="bottom" data-tooltip="Add Issue"><i class="material-icons">add</i></a>--></h4>	
			   ...
			   <!-- <div class="input-field col s12 m12">
			    ...
			    <div class="input-field col s4 m4">
				            <select id="IRTStatusHR" name="IRTStatusHR" required>
					            ...
					            <?php
					             if(isset($_POST['SearchBy']) && $_POST['SearchBy'] == 'Refer to HR')
					            {
        ...
        <form method="post">
		     ...
		     <div id="pnlTable">
				...
				if(isset($_POST['btnSearch']))
				{
					...
					{
						 $sqlConnect = "SELECT wr.* FROM warning_rth wr inner join personal_details pd on wr.EmployeeID = pd.EmployeeID where ((wr.ah_status = '".$_POST['SearchBy']."' and wr.hr_status is null) or (wr.ah_status = 'Refer to QH' and wr.qh_status ='Approved' and wr.hr_status is null)) and pd.location in (1,2);";
					}
					else
					{
						 $sqlConnect = "SELECT wr.* FROM warning_rth wr inner join personal_details pd on wr.EmployeeID = pd.EmployeeID where ((wr.ah_status = '".$_POST['SearchBy']."' and wr.hr_status is null) or (wr.ah_status = 'Refer to QH' and wr.qh_status ='Approved' and wr.hr_status is null)) and pd.location in ('".$_SESSION["__location"]."');";
					}
					//echo $sqlConnect;
...
$('#IRTStatusHR').change(function()
	{
		$('#month').hide();$('#month').val('1');
		$('#pli').hide();$('#PLIDeductionYN').prop('checked', false);
...
function EditData(el)
{
	...
	$('#myModal_content').modal('open');
		...
		$('#TerminationDateHR').datetimepicker({timepicker:false,format:'Y-m-d',minDate: new Date(TerminationDateAH),maxDate: new Date()});
		$('select').formSelect();
		 $.ajax({ url: <?php echo '"'.URL.'"';?>+"View/bindDocIR1.php?empid="+EmployeeID_ }).done(function(data) {$('#AppendDoc').html(data)});
