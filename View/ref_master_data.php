...
{
	//echo "add";
	$_Scheme=(isset($_POST['txt_Ref_Name'])? $_POST['txt_Ref_Name'] : null);
...
{	//echo "edit";
	...
	$_Name=(isset($_POST['txt_Ref_Name'])? $_POST['txt_Ref_Name'] : null);
...
$('#btn_Client_Edit,#btn_Client_Save').on('click', function(){
        ...
        if(DateFrom!="" && DateTo!="" ){
	        jQuery.ajax({url: <?php echo '"'.URL.'"';?>+"Controller/ref_validation.php?todate="+DateTo+"&fromdate="+DateFrom+"&ID="+ID}).done(function(data) {
	        	//alert(data);
				if(data==0){
					$('#txt_from').parent('.select-wrapper').find('.select-dropdown').addClass('has-error');
					if($('#stxt_txt_from').size() == 0)
						{
 ...
 function DeleteReq(el)
	{
		if(confirm("Do you Want to Delete Request"))
		{
			...
			var ID = tr.find('.ID').text();
			alert(ID);
			$.ajax({url: "../Controller/deleteRefScheme.php?ID="+$item.attr("ID"), success: function(result){
