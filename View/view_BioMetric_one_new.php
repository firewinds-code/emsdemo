...
if (isset($_POST['p_EmpID'])) {
	...
	$getUserDetails = $myDB->query("select EmployeeID,EmployeeName,designation,status,cm_id from whole_details_peremp where EmployeeID = '" . $_POST['p_EmpID'] . "'");
	if ($getUserDetails) {
	...
	$myDB = new MysqliDb1();
	if ($_SESSION["__view_Bio"] == "") {
		$getUserDetails = $myDB->query("select  EmployeeID,EmployeeName,designation,status from whole_details_peremp where EmployeeID = '" . $_SESSION['__user_logid'] . "'");
	} else {
...
class BioMetric
{
	...
	private $BioIn = array();
	private $BioOut = array();
	private $Bio_tt = array();
	private $APR = array();
	...
	public function __construct($EmployeeID, $DateFrom, $DateTo, $Emd_des, $Emp_status, $emp_dod, $emp_module)
	{
		...
		if (date('m') == date('m', strtotime($DateFrom))) {
			$sql_bio = 'select DateOn,PunchIn_Time,PunchOut_Time,Total_Hr from bioinout_report where EmployeeID ="' . $EmployeeID . '" and DateOn between "' . $DateFrom . '" and "' . $DateTo . '" order by dateon';
		} else {
			$sql_bio = 'select DateOn,PunchIn_Time,PunchOut_Time,Total_Hr from bioinout_report_pre where EmployeeID ="' . $EmployeeID . '" and DateOn between "' . $DateFrom . '" and "' . $DateTo . '" order by dateon';
		}

		$ds_bio = $myDB->query($sql_bio);

		if (count($ds_bio) > 0 && $ds_bio) {

			foreach ($ds_bio as $key => $val) {
				$date_for = $val['DateOn'];

				$this->BioIn[$date_for] = trim($val['PunchIn_Time']);
				$this->BioOut[$date_for] = trim($val['PunchOut_Time']);
				$this->Bio_tt[$date_for] = trim($val['Total_Hr']);
		...
		if ($__date != null) {
			...
			if ($this->Roster_type[$__date] == "4") {
				...
				if ($__date == date('Y-m-d')) {
					if (!is_numeric($this->RosterIn[$__date][0][0]) || !is_numeric($this->RosterOut[$__date][0][0])) {
						...
						$str_capping = 'select EmpID,PunchTime, DateOn from biopunchcurrentdata where EmpID="' . $__EmpID . '" and DateOn  = "' . $__date . '"';
						$ds_punchtime = $myDB->query($str_capping);
						if (count($ds_punchtime) > 0 && $ds_punchtime) {
					...
					$i_bioIN = $this->BioIn[$__date];
					$i_bioOUT = $this->BioOut[$__date];
					$b_tt = $this->Bio_tt[$__date];


					$pos = strpos($i_bioIN, '|');
					if ($pos != '') {
						$i_bioIN1 = substr($i_bioIN, 0, $pos);
						$i_bioOUT1 = substr($i_bioIN, $pos + 1, strlen($i_bioIN) - 1);
					} else {
						$i_bioIN1 = $i_bioIN;
						$i_bioOUT1 = $i_bioIN;
					}
					...
					$pos = strpos($i_bioOUT, '|');
					if ($pos != '') {
						$i_bioIN2 = substr($i_bioOUT, 0, $pos);
						$i_bioOUT2 = substr($i_bioOUT, $pos + 1, strlen($i_bioOUT) - 1);
					} else {
						$i_bioIN2 = $i_bioOUT;
						$i_bioOUT2 = $i_bioOUT;
					}
					...
					$pos = strpos($b_tt, '|');
					if ($pos != '') {
						$b_tt1 = substr($b_tt, 0, $pos);
						$b_tt2 = substr($b_tt, $pos + 1, strlen($b_tt) - 1);
					} else {
						$b_tt1 = $b_tt;
						$b_tt2 = $b_tt;
					}
			...
			//For rest employee biometric calculation
				...
				if (!empty($i_rosterIN) && trim($i_rosterIN) != '') {
					...
					$i_bioOUT = null;
					//echo '---' . $i_rosterIN[0] . '----';
					// if (!is_numeric($i_rosterIN[0])) {
					// }
					//echo '--' . $i_rosterIN[0] . '--';
					if ($__date == date('Y-m-d') || !is_numeric(trim($i_rosterIN[0]))) {
						//echo strlen($i_rosterIN[0]) . '--' . $i_rosterIN[0] . '||';
						$myDB = new MysqliDb1();
...
"CE121622102"= "'.$_SESSION['__user_logid'].'" ) and EmployeeID="'.EmployeeID_forPage.'"');*/
			...
			$location = '';
			$logineLocation = $_SESSION["__location"];
			if (count($elQuwery) > 0) {
				if (isset($elQuwery[0]['location']) && $elQuwery[0]['location'] != "") {
					$EmpLocation = $elQuwery[0]['location'];
				}
			}
			$locationHead = '';
			...
			$tmpcheck = $myDB->query('select EmployeeID from whole_details_peremp Where ("' . EmployeeID_forPage . '" = "' . $_SESSION['__user_logid'] . '" or account_head ="' . $_SESSION['__user_logid'] . '" or oh ="' . $_SESSION['__user_logid'] . '" or ReportTo = "' . $_SESSION['__user_logid'] . '" or th = "' . $_SESSION['__user_logid'] . '" or qh = "' . $_SESSION['__user_logid'] . '" or  ("' . $_SESSION['__user_logid'] . '"="' . $locationHead . '") or  ("' . $_SESSION['__user_logid'] . '"="' . $ID1 . '") or  ("' . $_SESSION['__user_logid'] . '"="' . $ID2 . '")  or "' . $_SESSION['__user_logid'] . '"  in (SELECT EmployeID FROM ems.empid_action where Code=1) ) and EmployeeID="' . EmployeeID_forPage . '"
union
select distinct EmployeeID from module_master_new where EmployeeID="' . EmployeeID_forPage . '" and (l1empid="' . $_SESSION['__user_logid'] . '" or l2empid="' . $_SESSION['__user_logid'] . '")
union
select distinct requestby from issue_tracker where requestby="' . EmployeeID_forPage . '" and lo1="' . $_SESSION['__user_logid'] . '"
union
select er_scop from new_client_master where cm_id in (select cm_id from employee_map where EmployeeID="' . EmployeeID_forPage . '" ) and er_scop="' . $_SESSION['__user_logid'] . '"
');
