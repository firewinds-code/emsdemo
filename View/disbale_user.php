...
if (isset($_REQUEST['submit'])) {
    ...
    $curl = curl_init();
    $url = 'https://verve.cogentlab.com/auth/DisableUser';
    ...
    $sql = "insert into auth_manage_userAPI(type,request,response,url) values('disable','" . $jsonErr . "', '" . $response . "', '" . $url . "')";
    $resultss = $myDB->query($sql);
    $mysql_error = $myDB->getLastError();
    // echo $sql = "insert into auth_manage_userAPI(type,request,response,url) values('disable',?,?,?)";
    ...
    if (empty($mysql_error)) {
        echo "<script>$(function(){toastr.success('Disable Successfully'); }); </script>";
    } else {
        echo "<script>$(function(){toastr.error('Failed !'); }); </script>";
    }
}
...
<div id="content" class="content">
    <span id="PageTittle_span" class="hidden">Disable User</span>
                    ...
                    <div class="col s12 m12">
                        <form method="post" enctype="multipart/form-data">
                            ...
                            <div class="input-field col s10 m10 right-align">
                                <input type="submit" class="btn btn-primary" id="submit" name="submit" value="Disable" />
                            </div>
                        </form>
                    </div>
                </div>
