			...
			$mysql_error = '';
				foreach($_POST['cb'] as $val)
				{
					...
					if($Status !='NA'||$Status !='Na')
					{
						...
						{
							$Status = 1;
							$sql='update resign_details set revoke_accept = "2",rg_status=11, revoke_hr="'.date('Y-m-d H:i:s').'",rv_hr_remark="'.$_POST['txt_remark_'.$EmpID].'"   where EmployeeID = "'.$EmpID.'"';
							$myDB=new MysqliDb();
							$result_upd = $myDB->rawQuery($sql);
							$rowCount = $myDB->count;
							$Status = 1;
							$sql1='delete from alert_details where EmployeeID = "'.$EmpID.'" and type like "RESIGN Accept%"';
							$myDB=new MysqliDb();
							$result_upd = $myDB->rawQuery($sql1);
							$rowCount = $myDB->count;
							$sql1='INSERT INTO alert_details(EmployeeID,alert_start,alert_end,type,createdon,createdby)VALUES("'.$EmpID.'","'.date('Y-m-d',time()).'","'.date('Y-m-d',strtotime('+1 day')).'","'."Resign cancellation request approved by HR".'","'.date('Y-m-d H:i:s',time()).'","'.$createBy.'")';
							$myDB=new MysqliDb();
							$result_upd = $myDB->rawQuery($sql1);
						...
						else
						{
							$myDB=new MysqliDb();
							$Status = 2;
							$sql='update resign_details set revoke_accept = "3", revoke_hr="'.date('Y-m-d H:i:s').'",rv_hr_remark="'.$_POST['txt_remark_'.$EmpID].'"  where EmployeeID = "'.$EmpID.'"';
							$result_upd = $myDB->rawQuery($sql);
							$mysql_error = $myDB->getLastError();
								$rowCount = $myDB->count;
						}
