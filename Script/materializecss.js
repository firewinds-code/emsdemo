...
M.AutoInit = function (context) {
  ...
  var registry = {
    ...
    Datepicker_M: root.querySelectorAll('.datepicker_M:not(.no-autoinit)'),
...
;(function ($) {
  ...
  var Datepicker_M = function (_Component15) {
    _inherits(Datepicker_M, _Component15);

    /**
     * Construct Datepicker_M instance and set up overlay
     * @constructor
     * @param {Element} el
     * @param {Object} options
     */
    function Datepicker_M(el, options) {
      _classCallCheck(this, Datepicker_M);

      var _this52 = _possibleConstructorReturn(this, (Datepicker_M.__proto__ || Object.getPrototypeOf(Datepicker_M)).call(this, Datepicker_M, el, options));

      _this52.el.M_Datepicker_M = _this52;

      _this52.options = $.extend({}, Datepicker_M.defaults, options);

      // make sure i18n defaults are not lost when only few i18n option properties are passed
      if (!!options && options.hasOwnProperty('i18n') && typeof options.i18n === 'object') {
        _this52.options.i18n = $.extend({}, Datepicker_M.defaults.i18n, options.i18n);
      }
      ...
      if (Datepicker_M._isDate(defDate)) {
      ...
      /**
       * Describes open/close state of datepicker_M
       * @type {Boolean}
       */
    ...
    _createClass(Datepicker_M, [{
      ...
      value: function destroy() {
        ...
        $(this.modalEl).remove();
        this.destroySelects();
        this.el.M_Datepicker_M = undefined;
      }
    }, {
      key: "destroySelects",
      ...
      value: function toString(format) {
        ...
        if (!Datepicker_M._isDate(this.date)) {
          return '';
        }
      ...
      value: function setDate(date, preventOnSelect) {
        ...
        if (Datepicker_M._isDate(min) && date < min) {
          date = min;
        } else if (Datepicker_M._isDate(max) && date > max) {
          date = max;
        }
        ...
        Datepicker_M._setToStartOfDay(this.date);
      ...
      value: function _renderDateDisplay() {
        var displayDate = Datepicker_M._isDate(this.date) ? this.date : new Date();
      ...
      value: function render(year, month, randId) {
        var opts = this.options,
            now = new Date(),
            days = Datepicker_M._getDaysInMonth(year, month),
            before = new Date(year, month, 1).getDay(),
            data = [],
            row = [];
        Datepicker_M._setToStartOfDay(now);
        if (opts.firstDay > 0) {
        ...
        var previousMonth = month === 0 ? 11 : month - 1,
            ...
            daysInPreviousMonth = Datepicker_M._getDaysInMonth(yearOfPreviousMonth, previousMonth);
        ...
        for (var i = 0, r = 0; i < cells; i++) {
          var day = new Date(year, month, 1 + (i - before)),
              isSelected = Datepicker_M._isDate(this.date) ? Datepicker_M._compareDates(day, this.date) : false,
              isToday = Datepicker_M._compareDates(day, now),
              hasEvent = opts.events.indexOf(day.toDateString()) !== -1 ? true : false,
              isEmpty = i < before || i >= days + before,
              dayNumber = 1 + (i - before),
              monthNumber = month,
              yearNumber = year,
              isStartRange = opts.startRange && Datepicker_M._compareDates(opts.startRange, day),
              isEndRange = opts.endRange && Datepicker_M._compareDates(opts.endRange, day),
              isInRange = opts.startRange && opts.endRange && opts.startRange < day && day < opts.endRange,
              isDisabled = opts.minDate && day < opts.minDate || opts.maxDate && day > opts.maxDate || opts.disableWeekends && Datepicker_M._isWeekend(day) || opts.disableDayFn && opts.disableDayFn(day);
      ...
      value: function renderDay(opts) {
        ...
        return '<td data-day="' + opts.day + '" class="' + arr.join(' ') + '" aria-selected="' + ariaSelected + '">' + '<button class="datepicker_M-day-button" type="button" ' + 'data-pika-year="' + opts.year + '" data-pika-month="' + opts.month + '" data-pika-day="' + opts.day + '">' + opts.day + '</button>' + '</td>';
      }
    }, {
      key: "renderRow",
      ...
      value: function renderTable(opts, data, randId) {
        return '<div class="datepicker_M-table-wrapper"><table cellpadding="0" cellspacing="0" class="datepicker_M-table" role="grid" aria-labelledby="' + randId + '">' + this.renderHead(opts) + this.renderBody(data) + '</table></div>';
      }
    }, {
      key: "renderHead",
      ...
      value: function renderTitle(instance, c, year, month, refYear, randId) {
        var i = void 0,
            ...
            html = '<div id="' + randId + '" class="datepicker_M-controls" role="heading" aria-live="assertive">',
            monthHtml = void 0,
            yearHtml = void 0,
            prev = true,
            next = true;
      ...
      value: function _setupVariables() {
        ...
        this.$modalEl = $(Datepicker_M._template);
        ...
        if (this.options.showClearBtn) {
          this.clearBtn = this.modalEl.querySelector('.datepicker_M-clear');
        }
        this.doneBtn = this.modalEl.querySelector('.datepicker_M-done');
        this.cancelBtn = this.modalEl.querySelector('.datepicker_M-cancel');
      ...
      value: function _handleCalendarClick(e) {
        ...
        if (!$target.hasClass('is-disabled')) {
          if ($target.hasClass('datepicker_M-day-button') && !$target.hasClass('is-empty') && !$target.parent().hasClass('is-disabled')) {
      ...
      /**
       * Set input value to the selected date and close Datepicker_M
       */
      ...
      key: "init",
      value: function init(els, options) {
        return _get(Datepicker_M.__proto__ || Object.getPrototypeOf(Datepicker_M), "init", this).call(this, this, els, options);
      }
    }, {
      key: "_isDate",
      ...
      value: function _setToStartOfDay(date) {
        if (Datepicker_M._isDate(date)) date.setHours(0, 0, 0, 0);
      }
    }, {
      key: "_getDaysInMonth",
      value: function _getDaysInMonth(year, month) {
        return [31, Datepicker_M._isLeapYear(year) ? 29 : 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31][month];
      }
    }, {
      key: "_isLeapYear",
  ...
  Datepicker_M._template = ['<div class= "modal datepicker_M-modal">', '<div class="modal-content datepicker_M-container">', '<div class="datepicker_M-date-display">', '<span class="year-text"></span>', '<span class="date-text"></span>', '</div>', '<div class="datepicker_M-calendar-container">', '<div class="pika-single"></div>', '<div class="datepicker_M-footer">', '<button class="btn-flat datepicker_M-clear waves-effect" style="visibility: hidden;" type="button"></button>', '<div class="confirmation-btns">', '<button class="btn-flat datepicker_M-cancel waves-effect" type="button"></button>', '<button class="btn-flat datepicker_M-done waves-effect" type="button"></button>', '</div>', '</div>', '</div>', '</div>', '</div>'].join('');

  M.Datepicker_M = Datepicker_M;

  if (M.jQueryLoaded) {
    M.initializeJqueryWrapper(Datepicker_M, 'datepicker_M', 'M_Datepicker_M');
  }
})(cash);
;(function ($) {
  'use strict';
