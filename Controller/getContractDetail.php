...
if(isset($_REQUEST['tablename']) && $_REQUEST['tablename']!="") {
	...
	$myDB=new MysqliDb();
 	$sqlcmid="call getCmidUsingAhVhId('".$_SESSION['__user_logid']."')";
		...
		$myDB=new MysqliDb();
		$para_array=$myDB->query("select  ack_flg,oh_ack,vh_ack, ack_date  from ctrctdetails_master where table_name='".$tablename."'" );
		$ack_flg='';
		$oh_ack='';
		$vh_ack='';
		if($para_array[0]['ack_flg']==1){
			$ack_flg='1';
		}
		if($para_array[0]['oh_ack']==1){
			$oh_ack='1';
		}
		if($para_array[0]['vh_ack']==1){
			$vh_ack='1';
		}
	 $sqlBankcQuery="select b.Client_Info,a.* from ".$tablename." a inner join (SELECT cm_id,concat(client_master.client_name,' | ',process,' | ',sub_process) as Client_info FROM ems.new_client_master inner join client_master on client_master.client_id = new_client_master.client_name  where cm_id='".$cmid."') b on a.cm_id=b.cm_id ";
			...
			if(count($comment_array)>0 && $error==""){
				...
				if(in_array($cmid,$cmid_array)){
					...
					if($acknowledge=="Acknowledged"){
				     ...
				     $data.='</div>';
				echo($data);
				$insert_Log="INSERT INTO view_contract_log set  createdBy='".$_SESSION['__user_logid']."', createdOn='".Date("Y-m-d h:i:s")."', table_name='".$tablename."', cm_id='".$cmid."'";
				$myDB=new MysqliDb();
