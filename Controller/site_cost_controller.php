...
//$sql = "SELECT s*.id as costmasterid,sm.id as sitemasterid, s.txt_date,s.costitem, s.price,s.createdon ,sm.site,l.item FROM site_cost_master as s left join site_master as sm on  s.site_id=sm.id left join costitem as l on l.id = s.costitem where s.site_id=? and s.txt_date=?";
