...
$myDB=new MysqliDb();
$db_doj = $myDB->query("select dateofjoin from employee_map where EmployeeID='".$EmpID."'");
$date_doj = (isset($db_doj[0]['dateofjoin']))?$db_doj[0]['dateofjoin']:'';
...
$counter_check =0;                                                      
    while ($Workday < 2)
    {
		...
		{
			$myDB=new MysqliDb();
			$db_app = $myDB->query("select LeaveID from leavehistry where employeeid='".$EmpID."' and ( MngrStatusID='Approve' or MngrStatusID='Pending' ) and leavetype = 'Leave' and cast('".$dd."' as date) between cast(Datefrom as date) and cast(DateTo as date)");
			if(count($db_app) > 0 && $db_app)
			{
				$Atts = getAttndForExcep($EmpID, $dd); 
...
else if ($Request == "Roster Change")
{
    ...
    for ($i = 1; $i <= 9; $i++)
    {
        ...
        $sql = "select InTime,OutTime from roster_temp where EmployeeID = '".$EmpID."' and DateOn ='".$dd."'";
        $myDB = new MysqliDb();
        $res = $myDB->query($sql);
        $mysqlError = $myDB->getLastError();
        $roaster='';
