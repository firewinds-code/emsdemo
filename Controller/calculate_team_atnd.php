...
$DateTo ='';
			...
			if(isset($_REQUEST['empid']) && !empty($DateTo))
			{
				...
				$myDB=new MysqliDb();
				$chk_task=$myDB->query("select whole_details_peremp.EmployeeID,whole_details_peremp.EmployeeName,des_id from whole_details_peremp where ReportTo ='".$_REQUEST['empid']."' or Qa_ops ='".$_REQUEST['empid']."'");
				$counter = 0;
				...
				{   
					...
					foreach($chk_task as $key => $value)
					{
			            ...
			            $myDB = new MysqliDb();
			            $ds_roster = $myDB->query('select DateOn,InTime,OutTime,type_ from roster_temp where EmployeeID ="'.$EmployeeID.'" and DateOn between "'.$i_first_date.'" and "'.$last_date.'"');
						$RosterIn = array();
						$RosterOut = array();
						$Roster_type = array();
