...
if(isset($_GET['mid'])  and $_GET['mid']!="" and isset($_GET['locid'])  and $_GET['locid']!="")
{
	 $mid=$_GET['mid'];
	 $locid=$_GET['locid'];
	 if($mid!=""){
	 	$myDB=new MysqliDb();
		$sqlConnect2 = 'select  a.emailID,a.cc_email,a.location,b.module_name,c.location from  manage_module_email a inner join module_manager  b on  b.ID=a.moduleID left outer join location_master c on a.location=c.id where a.moduleID="'.$mid.'" and a.location="'.$locid.'"'; 
		//echo "<br>";
		$email=array();
		$ccemail=array();
		$myDB=new MysqliDb();
