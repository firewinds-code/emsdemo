...
if(isset($_REQUEST['loc']) && $_REQUEST['loc']!="")
{
	...
	$sql='select nc.*,cm.*,t1.location from new_client_master nc inner join client_master cm  on nc.client_name = cm.client_id inner join location_master t1 on t1.id = nc.location where t1.id="'.$loc.'" and nc.cm_id not in (select cm_id from client_status_master) order by cm.client_name';
	$myDB=new MysqliDb();
